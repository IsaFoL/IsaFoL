 ...
 * Author:       Ghilain Bergeron <ghilain.bergeron at inria.fr>, 2023
                 Sophie Tourret <sophie.tourret at inria.fr>, 2025
 * Comments:     Relies on HOLZF to fix the type discrepency issue *)

theory FOL_Compactness_ZF
  imports
    Main
...
fun FV_fm :: \<open>('f, 'p, 'v) fm \<Rightarrow> 'v set\<close> where
  \<open>FV_fm (Rel _ args') = (\<Union> a \<in> List.set args'. FV_tm a)\<close>
...
next
  ...
  have \<open>\<forall> t \<in> List.set ts. is_ground_tm t \<longleftrightarrow> FV_tm t = {}\<close>
    using Fun.IH
    by (intro ballI)
...
| \<open>fn_sym_tm (Fun f ts) = {(f, length ts)} \<union> (\<Union> t \<in> List.set ts. fn_sym_tm t)\<close>
...
| \<open>fn_sym_fm (Rel p ts) = (\<Union> t \<in> List.set ts. fn_sym_tm t)\<close> 
...
locale struct =
  ...
  assumes
    ...
    FN_dom_to_dom: \<open>\<forall> f es. (\<forall> e \<in> List.set es. e \<in> M) \<longrightarrow> FN f es \<in> M\<close> and
    REL_to_dom: \<open>\<forall> p. \<forall> es \<in> REL p. \<forall> e \<in> List.set es. e \<in> M\<close> 
...
lemma FN_dom_to_dom: \<open>\<forall> t \<in> List.set ts. t \<in> dom \<M> \<Longrightarrow> interp_fn \<M> f ts \<in> dom \<M>\<close>
...
proof (induction \<phi> arbitrary: \<beta>\<^sub>1 \<beta>\<^sub>2)
  case (Rel p ts)
  then have \<open>\<forall> t \<in> List.set ts. \<forall> v \<in> FV_tm t. \<beta>\<^sub>1 v = \<beta>\<^sub>2 v\<close>
    by simp 
...
proof (intro allI impI, induction \<phi>)
  ...
  have all_fn_sym_in: \<open>(\<Union> t \<in> List.set ts. fn_sym_tm t) \<subseteq> fn_sym_fm (Rel p ts)\<close> (is \<open>?A \<subseteq> _\<close>)
    by simp 
  ...
  proof (induction t) 
    ...
    have \<open>\<forall> t' \<in> List.set ts'. fn_sym_tm t' \<subseteq> fn_sym_fm (Rel p ts)\<close>
      using Fun.prems
      by auto
...
next
  ...
  have \<open>FV_fm (Rel p ts \<cdot>\<^sub>f\<^sub>m \<sigma>) = (\<Union> t \<in> List.set ts. FV_tm (t \<cdot> \<sigma>))\<close>
    by force 
  also have \<open>... = (\<Union> t \<in> List.set ts. \<Union> (FV_tm ` \<sigma> ` FV_tm t))\<close>
    by (meson vars_term_subst)
  also have \<open>... = (\<Union> t \<in> List.set ts. \<Union> v \<in> FV_tm t. FV_tm (\<sigma> v))\<close>
    by blast
  also have \<open>... = (\<Union> v \<in> (\<Union> t \<in> List.set ts. FV_tm t). FV_tm (\<sigma> v))\<close>
    by blast
...
proof (induction t)   
  ...
  have \<open>\<lbrakk>Fun f ts\<rbrakk>\<^bsup>\<M>,\<beta>\<^esup> = interp_fn \<M> f [\<lbrakk>t\<rbrakk>\<^bsup>\<M>,\<beta>\<^esup>. t \<leftarrow> ts]\<close>
    by auto 
  moreover have \<open>\<forall> t \<in> List.set ts. \<lbrakk>t\<rbrakk>\<^bsup>\<M>,\<beta>\<^esup> \<in> dom \<M>\<close>
...
proof (intro conjI allI impI)
  ...
  show \<open>\<Union> \<C> \<subseteq> Pow S\<close>
    using CollectD Sup_least \<C>_filter_chain filter.F_family_of_subsets_of_S subset_chain_def
        subset_iff by (metis complete_lattice_class.Sup_le_iff)
...
definition REL :: \<open>'p \<Rightarrow> ('i \<Rightarrow> 'm) set list set\<close> where
  \<open>REL p = {e. (\<forall> x \<in> List.set e. x \<in> A) \<and> {i \<in> I. [rep e\<^sub>i i. e\<^sub>i \<leftarrow> e] \<in> interp_rel (\<M> i) p} \<in> F}\<close> 
...
next
  fix f es
  assume \<open>\<forall> e \<in> List.set es. e \<in> A\<close>
  then have \<open>\<forall> e \<in> List.set es. rep e \<in> (\<Pi> i \<in> I. dom (\<M> i))\<close>
    using A_nonempty rep_of_elem_ultraproduct_in_pi
    by auto 
  then have \<open>\<forall> e \<in> List.set [rep e\<^sub>i. e\<^sub>i \<leftarrow> es]. e \<in> (\<Pi> i \<in> I. dom (\<M> i))\<close>
    by simp 
  then have \<open>\<forall> e \<in> List.set [rep e\<^sub>i. e\<^sub>i \<leftarrow> es]. \<forall> i \<in> I. e i \<in> dom (\<M> i)\<close>
    by blast
  then have \<open>\<forall> i \<in> I. \<forall> e \<in> List.set [rep e\<^sub>i i. e\<^sub>i \<leftarrow> es]. e \<in> dom (\<M> i)\<close>
    by simp 
...
next
  ...
  assume
    ...
    e_in: \<open>e \<in> List.set es\<close>
  then have \<open>\<forall> e \<in> List.set es. e \<in> A\<close> and \<open>{i \<in> I. [rep e\<^sub>i i. e\<^sub>i \<leftarrow> es] \<in> interp_rel (\<M> i) p} \<in> F\<close> 
    unfolding REL_def
    by blast+  
...
lemma lists_related_to_inter_in_filter:
  ...
  shows \<open>\<Inter> (List.set (map2 (\<lambda> a b. {i \<in> I. a i = b i}) as bs)) \<in> F\<close>
proof -
  ...
  then show \<open>\<Inter> (List.set (map2 (\<lambda> a b. {i \<in> I. a i = b i}) as bs)) \<in> F\<close>
    using assms(2, 3)
  ...
  next
    ...
    then have \<open>\<Inter> (List.set (map2 (\<lambda> a b. {i \<in> I. a i = b i}) xs ys)) \<in> F\<close>
      using "2.IH" "2.prems"(1)
      by fastforce
    moreover have \<open>{i \<in> I. x i = y i} \<in> F\<close>
      using x_sym_y
      by blast
    ultimately have \<open>{i \<in> I. x i = y i} \<inter> \<Inter> (List.set (map2 (\<lambda> a b. {i \<in> I. a i = b i}) xs ys)) \<in> F\<close>
      using filter.closed_int[OF F_filter]
      by blast 
    then show ?case
      unfolding bs_is
...
lemma inter_map2_to_set_compr:
  assumes
    ...
    \<open>length as = length bs\<close> and
    \<open>list_all2 (in_rel (\<sim>)) as bs\<close>
  shows \<open>\<Inter> (List.set (map2 (\<lambda> a b. {i \<in> I. a i = b i}) as bs)) =
    {i \<in> I. \<forall> (a, b) \<in> List.set (zip as bs). a i = b i}\<close>
  using assms(2, 3)
proof (induction arbitrary: bs rule: list_nonempty_induct[OF assms(1)])
  case (1 x)
  then obtain y where
...
next
  ...
  then have \<open>\<Inter> (List.set (map2 (\<lambda> x y. {i \<in> I. x i = y i}) xs ys)) =
    {i \<in> I. \<forall> (a, b) \<in> List.set (zip xs ys). a i = b i}\<close>
    using "2.IH"
    by presburger
  then have \<open>{i \<in> I. x i = y i} \<inter> \<Inter> (List.set (map2 (\<lambda> x y. {i \<in> I. x i = y i}) xs ys)) =
    {i \<in> I. x i = y i} \<inter> {i \<in> I. \<forall> (a, b) \<in> List.set (zip xs ys). a i = b i}\<close>
    by blast 
  moreover have \<open>{i \<in> I. x i = y i} \<inter> \<Inter> (List.set (map2 (\<lambda> x y. {i \<in> I. x i = y i}) xs ys)) =
    \<Inter> (List.set (map2 (\<lambda> x y. {i \<in> I. x i = y i}) (x # xs) (y # ys)))\<close>
    by simp 
  moreover have \<open>{i \<in> I. x i = y i} \<inter> {i \<in> I. \<forall> (a, b) \<in> List.set (zip xs ys). a i = b i} =
    {i \<in> I. \<forall> (a, b) \<in> List.set (zip (x # xs) (y # ys)). a i = b i}\<close>
  proof -
    have \<open>{i \<in> I. x i = y i} \<inter> {i \<in> I. \<forall> (a, b) \<in> List.set (zip xs ys). a i = b i} =
      {i \<in> I. x i = y i \<and> (\<forall> (a, b) \<in> List.set (zip xs ys). a i = b i)}\<close>
      by blast
    also have \<open>... = {i \<in> I. \<forall> (a, b) \<in> List.set (zip (x # xs) (y # ys)). a i = b i}\<close>
      by auto 
    finally show ?thesis . 
  qed
  ultimately show ?case
...
next
  ...
  have dummy_A_in_F: \<open>\<Inter> (List.set (map2 (\<lambda> a b. {i \<in> I. a i = b i}) as bs)) \<in> F\<close>
    (is \<open>?A \<in> _\<close>)
    by (rule lists_related_to_inter_in_filter[OF False assms(1, 2)]) 

  have dummy_A_is: \<open>?A = {i \<in> I. \<forall> (a, b) \<in> List.set (zip as bs). a i = b i}\<close>
  ...
  proof -
    have \<open>?Pa \<inter> {i \<in> I. \<forall> (a, b) \<in> List.set (zip as bs). a i = b i} =
      {i \<in> I. [a i. a \<leftarrow> as] \<in> interp_rel (\<M> i) p \<and> (\<forall> (a, b) \<in> List.set (zip as bs). a i = b i)}\<close>
      (is \<open>?lhs = _\<close>)
      by fastforce 
    also have
      \<open>... = {i \<in> I. [b i. b \<leftarrow> bs] \<in> interp_rel (\<M> i) p \<and> (\<forall> (a, b) \<in> List.set (zip as bs). a i = b i)}\<close>
      using assms(1, 2)
    ...
    next
      ...
      then obtain y ys where
        ...
        by (smt (verit, best) in_rel_def list_all2_Cons1 list_all2_lengthD) 
      then have
        \<open>{i \<in> I. [a i. a \<leftarrow> xs] \<in> interp_rel (\<M> i) p \<and> (\<forall> (a, b) \<in> List.set (zip xs ys). a i = b i)} =
        {i \<in> I. [b i. b \<leftarrow> ys] \<in> interp_rel (\<M> i) p \<and> (\<forall> (a, b) \<in> List.set (zip xs ys). a i = b i)}\<close>
        using "2.IH"
        by presburger
      ...
      proof (intro subset_antisym subsetI; clarsimp)
        ...
        assume
          ...
          \<open>\<forall> (a, b) \<in> List.set (zip xs ys). a i = b i\<close>
    ...
    qed
    also have
      \<open>... = ?Pb \<inter> {i \<in> I. \<forall> (a, b) \<in> List.set (zip as bs). a i = b i}\<close>
      (is \<open>_ = ?rhs\<close>)
      by fastforce 
...
next
  ...
  moreover have \<open>\<lbrakk>t\<rbrakk>\<^bsup>\<M> i,\<lambda> v. rep (\<beta> v) i\<^esup> \<in> dom (\<M> i)\<close>
    if
      \<open>i \<in> I\<close> and
      \<open>t \<in> List.set ts\<close>
    for i t
    using Fun[OF that(2)]
    by (meson PiE restrict_Pi_cancel that(1)) 
  then have \<open>\<forall> t \<in> List.set ts. \<lbrakk>t\<rbrakk>\<^bsup>\<M> i,\<lambda> v. rep (\<beta> v) i\<^esup> \<in> dom (\<M> i)\<close>
    if \<open>i \<in> I\<close>
    for i
    using that
    by blast
...
proof (induction \<phi> arbitrary: \<beta>) 
  ...
  have \<open>\<A>,\<beta> \<Turnstile> Rel p e \<longleftrightarrow> [\<lbrakk>t\<rbrakk>\<^bsup>\<A>,\<beta>\<^esup>. t \<leftarrow> e] \<in> REL p\<close> (is \<open>_ \<longleftrightarrow> ?e \<in> _\<close>)
    ...
    by auto 
  also have
    \<open>... \<longleftrightarrow> (\<forall> x \<in> List.set ?e. x \<in> A) \<and> {i \<in> I. [rep e\<^sub>i i. e\<^sub>i \<leftarrow> ?e] \<in> interp_rel (\<M> i) p} \<in> F\<close>
    unfolding REL_def
    by blast 
  also have
    \<open>... \<longleftrightarrow> (\<forall> x \<in> List.set ?e. x \<in> A) \<and> {i \<in> I. [rep (\<lbrakk>t\<rbrakk>\<^bsup>\<A>,\<beta>\<^esup>) i. t \<leftarrow> e] \<in> interp_rel (\<M> i) p} \<in> F\<close>
    unfolding map_map_is_map
    by blast
  also have \<open>... \<longleftrightarrow> (\<forall> x \<in> List.set ?e. x \<in> A) \<and> {i \<in> I. 
    ...
    by simp 
  also have \<open>... \<longleftrightarrow> (\<forall> x \<in> List.set ?e. x \<in> A) \<and>
