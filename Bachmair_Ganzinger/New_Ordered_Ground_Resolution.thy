...
where
  ...
  "Cf' = \<Union># {#C'. (C', A, m) \<in># ZZ#} \<Longrightarrow>
   ...
   (mset (map side_clause CC)) = {#C' + replicate_mset (Suc m) (Pos A). (C', A, m) \<in># ZZ#} \<Longrightarrow>
   ...
   (\<forall>C. C \<in># (mset (map side_clause CC)) \<longrightarrow> S C = {#}) \<Longrightarrow>
   ord_resolve CC (D,As) (Cf' + D)"

lemma ord_resolve_sound:
  assumes
    res_e: "ord_resolve CC (D,As) E" and
    cc_true: "I \<Turnstile>m side_clauses CC" and
...
using res_e proof (cases rule: ord_resolve.cases)
  ...
  next
    ...
    have c_in_cc: "C' + Am \<in># side_clauses CC" using cam unfolding cc aaa by force
...
theorem ord_resolve_counterex_reducing:
  ...
  obtains DD E mC where
    ...
    "set_mset (side_clauses DD) \<subseteq> N"
    "INTERP N \<Turnstile>m side_clauses DD"
    "\<And>D. D \<in># side_clauses DD \<Longrightarrow> productive N D"
    "ord_resolve DD mC E"
...
proof -
  ...
  define DD where "DD \<equiv> inv side_clauses {#D' + replicate_mset (Suc m) (Pos A). (D', A, m) \<in># ZZ#}"
  have DD_def: "side_clauses DD \<equiv> {#D' + replicate_mset (Suc m) (Pos A). (D', A, m) \<in># ZZ#}"
  proof -
    ...
    then have "\<forall>D. D \<in># ({#D' + replicate_mset (Suc m) (Pos A). (D', A, m) \<in># ZZ#}) \<longrightarrow> D \<in> range side_clause"
      by auto
    then have in_range: "{#D' + replicate_mset (Suc m) (Pos A). (D', A, m) \<in># ZZ#} \<in> range side_clauses"
      using range_mset_map by blast
      
    
    have "DD = inv side_clauses {#D' + replicate_mset (Suc m) (Pos A). (D', A, m) \<in># ZZ#}" using DD_def by auto
    then have "side_clauses DD = side_clauses (inv (side_clauses) {#D' + replicate_mset (Suc m) (Pos A). (D', A, m) \<in># ZZ#})" by auto
    then show "side_clauses DD \<equiv> {#D' + replicate_mset (Suc m) (Pos A). (D', A, m) \<in># ZZ#}"
      using f_inv_into_f[of "{#D' + replicate_mset (Suc m) (Pos A). (D', A, m) \<in># ZZ#}" "side_clauses" UNIV]
      using in_range by auto
  qed

  have df': "Df' = \<Union># {#D'_of A. A \<in># AA#}"
    unfolding Df'_def ZZ_def by auto
  have dd: "side_clauses DD = {#D_of A. A \<in># AA#}"
    unfolding DD_def ZZ_def d_of
    apply simp 
    apply (rule image_mset_cong, metis Suc_pred m_nz neq0_conv replicate_mset_Suc
        union_mset_add_mset_right)
    done
  have prod_d: "\<And>D. D \<in># side_clauses DD \<Longrightarrow> productive N D"
    unfolding dd by (auto dest!: prod_d0)
  hence dd_subs_n: "set_mset (side_clauses DD) \<subseteq> N"
    using productive_in_N by auto
  have dd_true: "INTERP N \<Turnstile>m (side_clauses DD)"
    unfolding true_cls_mset_def using prod_d productive_imp_true_in_INTERP by blast 
  have s_dd_e: "\<And>D. D \<in># (side_clauses DD) \<Longrightarrow> S D = {#}"
  ...
  moreover have "side_clauses DD = {#C' + replicate_mset (Suc m) (Pos A). (C', A, m) \<in># ZZ#}"
  ...
  moreover have "\<forall>C. C \<in># side_clauses DD \<longrightarrow> S C = {#}" using s_dd_e by auto
  ...
  have "\<And>D. D \<in># side_clauses DD \<Longrightarrow> \<not> interp N D \<Turnstile> D"
