...
text \<open>The checker takes as input 3 files:
...
Remark that unlike DRAT checker, we do forward checking and check every derived polynomial.
\<close>
...
lemma coeff_MPoly_monomila[simp]:
