theory Functional_Substitution_Lifting
  imports Functional_Substitution
begin

locale functional_substitution_lifting = 
  sub: functional_substitution
  where subst = sub_subst and vars = sub_vars
  for
    sub_vars :: "'sub \<Rightarrow> 'var set" and
    sub_subst :: "'sub \<Rightarrow> ('var \<Rightarrow> 'base) \<Rightarrow> 'sub" +
  fixes
    map :: "('sub \<Rightarrow> 'sub) \<Rightarrow> 'expr \<Rightarrow> 'expr" and
    to_set :: "'expr \<Rightarrow> 'sub set"
  assumes
    map_comp: "\<And>expr f g. map (f \<circ> g) expr = map f (map g expr)" and
    map_id: "map id expr = expr" and
    map_cong: 
      "\<And>expr f g. (\<And>sub. sub \<in> to_set expr \<Longrightarrow> f sub = g sub) \<Longrightarrow> map f expr = map g expr" and
    to_set_map: "\<And>expr f. to_set (map f expr) = f ` to_set expr" and
    exists_expression: "\<And>sub. \<exists>expr. sub \<in> to_set expr" 
begin
...
locale based_functional_substitution_lifting = 
  base: base_functional_substitution 
  where subst = base_subst and vars = base_vars +
...
locale grounding_lifting = 
  functional_substitution_lifting 
  where sub_vars = sub_vars and sub_subst = sub_subst and map = map + 
  sub: grounding
  where vars = sub_vars and subst = sub_subst and to_ground = sub_to_ground and 
  ...
  assumes 
    to_set_from_ground_map: 
      "\<And>expr\<^sub>G f. to_set (from_ground_map f expr\<^sub>G) = f ` to_set_ground expr\<^sub>G" and
    map_ground_comp: "\<And>expr f g. map (f \<circ> g) expr = from_ground_map f (to_ground_map g expr)" and
    ground_map_comp: 
      "\<And>expr\<^sub>G f g. ground_map (f \<circ> g) expr\<^sub>G = to_ground_map f (from_ground_map g expr\<^sub>G)" and
    ground_map_id: "\<And>expr\<^sub>G. ground_map id expr\<^sub>G = expr\<^sub>G"
begin
...
proof unfold_locales
  ...
  proof-
    ...
    then have "\<exists>expr\<^sub>G. from_ground expr\<^sub>G = expr"
      using map_ground_comp[symmetric] map_id_cong
...
next
  show "\<And>expr\<^sub>G. to_ground (from_ground expr\<^sub>G) = expr\<^sub>G"
    ...
    by (metis ground_map_comp sub.to_ground_from_ground_id)
qed
...
locale all_subst_ident_iff_ground_lifting = 
  ...
  where map = map and id_subst = id_subst +  
  sub: all_subst_ident_iff_ground 
  where subst = sub_subst and is_ground = sub.is_ground
  for map :: "('sub \<Rightarrow> 'sub) \<Rightarrow> 'expr \<Rightarrow> 'expr" and id_subst :: "'var \<Rightarrow> 'base"
begin
