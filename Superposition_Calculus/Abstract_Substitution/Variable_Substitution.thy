...
lemma to_ground_inverse: 
  ...
  using assms 
  by (metis f_inv_into_f from_ground_inverse mem_Collect_eq range_from_ground_iff_is_ground)
...
assumes
  expanded_vars_vars: 
    "(\<forall>x. is_empty (expanded_vars (expanded_subst x \<gamma>))) \<longleftrightarrow> (\<forall>x. is_empty (vars (x \<cdot> \<gamma>)))" and
  is_ground_iff:
...
locale variable_substitution_lifting_set = 
  lifted: variable_substitution_set comp_subst id_subst base_subst base_vars
  for 
...
lemma is_ground_iff:
  ...
  by (smt (verit) UNION_empty_conv(1) UN_iff UN_simps(10) lifted_is_ground_iff subst_def variable_substitution_lifting_set.to_set_map variable_substitution_lifting_set.vars_def variable_substitution_lifting_set_axioms)
