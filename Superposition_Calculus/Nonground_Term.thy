...
sublocale nonground_term_properties where 
  ...
  vars = "vars :: ('f, 'v) term \<Rightarrow> 'v set" and subst_domain = subst_domain and 
...
next
  ...
  show "vars (t \<cdot>t \<sigma>) \<subseteq> vars t - subst_domain \<sigma> \<union> range_vars \<sigma>"
