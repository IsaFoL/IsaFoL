...
lemma Suc_Max_notin_set: "finite X \<Longrightarrow> Suc (v + Max (insert 0 X)) \<notin> X"
  by (metis Max.boundedE Suc_n_not_le_n empty_iff finite.insertI insertCI le_add2)
...
proof (induction Cs)
  ...
  then show ?case
    ...
    by (smt (verit, best) antisym_conv3 inf.strict_boundedE inf_commute inf_min length_Cons subst_cls_lists_length)
