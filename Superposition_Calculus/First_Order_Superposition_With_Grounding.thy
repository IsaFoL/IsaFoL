...
lemma (in first_order_superposition_calculus_with_grounding) 
  inference\<^sub>G_concl_in_clause_grounding: 
...
lemma (in first_order_superposition_calculus_with_grounding) 
  inference\<^sub>G_red_in_clause_grounding_of_concl: 
  "\<iota>\<^sub>G \<in> inference_groundings select\<^sub>G \<iota> \<Longrightarrow> \<iota>\<^sub>G \<in> ground.Red_I (clause_groundings (concl_of \<iota>))"
proof-
  assume a2: "\<iota>\<^sub>G \<in> inference_groundings select\<^sub>G \<iota>"

  moreover then have "\<iota>\<^sub>G \<in> ground.G_Inf"
    unfolding inference_groundings_def[OF select\<^sub>G]
    by blast

  moreover have "concl_of \<iota>\<^sub>G \<in> clause_groundings (concl_of \<iota>)"
    using a2 inference\<^sub>G_concl_in_clause_grounding
    by auto
