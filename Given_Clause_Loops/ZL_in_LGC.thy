...
locale zipperposition_loop = LGC? : discount_loop
  ...
  for
    ...
    active passive y :: "'l"
begin (* begin locale Zipperposition loop *)
...
inductive ZL :: 
  "('f inference) set \<times> ('f \<times> 'l) set \<Rightarrow> ('f inference) set \<times> ('f \<times> 'l) set \<Rightarrow> bool" 
                                                                            (infix "\<leadsto>ZL" 50) where
...
end (* end locale Zipperposition loop *)
