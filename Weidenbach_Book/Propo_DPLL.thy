...
lemma dpll_consistent_interp_inv:
  ...
  using assms
proof (induct rule: dpll.induct)
  case (backtrack S M' L M D) note extracted = this(1) and marked = this(2) and D = this(4) and cons = this(5) and no_dup = this(6)
  have no_dup': "no_dup M"
    using no_dup  marked D dpll.backtrack dpll_distinctinv extracted by fastforce
...
next
  ...
  moreover {
    ...
    moreover have 3: "\<And>a c. hd (get_all_marked_decomposition (trail S)) = (a, c) \<Longrightarrow> ((\<lambda>a. {#lit_of a#}) ` set a \<union> clauses S) \<Turnstile>p {#L#}"
      proof -
        ...
        have I: "set (map (\<lambda>a. {#lit_of a#})  a) \<union> clauses S \<union> (\<lambda>a. {#lit_of a#}) ` set c \<Turnstile>ps CNot C" using `?I \<Turnstile>ps CNot C` unfolding h' by (simp add: Un_commute Un_left_commute)
        have "atms_of_m (CNot C) \<subseteq> atms_of_m (set (map (\<lambda>a. {#lit_of a#}) a) \<union> clauses S)" and
          "atms_of_m ((\<lambda>a. {#lit_of a#}) ` set c) \<subseteq> atms_of_m (set (map (\<lambda>a. {#lit_of a#}) a) \<union> clauses S)"
  ...
  ultimately show ?case by auto
next
  case (backtrack S M' L M D) note extracted = this(1) and marked = this(2) and D = this(3) and cnot = this(4) and cons = this(4) and IH = this(5) and atms_incl = this(6)
  ...
  hence "all_decomposition_implies (clauses S) ((L # M, M') # tl (get_all_marked_decomposition (trail S)))"
  ...
  show ?case unfolding S all_decomposition_implies_def
    proof
      fix x P level
      assume x: "x \<in> set (get_all_marked_decomposition (fst (Propagated (- lit_of L) P # M, clauses S)))"
      ...
      moreover {
        ...
        have "x \<in> set (get_all_marked_decomposition (M' @ L # M))"
          using x' get_all_marked_decomposition_except_last_choice_equal[of M' "lit_of L" P M  Level] L' by (metis (no_types) M' list.set_sel(2) tl_Nil)
        hence "case x of (Ls, seen) \<Rightarrow> (\<lambda>a. {#lit_of a#}) ` set Ls \<union> clauses S \<Turnstile>ps (\<lambda>a. {#lit_of a#}) ` set seen"
      ...
      moreover {
        ...
        have tl: "tl (get_all_marked_decomposition (M' @ L # M)) \<noteq> []"
          proof -
            have f1: "\<And>ms. length (get_all_marked_decomposition (M' @ ms)) = length (get_all_marked_decomposition ms)"
            ...
            have "Suc (length (get_all_marked_decomposition M)) \<noteq> Suc 0"
              by blast
            thus ?thesis
              using f1 marked by (metis (no_types) get_all_marked_decomposition.simps(1) length_tl list.sel(3) list.size(3) marked_lit.collapse(1))
          qed
        obtain M0' M0 where
        ...
        have x'': "x = (M0, Propagated (-lit_of L) P # M0')"
          unfolding x' using get_all_marked_decomposition_last_choice tl M' L0 by (metis marked marked_lit.collapse(1))
        obtain l_get_all_marked_decomposition where
          ...
          using get_all_marked_decomposition_backtrack_split extracted by (metis (no_types) L0 S hd_Cons_tl n tl)
      ...
      ultimately show "case x of (Ls, seen) \<Rightarrow> (\<lambda>a. {#lit_of a#}) ` set Ls \<union> snd (?M', clauses S) \<Turnstile>ps (\<lambda>a. {#lit_of a#}) ` set seen" unfolding snd_conv by blast
    qed
qed
...
theorem dpll_propagate_is_conclusion_of_decided:
  ...
  and "all_decomposition_implies (clauses S) (get_all_marked_decomposition (trail S))"
  and "atm_of ` lit_of ` set (trail S) \<subseteq> atms_of_m (clauses S)"
  shows "clauses S' \<union> {{#lit_of L#} |L. is_marked L \<and> L \<in> set (trail S')} \<Turnstile>ps (\<lambda>a. {#lit_of a#}) ` \<Union>(set ` snd ` set (get_all_marked_decomposition (trail S')))"
...
next
  ...
  and "no_dup (trail S)"
  and "finite (clauses S)"
  ultimately have decomp: "all_decomposition_implies (clauses S') (get_all_marked_decomposition (trail S') )"
  and atm_incl': "atm_of ` lit_of ` set (trail S') \<subseteq> atms_of_m (clauses S')"
  ...
  show "all_decomposition_implies (clauses S'') (get_all_marked_decomposition (trail S''))" using  dpll_propagate_is_conclusion[OF dpll] decomp atm_incl' by auto
  show "atm_of ` lit_of ` set (trail S'') \<subseteq> atms_of_m (clauses S'')" using dpll_vars_in_snd_inv[OF dpll]  atm_incl atm_incl' by auto
  show "no_dup (trail S'')" using dpll_distinctinv[OF dpll] no_dup' dpll by auto
  show "consistent_interp (lits_of (trail S''))" using cons' no_dup' dpll_consistent_interp_inv[OF dpll] by auto
  show "finite (clauses S'')" using dpll_finite_inv[OF dpll] finite' by auto
qed

definition "dpll_all_inv S \<equiv> (all_decomposition_implies (clauses S) (get_all_marked_decomposition (trail S)) \<and> atm_of ` lit_of ` (set (trail S))  \<subseteq> atms_of_m (clauses S) \<and> consistent_interp (lits_of (trail S)) \<and> no_dup (trail S) \<and> finite (clauses S))"
...
lemma rtranclp_dpll_inv_starting_from_0:
  ...
  shows "dpll_all_inv S'"
proof -
  have "dpll_all_inv S" using assms unfolding all_decomposition_implies_def unfolding dpll_all_inv_def by auto
...
definition "final_dpll_state (S:: 'v dpll_state) \<longleftrightarrow> (trail S \<Turnstile>as clauses S \<or> ((\<forall>L \<in> set (trail S). \<not>is_marked L) \<and> (\<exists>C \<in> clauses S. trail S \<Turnstile>as CNot C)))"
  ...
  show ?A
    proof (rule ccontr)
      ...
      have "(\<exists>L. undefined_lit L M \<and> atm_of L \<in> atms_of_m N) \<or> (\<exists>D\<in>N. M \<Turnstile>as CNot D)"
        proof -
          obtain D :: "'a clause" where D: "D \<in> N" and "\<not> M \<Turnstile>a D" using n unfolding true_annots_def Ball_def by metis
...
definition "dpll_mes M n = map (\<lambda>l. if is_marked l then 2 else (1::nat)) (rev M) @ replicate (n - length M) 3"
...
lemma dpll_card_decrease:
  ...
  shows "(dpll_mes (trail S') (card vars), dpll_mes (trail S) (card vars)) \<in> lexn {(a, b). a < b} (card vars)"
  using assms
proof (induct rule: dpll.induct)
  case (propagate C L S)
  have m: "map (\<lambda>l. if is_marked l then 2 else 1) (rev (trail S)) @ replicate (card vars - length (trail S)) 3  =  map (\<lambda>l. if is_marked l then 2 else 1) (rev (trail S)) @ 3 # replicate (card vars - Suc (length (trail S))) 3" using propagate.prems[simplified] using Suc_diff_le by fastforce
  thus ?case
    using propagate.prems(1) unfolding dpll_mes_def by (fastforce simp add: lexn_conv assms(2))
next
  case (decided L S)
  have m: "map (\<lambda>l. if is_marked l then 2 else 1) (rev (trail S)) @ replicate (card vars - length (trail S)) 3  =  map (\<lambda>l. if is_marked l then 2 else 1) (rev (trail S)) @ 3 # replicate (card vars - Suc (length (trail S))) 3" using decided.prems[simplified] using Suc_diff_le by fastforce
  thus ?case
...
next
  ...
  have S: "trail S = M' @ L # M" using backtrack.hyps(1) backtrack_split_list_eq[of "trail S"] by auto
  show ?case
...
lemma dpll_card_decrease':
  ...
  and no_dup: "no_dup (trail S)"
  and fin: "finite (clauses S)"
  shows "(dpll_mes (trail S') (card (atms_of_m (clauses S'))), dpll_mes (trail S) (card (atms_of_m (clauses S)))) \<in> lex {(a, b). a < b}"
proof -
  ...
  ultimately have "(dpll_mes (trail S') (card (atms_of_m (clauses S))), dpll_mes (trail S) (card (atms_of_m (clauses S)))) \<in> lexn {(a, b). a < b} (card (atms_of_m (clauses S)))" using dpll_card_decrease[OF assms(1), of "atms_of_m (clauses S)"] by blast
  hence "(dpll_mes (trail S') (card (atms_of_m (clauses S))), dpll_mes (trail S) (card (atms_of_m (clauses S)))) \<in> lex {(a, b). a < b}"
    unfolding lex_def by auto
  thus "(dpll_mes (trail S') (card (atms_of_m (clauses S'))), dpll_mes (trail S) (card (atms_of_m (clauses S)))) \<in> lex {(a, b). a < b}"
...
lemma dpll_wf:
  ...
  apply (rule wf_wf_if_measure'[OF wf_lex_less, of _ _ "\<lambda>S. dpll_mes (trail S) (card (atms_of_m (clauses S)))"])
  using dpll_card_decrease' by fast


lemma dpll_tranclp_star_commute:
  "{(S', S). dpll_all_inv S \<and> dpll S S'}\<^sup>+ = {(S', S). dpll_all_inv S \<and> tranclp dpll S S'}" (is "?A = ?B")
proof
  { fix S S'
    assume "(S, S') \<in> ?A"
    hence "(S, S') \<in> ?B"
