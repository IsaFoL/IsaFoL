...
lemma propagate_no_more_Decided_lit:
  ...
  shows "Decided K i \<in> set (trail S) \<longleftrightarrow> Decided K i \<in> set (trail S')"
...
lemma cdcl\<^sub>W_o_no_more_Decided_lit:
  ...
  shows "Decided K i \<in> set (trail S') \<longrightarrow> Decided K i \<in> set (trail S)"
  using assms
...
proof (induct rule: cdcl\<^sub>W_stgy.induct)
  ...
  then have "Decided L i \<in> set (trail S')" and "Decided L i \<notin> set (trail S)"
...
next
  ...
  then have "Decided L i \<in> set (trail U)" and "Decided L i \<notin> set (trail S)"
    using no_dup unfolding S S' cdcl\<^sub>W_M_level_inv_def by (auto simp add: rev_image_eqI)
  then have "Decided L i \<in> set (trail T)"
    using st rtranclp_cdcl\<^sub>W_cp_no_more_Decided_lit unfolding full_def by blast
  then show ?case
    using cdcl\<^sub>W_o_no_more_Decided_lit[OF o] \<open>Decided L i \<notin> set (trail S)\<close> ns lev by meson
qed

lemma cdcl\<^sub>W_o_is_decide:
  assumes "cdcl\<^sub>W_o S T" and lev: "cdcl\<^sub>W_M_level_inv S"
  "trail T = drop (length M\<^sub>0) M' @ Decided L i # H @ M"and
  "\<not> (\<exists>M'. trail S = M' @ Decided L i # H @ M)"
  shows "decide S T"
  using assms
proof (induction rule:cdcl\<^sub>W_o_induct_lev2)
  ...
  show ?case
    ...
    using \<open>trail S = c @ M2 @ Decided K (Suc i) # M1\<close>
...
lemma rtranclp_cdcl\<^sub>W_new_decided_at_beginning_is_decide:
  ...
  shows
    "\<exists>S T T'. cdcl\<^sub>W_stgy\<^sup>*\<^sup>* R S \<and> decide S T \<and> cdcl\<^sub>W_stgy\<^sup>*\<^sup>* T U \<and> cdcl\<^sub>W_stgy\<^sup>*\<^sup>* S U \<and>
      no_step cdcl\<^sub>W_cp S \<and> trail T = Decided L i # H @ M \<and> trail S = H @ M \<and> cdcl\<^sub>W_stgy S T' \<and>
...
next
  ...
  show ?case
    proof (cases "\<exists>M'. trail T = M' @ Decided L i # H @ M")
      case False
      with s show ?thesis using U s st S
        proof induction
          ...
          then have MV: "M' @ Decided L i # H @ M = M\<^sub>0 @ trail T" unfolding W by simp
          then have V: "trail T = drop (length M\<^sub>0) (M' @ Decided L i # H @ M)"
            by auto
        ...
        next
          ...
          then have MV: "M' @ Decided L i # H @ M = M\<^sub>0 @ trail T'" unfolding U by simp
          then have V: "trail T' = drop (length M\<^sub>0) (M' @ Decided L i # H @ M)"
            by auto
          have "takeWhile (Not o is_decided) M' = M\<^sub>0  @ takeWhile (Not \<circ> is_decided) (trail T')"
            using arg_cong[OF MV, of "takeWhile (Not o is_decided)"] ndecided
          ...
          from arg_cong[OF this, of length]  have "length M\<^sub>0 \<le> length M'"
            unfolding length_append by (metis (no_types, lifting) Nat.le_trans le_add1
              length_takeWhile_le)
          then have tr_T': "trail T' = drop (length M\<^sub>0) M' @ Decided L i # H @ M" using V by auto
          then have LT': "Decided L i \<in> set (trail T')" by auto
          moreover
            have "cdcl\<^sub>W_M_level_inv T"
          ...
          have [simp]: "drop (length M\<^sub>0) M' = []"
            using \<open>decide T T'\<close> \<open>Decided L i \<in> set (trail T')\<close>  nd tr_T'
            by (auto simp add: Cons_eq_append_conv elim: decideE)
          have T': "drop (length M\<^sub>0) M' @ Decided L i # H @ M = Decided L i # trail T"
            using \<open>decide T T'\<close> \<open>Decided L i \<in> set (trail T')\<close>  nd tr_T'
            by (auto elim: decideE)
          have "trail T' = Decided L i # trail T"
            using \<open>decide T T'\<close> \<open>Decided L i \<in> set (trail T')\<close> tr_T'
            by (auto elim: decideE)
          then have 5: "trail T' = Decided L i # H @ M"
              using append.simps(1) list.sel(3) local.other'(5) tl_append2 by (simp add: tr_T')
          have 6: "trail T = H @ M"
            by (metis (no_types) \<open>trail T' = Decided L i # trail T\<close>
              \<open>trail T' = drop (length M\<^sub>0) M' @ Decided L i # H @ M\<close> append_Nil list.sel(3) nd
              tl_append2)
...
lemma rtranclp_cdcl\<^sub>W_new_decided_at_beginning_is_decide':
  ...
  "trail R = M" and
  "cdcl\<^sub>W_M_level_inv R"
  shows "\<exists>y y'. cdcl\<^sub>W_stgy\<^sup>*\<^sup>* R y \<and> cdcl\<^sub>W_stgy y y' \<and> \<not> (\<exists>c. trail y = c @ Decided L i # H @ M)
    \<and> (\<lambda>a b. cdcl\<^sub>W_stgy a b \<and> (\<exists>c. trail a = c @ Decided L i # H @ M))\<^sup>*\<^sup>* y' U"
proof -
  fix T'
  obtain S' T T' where
  ...
  have n: "\<not> (\<exists>c. trail S' = c @ Decided L i # H @ M)" using trS' by auto
  show ?thesis
    using rtranclp_trans[OF st] rtranclp_exists_last_with_prop[of cdcl\<^sub>W_stgy S' T' _
        "\<lambda>a _. \<not>(\<exists>c. trail a = c @ Decided L i # H @ M)", OF S'T' T'U n]
    by meson
qed
...
lemma cdcl\<^sub>W_stgy_trail_has_new_decided_is_decide_step:
  ...
  "\<not> (\<exists>c. trail S = c @ Decided L i # H @ M)" and
  "(\<lambda>a b. cdcl\<^sub>W_stgy a b \<and> (\<exists>c. trail a = c @ Decided L i # H @ M))\<^sup>*\<^sup>* T U" and
...
next
  ...
  then show ?case
    ...
    next
      ...
      have "\<exists>M'. trail T = M' @ Decided L i # H @ M"
...
lemma rtranclp_cdcl\<^sub>W_stgy_with_trail_end_has_trail_end:
  assumes "(\<lambda>a b. cdcl\<^sub>W_stgy a b \<and> (\<exists>c. trail a = c @ Decided L i # H @ M))\<^sup>*\<^sup>* T U" and
...
proof (induction rule: cdcl\<^sub>W_o_induct_lev2)
  ...
  have H: "get_all_levels_of_ann (trail y) = rev [1..<1 + backtrack_lvl y]"
    using lev unfolding cdcl\<^sub>W_M_level_inv_def by auto
  have "c' @ Decided Kh i # H = Propagated L' (mset_ccls D') # trail (reduce_trail_to M1 y)"
  ...
  have "i \<in> set (get_all_levels_of_ann (M3 @ M2 @ Decided K (Suc j) # d @ Decided Kh i # H))"
    by auto
  ...
  show ?case
    proof
      ...
      then have DLD': "D = mset_ccls D'"
        using DL T neq0_conv undef decomp lev by (fastforce simp: cdcl\<^sub>W_M_level_inv_def)
      have L_cKh: "atm_of L \<in> atm_of ` lits_of_l (c @ [Decided Kh i])"
      ...
      have "get_all_levels_of_ann (M3 @ M2 @ Decided K (j + 1) # M1)
      ...
      have DD'[simp]: "remove1_mset L D = mset_ccls D' - {#L'#}"
        proof (rule ccontr)
          ...
          moreover {
            ...
            moreover
              have "get_all_levels_of_ann (trail y) = rev [1..<1 + backtrack_lvl y]"
                using lev unfolding cdcl\<^sub>W_M_level_inv_def by auto
              then have "get_all_levels_of_ann H = rev [1..< i]"
                unfolding M by (auto dest: append_cons_eq_upt_length_i
                  simp add: rev_swap[symmetric])
              then have "get_maximum_possible_level H < i"
                using get_maximum_possible_level_max_get_all_levels_of_ann[of H] \<open>i > 0\<close> by auto
          ...
          moreover
            ...
            then have "get_maximum_level (trail y) (remove1_mset L' (mset_ccls D')) \<ge>
              ...
              using get_maximum_level_ge_get_level by blast
          moreover {
            have "get_all_levels_of_ann (c @ [Decided Kh i]) = rev [i..< backtrack_lvl y+1]"
              using append_cons_eq_upt_length_i_end[of " rev (get_all_levels_of_ann H)" i
                "rev (get_all_levels_of_ann c)" "Suc 0" "Suc (backtrack_lvl y)"] H
            ...
            have "get_level (c @ [Decided Kh i]) L \<ge> i"
              using L_cKh levL
                \<open>get_all_levels_of_ann (c @ [Decided Kh i]) = rev [i..<backtrack_lvl y + 1]\<close>
               calculation(1,2) by auto
            then have "get_level (trail y) L \<ge> i"
              using M \<open>get_level (trail y) L = get_level (c @ [Decided Kh i]) L\<close> by auto }
          moreover
            have "get_maximum_level (trail y) (remove1_mset L' (mset_ccls D'))
               < get_level (trail y) L"
            using \<open>j \<le> backtrack_lvl y\<close> levL j calculation(1-4) by linarith
      ...
      { assume D: "remove1_mset L (mset_ccls D') = {#}"
        ...
        have "\<forall>m \<in> set M1. \<not>is_decided m"
          ...
          by (auto simp add: rev_swap[symmetric] get_all_levels_of_ann_no_decided
      ...
      moreover {
        ...
        have L''M: "atm_of L'' \<in> atm_of ` lits_of_l (trail y)"
          using get_rev_level_ge_0_atm_of_in[of 0 "rev (trail y)" L''] \<open>j>0\<close> levD L''D'
          \<open>j \<le> backtrack_lvl y\<close> levL by (simp add: LL' j)
        then have "L'' \<in> lits_of_l (Decided Kh i # d)"
          proof -
            {
              assume L''H: "atm_of L'' \<in> atm_of ` lits_of_l H"
              have "get_all_levels_of_ann H = rev [1..<i]"
                using H unfolding M
              ...
              ultimately have False
                using levD \<open>j>0\<close> get_rev_level_in_levels_of_decided[of "rev H" 0 L''] \<open>i \<le> j\<close>
                unfolding L''D'[symmetric] nd
                by (metis L''D' LL' Max_n_upt Nat.le_trans One_nat_def Suc_pred \<open>0 < i\<close>
                  get_all_levels_of_ann_rev_eq_rev_get_all_levels_of_ann
                  get_rev_level_less_max_get_all_levels_of_ann j lessI list.simps(15)
                  not_less rev_rev_ident set_upt)
            }
            moreover
...
lemma rtranclp_cdcl\<^sub>W_stgy_with_trail_end_has_not_been_learned:
  assumes
    "(\<lambda>a b. cdcl\<^sub>W_stgy a b \<and> (\<exists>c. trail a = c @ Decided K i # H @ []))\<^sup>*\<^sup>* S z" and
    ...
    DH: "atms_of (remove1_mset L D) \<subseteq> atm_of ` lits_of_l H" and
    LH: "atm_of L \<notin> atm_of ` lits_of_l H" and
    "\<exists>c'. trail z = c' @ Decided K i # H"
  shows "D \<notin># learned_clss z"
  using assms(1-4,8)
proof (induction rule: rtranclp_induct)
  case base
...
proof -
  ...
  with bt obtain L M1 M2_loc K i where
     T: "T \<sim> cons_trail (Propagated L (cls_of_ccls E))
      ...
      and
    decomp: "(Decided K (Suc i) # M1, M2_loc) \<in>
    ...
    using confl by (induction rule: backtrack_induction_lev2) fastforce
  obtain M2 where
  ...
  have get_lvls_M: "get_all_levels_of_ann (trail S) = rev [1..<Suc (backtrack_lvl S)]"
    using lev' unfolding cdcl\<^sub>W_M_level_inv_def by auto
  ...
  obtain M1' K' Ls where
    ...
    proof -
      ...
      moreover
        have "get_all_levels_of_ann (trail S)
                = K'k # get_all_levels_of_ann(tl (dropWhile (Not \<circ> is_decided) (trail S)))"
          apply (subst MLs, subst K'k)
          using calculation(2) by (auto simp add: get_all_levels_of_ann_no_decided)
  ...
  have lvls_M1': "get_all_levels_of_ann M1' = rev [1..<backtrack_lvl S]"
    using get_lvls_M Ls by (auto simp add: get_all_levels_of_ann_no_decided M' upt.simps(2)
      split: if_split_asm)
  have L_notin: "atm_of L \<in> atm_of ` lits_of_l Ls \<or> atm_of L = atm_of K'"
    proof (rule ccontr)
      assume "\<not> ?thesis"
      then have "atm_of L \<notin> atm_of ` lits_of_l (Decided K' (backtrack_lvl S) # rev Ls)" by simp
      then have "get_level (trail S) L = get_level M1' L"
        unfolding M' by auto
      then show False using get_level_in_levels_of_decided[of M1' L] \<open>backtrack_lvl S > 0\<close>
      unfolding k lvls_M1' by auto
    qed
  obtain Y Z where
    RY: "cdcl\<^sub>W_stgy\<^sup>*\<^sup>* R Y" and
    YZ: "cdcl\<^sub>W_stgy Y Z" and
    nt: "\<not> (\<exists>c. trail Y = c @ Decided K' (backtrack_lvl S) # M1' @ [])" and
    Z: "(\<lambda>a b. cdcl\<^sub>W_stgy a b \<and> (\<exists>c. trail a = c @ Decided K' (backtrack_lvl S) # M1' @ []))\<^sup>*\<^sup>* Z S"
    using rtranclp_cdcl\<^sub>W_new_decided_at_beginning_is_decide'[OF  st' _ _ lev, of Ls K'
      "backtrack_lvl S" M1' "[]"] unfolding R M' by auto
      ...
      show ?thesis
        ...
        proof (cases rule: cdcl\<^sub>W_o_rule_cases)
          ...
          ultimately obtain E where
            ...
            by (induction rule: backtrack_induction_lev2) (auto simp: cdcl\<^sub>W_M_level_inv_decomp)
...
proof (induct rule: cdcl\<^sub>W_all_induct_lev2)
  ...
  then have no_dup': "no_dup (Propagated L (mset_cls C) # trail S)"
  ...
  then have  "atm_of ` lits_of_l (Propagated L (mset_cls C) # trail S)
    \<subseteq> atms_of_mm (init_clss S)"
    using undef unfolding no_strange_atm_def by auto
  then have "card (atm_of ` lits_of_l (Propagated L (mset_cls C) # trail S))
    \<le> card (atms_of_mm (init_clss S))"
    by (meson atms_of_ms_finite card_mono finite_set_mset)
  then have "length (Propagated L (mset_cls C) # trail S) \<le> card (atms_of_mm ?N)"
...
next
  ...
  moreover
    ...
    then have cdcl\<^sub>W:"cdcl\<^sub>W S (cons_trail (Decided L (backtrack_lvl S + 1)) (incr_lvl S))"
      using cdcl\<^sub>W.simps cdcl\<^sub>W_o.intros by blast
  moreover
    have lev: "cdcl\<^sub>W_M_level_inv (cons_trail (Decided L (backtrack_lvl S + 1)) (incr_lvl S))"
...
next
  case (forget C T) note no_forget = this(8)
  then have "mset_cls C \<in># learned_clss S" and "mset_cls C \<notin># learned_clss T"
    using forget.hyps by auto
  then show ?case using no_forget by (auto simp add: mset_leD)
qed
