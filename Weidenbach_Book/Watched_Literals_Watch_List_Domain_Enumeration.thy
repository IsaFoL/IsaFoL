theory Watched_Literals_Watch_List_Domain_Enumeration
  imports Watched_Literals_Watch_List_Enumeration Watched_Literals_Watch_List_Domain
begin
...
lemma find_decomp_target_wl_D_find_decomp_target_wl:
  ...
  by (cases S') (auto intro!: RES_refine)
...
lemma negate_mode_bj_unit_wl_D_negate_mode_bj_unit_wl:
  ...
  apply (auto simp: mset_take_mset_drop_mset' uminus_\<A>\<^sub>i\<^sub>n_iff all_lits_of_mm_add_mset
     all_lits_of_m_add_mset in_all_lits_of_mm_uminus_iff
...
proof -
  have find_deomp: \<open>find_decomp_target_wl (count_decided (get_trail_wl S)) S
    ...
    by (cases S') (auto simp: intro!: RES_refine)
  have fresh: \<open>get_fresh_index (get_clauses_wl T)
    \<le> \<Down> {(i, j). i = j \<and> i \<notin># dom_m (get_clauses_wl T)} (get_fresh_index (get_clauses_wl T'))\<close>
...
proof -
  ...
  show ?thesis
    ...
    subgoal using TK SS' ij unfolding restart_nonunit_and_add_wl_D_inv_def
      by auto
    subgoal using TK SS' j unfolding restart_nonunit_and_add_wl_inv_def
      by (fastforce simp: ran_m_mapsto_upd_notin all_lits_of_mm_add_mset
...
proof -
  have fresh: \<open>get_fresh_index (get_clauses_wl T)
    \<le> \<Down> {(i, j). i = j \<and> i \<notin># dom_m (get_clauses_wl S)}
       (get_fresh_index (get_clauses_wl T'))\<close>
