...
"full cdcl_sr S (M, N, U, k, C_Clause (D + {#L#}))
  ...
  \<Longrightarrow> backjump S (Propagated L (D+{#L#}) # M1 , N, U \<union> {D + {#L#}}, i, C_True)"
...
interpretation cdcl_CW: dpll_state trail "\<lambda>S::'v cdcl_state. clauses S \<union> learned_clauses S" "\<lambda>M (_, S'). (M, S')"
  "\<lambda>C (M, N, U, k, D). (M, N, insert C U, k, D)" "\<lambda>C (M, N, U, k, D). (M, N - {C}, U - {C}, k, D)"
  by unfold_locales auto

interpretation cdcl_CW: cdcl_merge_conflict_propagate backjump trail "\<lambda>S::'v::linorder cdcl_state. clauses S \<union> learned_clauses S" "\<lambda>M (_, S'). (M, S')"
  "\<lambda>C (M, N, U, k, D). (M, N, insert C U, k, D)" "\<lambda>C (M, N, U, k, D). (M, N - {C}, U - {C}, k, D)"  cdcl_all_inv_mes "\<lambda>_ _. True"
