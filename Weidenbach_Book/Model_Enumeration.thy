...
lemma Ex_option_model:
  ...
  by (cases \<open>satisfiable (set_mset N)\<close>)
    (fastforce dest!: Ex_sat_model simp: option_model_def intro: exI[of _ \<open>Some _\<close>])+
...
lemma get_model_SomeE':
  assumes
    mod: \<open>get_option_model N = Some M\<close> and
    H: \<open>set M \<Turnstile>sm N  \<Longrightarrow> consistent_interp (set M) \<Longrightarrow>  distinct M \<Longrightarrow>
           consistent_interp (set M)  \<Longrightarrow> atm_of ` set M \<subseteq> atms_of_mm N \<Longrightarrow> P M\<close>
  shows \<open>P M\<close>
  apply (rule H)
...
lemma get_model_NoneE:
  assumes
    mod: \<open>get_option_model N = None\<close> and
    H: \<open>unsatisfiable (set_mset N) \<Longrightarrow> P N\<close>
  shows \<open>P N\<close>
  apply (rule H)
  using mod by (metis Ex_option_model mod option_model_def someI)+


lemma enumerate_model_unless_decreasing:
  assumes
    \<open>SM = get_option_model N\<close> and
...
proof -
  have \<open>M \<in> all_models N\<close>
    using assms unfolding all_models_def
    by (auto elim!: get_model_SomeE simp: true_clss_def true_cls_mset_def)
  moreover {
    have \<open>\<not> set M \<Turnstile> image_mset uminus (mset M)\<close>
      using assms unfolding true_cls_def all_models_def
...
function enumerate_model_unless where
  [simp del]: \<open>enumerate_model_unless P N =
     (let M = get_option_model N in
      case M of
        None \<Rightarrow> None
      | Some M \<Rightarrow>
        if \<not>P (set M)
        then enumerate_model_unless P (add_mset (image_mset uminus (mset M)) N)
        else Some M)\<close>
  by pat_completeness auto

termination enumerate_model_unless
  apply (relation \<open>measure (\<lambda>(P, N). card (all_models N))\<close>)
  subgoal by auto
  subgoal by (rule enumerate_model_unless_decreasing)
  done

lemma get_option_model_Some_next_model: \<open>get_option_model N = Some M \<Longrightarrow> next_model M N\<close>
  by (auto simp: next_model.simps
      elim!: get_model_SomeE')

lemma unsatisfiable_no_next_model: \<open>unsatisfiable (set_mset N) \<Longrightarrow> \<not>next_model M N\<close>
  by (auto simp: next_model.simps)

lemma enumerate_model_unless_full_next_model_filtered:
  \<open>enumerate_model_unless P N \<in> {M. \<exists>N'. full (next_model_filtered P) (None, N) (M, N')}\<close>
proof (induction P N rule: enumerate_model_unless.induct)
  case (1 P N) note IH = this(1)
  consider
    (sat_ok) M where \<open>get_option_model N = Some M\<close> and \<open>P (set M)\<close> |
    (sat_not_ok) M where \<open>get_option_model N = Some M\<close> and \<open>\<not>P (set M)\<close> |
    (unsat) \<open>get_option_model N = None\<close>
    by blast
  then show ?case
  proof cases
    case unsat
    then have \<open>no_step (next_model_filtered P) (None, N)\<close>
      by (auto simp: next_model_filtered.simps
          elim!: get_model_NoneE dest: unsatisfiable_no_next_model)
    then show ?thesis
      using unsat
      by (subst enumerate_model_unless.simps)
        (auto simp: full_def)
  next
    case sat_ok
    then have \<open>(next_model_filtered P) (None, N) (Some M, N)\<close>
      by (auto simp: get_option_model_Some_next_model intro!: next_model_filtered.intros)
    moreover have \<open>no_step (next_model_filtered P) (Some M, N)\<close>
      by (auto simp: next_model_filtered.simps)
  ...
  next
    case sat_not_ok
    from IH[OF this(1)[symmetric], OF refl this(2)]
    obtain N' where
      \<open>full (next_model_filtered P) (None, add_mset (image_mset uminus (mset M)) N)
         (enumerate_model_unless P (add_mset (image_mset uminus (mset M)) N), N')\<close>
      by auto
    moreover have \<open>(next_model_filtered P) (None, N)
               (None, add_mset (image_mset uminus (mset M)) N)\<close>
      using sat_not_ok
      by (auto simp: get_option_model_Some_next_model intro!: next_model_filtered.intros)
    ultimately have \<open>full (next_model_filtered P) (None, N)
               (enumerate_model_unless P (add_mset (image_mset uminus (mset M)) N), N')\<close>
      by (auto dest: full_fullI)
    then show ?thesis
      using sat_not_ok
      by (subst enumerate_model_unless.simps)
        (auto simp: full_def)
