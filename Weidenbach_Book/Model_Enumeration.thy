...
function enumerate_model_while where
  \<open>enumerate_model_while P N =
     (let M = get_option_model N in
      ...
      | Some M \<Rightarrow>
        if P (mset M)
        then enumerate_model_while P (add_mset (image_mset uminus (mset M)) N)
        else Some M)\<close>
...
lemma no_step_next_model_filtered_unsat:
  ...
  by (metis Ex_option_model Model_Enumeration.next_model_filtered.simps
      assms next_model.intros option_model_def)
