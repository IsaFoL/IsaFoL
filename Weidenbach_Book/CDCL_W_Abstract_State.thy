...
definition "conc_init_clss \<equiv> \<lambda>S. mset_clss (raw_clauses S) - conc_learned_clss S"
...
lemma
  ...
  resolve_conflicting[of st _ _ _ _ F L' D]
  unfolding state_def Product_Type.prod.inject by fast+
...
lemma clauses_state_conc_clauses[simp]: "cdcl\<^sub>W_restart_mset.clauses (state S) = conc_clauses S"
  apply (cases "state S")
  unfolding cdcl\<^sub>W_restart_mset.clauses_def conc_clauses_init_learned
  unfolding cdcl\<^sub>W_restart_mset_state state_def
  by simp
...
lemma tl_trail_state_tl_con_trail[simp]:
  ...
  by (auto simp: cdcl\<^sub>W_restart_mset_state state_def simp del: trail_state_conc_trail
...
lemma add_learned_cls_state_add_conc_confl_to_learned_cls[simp]:
  ...
  using assms by (auto simp: cdcl\<^sub>W_restart_mset_state state_def simp del: trail_state_conc_trail
    init_clss_state_conc_init_clss
    learned_clss_state_conc_learned_clss local.state_simp)
...
lemma conc_backtrack_lvl_backtrack_lvl[simp]:
  ...
  unfolding state_def by (auto simp: cdcl\<^sub>W_restart_mset_state)
...
proof
  ...
  show ?mset
    ...
    using undef T unfolding cdcl\<^sub>W_restart_mset_state_eq_eq state_def cons_trail_def by simp
next
  assume ?mset
  then obtain E L where
    ...
    by (auto elim!: cdcl\<^sub>W_restart_mset.propagateE dest!: in_clauses_preimage
...
proof -
  ...
  moreover have "T \<sim>m state ?U"
    using T SS' undef by (auto simp: cdcl\<^sub>W_restart_mset_state_eq_eq)
...
proof -
  ...
  have "conflict_abs S' ?U"
    ...
    using T by auto
  moreover have "T \<sim>m state ?U"
    using T SS' confl by (auto simp: cdcl\<^sub>W_restart_mset_state_eq_eq)
...
proof
  ...
  show ?conc
    apply (rule cdcl\<^sub>W_restart_mset.backtrack_rule)
      ...
      using i apply simp
     using lev_K apply simp
    using T undef n_d tr D unfolding cdcl\<^sub>W_restart_mset.state_eq_def
    by auto
next
  assume ?conc
...
proof -
  ...
  moreover have "T \<sim>m state ?U"
    using undef decomp T n_d SS'[unfolded cdcl\<^sub>W_restart_mset_state_eq_eq] confl' by auto
  ultimately show thesis using that[of ?U] by fast
qed
