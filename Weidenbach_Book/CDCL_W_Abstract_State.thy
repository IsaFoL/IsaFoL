...
locale relation_implied_relation_abs =
  fixes
    R :: "'v cdcl\<^sub>W_restart_mset \<Rightarrow> 'v cdcl\<^sub>W_restart_mset \<Rightarrow> bool" and
    R_abs :: "'st \<Rightarrow> 'st \<Rightarrow> bool" and
    state :: "'st \<Rightarrow> 'v cdcl\<^sub>W_restart_mset" and
    inv :: "'v cdcl\<^sub>W_restart_mset \<Rightarrow> bool"
  assumes
...
proof -
  ...
  then show ?thesis
    using relation_abs_right_compatible[OF _ S'U, of T'] TT' inv SS'[unfolded cdcl\<^sub>W_restart_mset_state_eq_eq]
    cdcl\<^sub>W_restart_mset.state_eq_trans[of "state T'" "state T" "state U"]
    by (auto simp add: cdcl\<^sub>W_restart_mset.state_eq_sym)
qed
...
lemma no_step_iff:
  ...
  using relation_compatible_state relation_compatible_abs cdcl\<^sub>W_restart_mset.state_eq_ref
  by blast
...
proof (induction arbitrary: U rule: tranclp_induct)
  ...
  moreover then have "inv (state U)"
    by (metis (full_types) cdcl\<^sub>W_restart_mset_state_eq_eq inv relation_compatible_state relation_invariant)
...
proof
  ...
  then have "R_abs\<^sup>+\<^sup>+ S T"
    ...
    tranclp_relation_compatible_eq[of S _ S T] cdcl\<^sub>W_restart_mset.state_eq_sym by blast
...
lemma full1_iff_compatible:
  ...
  using full1_iff assms unfolding cdcl\<^sub>W_restart_mset_state_eq_eq by simp

lemma full_if_full_abs:
  assumes "inv (state S)" and "full R_abs S T"
  shows "full R (state S) (state T)"
  using assms full1_iff cdcl\<^sub>W_restart_mset_state_eq_eq relation_compatible_abs
...
proof -
  ...
  using full unfolding full_unfold cdcl\<^sub>W_restart_mset_state_eq_eq by fast
  then show ?thesis
    proof cases
      case 0
      then show ?thesis using that[of S] unfolding full_def
        using cdcl\<^sub>W_restart_mset.state_eq_ref inv relation_compatible_state rtranclp.rtrancl_refl by blast
    next
      case full1
      then obtain U where
      ...
      then show ?thesis
        ...
        unfolding cdcl\<^sub>W_restart_mset_state_eq_eq by blast
    qed
qed
...
proof -
  obtain U where
    ...
    by blast
  then show ?thesis
    using full1 that[of U] full1_iff[OF inv] unfolding cdcl\<^sub>W_restart_mset_state_eq_eq by blast
qed
...
lemma full1_right_compatible:
  ...
  by (metis (full_types) TV assms(1) cdcl\<^sub>W_restart_mset_state_eq_eq full1 full1_iff)
...
lemma relation_compatible_eq:
  ...
  by (simp add: cdcl\<^sub>W_restart_mset_state_eq_eq relation_compatible_state[symmetric])
...
definition state :: "'st \<Rightarrow> 'v cdcl\<^sub>W_restart_mset" where
"state = (\<lambda>S. (conc_trail S, conc_init_clss S, conc_learned_clss S, conc_conflicting S))"
...
text \<open>
  ...
  the conflicting clause to @{term None}.


  To ease the representation, we consider the clauses all together, where some of them are learned.
  This eases representation like arrays where the initial set of clause is at the beginning and
  avoid having an explicit @{term union} operator.\<close>
locale abs_state\<^sub>W =
  abs_state\<^sub>W_ops
    ...
    conc_trail hd_raw_conc_trail raw_clauses
    raw_conflicting conc_learned_clss
      \<comment> \<open>setter:\<close>
    cons_conc_trail tl_conc_trail add_conc_confl_to_learned_cls remove_cls
  ...
  assumes
    ...
    hd_raw_conc_trail:
      "conc_trail st \<noteq> [] \<Longrightarrow>
        mmset_of_mlit (raw_clauses st) (hd_raw_conc_trail st) = hd (conc_trail st)" and

    cons_conc_trail:
      "\<And>S'. undefined_lit (conc_trail st) (lit_of L) \<Longrightarrow>
        state st = (M, S') \<Longrightarrow> valid_annotation st L \<Longrightarrow>
        state (cons_conc_trail L st) = (mmset_of_mlit (raw_clauses st) L # M, S')" and
    ...
    add_conc_confl_to_learned_cls:
      "no_dup (conc_trail st) \<Longrightarrow> state st = (M, N, U, Some F) \<Longrightarrow>
        state (add_conc_confl_to_learned_cls st) =
          (M, N, {#F#} + U, None)" and

    mark_conflicting:
      "state st = (M, N, U, None) \<Longrightarrow> E \<in>\<Down> raw_clauses st \<Longrightarrow>
        state (mark_conflicting E st) = (M, N, U, Some (mset_cls (raw_clauses st \<Down> E)))" and

    resolve_conflicting:
      "state st = (M, N, U, Some F) \<Longrightarrow> -L' \<in># F \<Longrightarrow> L' \<in># mset_cls D \<Longrightarrow>
        state (resolve_conflicting L' D st) =
          (M, N, U, Some (cdcl\<^sub>W_restart_mset.resolve_cls L' F (mset_cls D)))" and

    conc_init_state:
      "state (conc_init_state Ns) = ([], clauses_of_clss Ns, {#}, None)" and
    ...
    reduce_conc_trail_to[simp]:
      "\<And>S'. conc_trail st = M2 @ M1 \<Longrightarrow> state st = (M, S') \<Longrightarrow>
        state (reduce_conc_trail_to M1 st) = (M1, S')" and

    learned_clauses:
      "conc_learned_clss S \<subseteq># conc_clauses S"
begin
...
lemma
  ...
  using tl_conc_trail[of st]
  add_conc_confl_to_learned_cls[of st  "conc_trail st"]
  mark_conflicting[of st _ _ _ E]
  remove_cls[of st _ _ _ _ C]
  reduce_conc_trail_to[of st M2 M1]
  resolve_conflicting[of st _ _ _ F L' D]
  unfolding state_def Product_Type.prod.inject
  by (fastforce; fail)+

lemma
    \<comment> \<open>Properties about the trail @{term conc_trail}:\<close>
    conc_trail_cons_conc_trail[simp]:
      "undefined_lit (conc_trail st) (lit_of L) \<Longrightarrow> valid_annotation st L \<Longrightarrow>
        conc_trail (cons_conc_trail L st) = mmset_of_mlit (raw_clauses st) L # conc_trail st" and
    ...
    conc_conflicting_clss_clss_resolve_conflicting[simp]:
      "conc_conflicting st = Some F \<Longrightarrow> -L' \<in># F \<Longrightarrow> L' \<in># mset_cls D \<Longrightarrow>
        conc_conflicting (resolve_conflicting L' D st) =
          Some (cdcl\<^sub>W_restart_mset.resolve_cls L' F (mset_cls D))" and
    ...
    conc_init_state_clss[simp]: "conc_init_clss (conc_init_state Ns) = clauses_of_clss Ns" and
  ...
  resolve_conflicting[of st _ _ _ F L' D]
  unfolding state_def Product_Type.prod.inject by auto
  (* TODO very slow ~ 12s, but very stupid proofs *)

text \<open>Kill once porting finished\<close>
abbreviation incr_lvl :: "'st \<Rightarrow> bool" where
"incr_lvl S \<equiv> \<not>False"
...
lemma state_eq_sym:
  ...
  using cdcl\<^sub>W_restart_mset.state_eq_sym by blast

lemma state_eq_trans:
  "S \<sim> T \<Longrightarrow> T \<sim> U \<Longrightarrow> S \<sim> U"
  using cdcl\<^sub>W_restart_mset.state_eq_trans by blast

lemma conc_clauses_init_learned: "conc_clauses S = conc_init_clss S + conc_learned_clss S"
  using learned_clauses[of S] by (auto simp: conc_init_clss_def multiset_eq_iff subseteq_mset_def)

lemma
  init_clss_conc_init_clss[simp]:
    "init_clss (state S) = conc_init_clss S" and
  learned_clss_conc_learned_clss[simp]:
    "learned_clss (state S) = conc_learned_clss S"
  by (auto simp: cdcl\<^sub>W_restart_mset_state state_def)

lemma clauses_conc_clauses[simp]:
  "cdcl\<^sub>W_restart_mset.clauses (state S) = conc_clauses S"
  unfolding conc_clauses_init_learned cdcl\<^sub>W_restart_mset.clauses_def by auto

lemma
  trail_conc_trail[simp]:
    "trail (state S) = conc_trail S" and
  conflicting_conc_conflicting[simp]:
...
lemmas state_simp = state_eq_conc_trail state_eq_conc_init_clss state_eq_conc_learned_clss
  state_eq_conc_conflicting state_eq_clauses state_eq_undefined_lit
...
lemma clauses_reduce_conc_trail_to[simp]:
  ...
  unfolding conc_clauses_init_learned by auto

lemma in_get_all_ann_decomposition_clauses_reduce_conc_trail_to[simp]:
  "(L # M1, M2) \<in> set (get_all_ann_decomposition (conc_trail S)) \<Longrightarrow>
    conc_clauses (reduce_conc_trail_to M1 S) = conc_clauses S"
  unfolding conc_clauses_init_learned by auto
...
lemma state_cons_cons_trail_cons_trail[simp]:
  "undefined_lit (trail (state S)) (lit_of L) \<Longrightarrow> valid_annotation S L \<Longrightarrow>
    cons_trail (mmset_of_mlit (raw_clauses S) L) (state S) = state (cons_conc_trail L S)"
  by (auto simp: cdcl\<^sub>W_restart_mset_state state_def)

lemma state_cons_cons_trail_cons_trail_propagated[simp]:
  "undefined_lit (trail (state S)) K \<Longrightarrow>  C \<in>\<Down> raw_clauses S \<Longrightarrow>
    cons_trail (Propagated K (mset_cls (raw_clauses S \<Down> C))) (state S)
      = state (cons_conc_trail (Propagated K C) S)"
...
mset_cls (T \<Down> D') = mset_ccls D \<Longrightarrow>
  add_learned_cls (mset_cls (T \<Down> D')) (update_conflicting None (state S)) =
  state (add_conc_confl_to_learned_cls S)"
  by (auto simp: cdcl\<^sub>W_restart_mset_state state_def)

abbreviation conc_backtrack_lvl where
\<open>conc_backtrack_lvl S \<equiv> count_decided (conc_trail S)\<close>

lemma backtrack_lvl_state[simp]:
  \<open>backtrack_lvl (local.state S) = conc_backtrack_lvl S\<close>
  unfolding state_def by (simp add: trail.simps)

lemma state_reduce_conc_trail_to_reduce_conc_trail_to[simp]:
  assumes [simp]: "conc_trail S = M2 @ M1"
  shows "cdcl\<^sub>W_restart_mset.reduce_trail_to M1 (state S) = state (reduce_conc_trail_to M1 S)" (is "?RS = ?SR")
proof -
...
lemma conc_clauses_add_conc_confl_to_learned_cls[simp]:
  "conc_conflicting S = Some C \<Longrightarrow> no_dup (conc_trail S) \<Longrightarrow>
...
lemma in_clauses_preimage:
  assumes b: "b \<in># cdcl\<^sub>W_restart_mset.clauses (state C)"
  shows "\<exists>b'. b' \<in>\<Down> raw_clauses C \<and> mset_cls ((raw_clauses C) \<Down> b') = b"
proof -
...
lemma state_reduce_conc_trail_to_reduce_conc_trail_to_decomp[simp]:
  ...
  shows "cdcl\<^sub>W_restart_mset.reduce_trail_to M1 (state S) = state (reduce_conc_trail_to M1 S)"
  using assms by auto

end \<comment> \<open>end of \<open>abs_state\<^sub>W\<close> locale\<close>
...
lemma propagate_propagate_abs:
  "cdcl\<^sub>W_restart_mset.propagate (state S) (state T) \<longleftrightarrow> propagate_abs S T" (is "?mset \<longleftrightarrow> ?abs")
proof
  assume ?abs
  then obtain E L where
    confl: "conc_conflicting S = None" and
    E: "E \<in>\<Down> raw_clauses S" and
    L: "L \<in># mset_cls (raw_clauses S \<Down> E)" and
  ...
  show ?mset
    apply (rule cdcl\<^sub>W_restart_mset.propagate_rule)
...
next
  ...
  then obtain E L where
    ...
    by (fastforce elim!: cdcl\<^sub>W_restart_mset.propagateE dest!: in_clauses_preimage
      simp: cdcl\<^sub>W_restart_mset.clauses_def)
  then show ?abs
    by (auto intro!: propagate_abs_rule)
qed

lemma propagate_compatible_abs:
  assumes SS': "S \<sim>m state S'" and abs: "cdcl\<^sub>W_restart_mset.propagate S T"
  obtains U where "propagate_abs S' U" and "T \<sim>m state U"
proof -
  obtain E L where
    confl: "conflicting S = None" and
    E: "E \<in># cdcl\<^sub>W_restart_mset.clauses S" and
    L: "L \<in># E" and
    tr: "trail S \<Turnstile>as CNot (E - {#L#})" and
    undef: "undefined_lit (trail S) L" and
    T: "T \<sim>m cons_trail (Propagated L E) S"
    using abs by (auto elim!: cdcl\<^sub>W_restart_mset.propagateE dest!: in_clauses_preimage
      simp: cdcl\<^sub>W_restart_mset.clauses_def)
  then obtain E' where
    E': "E' \<in>\<Down> raw_clauses S'" and [simp]: "E = mset_cls (raw_clauses S' \<Down> E')"
    by (metis SS' cdcl\<^sub>W_restart_mset.state_eq_clauses in_clauses_preimage)
  ...
  moreover have "T \<sim>m state ?U"
    using T SS' undef E' by (auto simp: cdcl\<^sub>W_restart_mset_state_eq_eq)
  ultimately show thesis using that by blast
qed

interpretation propagate_abs: relation_relation_abs cdcl\<^sub>W_restart_mset.propagate propagate_abs state
  "\<lambda>_. True"
  apply unfold_locales
...
conflict_abs_rule: "
  ...
  D \<in>\<Down> raw_clauses S \<Longrightarrow>
  conc_trail S \<Turnstile>as CNot (mset_cls (raw_clauses S \<Down> D)) \<Longrightarrow>
  T \<sim> mark_conflicting D S \<Longrightarrow>
  conflict_abs S T"

inductive_cases conflict_absE: "conflict_abs S T"

lemma conflict_conflict_abs:
  "cdcl\<^sub>W_restart_mset.conflict (state S) (state T) \<longleftrightarrow> conflict_abs S T" (is "?mset \<longleftrightarrow> ?abs")
proof
  assume ?abs
  then obtain D where
    confl: "conc_conflicting S = None" and
    D: "D \<in>\<Down> raw_clauses S" and
    tr_D: "conc_trail S \<Turnstile>as CNot (mset_cls (raw_clauses S \<Down> D))" and
    T: "T \<sim> mark_conflicting D S"
    by (auto elim!: conflict_absE)
  show ?mset
    apply (rule cdcl\<^sub>W_restart_mset.conflict_rule)
...
next
  ...
  then obtain D where
    ...
    D: "D \<in># cdcl\<^sub>W_restart_mset.clauses (state S)" and
    tr_D: "trail (state S) \<Turnstile>as CNot D" and
    T: "state T \<sim>m update_conflicting (Some D) (state S)"
    by (cases "state S") (auto elim: cdcl\<^sub>W_restart_mset.conflictE)
  ...
  show ?abs
    apply (rule conflict_abs_rule)
      ...
      using D' apply simp
     using tr_D apply simp
    using T confl D' by auto
qed

lemma conflict_compatible_abs:
  assumes SS': "S \<sim>m state S'" and conflict: "cdcl\<^sub>W_restart_mset.conflict S T"
  obtains U where "conflict_abs S' U" and "T \<sim>m state U"
proof -
  obtain D where
    confl: "conflicting S = None" and
    D: "D \<in># cdcl\<^sub>W_restart_mset.clauses S" and
    tr_D: "trail S \<Turnstile>as CNot D" and
    T: "T \<sim>m update_conflicting (Some D) S"
    using conflict by (auto elim: cdcl\<^sub>W_restart_mset.conflictE)
  obtain D' where D': "D' \<in>\<Down> raw_clauses S'" and DD'[simp]: "D = mset_cls (raw_clauses S' \<Down> D')"
  ...
  have "conflict_abs S' ?U"
    ...
    using T by auto
  moreover have "T \<sim>m state ?U"
    using T SS' confl D' by (auto simp: cdcl\<^sub>W_restart_mset_state_eq_eq)
  ultimately show thesis using that[of ?U] by fast
qed

interpretation conflict_abs: relation_relation_abs cdcl\<^sub>W_restart_mset.conflict conflict_abs state
  "\<lambda>_. True"
  apply unfold_locales
...
text \<open>In the backtrack rule, we assume the existence of an index @{term D'} such that the clause is
  equal to the one use to backtrack.
  \<^enum> the clause @{term D} was added to the state by @{term add_conc_confl_to_learned_cls}
  \<^enum> therefore, the index @{term D'} exists. \<close>
inductive backtrack_abs :: "'st \<Rightarrow> 'st \<Rightarrow> bool" for S :: 'st where
backtrack_abs_rule: "
  conc_conflicting S = Some D \<Longrightarrow>
  ...
  get_level (conc_trail S) L = get_maximum_level (conc_trail S) D \<Longrightarrow>
  get_maximum_level (conc_trail S) (D - {#L#}) \<equiv> i \<Longrightarrow>
  get_level (conc_trail S) K = i + 1 \<Longrightarrow>
  mset_cls
    (raw_clauses (reduce_conc_trail_to M1 (add_conc_confl_to_learned_cls S)) \<Down> D') = D \<Longrightarrow>
  D' \<in>\<Down> raw_clauses (reduce_conc_trail_to M1 (add_conc_confl_to_learned_cls S))  \<Longrightarrow>
  T \<sim> cons_conc_trail (Propagated L D')
        (reduce_conc_trail_to M1
          (add_conc_confl_to_learned_cls S)) \<Longrightarrow>
  backtrack_abs S T"
...
proof
  assume ?abs
  then obtain D D' L K M1 M2 i where
  D: "conc_conflicting S = Some D" and
  L: "L \<in># D" and
  ...
  lev_Max: "get_level (conc_trail S) L = get_maximum_level (conc_trail S) D" and
  i: "get_maximum_level (conc_trail S) (D - {#L#}) \<equiv> i" and
  lev_K: "get_level (conc_trail S) K = i + 1" and
  D': "mset_cls (raw_clauses (reduce_conc_trail_to M1 (add_conc_confl_to_learned_cls S)) \<Down> D') = D" and
  D'T: "D' \<in>\<Down> raw_clauses (reduce_conc_trail_to M1 (add_conc_confl_to_learned_cls S))" and
  T: "T \<sim> cons_conc_trail (Propagated L D')
        (reduce_conc_trail_to M1
          (add_conc_confl_to_learned_cls S))"
...
next
  ...
  then obtain L D K M1 M2 i where
    ...
    by (auto elim!: cdcl\<^sub>W_restart_mset.backtrackE)
  let ?S' = "reduce_conc_trail_to M1 (add_conc_confl_to_learned_cls S)"
   have n_d: "no_dup (trail (state S))"
    using lev_L inv unfolding cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_all_struct_inv_def cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_M_level_inv_def
    by simp
  have "atm_of L \<notin> atm_of ` lits_of_l M1"
  ...
  then have undef: "undefined_lit M1 L"
    by (auto simp add: defined_lit_map lits_of_def)
  have "\<exists>z'. raw_conflicting S = Some z' \<and> mset_ccls z' = D"
    using confl decomp n_d by auto
  then have "D \<in># conc_clauses ?S'"
     using confl decomp n_d by auto
  with in_clauses_preimage[of D ?S'] obtain D' where
     confl': "D' \<in>\<Down> raw_clauses ?S'" and D[simp]: "D = mset_cls (raw_clauses ?S' \<Down> D')"
     by (auto simp: )
  ...
  show ?abs
    apply (rule backtrack_abs_rule)
      ...
      using i apply (simp; fail)
     using lev_K apply (simp; fail)
    using T undef n_d decomp confl' confl by auto
qed
...
proof -
  ...
  then have undef: "undefined_lit M1 L"
    by (auto simp add: defined_lit_map lits_of_def)
  let ?S = "reduce_conc_trail_to M1
            (add_conc_confl_to_learned_cls S')"
  have "D \<in># conc_clauses ?S"
   using confl decomp n_d SS' by auto
  then obtain D'' where
    D'': "D'' \<in>\<Down> raw_clauses ?S" and [simp]: "mset_ccls D' = mset_cls (raw_clauses ?S \<Down> D'')"
    using in_clauses_preimage[of D ?S] by auto
  let ?U = "cons_conc_trail (Propagated L D'') ?S"
  have "backtrack_abs S' ?U"
    apply (rule backtrack_abs_rule)
           using confl' apply (simp; fail)
          using L apply (simp; fail)
         using decomp SS' apply (simp; fail)
        using lev_L SS' apply (simp; fail)
       using lev_max SS' apply (simp; fail)
      using i SS' apply (simp; fail)
     using lev_K  SS' apply (simp; fail)
    using T undef n_d D'' decomp by auto
  moreover have "T \<sim>m state ?U"
    using undef decomp T n_d SS'[unfolded cdcl\<^sub>W_restart_mset_state_eq_eq] confl' D''
    by auto
  ultimately show thesis using that[of ?U] by fast
qed
...
decide_abs_rule:
  ...
  T \<sim> cons_conc_trail (Decided L) S \<Longrightarrow>
  decide_abs S T"

inductive_cases decide_absE: "decide_abs S T"

lemma decide_decide_abs:
  "cdcl\<^sub>W_restart_mset.decide (state S) (state T) \<longleftrightarrow> decide_abs S T"
  by (auto elim!: cdcl\<^sub>W_restart_mset.decideE decide_absE intro!: cdcl\<^sub>W_restart_mset.decide_rule decide_abs_rule)

interpretation decide_abs: relation_relation_abs cdcl\<^sub>W_restart_mset.decide decide_abs state
  "\<lambda>_. True"
  apply unfold_locales
     apply (simp add: decide_decide_abs; fail)
    apply (metis (full_types) cdcl\<^sub>W_restart_mset.decide.cases cdcl\<^sub>W_restart_mset_state_eq_eq
      decide_decide_abs state_cons_cons_trail_cons_trail_decided trail_conc_trail)
  using cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_all_struct_inv_inv cdcl\<^sub>W_restart_mset.decide cdcl\<^sub>W_restart_mset.other
  by blast
...
lemma skip_skip_abs:
  "cdcl\<^sub>W_restart_mset.skip (state S) (state T) \<longleftrightarrow> skip_abs S T" (is "?conc \<longleftrightarrow> ?abs")
proof
  assume ?abs
  then show ?conc
    by (auto elim!: skip_absE intro!: cdcl\<^sub>W_restart_mset.skip_rule)
next
  assume ?conc
  then obtain L C' E M where
    ...
    by (auto elim: cdcl\<^sub>W_restart_mset.skipE)
  obtain E' where
...
lemma skip_exists_skip_abs:
  assumes skip: "cdcl\<^sub>W_restart_mset.skip S T" and SS': "S \<sim>m state S'"
...
proof -
  obtain L C' E M where
    ...
    using skip by (auto elim: cdcl\<^sub>W_restart_mset.skipE)
  obtain E' where
    confl': "raw_conflicting S' = Some E'" and [simp]: "E = mset_ccls E'"
    using confl SS' by auto
  have "skip_abs S' (tl_conc_trail S')"
    ...
    using T by simp
  then show ?thesis
    using that[of "tl_conc_trail S'"] T SS'[unfolded cdcl\<^sub>W_restart_mset_state_eq_eq ] by auto
qed

interpretation skip_abs: relation_relation_abs cdcl\<^sub>W_restart_mset.skip skip_abs state
  "\<lambda>_. True"
  apply unfold_locales
...
lemma resolve_resolve_abs:
  "cdcl\<^sub>W_restart_mset.resolve (state S) (state T) \<longleftrightarrow> resolve_abs S T" (is "?conc \<longleftrightarrow> ?abs")
proof
  assume ?conc
  then obtain L E D where
    tr: "trail (state S) \<noteq> []" and
    hd: "cdcl\<^sub>W_restart_mset.hd_trail (state S) = Propagated L E" and
    LE: "L \<in># E" and
    confl: "conflicting (state S) = Some D" and
    LD: "-L \<in># D" and
    lvl_max: "get_maximum_level (trail (state S)) ((remove1_mset (-L) D)) = backtrack_lvl (state S)" and
    T: "state T \<sim>m update_conflicting (Some (cdcl\<^sub>W_restart_mset.resolve_cls L D E)) (tl_trail (state S))"
    by (auto elim!: cdcl\<^sub>W_restart_mset.resolveE)
  obtain E' where
    hd': "hd_raw_conc_trail S = Propagated L E'" and
    [simp]: "E = mset_cls (raw_clauses S \<Down> E')"
...
next
  assume ?abs
  then show ?conc
    using hd_raw_conc_trail[of S] by (auto elim!: resolve_absE intro!: cdcl\<^sub>W_restart_mset.resolve_rule)
qed

lemma resolve_exists_resolve_abs:
  assumes
    res: "cdcl\<^sub>W_restart_mset.resolve S T" and
    SS': "S \<sim>m state S'"
  obtains U where "resolve_abs S' U" and "T \<sim>m state U"
proof -
  obtain L E D where
    tr: "trail S \<noteq> []" and
    hd: "cdcl\<^sub>W_restart_mset.hd_trail S = Propagated L E" and
    LE: "L \<in># E" and
    confl: "conflicting S = Some D" and
    LD: "-L \<in># D" and
    lvl_max: "get_maximum_level (trail S) ((remove1_mset (-L) D)) = backtrack_lvl S" and
    T: "T \<sim>m update_conflicting (Some (cdcl\<^sub>W_restart_mset.resolve_cls L D E)) (tl_trail S)"
    using res
    by (auto elim!: cdcl\<^sub>W_restart_mset.resolveE)
  obtain E' where
    hd': "hd_raw_conc_trail S' = Propagated L E'" and
...
interpretation resolve_abs: relation_relation_abs cdcl\<^sub>W_restart_mset.resolve resolve_abs state
  "\<lambda>_. True"
  apply unfold_locales
...
resolve: "resolve_abs S S' \<Longrightarrow> cdcl\<^sub>W_restart_abs_bj S S'" |
backtrack: "backtrack_abs S S' \<Longrightarrow> cdcl\<^sub>W_restart_abs_bj S S'"
...
bj: "cdcl\<^sub>W_restart_abs_bj S S' \<Longrightarrow> cdcl\<^sub>W_restart_abs_o S S'"
...
fw_s_conflict': "conflict_abs S T \<Longrightarrow> cdcl\<^sub>W_merge_abs_stgy S T" |
fw_s_propagate': "propagate_abs S T \<Longrightarrow> cdcl\<^sub>W_merge_abs_stgy S T" |
fw_s_decide': "no_step conflict_abs S \<Longrightarrow> no_step propagate_abs S \<Longrightarrow> decide_abs S T
  \<Longrightarrow> cdcl\<^sub>W_merge_abs_stgy S T" |
fw_s_bj': "full1 cdcl\<^sub>W_restart_abs_bj S T \<Longrightarrow> cdcl\<^sub>W_merge_abs_stgy S T"
...
lemma cdcl\<^sub>W_cp_cdcl\<^sub>W_restart_abs_cp:
  ...
  shows "cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_s' (state S) (state T)"
  using stgy
proof (induction rule: cdcl\<^sub>W_merge_abs_stgy.induct)
  case (fw_s_conflict' T)
  show ?case
    apply (rule cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_s'.conflict')
    using fw_s_conflict' inv conflict_abs.relation_compatible_state by blast
next
  case (fw_s_propagate' T)
  show ?case
    apply (rule cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_s'.propagate')
    using fw_s_propagate' inv propagate_abs.relation_compatible_state by blast
next
  case (fw_s_decide' T)
  show ?case
    apply (rule cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_s'.decide')
      using fw_s_decide' inv conflict_abs.no_step_iff apply blast
     using fw_s_decide' inv propagate_abs.no_step_iff apply blast
    using fw_s_decide' inv decide_abs.relation_compatible_state by blast
next
  case (fw_s_bj' T)
  show ?case
    apply (rule cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_s'.bj')
    using fw_s_bj' inv cdcl\<^sub>W_restart_abs_bj.full1_iff[of S T] by blast
qed

lemma cdcl\<^sub>W_merge_abs_stgy_exists_cdcl\<^sub>W_s':
  assumes
    inv: "cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_all_struct_inv S" and
    SS': "S \<sim>m state S'" and
    st: "cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_s' S T"
  shows "\<exists>U. cdcl\<^sub>W_merge_abs_stgy S' U \<and> T \<sim>m state U"
  using st
proof (cases rule: cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_s'.cases)
  case conflict'
  then show ?thesis
    by (meson SS' conflict_compatible_abs fw_s_conflict')
next
  case propagate'
  then show ?thesis
    by (meson SS' propagate_compatible_abs fw_s_propagate')
next
  case decide' note dec = this(3) and n_s = this(1,2) and full = this(3)
  have SS': "S = state S'"
    using SS' unfolding cdcl\<^sub>W_restart_mset_state_eq_eq .
  obtain T' where "decide_abs S' T'" and TT': "T \<sim>m state T'"
    using dec decide_abs.relation_compatible_abs[of S S' T] SS' by auto
  moreover have "no_step conflict_abs S'" "no_step propagate_abs S'"
    using n_s conflict_abs.no_step_iff propagate_abs.no_step_iff inv unfolding SS' by blast+
...
lemma cdcl\<^sub>W_merge_abs_stgy_right_compatible:
  ...
  using st TU
proof (cases rule: cdcl\<^sub>W_merge_abs_stgy.cases)
  case (fw_s_conflict')
  then show ?thesis
     using inv TU cdcl\<^sub>W_merge_abs_stgy.fw_s_conflict' conflict_abs.relation_right_compatible
     by blast
next
  case fw_s_propagate'
  then show ?thesis
     using inv TU  cdcl\<^sub>W_merge_abs_stgy.fw_s_propagate' propagate_abs.relation_right_compatible
     by blast
next
  case fw_s_decide' note dec = this(3) and n_s = this(1,2) and full = this(3)
  moreover have "decide_abs S V"
    using TU decide_abs.relation_right_compatible full by blast
  ultimately show ?thesis
    using cdcl\<^sub>W_merge_abs_stgy.fw_s_decide' by blast
next
  case fw_s_bj'
  show ?thesis
    apply (rule cdcl\<^sub>W_merge_abs_stgy.fw_s_bj')
    using TU cdcl\<^sub>W_restart_abs_bj.full1_right_compatible inv local.fw_s_bj' by blast
qed

interpretation cdcl\<^sub>W_merge_abs_stgy: relation_implied_relation_abs
  cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_s' cdcl\<^sub>W_merge_abs_stgy state cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_all_struct_inv
  apply unfold_locales
...
lemma cdcl\<^sub>W_merge_abs_stgy_final_State_conclusive:
  ...
  assumes
    ...
    n_d: "distinct_mset_mset (clauses_of_clss N)"
  shows "(conc_conflicting T = Some {#} \<and> unsatisfiable (set_mset (clauses_of_clss N)))
    \<or> (conc_conflicting T = None \<and> conc_trail T \<Turnstile>asm clauses_of_clss N
      \<and> satisfiable (set_mset (clauses_of_clss N)))"
proof -
  have inv: "cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_all_struct_inv (state (conc_init_state N))"
    using n_d unfolding cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_all_struct_inv_def by (auto simp: state_conc_init_state)
  then show ?thesis
    using cdcl\<^sub>W_merge_abs_stgy.full_if_full_abs[of "conc_init_state N" T] full
    using cdcl\<^sub>W_restart_mset.full_cdcl\<^sub>W_stgy_iff_full_cdcl\<^sub>W_s'
      [of "state (conc_init_state N)" "state T", OF inv]
    cdcl\<^sub>W_restart_mset.full_cdcl\<^sub>W_stgy_final_state_conclusive_from_init_state
      [of "init_clss (state (conc_init_state N))" "state T"]
