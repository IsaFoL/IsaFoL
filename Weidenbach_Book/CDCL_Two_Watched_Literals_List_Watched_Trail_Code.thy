...
definition trailt_ref :: \<open>(trail_int \<times> (nat, nat) ann_lits) set\<close> where
  \<open>trailt_ref = {((M', xs, lvls, k), M). M = M' \<and> no_dup M \<and>
    (\<forall>L \<in># \<L>\<^sub>a\<^sub>l\<^sub>l. atm_of L < length xs \<and> xs ! (atm_of L) = valued_atm_on_trail M (atm_of L)) \<and>
...
definition literals_are_in_\<A>\<^sub>i\<^sub>n_int where
  \<open>literals_are_in_\<A>\<^sub>i\<^sub>n_int S = literals_are_in_\<A>\<^sub>i\<^sub>n_mm (mset `# mset (tl (get_clauses_wl_int S)))\<close>

lemma literals_are_in_\<A>\<^sub>i\<^sub>n_int_in_D\<^sub>0:
  assumes \<open>literals_are_in_\<A>\<^sub>i\<^sub>n_int S\<close> and
  ...
  using assms literals_are_in_\<A>\<^sub>i\<^sub>n_mm_in_\<L>\<^sub>a\<^sub>l\<^sub>l[of \<open>tl (get_clauses_wl_int S)\<close> \<open>i - 1\<close> j]
  by (auto simp: literals_are_in_\<A>\<^sub>i\<^sub>n_int_def image_image nth_tl)
...
lemma literals_are_in_\<A>\<^sub>i\<^sub>n_int_in_D\<^sub>0':
  ...
  using literals_are_in_\<A>\<^sub>i\<^sub>n_int_in_D\<^sub>0[OF assms(1-4)] unfolding N .
...
lemma Pos_unat_lit_assn:
  \<open>(return o (\<lambda>n. two_uint32 * n), RETURN o Pos) \<in> [\<lambda>L. Pos L \<in># \<L>\<^sub>a\<^sub>l\<^sub>l]\<^sub>a uint32_nat_assn\<^sup>k \<rightarrow> unat_lit_assn\<close>
  apply sepref_to_hoare
...
lemma Neg_unat_lit_assn:
  \<open>(return o (\<lambda>n. two_uint32 * n +1), RETURN o Neg) \<in> [\<lambda>L. Pos L \<in># \<L>\<^sub>a\<^sub>l\<^sub>l]\<^sub>a uint32_nat_assn\<^sup>k \<rightarrow> unat_lit_assn\<close>
  apply sepref_to_hoare
  using in_N1_less_than_upperN
  by (sep_auto simp: unat_lit_rel_def nat_lit_rel_def uint32_nat_rel_def br_def Collect_eq_comp
...
definition set_conflict_and_clvls :: \<open>(nat, nat) ann_lits \<Rightarrow> nat clauses_l \<Rightarrow> nat \<Rightarrow> nat clause option \<Rightarrow> nat \<Rightarrow>
  (nat clause option \<times> nat) nres\<close> where
\<open>set_conflict_and_clvls M N i _ _ = RES{(Some (mset (N!i)), counts_max_lvl M (mset (N!i)))}\<close>
...
definition conflict_merge_aa :: \<open>(nat, nat) ann_lits \<Rightarrow> nat clauses_l \<Rightarrow> nat \<Rightarrow> conflict_option_rel \<Rightarrow> nat \<Rightarrow> (conflict_option_rel \<times> nat) nres\<close> where
  \<open>conflict_merge_aa M C i xs = conflict_merge M (C ! i) xs\<close>

lemma literals_are_in_\<A>\<^sub>i\<^sub>n_in_\<L>\<^sub>a\<^sub>l\<^sub>l:
  assumes
    N1: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (mset xs)\<close> and
...
proof -
  ...
  then show ?thesis
    using N1 unfolding literals_are_in_\<A>\<^sub>i\<^sub>n_def by blast
qed
...
sepref_thm conflict_merge_code
  ...
  :: \<open>[\<lambda>((((M, N), i), (_, xs)), _). i < length N \<and> (\<forall>j<length (N!i). atm_of (N!i!j) < length (snd xs)) \<and>
        literals_are_in_\<A>\<^sub>i\<^sub>n (mset (N!i))]\<^sub>a
  ...
  supply length_rll_def[simp] nth_rll_def[simp] upperN_def[simp] uint32_nat_assn_one[sepref_fr_rules]
  image_image[simp] literals_are_in_\<A>\<^sub>i\<^sub>n_in_\<L>\<^sub>a\<^sub>l\<^sub>l[simp]
...
definition conflict_merge'_step :: \<open>(nat, nat) ann_lits \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> conflict_rel \<Rightarrow> nat clause_l \<Rightarrow>
  ...
  \<open>conflict_merge'_step M i clvls zs D C = (
      ...
      literals_are_in_\<A>\<^sub>i\<^sub>n E \<and> clvls = counts_max_lvl M E)\<close>
...
lemma conflict_add_option_conflict_rel:
  assumes ocr: \<open>((False, (n, zs)), Some D) \<in> option_conflict_rel\<close> and
    \<open>literals_are_in_\<A>\<^sub>i\<^sub>n D\<close> and L_\<L>\<^sub>a\<^sub>l\<^sub>l: \<open>L \<in># \<L>\<^sub>a\<^sub>l\<^sub>l\<close>
  shows
...
proof -
  ...
  from conflict_add_conflict_rel[OF this _ L_\<L>\<^sub>a\<^sub>l\<^sub>l] have \<open>((False, (if zs!(atm_of L) = None then n+1 else n, zs[atm_of L := Some (is_pos L)])), Some (add_mset L (D - {#L, -L#}))) \<in> option_conflict_rel\<close>
...
lemma literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (add_mset L A) \<longleftrightarrow> literals_are_in_\<A>\<^sub>i\<^sub>n A \<and> L \<in># \<L>\<^sub>a\<^sub>l\<^sub>l\<close>
  unfolding literals_are_in_\<A>\<^sub>i\<^sub>n_def by (auto simp: all_lits_of_m_add_mset in_\<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_in_atms_of_iff)
...
lemma literals_are_in_\<A>\<^sub>i\<^sub>n_remdups[simp]: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (remdups_mset N) = literals_are_in_\<A>\<^sub>i\<^sub>n N\<close>
  by (auto simp: literals_are_in_\<A>\<^sub>i\<^sub>n_def all_lits_of_m_remdups_mset)
...
lemma conflict_merge'_spec:
  assumes
      ...
      lits: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (mset D)\<close> and
      tauto: \<open>\<not>tautology (mset D)\<close> and
      \<open>literals_are_in_\<A>\<^sub>i\<^sub>n C\<close> and
  ...
  shows \<open>conflict_merge M D (b, n, xs) clvls \<le> \<Down> (option_conflict_rel \<times>\<^sub>r Id)
             (RES {(Some (mset D \<union># (C - mset D - uminus `# mset D)),
              counts_max_lvl M (mset D \<union># (C - mset D - uminus `# mset D)))})\<close>
proof -
  ...
  have [intro]: \<open>((b, a, ba), Some C) \<in> option_conflict_rel \<Longrightarrow> literals_are_in_\<A>\<^sub>i\<^sub>n C \<Longrightarrow>
  ...
  proof -
    ...
    have aa: \<open>aa = counts_max_lvl M (remdups_mset (?C' a))\<close> and
      ...
      lits: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (remdups_mset (?C' a))\<close>
      using I'
    ...
    have ab_upper: \<open>Suc (Suc ab) < upperN\<close>
      ...
      unfolding ab literals_are_in_\<A>\<^sub>i\<^sub>n_remdups upperN_def by auto
    show ?I
    ...
    have [simp]: \<open>atm_of (D ! a) < length xs\<close> \<open>D ! a \<in># \<L>\<^sub>a\<^sub>l\<^sub>l\<close>
      using literals_are_in_\<A>\<^sub>i\<^sub>n_in_\<L>\<^sub>a\<^sub>l\<^sub>l[OF \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (mset D)\<close> a_le_D] atms_le_xs
    ...
    show ?I'
      ...
      unfolding I'_def conflict_merge'_step_def Let_def
      by (auto simp: minus_notin_trivial literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset
  ...
  proof -
    ...
    have \<open>size (counts_max_lvl M (remdups_mset (?C' a))) \<le> size (remdups_mset (?C' a))\<close>
      unfolding counts_max_lvl_def
      by auto
    then have Suc_Suc_aa: \<open>Suc (Suc aa) < upperN\<close>
      using size_C_upperN lits
      simple_clss_size_upper_div2[of \<open>remdups_mset (?C' a)\<close>]
      unfolding upperN_def aa[symmetric]
      by (auto simp: tauto_C')
    ...
    next
      ...
      show ?thesis
        ...
        by (auto dest:  simp: distinct_mset_in_diff minus_notin_trivial
            intro: mset_as_position.intros)
    qed
    have \<open>literals_are_in_\<A>\<^sub>i\<^sub>n
     ((mset (take a D) +
      (C -
       add_mset (- D ! a)
        (add_mset (D ! a)
          (mset (take a D) + uminus `# mset (take a D))))))\<close>
      by (rule literals_are_in_\<A>\<^sub>i\<^sub>n_mono[OF lits[unfolded literals_are_in_\<A>\<^sub>i\<^sub>n_remdups]])
         (auto simp: diff_le_mono2_mset)
    moreover {
      have K: \<open>D ! a = Neg L \<Longrightarrow>is_in_conflict_rel (ab, b) (Neg L) \<Longrightarrow> L < length xs  \<Longrightarrow>
      ...
      by (cases \<open>D ! a\<close>)
       (auto simp: is_in_conflict_rel_def dist_C distinct_mset_in_diff
          split: option.splits bool.splits
          dest: in_diffD)
  ...
  have confl: \<open>conflict_merge M D (b, n, xs) clvls
    ...
    subgoal for b' n' s j zs
      ...
      by (auto simp: option_conflict_rel_def take_Suc_conv_app_nth
          literals_are_in_\<A>\<^sub>i\<^sub>n_in_\<L>\<^sub>a\<^sub>l\<^sub>l)
...
lemma conflict_merge_aa_mark_conflict:
  \<open>(uncurry4 conflict_merge_aa, uncurry4 set_conflict_and_clvls) \<in>
    [\<lambda>((((M, N), i), xs), clvls). i < length N \<and> xs = None \<and> distinct (N ! i) \<and>
       literals_are_in_\<A>\<^sub>i\<^sub>n (mset (N ! i)) \<and> \<not>tautology (mset (N ! i)) \<and> clvls = 0]\<^sub>f
...
proof -
  ...
  have H: \<open>conflict_merge_aa M N i (b, n, xs) clvls \<le> \<Down> (option_conflict_rel \<times>\<^sub>r nat_rel) (set_conflict_and_clvls M N i None clvls)\<close>
    if
      \<open>i < length N\<close> and
      ocr: \<open>((b, n, xs), None) \<in> option_conflict_rel\<close> and
     dist: \<open>distinct (N! i)\<close> and
     lits: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (mset (N ! i))\<close> and
...
proof -
  ...
  have pre: \<open>?pre' = ?pre\<close>
    by (intro ext) (auto simp: comp_PRE_def in_br_conv list_mset_rel_def in_\<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_in_atms_of_iff
        ...
        dest: literals_are_in_\<A>\<^sub>i\<^sub>n_in_\<L>\<^sub>a\<^sub>l\<^sub>l)
  have im: \<open>?im' = ?im\<close>
...
lemma unit_prop_body_wl_D_invD:
  ...
  shows
    ...
    \<open>literals_are_\<A>\<^sub>i\<^sub>n S\<close> and
    \<open>get_conflict_wl S = None\<close> and
    \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (mset (get_clauses_wl S ! watched_by_app S L w))\<close> and
...
proof -
  ...
  obtain M N U D NP UP W Q where
    S: \<open>S = (M, N, U, D, NP, UP, Q, W)\<close>
    by (cases S)
  show lits_N: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (mset (get_clauses_wl S ! watched_by_app S L w))\<close>
    using assms unfolding unit_prop_body_wl_D_inv_def apply -
    apply (rule literals_are_in_\<A>\<^sub>i\<^sub>n_nth[of _ _ M U D NP UP Q W])
  ...
  then show \<open>get_clauses_wl S ! watched_by_app S L w ! 0 \<in> snd ` D\<^sub>0\<close>
    ...
    by (auto simp: literals_are_in_\<A>\<^sub>i\<^sub>n_def all_lits_of_m_add_mset)
  ...
  show S_L_W_ge_0: \<open>watched_by_app S L w > 0\<close> and \<open>literals_are_\<A>\<^sub>i\<^sub>n S\<close> and \<open>get_conflict_wl S = None\<close>
    using assms unfolding unit_prop_body_wl_D_inv_def unit_prop_body_wl_inv_def watched_by_app_def
    by fast+
  have
    struct: \<open>twl_struct_invs (twl_st_of (Some L) (st_l_of_wl (Some (L, w)) S))\<close> and
...
(* TODO Move? *)
type_synonym twl_st_wll_trail_no_lvl =
  \<open>trail_int_assn \<times> clauses_wl \<times> nat \<times> conflict_option_assn \<times>
    lit_queue_l \<times> watched_wl \<times> vmtf_remove_assn \<times> phase_saver_assn \<times> uint32\<close>

definition twl_st_int_trail_ref_assn
  :: \<open>twl_st_wl_int_trail_ref \<Rightarrow> twl_st_wll_trail_no_lvl \<Rightarrow> assn\<close>
where
...
sepref_thm find_unwatched_wl_s_int_code
  ...
  :: \<open>[\<lambda>(S, i). i < length (get_clauses_wl_int S) \<and> i > 0 \<and> literals_are_in_\<A>\<^sub>i\<^sub>n_int S]\<^sub>a
         twl_st_int_assn\<^sup>k *\<^sub>a nat_assn\<^sup>k \<rightarrow> option_assn nat_assn\<close>
  supply [[goals_limit = 1]] literals_are_in_\<A>\<^sub>i\<^sub>n_int_in_D\<^sub>0'[intro] nth_rll_def[simp]
    length_rll_def[simp]
...
theorem find_unwatched_wl_s_int_find_unwatched_wl_s:
  \<open>(uncurry find_unwatched_wl_s_int, uncurry find_unwatched_wl_s)
    \<in> [\<lambda>(S, i). i < length (get_clauses_wl S) \<and> 0 < i \<and> literals_are_\<A>\<^sub>i\<^sub>n S \<and>
...
proof -
  have H: \<open>(uncurry find_unwatched_wl_s_int_code, uncurry find_unwatched_wl_s)
    \<in> [comp_PRE (twl_st_ref \<times>\<^sub>f nat_rel)
         (\<lambda>(S, i). i < length (get_clauses_wl S) \<and> 0 < i \<and> literals_are_\<A>\<^sub>i\<^sub>n S \<and>
            2 \<le> length (get_clauses_wl S ! i))
         (\<lambda>_ (S, i). i < length (get_clauses_wl_int S) \<and> 0 < i \<and> literals_are_in_\<A>\<^sub>i\<^sub>n_int S)
         (\<lambda>_. True)]\<^sub>a
  ...
  proof -
    have [simp]: \<open>mset `# mset (take ai (tl ah)) + ak + (mset `# mset (drop (Suc ai) ah) + al) =
      ...
      by auto
    have [intro]: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n_int T\<close> and \<open>get_clauses_wl_int T = get_clauses_wl S\<close> if
      ...
      using that apply (auto simp: twl_st_ref_def mset_take_mset_drop_mset
          mset_take_mset_drop_mset' clauses_def literals_are_in_\<A>\<^sub>i\<^sub>n_int_def)
      apply (auto simp add: all_lits_of_mm_union literals_are_in_\<A>\<^sub>i\<^sub>n_mm_def is_\<L>\<^sub>a\<^sub>l\<^sub>l_def)
      done
    show ?thesis
      unfolding comp_PRE_def
...
proof -
  ...
  have H: \<open>xa \<in># \<L>\<^sub>a\<^sub>l\<^sub>l \<Longrightarrow> atm_of xa < upperN div 2\<close> for xa
    using in_N1_less_than_upperN
    by (cases xa) (auto simp: upperN_def)
  have \<open>remdups_mset (atm_of `# \<L>\<^sub>a\<^sub>l\<^sub>l) \<subseteq># mset [0..<upperN div 2]\<close>
    apply (subst distinct_subseteq_iff[THEN iffD1])
  ...
  moreover have \<open>size (nat_of_lit `# remdups_mset \<L>\<^sub>a\<^sub>l\<^sub>l) = size (remdups_mset \<L>\<^sub>a\<^sub>l\<^sub>l)\<close>
    by simp
  ultimately have 2: \<open>size (remdups_mset (atm_of `# \<L>\<^sub>a\<^sub>l\<^sub>l)) \<le> upperN div 2\<close>
    by auto
...
definition mark_conflict_wl'_int :: \<open>nat \<Rightarrow> twl_st_wl_int \<Rightarrow> twl_st_wl_int nres\<close> where
  \<open>mark_conflict_wl'_int = (\<lambda>C (M, N, U, D, Q, W, vmtf, \<phi>, clvls). do {
    let n = zero_uint32_nat;
    (D, clvls) \<leftarrow> set_conflict_and_clvls M N C D n;
    RETURN (M, N, U, D, {#}, W, vmtf, \<phi>, clvls)})\<close>

sepref_thm mark_conflict_wl'_int_code
  is \<open>uncurry mark_conflict_wl'_int\<close>
  :: \<open>[\<lambda>(C, S). get_conflict_wl_int S = None \<and> C < length (get_clauses_wl_int S) \<and>
      distinct (get_clauses_wl_int S ! C) \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (mset (get_clauses_wl_int S ! C)) \<and>
  ...
  supply [[goals_limit=1]]
    conflict_merge_code_mark_conflict[unfolded twl_st_int_assn_def, sepref_fr_rules]
...
lemma mark_conflict_wl'_int_mark_conflict_wl':
  ...
  by (intro nres_relI frefI)
     (auto simp: twl_st_ref_def mark_conflict_wl'_int_def mark_conflict_wl'_def
        set_conflict_and_clvls_def bind_RES_RETURN2_eq RETURN_def[symmetric]
...
lemma mark_conflict_wl'_int_code_mark_conflict_wl'[sepref_fr_rules]:
  \<open>(uncurry mark_conflict_wl'_int_code, uncurry (RETURN oo mark_conflict_wl')) \<in>
    [\<lambda>(C, S). get_conflict_wl S = None \<and> C < length (get_clauses_wl S) \<and>
      distinct (get_clauses_wl S ! C) \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (mset (get_clauses_wl S ! C)) \<and>
...
proof (rule ccontr)
  ...
  have \<open>literals_are_\<A>\<^sub>i\<^sub>n S\<close>
    using inv by (blast intro: unit_prop_body_wl_D_invD)
...
proof -
  have H: \<open>(uncurry5 update_clause_wl_int_code, uncurry5 update_clause_wl) \<in>
    ...
    .
  have [dest]: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n_mm (mset `# mset (tl (get_clauses_wl S)))\<close>
  ...
  proof -
    ...
    then  have \<open> set_mset (all_lits_of_mm (mset `# mset (tl (get_clauses_wl S)))) \<subseteq>
      ...
      apply (cases S)
      by (auto simp:  mset_take_mset_drop_mset' literals_are_in_\<A>\<^sub>i\<^sub>n_mm_def clauses_def
           drop_Suc all_lits_of_mm_union is_\<L>\<^sub>a\<^sub>l\<^sub>l_def)
    then show ?thesis
      unfolding literals_are_in_\<A>\<^sub>i\<^sub>n_mm_def by blast
  qed
  have pre: \<open>?pre' x\<close> if \<open>?pre x\<close> for x
    unfolding comp_PRE_def
    apply (cases x)
    ...
    subgoal for L C w i f M N U D NP UP Q W y
      apply (subgoal_tac \<open>literals_are_in_\<A>\<^sub>i\<^sub>n_mm (mset `# mset (tl N))\<close>)
      subgoal
        using literals_are_in_\<A>\<^sub>i\<^sub>n_mm_in_\<L>\<^sub>a\<^sub>l\<^sub>l[of \<open>tl N\<close> \<open>(W L ! w - 1)\<close> f]
...
lemma twl_st_wl_int_W_list_rel_twl_st_rel: \<open>twl_st_wl_int_W_list_rel O twl_st_ref =
   {((M', N', U', D', Q', W', vm, \<phi>, clvls), M, N, U, D, NP, UP, Q, W).
     ...
     (W', W) \<in> \<langle>Id\<rangle>map_fun_rel D\<^sub>0 \<and>
     vm \<in> vmtf_imp M \<and> phase_saving \<phi> \<and> no_dup M \<and> clvls \<in> counts_maximum_level M D}\<close>
...
lemma in_literals_are_in_\<A>\<^sub>i\<^sub>n_in_D\<^sub>0:
  assumes \<open>literals_are_in_\<A>\<^sub>i\<^sub>n D\<close> and \<open>L \<in># D\<close>
  shows \<open>L \<in> snd ` D\<^sub>0\<close>
  using assms by (cases L) (auto simp: image_image literals_are_in_\<A>\<^sub>i\<^sub>n_def all_lits_of_m_def)
...
definition (in -) hd_trail_code :: \<open>trail_int_assn \<Rightarrow> uint32 \<times> nat option\<close> where
...
lemma is_in_conflict_option_assn_code_is_in_conflict[sepref_fr_rules]:
  \<open>(uncurry is_in_conflict_option_assn_code,
    ...
    [\<lambda>(L, C). L \<in> snd ` D\<^sub>0 \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (the C) \<and> C \<noteq> None]\<^sub>a
...
sepref_thm literal_is_in_conflict_int_code
  ...
  :: \<open>[\<lambda>(L, S). L \<in> snd ` D\<^sub>0 \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (the (get_conflict_wl_int S)) \<and> get_conflict_wl_int S \<noteq> None]\<^sub>a
...
lemma literal_is_in_conflict_int_code_literal_is_in_conflict[sepref_fr_rules]:
  \<open>(uncurry literal_is_in_conflict_int_code,
    ...
    [\<lambda>(L, S). L \<in> snd ` D\<^sub>0 \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (the (get_conflict_wl S)) \<and> get_conflict_wl S \<noteq> None]\<^sub>a
      unat_lit_assn\<^sup>k *\<^sub>a twl_st_assn\<^sup>k  \<rightarrow> bool_assn\<close>
  (is \<open>?c \<in> [?pre]\<^sub>a ?im \<rightarrow> ?f\<close>)
proof -
  have H: \<open>?c
    \<in> [comp_PRE (Id \<times>\<^sub>f twl_st_ref) (\<lambda>_. True)
         (\<lambda>_ (L, S). L \<in> snd ` D\<^sub>0 \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (the (get_conflict_wl_int S)) \<and>
...
definition tl_state_wl_int :: \<open>twl_st_wl_int \<Rightarrow> twl_st_wl_int\<close> where
  \<open>tl_state_wl_int = (\<lambda>(M, N, U, D, WS, Q, vmtf, \<phi>, clvls).
...
lemma literals_are_\<A>\<^sub>i\<^sub>n_hd_trail_in_D\<^sub>0:
  assumes
    \<L>\<^sub>a\<^sub>l\<^sub>l: \<open>literals_are_\<A>\<^sub>i\<^sub>n S\<close> and
...
sepref_thm tl_state_wl_int_code
  ...
  option.splits[split]
  supply [[goals_limit=1]] option.splits[split] literals_are_\<A>\<^sub>i\<^sub>n_hd_trail_in_D\<^sub>0[intro]
...
lemma counts_max_lvl_Cons:
  ...
  shows \<open>counts_max_lvl (L # a) y =
    (if (lit_of L \<in># y \<or> -lit_of L \<in># y) \<and> count_decided a \<noteq> 0 then counts_max_lvl a y + 1
    else counts_max_lvl a y)\<close>
proof -
  ...
  have [simp]: \<open>lit_of L \<notin># A \<Longrightarrow>
    ...
    by auto
  show ?thesis
    using assms by (auto simp: counts_max_lvl_def get_level_cons_if tautology_add_mset
        atm_of_eq_atm_of
        dest!: multi_member_split)
qed

lemma counts_max_lvl_tl:
  ...
  shows \<open>counts_max_lvl (tl a) y =
      (if (lit_of(hd a) \<in># y \<or> -lit_of(hd a) \<in># y) \<and> count_decided a \<noteq> 0
       then counts_max_lvl a y - 1 else counts_max_lvl a y)\<close>
  using assms by (cases a) (auto simp: counts_max_lvl_Cons)
...
lemma tl_state_wl_int_tl_state_wl: \<open>(RETURN o tl_state_wl_int, RETURN o tl_state_wl) \<in>
  ...
  ]\<^sub>f twl_st_ref \<rightarrow> \<langle>twl_st_ref\<rangle>nres_rel\<close>
  by (intro frefI nres_relI)
...
lemma tl_state_wl_refine[sepref_fr_rules]:
  \<open>(tl_state_wl_int_code, RETURN o tl_state_wl) \<in>
    [\<lambda>S. get_trail_wl S \<noteq> [] \<and> literals_are_\<A>\<^sub>i\<^sub>n S \<and>
...
proof -
  have H: \<open>?c
   \<in> [comp_PRE twl_st_ref
     ...
     (\<lambda>_. True)]\<^sub>a hrp_comp (twl_st_int_assn\<^sup>d)
                    twl_st_ref \<rightarrow> hr_comp twl_st_int_assn
    twl_st_ref\<close>
      (is \<open>_ \<in> [?pre']\<^sub>a ?im' \<rightarrow> ?f'\<close>)
    using  hfref_compI_PRE_aux[OF tl_state_wl_int_code_refine tl_state_wl_int_tl_state_wl]
    .
  have pre: \<open>?pre' x\<close> if \<open>?pre x\<close> for x
    using that literals_are_\<A>\<^sub>i\<^sub>n_hd_trail_in_D\<^sub>0[of x]
    ...
    by (auto simp: image_image twl_st_ref_def phase_saving_def in_\<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_in_atms_of_iff
      vmtf_imp_def)
  have im: \<open>?im' = ?im\<close>
    unfolding prod_hrp_comp hrp_comp_dest hrp_comp_keep
    twl_st_assn_W_list[symmetric] twl_st_assn_def
...
proof -
  ...
  have pre: \<open>?pre' x\<close> if \<open>?pre x\<close> for x
    ...
    unfolding comp_PRE_def option_conflict_rel_def conflict_rel_def
    by (fastforce simp: image_image twl_st_ref_def phase_saving_def in_\<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_in_atms_of_iff
      vmtf_imp_def)
  have im: \<open>?im' = ?im\<close>
...
definition (in -) conflict_merge_abs_union'
...
where
  \<open>conflict_merge_abs_union' M N i C _ =
...
where
  \<open>conflict_merge_abs_union M N i C _ = RES {(Some (mset (N!i) \<union># (the C - (mset (N!i) + uminus `# mset (N!i)))),
    counts_max_lvl M (mset (N!i) \<union># (the C - (mset (N!i) + uminus `# mset (N!i)))))}\<close>

lemma conflict_merge_abs_union_alt_def:
  \<open>conflict_merge_abs_union M N i C clvls = RES {(conflict_merge_abs_union' M N i C clvls,
    counts_max_lvl M (the (conflict_merge_abs_union' M N i C clvls)))}\<close>
  unfolding conflict_merge_abs_union_def conflict_merge_abs_union'_def by auto

lemma counts_max_lvl_tl_remove_hd:
  assumes
  ...
  shows \<open>counts_max_lvl (tl M) (C - unmark (hd M)) =
         counts_max_lvl M C - 1\<close>
  using assms
  by (cases M) (auto simp: counts_max_lvl_Cons counts_max_lvl_add_mset
      dest!: multi_member_split)
...
lemma
  ...
  defines [simp]: \<open>E \<equiv> the (conflict_merge_abs_union' (get_trail_wl S)  (get_clauses_wl S) C (get_conflict_wl S) clvls)\<close>
  ...
  shows
    ...
    resolve_cls_wl'_counts_max_lvl:
       \<open>C > 0 \<Longrightarrow> counts_max_lvl (get_trail_wl S) E = counts_max_lvl (tl (get_trail_wl S))
...
proof -
  ...
  ultimately show ?Res
    using C C' unfolding S by (auto simp: C'' D resolve_cls_wl'_def ac_simps
        conflict_merge_abs_union'_def minus_notin_trivial S)
  show ?L_in_union
    using C C' unfolding S by (auto simp: C'' D resolve_cls_wl'_def ac_simps
        conflict_merge_abs_union'_def S)
  show ?L_notin_union
    using C C' uL_C' uL_C'' dist_D unfolding S by (auto simp: C'' D resolve_cls_wl'_def ac_simps
        conflict_merge_abs_union'_def S dest: in_diffD)
  have [simp]: \<open>L \<notin># D'\<close>
   ...
   have \<open>D' - (C' + uminus `# C') = (D' -uminus `# C') - C'\<close>
    ...
    apply (auto simp: S conflict_merge_abs_union'_def C'' tautology_add_mset
  ...
  then have \<open>counts_max_lvl (Propagated L C # M)
     (add_mset L (C' \<union># (D' - add_mset L (C' + uminus `# C')))) =
    counts_max_lvl M (C' \<union># (D' - add_mset L (C' + uminus `# C'))) + 1\<close>
    apply (subst counts_max_lvl_Cons)
    using undef_L n_d tauto_C dist_C uL_C'
    by (auto simp: S C'' conflict_merge_abs_union'_def D
        counts_max_lvl_add_mset)
  then show ?Max
    by (auto simp: S resolve_cls_wl'_def conflict_merge_abs_union'_def C'' D)
qed

lemma conflict_merge_aa_conflict_merge_abs_union_aa:
  \<open>(uncurry4 (conflict_merge_aa), uncurry4 conflict_merge_abs_union) \<in>
     [\<lambda>((((M, N), i), C), clvls). distinct (N!i) \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (mset (N!i)) \<and>
          \<not> tautology (mset (N!i)) \<and>
         literals_are_in_\<A>\<^sub>i\<^sub>n (the C) \<and> C \<noteq> None \<and>
         clvls = counts_max_lvl M (the C)]\<^sub>f
  ...
  subgoal for M N i b j xs clvls M' N' i' _ clvls' C
    using conflict_merge'_spec[of b j xs C \<open>N' ! i'\<close> clvls M]
...
lemma conflict_merge_code_conflict_merge_abs_union[sepref_fr_rules]:
  \<open>(uncurry4 conflict_merge_code, uncurry4 conflict_merge_abs_union) \<in>
    [\<lambda>((((M, N), i), C), clvls). distinct (N!i) \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (mset (N!i)) \<and> \<not> tautology (mset (N!i)) \<and>
         literals_are_in_\<A>\<^sub>i\<^sub>n (the C) \<and> C \<noteq> None \<and> i < length N \<and> clvls = counts_max_lvl M (the C)]\<^sub>a
    trail_assn\<^sup>k *\<^sub>a clauses_ll_assn\<^sup>k *\<^sub>a nat_assn\<^sup>k *\<^sub>a (conflict_option_assn)\<^sup>d *\<^sub>a uint32_nat_assn\<^sup>k \<rightarrow>
     conflict_option_assn *assn uint32_nat_assn \<close>
  (is \<open>?c \<in> [?pre]\<^sub>a ?im \<rightarrow> ?f\<close>)
proof -
  have H: \<open>?c
     ...
     (\<lambda>((((M, N), i), C), clvls).
         ...
         literals_are_in_\<A>\<^sub>i\<^sub>n (mset (N ! i)) \<and>
         \<not> tautology (mset (N ! i)) \<and>
         literals_are_in_\<A>\<^sub>i\<^sub>n (the C) \<and>
         C \<noteq> None \<and> clvls = counts_max_lvl M (the C))
     (\<lambda>_ ((((M, N), i), _, xs), _).
         i < length N \<and>
         (\<forall>j<length (N ! i).
   ...
   (option_conflict_rel \<times>\<^sub>f nat_rel)\<close>
    ...
    .
  have pre: \<open>?pre' x\<close> if \<open>?pre x\<close> for x
    using that literals_are_in_\<A>\<^sub>i\<^sub>n_in_\<L>\<^sub>a\<^sub>l\<^sub>l
    unfolding comp_PRE_def option_conflict_rel_def conflict_rel_def
    by (auto simp: image_image twl_st_ref_def phase_saving_def in_\<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_in_atms_of_iff
      vmtf_imp_def)
  have im: \<open>?im' = ?im\<close>
...
definition update_confl_tl_wl_int
  :: \<open>nat \<Rightarrow> nat literal \<Rightarrow> twl_st_wl_int \<Rightarrow> (bool \<times> twl_st_wl_int) nres\<close>
where
  \<open>update_confl_tl_wl_int = (\<lambda>C L (M, N, U, D, Q, W, vmtf, \<phi>, clvls).
     (if C = 0 then do {
         let D' = remove1_mset (-L) (the D);
         let L' = atm_of L;
         ASSERT (clvls \<ge> 1);
         RETURN (D' = {#}, (tl M, N, U, Some D', Q, W, vmtf_dump_and_unset L' vmtf, save_phase L \<phi>,
            fast_minus clvls one_nat_uint32))
         }
      else do {
        let L' = atm_of L;
        (D', clvls) \<leftarrow> conflict_merge_abs_union M N C D clvls;
        let D' = remove1_mset L (the D');
        RETURN (D' = {#}, (tl M, N, U, Some D', Q, W, vmtf_dump_and_unset L' vmtf, save_phase L \<phi>,
           fast_minus clvls one_nat_uint32))
      }))\<close>

lemma counts_max_lvl_remove1_mset_hd:
  \<open>-lit_of (hd M) \<in># y \<Longrightarrow> is_proped (hd M) \<Longrightarrow>
     counts_max_lvl M (remove1_mset (-lit_of (hd M)) y) = counts_max_lvl M y - 1\<close>
  by (cases M)  (auto dest!: multi_member_split simp: counts_max_lvl_add_mset)
...
lemma resolve_cls_wl'_if_conflict_merge_abs_union:
  ...
  shows \<open>resolve_cls_wl' S C L = (if C = 0 then remove1_mset (-L) (the (get_conflict_wl S))
               else remove1_mset L (the (conflict_merge_abs_union' (get_trail_wl S) (get_clauses_wl S) C (get_conflict_wl S) clvls)))\<close>
...
lemma counts_max_lvl_ge_1:
  assumes \<open>twl_stgy_invs (twl_st_of_wl None S)\<close> and
    \<open>get_conflict_wl S \<noteq> None\<close> and
    \<open>get_conflict_wl S \<noteq> Some {#}\<close>
  shows
   \<open>counts_max_lvl (get_trail_wl S) (the (get_conflict_wl S)) \<ge> Suc 0\<close>
...
lemma update_confl_tl_wl_int_update_confl_tl_wl:
  ...
  subgoal for CLS' CLS
    ...
    apply (cases \<open>CLS'\<close>; cases CLS)
    apply (auto simp: twl_st_ref_def update_confl_tl_wl_int_def update_confl_tl_wl_def
        abs_l_vmtf_unset_vmtf_unset' Let_def save_phase_def
        in_\<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_in_atms_of_iff phase_saving_def abs_l_vmtf_unset_vmtf_dump_unset
        conflict_merge_abs_union_alt_def
        RES_RETURN_RES RETURN_def no_dup_tlD counts_maximum_level_def
        RES_RES_RETURN_RES (* conflict_merge_abs_union'_def *)
        eq_commute[of \<open>remove1_mset _ _\<close> \<open>resolve_cls_wl' _ _ _\<close>]
        in_multiset_nempty
        intro!: RES_refine ASSERT_refine_left
        dest: )
    subgoal
      by (subst counts_max_lvl_tl)
         (auto simp: counts_max_lvl_tl distinct_mset_in_diff not_tautology_minus
          counts_max_lvl_remove1_mset_hd)
  done
  done
...
lemma conflict_merge_abs_union_None: \<open>conflict_merge_abs_union' M a b c clvls \<noteq> None\<close>
  unfolding conflict_merge_abs_union'_def by auto
...
sepref_thm update_confl_tl_wl_code
  is \<open>uncurry2 update_confl_tl_wl_int\<close>
  :: \<open>[\<lambda>((i, L), (M, N, U, D, W, Q, ((A, m, lst, next_search), _), \<phi>, clvls)).
      (i > 0 \<longrightarrow> distinct (N ! i)) \<and>
      (i > 0 \<longrightarrow> literals_are_in_\<A>\<^sub>i\<^sub>n (mset (N! i))) \<and>
      (i > 0 \<longrightarrow> \<not> tautology (mset (N ! i))) \<and>
      i < length N \<and>
      literals_are_in_\<A>\<^sub>i\<^sub>n (the D) \<and> D \<noteq> None \<and>
      M \<noteq> [] \<and>
      L \<in> snd ` D\<^sub>0 \<and> -L \<in># the D \<and> L \<notin># the D \<and>
      (i > 0 \<longrightarrow> (L \<in> set (N ! i) \<and> -L \<notin> set (N ! i))) \<and>
      (i > 0 \<longrightarrow> (-L \<notin># the (conflict_merge_abs_union' M N i D clvls) \<and>
           L \<in># the (conflict_merge_abs_union' M N i D clvls))) \<and>
      (i > 0 \<longrightarrow> counts_max_lvl M (the (conflict_merge_abs_union' M N i D clvls)) \<ge> 1) \<and>
      atm_of (lit_of (hd M)) < length \<phi> \<and>
      atm_of (lit_of (hd M)) < length A \<and> (next_search \<noteq> None \<longrightarrow>  the next_search < length A) \<and>
      L = lit_of (hd M) \<and>
      clvls = counts_max_lvl M (the D)
         ]\<^sub>a
  ...
  supply conflict_merge_abs_union'_def[simp] conflict_merge_abs_union_def[simp]
...
lemma update_confl_tl_wl_code_update_confl_tl_wl[sepref_fr_rules]:
  \<open>(uncurry2 update_confl_tl_wl_code, uncurry2 (RETURN ooo update_confl_tl_wl))
    \<in> [\<lambda>((C, L), S). twl_struct_invs (twl_st_of_wl None S) \<and>
        ...
        literals_are_\<A>\<^sub>i\<^sub>n S \<and>
...
[comp_PRE (nat_rel \<times>\<^sub>f Id \<times>\<^sub>f twl_st_ref)
       ...
       (\<lambda>_ ((i, L), M, N, U, D, W, Q, ((A, m, lst,
           ...
           (0 < i \<longrightarrow> literals_are_in_\<A>\<^sub>i\<^sub>n (mset (N ! i))) \<and>
           (0 < i \<longrightarrow> \<not> tautology (mset (N ! i))) \<and>
           i < length N \<and>
           literals_are_in_\<A>\<^sub>i\<^sub>n (the D) \<and>
           D \<noteq> None \<and>
           M \<noteq> [] \<and>
           L \<in> snd ` D\<^sub>0 \<and>
           - L \<in># the D \<and>
           ...
           (0 < i \<longrightarrow>
            - L
            \<notin># the (conflict_merge_abs_union' M N i D
                      clvls) \<and>
            L \<in># the (conflict_merge_abs_union' M N i D
                        clvls)) \<and>
           (0 < i \<longrightarrow>
            1 \<le> counts_max_lvl M
                  (the (conflict_merge_abs_union' M N i D
                         clvls))) \<and>
           atm_of (lit_of (hd M)) < length \<phi> \<and>
           atm_of (lit_of (hd M)) < length A \<and>
       ...
       (\<lambda>_. True)]\<^sub>a hrp_comp
        ...
        (bool_assn *assn twl_st_int_assn)
        (bool_rel \<times>\<^sub>f twl_st_ref)\<close>
      (is \<open>_ \<in> [?pre']\<^sub>a ?im' \<rightarrow> ?f'\<close>)
  ...
  proof (intro allI impI conjI)
    ...
    have LC: \<open>(L, C) = lit_and_ann_of_propagated (hd (get_trail_wl S))\<close> and
      lits_\<A>\<^sub>i\<^sub>n: \<open>literals_are_\<A>\<^sub>i\<^sub>n S\<close> and
    ...
    have lits_D: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (the (get_conflict_wl S))\<close>
      by (rule literals_are_\<A>\<^sub>i\<^sub>n_conflict_literals_are_in_\<A>\<^sub>i\<^sub>n)
    ...
    moreover have L_D\<^sub>0: \<open>L \<in> snd ` D\<^sub>0\<close>
      ...
      by (cases \<open>get_conflict_wl S\<close>)
        (auto simp: image_image literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset uminus_\<A>\<^sub>i\<^sub>n_iff
            dest: multi_member_split)
    ...
    have in_atms_le: \<open>\<forall>L\<in>atms_of \<L>\<^sub>a\<^sub>l\<^sub>l. L < length A\<close> and \<phi>: \<open>phase_saving \<phi>\<close> and
      vmtf: \<open>\<exists>xs' ys'. l_vmtf (ys' @ xs') m A \<and> lst = option_hd (ys' @ xs') \<and> next_search = option_hd xs'\<close> and
    ...
    have lits_NC: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (mset (N ! C))\<close> if \<open>C > 0\<close>
      by (rule literals_are_in_\<A>\<^sub>i\<^sub>n_nth) (use C_le that lits_\<A>\<^sub>i\<^sub>n in auto)
    ...
    then have uL_conflict_merge: \<open>- L \<notin># the (conflict_merge_abs_union' M N C D clvls)\<close> if \<open>C > 0\<close>
      using confl L_notin_D that resolve_cls_wl'_notin[of S C L] struct_invs C_le LC proped
       trail_nempty
      by (auto simp: conflict_merge_abs_union'_def dest: in_diffD)
    then have L_conflict_merge: \<open>L \<in># the (conflict_merge_abs_union' M N C D clvls)\<close> if \<open>C > 0\<close>
      using confl L_notin_D that resolve_cls_wl'_in[of S C L] struct_invs C_le LC proped
       trail_nempty L_confl
      by (auto dest: in_diffD)
    have [simp]: \<open>Suc 0 \<le> counts_max_lvl M (the (conflict_merge_abs_union' M N C D
                    (counts_max_lvl M (the D))))\<close>
      if \<open>C > 0\<close>
      using resolve_cls_wl'_counts_max_lvl[of S C L clvls] LC confl C_le L_D\<^sub>0 clvls that pre
      by (auto simp: counts_maximum_level_def)
    show \<open>?\<Psi> x x'\<close>
      using confl L_confl dist_NC lits_NC C_le trail_nempty L_D\<^sub>0 tauto lits_D L_notin_D L_NC
      uL_conflict_merge L_conflict_merge atm_L_le_A atm_L_le_\<phi> next_search clvls
...
lemma literals_are_\<A>\<^sub>i\<^sub>n_trail_literals_are_in_\<A>\<^sub>i\<^sub>n:
  assumes
    \<A>\<^sub>i\<^sub>n: \<open>literals_are_\<A>\<^sub>i\<^sub>n S\<close> and
    struct: \<open>twl_struct_invs (twl_st_of_wl None S)\<close>
  shows \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset (get_trail_wl S))\<close> (is \<open>literals_are_in_\<A>\<^sub>i\<^sub>n ?M\<close>)
proof -
...
lemma skip_and_resolde_hd_D\<^sub>0:
  assumes
    ...
    \<open>literals_are_\<A>\<^sub>i\<^sub>n S\<close>
  shows \<open>- x21 \<in> snd ` D\<^sub>0\<close>
  using literals_are_\<A>\<^sub>i\<^sub>n_trail_literals_are_in_\<A>\<^sub>i\<^sub>n[of S] assms
  by (cases S)
     (auto simp: literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset image_image
...
sepref_thm skip_and_resolve_loop_wl_D
  ...
  supply [[goals_limit=1]] get_trail_twl_st_of_wl_get_trail_wl_empty_iff[simp] is_decided_hd_trail_wl_def[simp]
    ...
    literals_are_\<A>\<^sub>i\<^sub>n_conflict_literals_are_in_\<A>\<^sub>i\<^sub>n[intro]
...
lemma extract_shorter_conflict_list_removed_extract_shorter_conflict_l_trivial:
  shows \<open>(uncurry extract_shorter_conflict_list_removed, uncurry (RETURN oo extract_shorter_conflict_l_trivial)) \<in>
      [\<lambda>(M', D). literals_are_in_\<A>\<^sub>i\<^sub>n (the D) \<and> D \<noteq> None \<and> M = M']\<^sub>f Id \<times>\<^sub>f option_conflict_rel \<rightarrow>
...
proof -
  have H: \<open>extract_shorter_conflict_list_removed M (b, n, xs)
    ...
    if lits: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n C\<close> and ocr: \<open>((b, n, xs), Some C) \<in> option_conflict_rel\<close>
    for C b n xs
  proof -
    ...
    have n_ge_upperN: \<open>n > upperN div 2 \<Longrightarrow> {#L \<in># C. n \<le> atm_of L#} = {#}\<close> for n
      ...
      by (auto simp: filter_mset_empty_conv literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset nat_of_lit_def upperN_def
    ...
    have init_I: \<open>I n (0, n, n, xs, None)\<close>
      ...
      by (auto simp: conflict_rel_def highest_lit_def option_conflict_rel_def upperN_def
          mset_as_position_length_not_None[OF map] xs_Some literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset
    ...
    proof -
      ...
      have 3: \<open>Suc ab < length ae \<Longrightarrow> ae ! Suc ab = Some y \<Longrightarrow> Pos (Suc ab) \<in> snd ` D\<^sub>0\<close> for y
        ...
        by (auto dest!: multi_member_split simp: literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset image_image
    ...
    proof -
      ...
      have Suc_D\<^sub>0: \<open>Suc ab < length ae \<Longrightarrow> ae ! Suc ab = Some y \<Longrightarrow> Pos (Suc ab) \<in> snd ` D\<^sub>0\<close> for y
        using mset_as_position_in_iff_nth[OF I'_mapD[OF I'[unfolded s]], of \<open>if y then Pos (Suc ab) else Neg (Suc ab)\<close>]
          lits
        by (cases y) (auto simp: image_image in_\<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_in_atms_of_iff literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset
            dest: multi_member_split)
      ...
      have ab_upperN: \<open>0 < size {#L \<in># C. Suc ab \<le> atm_of L#} \<Longrightarrow> Suc ab \<le> upperN div 2\<close>
        using n_ge_upperN[of \<open>Suc ab\<close>] by (cases \<open>upperN div 2 \<le> ab\<close>; auto simp: upperN_def)
      then show ?thesis
        using ab_le Some shl' ab_\<L>\<^sub>a\<^sub>l\<^sub>l ocr' ab_ac_0 ac_size Suc_ab_drop_ae Suc_D\<^sub>0 ab_upperN cond
    ...
    proof -
      ...
      have ab_upperN: \<open>0 < size {#L \<in># C. Suc ab \<le> atm_of L#} \<Longrightarrow> Suc ab \<le> upperN div 2\<close>
        using n_ge_upperN[of \<open>Suc ab\<close>] by (cases \<open>upperN div 2 \<le> ab\<close>; auto simp: upperN_def)
      show ?thesis
        using ab_le Some shl' ab_\<L>\<^sub>a\<^sub>l\<^sub>l ocr' ab_ac_0 ac_size Suc_ab_drop_ae Suc_D\<^sub>0 ab_upperN
        unfolding I'_def
        by auto
    qed
...
lemma extract_shorter_conflict_list_extract_shorter_conflict_l_trivial_spec:
  \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (the ba) \<and> (\<exists>y. ba = Some y) \<and> ((a, aa, b), ba) \<in> option_conflict_rel \<and> M = M' \<Longrightarrow>
...
lemma literals_are_in_\<A>\<^sub>i\<^sub>n_sub:
  \<open>literals_are_in_\<A>\<^sub>i\<^sub>n y \<Longrightarrow> literals_are_in_\<A>\<^sub>i\<^sub>n (y - z)\<close>
  using literals_are_in_\<A>\<^sub>i\<^sub>n_mono[of y \<open>y - z\<close>] by auto
...
lemma extract_shorter_conflict_list_int_extract_shorter_conflict_list:
  \<open>(uncurry extract_shorter_conflict_list_int, uncurry extract_shorter_conflict_list)
   \<in> [\<lambda>(M', D). literals_are_in_\<A>\<^sub>i\<^sub>n (the D) \<and> D \<noteq> None \<and> M = M' \<and> M \<noteq> [] \<and>
         literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset M) \<and>  - lit_of (hd M) \<in># the D \<and>
  ...
  subgoal
    by (cases M)
      (auto simp: literals_are_in_\<A>\<^sub>i\<^sub>n_sub option_conflict_rel_def conflict_rel_def
        literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset in_\<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_in_atms_of_iff)
  subgoal by (auto simp: literals_are_in_\<A>\<^sub>i\<^sub>n_sub option_conflict_rel_def conflict_rel_def
        literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset in_\<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_in_atms_of_iff
        dest: multi_member_split)
  ...
  subgoal for a aa ab b ac ba y
    ...
    apply (auto intro!: ASSERT_refine_left
        simp: literals_are_in_\<A>\<^sub>i\<^sub>n_sub option_conflict_rel_def conflict_rel_def
  ...
  subgoal for M' b n zs ac ba D x' x1 x1a x2 x1b x2a L
    ...
    subgoal
      apply (cases M)
       apply (auto simp: literals_are_in_\<A>\<^sub>i\<^sub>n_sub option_conflict_rel_def conflict_rel_def
          literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset in_\<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_in_atms_of_iff is_pos_neg_not_is_pos
          extract_shorter_conflict_l_trivial_def
          intro!: mset_as_position.add
          dest!: multi_member_split[of _ D])
      done
    subgoal for D'
      using simple_clss_size_upper_div2[of D'] literals_are_in_\<A>\<^sub>i\<^sub>n_mono[of D D']
        distinct_mset_mono[of D' D] not_tautology_mono[of D' D]
      by (auto simp: literals_are_in_\<A>\<^sub>i\<^sub>n_sub option_conflict_rel_def conflict_rel_def
          literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset in_\<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_in_atms_of_iff upperN_def
...
\<open>extract_shorter_conflict_l_trivial_pre \<equiv> \<lambda>(M, D). literals_are_in_\<A>\<^sub>i\<^sub>n (mset (fst D))\<close>
...
\<open>twl_st_confl_extracted_assn = hr_comp twl_st_confl_extracted_int_assn
  (twl_st_ref_confl_extracted O twl_st_ref_no_clvls)\<close>
...
\<open>twl_st_confl_extracted_assn2 L = hr_comp twl_st_confl_extracted_int_assn
  (twl_st_ref_confl_extracted2 L O twl_st_ref_no_clvls)\<close>
...
lemma extract_shorter_conflict_list_st_int_extract_shorter_conflict_st_trivial_int:
  \<open>(extract_shorter_conflict_list_st_int, extract_shorter_conflict_st_trivial_int) \<in>
     ...
     literals_are_in_\<A>\<^sub>i\<^sub>n (the (get_conflict_wl_int S)) \<and>
         ...
         literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset (get_trail_wl_int S)) \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (the (get_conflict_wl_int S)) \<and>
...
proof -
  ...
  have H':
    \<open>M \<noteq> [] \<and>
     ...
     literals_are_in_\<A>\<^sub>i\<^sub>n (the ba) \<and>
     M \<noteq> [] \<and>
     literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset M) \<and>
     - lit_of (hd M) \<in># the ba \<and>
  ...
  \<open>find_decomp_wl_imp = (\<lambda>M\<^sub>0 D L vm. do {
    ...
    (_, M, vm') \<leftarrow>
       WHILE\<^sub>T\<^bsup>\<lambda>(j, M, vm'). j = count_decided M \<and> j \<ge> lev \<and>
           ...
           vm' \<in> vmtf_imp M \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset M)\<^esup>
         (\<lambda>(j, M, vm). j > lev)
         (\<lambda>(j, M, vm). do {
            ASSERT(M \<noteq> []);
            if is_decided (hd M)
...
sepref_thm vmtf_unset_code
  ...
  :: \<open>[\<lambda>(L, vm). \<exists>M. L = atm_of(lit_of (hd M)) \<and> vm \<in> vmtf_imp M \<and> M \<noteq> [] \<and>
          literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset M)]\<^sub>a
  ...
  supply [[goals_limit=1]] option.splits[split] vmtf_imp_def[simp] in_\<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_in_atms_of_iff[simp]
    neq_NilE[elim!] literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset[simp]
...
lemmas vmtf_unset_code_code[sepref_fr_rules] =
   vmtf_unset_code.refine[of \<A>\<^sub>i\<^sub>n, OF twl_array_code_axioms]

definition find_decomp_wl_imp_pre where
  \<open>find_decomp_wl_imp_pre = (\<lambda>(((M, D), L), vm). M \<noteq> [] \<and> D \<noteq> None \<and>
      literals_are_in_\<A>\<^sub>i\<^sub>n (the D) \<and> -L \<in># the D \<and>
      literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset M) \<and> vm \<in> vmtf_imp M)\<close>
...
sepref_thm find_decomp_wl_imp_code
  ...
  supply [[goals_limit=1]]   literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset[simp]
  supply uint32_nat_assn_one[sepref_fr_rules]
...
lemma
  assumes
   ...
   lits: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset M\<^sub>0)\<close>
  shows
...
proof -
  ...
  have atm_of_N:
    \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset aa) \<Longrightarrow> aa \<noteq> [] \<Longrightarrow> atm_of (lit_of (hd aa)) \<in> atms_of \<L>\<^sub>a\<^sub>l\<^sub>l\<close>
    for aa
    by (cases aa) (auto simp: literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset in_\<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_in_atms_of_iff)
  thm WHILEIT_rule
  show ?decomp
    ...
    subgoal for s a b aa ba x1 x2 x1a x2a  by (cases aa)
        (auto  simp: literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset)
    subgoal by auto
...
definition find_decomp_wl_vmtf_pre where
  \<open>find_decomp_wl_vmtf_pre = (\<lambda>(((M, E), L), vm).
      \<exists>N U D NP UP Q W. twl_struct_invs (twl_st_of_wl None (M, N, U, D, NP, UP, Q, W)) \<and>
       ...
       the E \<subseteq># the D \<and> D \<noteq> None \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset M) \<and>
...
definition find_decomp_wl_vmtf_pre_full where
  \<open>find_decomp_wl_vmtf_pre_full M' = (\<lambda>(((M, E), L), vm).
      \<exists>N U D NP UP Q W. twl_struct_invs (twl_st_of_wl None (M, N, U, D, NP, UP, Q, W)) \<and>
       ...
       M \<noteq> [] \<and> ex_decomp_of_max_lvl M D L \<and>
       the E \<subseteq># the D \<and> D \<noteq> None \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset M) \<and>
      vm \<in> vmtf_imp M \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (the E) \<and> -L \<in># the E \<and> M = M')\<close>
...
lemma find_decomp_wl_st_int_find_decomp_wl_st:
  \<open>(uncurry find_decomp_wl_st_int, uncurry find_decomp_wl_st) \<in>
   ...
   nat_lit_lit_rel \<times>\<^sub>r twl_st_ref_no_clvls \<rightarrow>
   \<langle>{(S', S). (S', S) \<in> twl_st_ref_no_clvls \<and>
  ...
  subgoal for L' M' N' U' D' K' W' Q' A' m'  _ _ _ \<phi> L M N U D NP UP W Q
    apply (auto simp: find_decomp_wl_st_int_def find_decomp_wl_st_def
        list_mset_rel_def br_def twl_st_ref_def twl_st_ref_no_clvls_def
...
fun set_conflict_and_clvls_wl where
  \<open>set_conflict_and_clvls_wl D (M, N, U, _, oth) = (M, N, U, D, oth)\<close>
...
lemma twl_st_ref_confl_extracted2_twl_st_ref:
  \<open>twl_st_ref_confl_extracted2 L O twl_st_ref_no_clvls =
    {((M', N', U', D', Q', W', vm, \<phi>, clvls), M, N, U, D, NP, UP, Q, W).
     M = M' \<and>
     N' = N \<and>
     U' = U \<and>
     ...
     (W', W) \<in> \<langle>Id\<rangle>map_fun_rel D\<^sub>0 \<and>
     vm \<in> vmtf_imp M \<and> phase_saving \<phi> \<and> no_dup M}\<close>
  unfolding twl_st_ref_confl_extracted2_def twl_st_ref_no_clvls_def
  by fast
...
definition find_decomp_wl_pre where
   \<open>find_decomp_wl_pre M = (\<lambda>(L, S). \<exists>D\<^sub>0. get_conflict_wl S \<noteq> None \<and>
               ...
               L = lit_of (hd (get_trail_wl S)) \<and>
               twl_struct_invs (twl_st_of_wl None (set_conflict_and_clvls_wl D\<^sub>0 S)) \<and>
               the (get_conflict_wl S) \<subseteq># the D\<^sub>0 \<and> D\<^sub>0 \<noteq> None \<and> get_trail_wl S = M \<and> L = lit_of (hd M) \<and>
               literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset M) \<and>
               -L \<in># the (get_conflict_wl S) \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (the D\<^sub>0))\<close>
...
proof -
  ...
  have H: \<open>?c
       \<in> [comp_PRE (nat_lit_lit_rel \<times>\<^sub>f Id) (\<lambda>_. True)
            (\<lambda>_. comp_PRE (nat_lit_lit_rel \<times>\<^sub>f twl_st_ref_no_clvls)
  ...
  have decomp_1: \<open>hr_comp (twl_st_confl_extracted_int_assn' M) twl_st_ref_no_clvls
        (M, aa, ab, ac, ad, ae, af, b) =
        hr_comp (twl_st_confl_extracted_int_assn' M)
            (twl_st_ref')
       (M, aa, ab, ac, ad, ae, af, b)\<close> for aa ab ac ad ae af b
    by (auto simp: hr_comp_def twl_st_ref_no_clvls_def twl_st_ref'_def ex_assn_pair_split
        eq_commute[of ac] intro!: ext)
  have decomp_2: \<open>hr_comp twl_st_confl_extracted_int_assn
     (twl_st_ref_confl_extracted O twl_st_ref_no_clvls) (M, aa, ab, ac, ad, ae, af, b) =
  ...
  proof -
    have [simp]: \<open>(a, M, X) \<in> twl_st_ref_confl_extracted O twl_st_ref_no_clvls \<longleftrightarrow>
          (a, M, X) \<in> twl_st_ref_confl_extracted O twl_st_ref'\<close> for X a
      unfolding twl_st_ref_confl_extracted_def twl_st_ref_no_clvls_def twl_st_ref'_def
      by fastforce
    show ?thesis
  ...
  have 1: \<open>hr_comp (twl_st_confl_extracted_int_assn' M) twl_st_ref_no_clvls
        (M, aa, ab, ac, ad, ae, af, b) =
        hr_comp twl_st_confl_extracted_int_assn
         (twl_st_ref_confl_extracted O twl_st_ref_no_clvls)
       (M, aa, ab, ac, ad, ae, af, b)\<close> for aa ab ac ad ae af b
  ...
  have 0:
    ...
    by simp
  have pre: \<open>?pre x \<Longrightarrow> ?pre' x\<close> for x
    unfolding comp_PRE_def twl_st_ref_no_clvls_def find_decomp_wl_vmtf_pre_full_def find_decomp_wl_pre_def
    apply (auto dest: 0)
    by (rule_tac x=aa in exI, rule_tac x=ab in exI, rule_tac x=\<open>Some ya\<close> in exI)
      (use literals_are_in_\<A>\<^sub>i\<^sub>n_mono in auto)
  ...
  have incl: \<open>?ref
         \<subseteq> twl_st_ref_confl_extracted2 L O twl_st_ref_no_clvls\<close>
    unfolding twl_st_ref_confl_extracted2_twl_st_ref
    by (auto  simp: twl_st_ref_no_clvls_def option_conflict_rel_with_cls_with_highest_def
...
definition extract_shorter_conflict_wl_pre where
  \<open>extract_shorter_conflict_wl_pre S \<longleftrightarrow>
      twl_struct_invs (twl_st_of_wl None S) \<and>
            ...
            literals_are_\<A>\<^sub>i\<^sub>n S\<close>
...
lemma backtrack_wl_D_invD:
  ...
  apply (cases S; auto)
  using assms literals_are_\<A>\<^sub>i\<^sub>n_conflict_literals_are_in_\<A>\<^sub>i\<^sub>n[of S]
...
proof -
  ...
  have
    lits: \<open>literals_are_\<A>\<^sub>i\<^sub>n (M, N, U, D, NP, UP, W, Q)\<close> and
  ...
  show ?thesis
    ...
    using literals_are_\<A>\<^sub>i\<^sub>n_trail_literals_are_in_\<A>\<^sub>i\<^sub>n[OF lits struct]
      literals_are_\<A>\<^sub>i\<^sub>n_conflict_literals_are_in_\<A>\<^sub>i\<^sub>n[OF lits _ struct]
      uL_E E_D' struct
    unfolding x D' xc
...
lemma size_conflict_wl_int_size_conflict_wl:
  \<open>(RETURN o size_conflict_wl_int, RETURN o size_conflict_wl) \<in>
   [\<lambda>S. get_conflict_wl S \<noteq> None]\<^sub>f twl_st_ref_confl_extracted2 L O twl_st_ref_no_clvls \<rightarrow>
...
proof -
  have H: \<open>?c
    \<in> [comp_PRE (twl_st_ref_confl_extracted2 L O twl_st_ref_no_clvls)
     ...
     (\<lambda>_. True)]\<^sub>a hrp_comp (twl_st_confl_extracted_int_assn\<^sup>k)
                    (twl_st_ref_confl_extracted2 L O
                     twl_st_ref_no_clvls) \<rightarrow> hr_comp uint32_nat_assn nat_rel\<close>
    (is \<open>_ \<in> [?pre']\<^sub>a ?im' \<rightarrow> ?f'\<close>)
...
lemma find_lit_of_max_level_wl_int_find_lit_of_max_level_wl:
  \<open>(uncurry (RETURN oo find_lit_of_max_level_wl_int), uncurry find_lit_of_max_level_wl) \<in>
    [\<lambda>(S, L'). L' = -L \<and> get_conflict_wl S \<noteq> None \<and> size (the (get_conflict_wl S)) > 1]\<^sub>f
     twl_st_ref_confl_extracted2 L O twl_st_ref_no_clvls \<times>\<^sub>r nat_lit_lit_rel \<rightarrow> \<langle>Id\<rangle> nres_rel\<close>
  by (intro frefI nres_relI)
...
proof -
  have H: \<open>?c
    \<in> [comp_PRE (twl_st_ref_confl_extracted2 L O twl_st_ref_no_clvls \<times>\<^sub>f nat_lit_lit_rel)
     ...
     (\<lambda>_. True)]\<^sub>a hrp_comp (twl_st_confl_extracted_int_assn\<^sup>k *\<^sub>a unat_lit_assn\<^sup>k)
                    (twl_st_ref_confl_extracted2 L O twl_st_ref_no_clvls \<times>\<^sub>f
  ...
  have pre: \<open>?pre' x\<close> if \<open>?pre x\<close> for x
    using that by (auto simp: comp_PRE_def highest_lit_def remove1_mset_empty_iff
        ...
        option_conflict_rel_def conflict_rel_def twl_st_ref_no_clvls_def)
  have im: \<open>?im' = ?im\<close>
...
lemma extract_shorter_conflict_l_trivial_int_extract_shorter_conflict_l_trivial:
  \<open>(extract_shorter_conflict_st_trivial_int, extract_shorter_conflict_st_trivial) \<in>
    [\<lambda>S. get_conflict_wl S \<noteq> None]\<^sub>f twl_st_ref \<rightarrow> \<langle>twl_st_ref_no_clvls\<rangle> nres_rel\<close>
  by (intro frefI nres_relI)
     (auto simp: extract_shorter_conflict_st_trivial_int_def twl_st_ref_no_clvls_def
...
lemma conflict_to_conflict_with_cls_id:
  \<open>(uncurry conflict_to_conflict_with_cls, uncurry (RETURN oo conflict_to_conflict_with_cls_spec)) \<in>
    [\<lambda>(D, C). C \<noteq> None \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (the C) \<and> length D = size (the C) + 2 \<and>
...
proof -
  have H: \<open>conflict_to_conflict_with_cls D (b, n, xs) \<le> \<Down> (option_conflict_rel_with_cls_removed L L') (RETURN (Some C))\<close>
    if
      ocr: \<open>((b, n, xs), Some C) \<in> option_conflict_rel_removed\<close> and
      lits_\<A>\<^sub>i\<^sub>n: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n C\<close> and
  ...
  proof -
    ...
    have final: "((ad, bd), Some C) \<in> option_conflict_rel_with_cls_removed L L'"
      if
        s0: "?I baa aa s" and
        s1: "?I' s" and
        s:
    ...
    proof -
      have [simp]: \<open>ac = 2\<close> \<open>s = (ab, 2, ad, bd)\<close> \<open>bb = (2, ad, bd)\<close> \<open>bc = (ad, bd)\<close> \<open>ba = (aa, baa)\<close>
    ...
    proof -
      ...
      then show ?thesis
        ...
        by (auto dest!: multi_member_split simp: literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset in_\<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_in_atms_of_iff)
    qed
    ...
    proof (rule ccontr)
      ...
      have \<open>L \<in># C \<Longrightarrow> atm_of L < upperN div 2\<close> for L
        using lits_\<A>\<^sub>i\<^sub>n in_N1_less_than_upperN
        by (cases L)  (auto dest!: multi_member_split simp: literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset upperN_def)
  ...
  show ?thesis
    ...
    apply clarify
    subgoal for a aa ab b ac ba y
      using H[of aa ab b y ac]
      by (auto simp: conflict_to_conflict_with_cls_spec_def)
    done
qed
...
lemma conflict_to_conflict_with_cls_code_helper:
  ...
  by (auto simp: upperN_def zero_uint32_nat_def one_nat_uint32_def fast_minus_def)
...
lemma extract_shorter_conflict_with_cls_code_conflict_to_conflict_with_cls_spec[sepref_fr_rules]:
  \<open>(uncurry conflict_to_conflict_with_cls_code,  uncurry (RETURN \<circ>\<circ> conflict_to_conflict_with_cls_spec))
    \<in> [\<lambda>(D, C). C \<noteq> None \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (the C) \<and>
  ...
  using conflict_to_conflict_with_cls_code_refine[unfolded PR_CONST_def,
    FCOMP conflict_to_conflict_with_cls_id, of L L']
  unfolding conflict_with_cls_assn_removed_def[symmetric]
  by simp
...
proof -
  ...
  have pre: \<open>?pre' x\<close> if \<open>?pre x\<close> for x
    using that by (auto simp: comp_PRE_def twl_st_trail_ref_def trailt_ref_def option_conflict_rel_def
...
definition add2_from_conflict :: \<open>nat literal \<Rightarrow> nat literal \<Rightarrow> nat cconflict \<Rightarrow> nat cconflict\<close> where
  \<open>add2_from_conflict L L' C = Some (add_mset L (add_mset L' (the C)))\<close>

definition add2_from_conflict_int
  :: \<open>nat literal \<Rightarrow> nat literal \<Rightarrow> conflict_rel_with_cls \<Rightarrow> conflict_rel_with_cls\<close> where
  \<open>add2_from_conflict_int _ _ C = C\<close>


lemma add2_from_conflict_int_add2_from_conflict:
  ...
  subgoal for K K' D zs ab bb bc C
    by (cases D; cases \<open>tl D\<close>)
      (auto simp: option_conflict_rel_with_cls_def add2_from_conflict_int_def
        add2_from_conflict_def list_mset_rel_def option_conflict_rel_with_cls_removed_def
        br_def)
  done

sepref_thm add2_from_conflict_code
  is \<open>uncurry2 (RETURN ooo add2_from_conflict_int)\<close>
  :: \<open>unat_lit_assn\<^sup>k *\<^sub>a unat_lit_assn\<^sup>k *\<^sub>a conflict_with_cls_int_assn\<^sup>d \<rightarrow>\<^sub>a conflict_with_cls_int_assn\<close>
  unfolding add2_from_conflict_int_def by sepref

concrete_definition (in -) add2_from_conflict_code
   uses twl_array_code.add2_from_conflict_code.refine_raw
   is \<open>(uncurry2 ?f, _)\<in>_\<close>

prepare_code_thms (in -) add2_from_conflict_code_def

lemmas add2_from_conflict_code_hnr[sepref_fr_rules] =
   add2_from_conflict_code.refine[of \<A>\<^sub>i\<^sub>n, OF twl_array_code_axioms]
...
definition list_of_mset2_None_int where
  \<open>list_of_mset2_None_int L L' C\<^sub>0 =  do {
     ...
     ASSERT(C \<noteq> None \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (the C) \<and> size (the C\<^sub>0) = size (the C) + 2 \<and>
       D!0 = L \<and> D!1 = L');
...
lemma list_of_mset2_None_int_list_of_mset2_None:
  \<open>(uncurry2 (list_of_mset2_None_int), uncurry2 list_of_mset2_None) \<in>
     [\<lambda>((L, L'), C). C \<noteq> None \<and> L \<in># the C \<and> L' \<in># the C \<and> L \<noteq> L' \<and>
        literals_are_in_\<A>\<^sub>i\<^sub>n (the C) \<and> distinct_mset (the C)]\<^sub>f Id \<times>\<^sub>f Id \<times>\<^sub>f Id \<rightarrow> \<langle>Id\<rangle>nres_rel\<close>
  by (intro frefI nres_relI)
   (auto simp: list_of_mset2_None_int_def list_of_mset2_None_def
      ...
      literals_are_in_\<A>\<^sub>i\<^sub>n_sub literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset
      dest!: multi_member_split)
...
definition (in -) conflict_with_cls_split where
...
sepref_thm list_of_mset2_None_code
  ...
  :: \<open>[\<lambda>((L, L'), C). C \<noteq> None \<and> L \<in># the C \<and> L' \<in># the C \<and> L \<noteq> L' \<and> L \<in># \<L>\<^sub>a\<^sub>l\<^sub>l \<and> L' \<in># \<L>\<^sub>a\<^sub>l\<^sub>l \<and>
       literals_are_in_\<A>\<^sub>i\<^sub>n (the C)]\<^sub>a
...
lemma list_of_mset2_None_hnr[sepref_fr_rules]:
  \<open>(uncurry2 list_of_mset2_None_code, uncurry2 list_of_mset2_None)
   \<in> [\<lambda>((a, b), ba). ba \<noteq> None \<and> a \<in># the ba \<and> b \<in># the ba \<and> a \<noteq> b \<and>
       literals_are_in_\<A>\<^sub>i\<^sub>n (the ba) \<and> distinct_mset (the ba) \<and> a \<in># \<L>\<^sub>a\<^sub>l\<^sub>l \<and> b \<in># \<L>\<^sub>a\<^sub>l\<^sub>l]\<^sub>a
      unat_lit_assn\<^sup>k *\<^sub>a unat_lit_assn\<^sup>k *\<^sub>a conflict_option_assn\<^sup>d \<rightarrow>
...
lemma extract_shorter_conflict_l_trivial_code_extract_shorter_conflict_l_trivial:
  \<open>(uncurry extract_shorter_conflict_l_trivial_code,
    ...
    \<in> [\<lambda>(M', D). M' \<noteq> [] \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (the D) \<and> D \<noteq> None \<and> M' = M \<and>
         -lit_of (hd M) \<in># the D \<and>  0 < get_level M (lit_of (hd M)) \<and>
         literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset M) \<and> literals_are_in_\<A>\<^sub>i\<^sub>n (the D) \<and>
...
proof -
  ...
  \<in> [comp_PRE (Id \<times>\<^sub>f \<langle>Id\<rangle>option_rel)
       ...
       (\<lambda>_. comp_PRE (Id \<times>\<^sub>f option_conflict_rel)
              (\<lambda>(M', D). literals_are_in_\<A>\<^sub>i\<^sub>n (the D) \<and> D \<noteq> None \<and> M = M' \<and> M \<noteq> [] \<and>
                  literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset M) \<and> - lit_of (hd M) \<in># the D \<and>
...
definition extract_shorter_conflict_st_trivial_pre where
  \<open>extract_shorter_conflict_st_trivial_pre S \<longleftrightarrow> (get_conflict_wl S \<noteq> None \<and>
          get_trail_wl S \<noteq> [] \<and>
          literals_are_in_\<A>\<^sub>i\<^sub>n (the (get_conflict_wl S)) \<and>
          - lit_of (hd (get_trail_wl S)) \<in># the (get_conflict_wl S) \<and>
          0 < get_level (get_trail_wl S) (lit_of (hd (get_trail_wl S))) \<and>
          literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset (get_trail_wl S)) \<and>
          literals_are_in_\<A>\<^sub>i\<^sub>n (the (get_conflict_wl S)) \<and>
...
proof -
  have H: \<open>?c
       \<in> [comp_PRE twl_st_ref (\<lambda>S. get_conflict_wl S \<noteq> None)
               (\<lambda>_. comp_PRE twl_st_wl_int_conflict_rel
                  (\<lambda>S. get_conflict_wl_int S \<noteq> None \<and>
                     ...
                     - lit_of (hd (get_trail_wl_int S)) \<in># the (get_conflict_wl_int S) \<and>
                     0 < get_level (get_trail_wl_int S) (lit_of (hd (get_trail_wl_int S))) \<and>
                     literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset (get_trail_wl_int S)) \<and>
                     literals_are_in_\<A>\<^sub>i\<^sub>n (the (get_conflict_wl_int S)) \<and> distinct_mset (the (get_conflict_wl_int S)) \<and> \<not> tautology (the (get_conflict_wl_int S)))
     ...
     hr_comp (hr_comp twl_st_confl_extracted_int_assn twl_st_ref_confl_extracted) twl_st_ref_no_clvls\<close>
    (is \<open>_ \<in> [?pre']\<^sub>a ?im' \<rightarrow> ?f'\<close>)
    using hfref_compI_PRE_aux[OF
...
proof -
  have
    ...
    lits: \<open>literals_are_\<A>\<^sub>i\<^sub>n S\<close>
  ...
  have lits_conf: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (the (get_conflict_wl S))\<close>
    using literals_are_\<A>\<^sub>i\<^sub>n_conflict_literals_are_in_\<A>\<^sub>i\<^sub>n[OF lits confl struct_invs] .
  ...
  have lits_trail: \<open>literals_are_in_\<A>\<^sub>i\<^sub>n (lit_of `# mset (get_trail_wl S))\<close>
    using literals_are_\<A>\<^sub>i\<^sub>n_trail_literals_are_in_\<A>\<^sub>i\<^sub>n[OF lits struct_invs] .
...
lemma vmtf_rescore_score_clause:
  \<open>(uncurry3 vmtf_rescore, uncurry3 rescore_clause) \<in>
     [\<lambda>(((C, M), vm), \<phi>). literals_are_in_\<A>\<^sub>i\<^sub>n (mset C) \<and> vm \<in> vmtf_imp M \<and> phase_saving \<phi>]\<^sub>f
...
proof -
  have H: \<open>vmtf_rescore_body C M vm \<phi> \<le>
    ...
    if M: \<open>vm \<in> vmtf_imp M\<close>\<open>phase_saving \<phi>\<close> and C: \<open>\<forall>c\<in>set C. atm_of c \<in> atms_of \<L>\<^sub>a\<^sub>l\<^sub>l\<close>
    for C vm \<phi> M
  ...
  show ?thesis
    ...
    subgoal
      by (meson atm_of_lit_in_atms_of contra_subsetD in_all_lits_of_m_ain_atms_of_iff
          in_multiset_in_set literals_are_in_\<A>\<^sub>i\<^sub>n_def)
    subgoal by auto
    done
qed
...
lemma vmtf_rescore_code_rescore_clause[sepref_fr_rules]:
  \<open>(uncurry3 vmtf_rescore_code, uncurry3 (PR_CONST rescore_clause))
     \<in> [\<lambda>(((b, a), c), d). literals_are_in_\<A>\<^sub>i\<^sub>n (mset b) \<and> c \<in> vmtf_imp a \<and>
...
lemma twl_st_wl_W_conflict_alt_def:
  \<open>twl_st_wl_W_conflict =
     (Id \<times>\<^sub>r Id \<times>\<^sub>r Id \<times>\<^sub>r option_conflict_rel \<times>\<^sub>r Id \<times>\<^sub>r Id \<times>\<^sub>r Id) O twl_st_ref_no_clvls\<close>
  unfolding twl_st_wl_W_conflict_def twl_st_ref_no_clvls_def
  by force

definition twl_st_W_conflict_int_assn :: \<open>twl_st_wl_int_conflict \<Rightarrow> twl_st_wll_trail \<Rightarrow> assn\<close> where
...
lemma st_remove_highest_lvl_from_confl_int_st_remove_highest_lvl_from_confl:
  \<open>(RETURN o st_remove_highest_lvl_from_confl_int, RETURN o st_remove_highest_lvl_from_confl) \<in>
   (twl_st_ref_confl_extracted2 L O twl_st_ref_no_clvls) \<rightarrow>\<^sub>f \<langle>twl_st_wl_W_conflict\<rangle> nres_rel\<close>
  by (intro frefI nres_relI)
     (auto simp: st_remove_highest_lvl_from_confl_int_def st_remove_highest_lvl_from_confl_def
      twl_st_wl_W_conflict_def twl_st_ref_confl_extracted2_def twl_st_ref_no_clvls_def
      option_conflict_rel_with_cls_with_highest2_def)


sepref_thm st_remove_highest_lvl_from_confl_code
  is \<open>RETURN o st_remove_highest_lvl_from_confl_int\<close>
...
lemma twl_st_assn_twl_st_wl_W_conflict:
  ...
  unfolding twl_st_assn_no_clvls_def twl_st_wl_W_conflict_alt_def twl_st_W_conflict_int_assn_def
    twl_st_int_assn_def hr_comp_assoc[symmetric] conflict_option_assn_def
  by simp
...
lemma st_remove_highest_lvl_from_confl_hnr[sepref_fr_rules]:
  ...
  using st_remove_highest_lvl_from_confl_int_hnr[FCOMP st_remove_highest_lvl_from_confl_int_st_remove_highest_lvl_from_confl]
...
definition twl_st_confl_extracted_int_assn2
  :: \<open>nat literal \<Rightarrow> _ \<Rightarrow> _ \<Rightarrow> twl_st_wll_trail_confl_extracted \<Rightarrow> assn\<close> where
\<open>twl_st_confl_extracted_int_assn2 L M =
  (trail_assn *assn clauses_ll_assn *assn nat_assn *assn
  conflict_with_cls_with_cls_with_highest_assn2 L M *assn
  clause_l_assn *assn
  arrayO_assn (arl_assn nat_assn) *assn
  vmtf_remove_conc *assn phase_saver_conc *assn uint32_nat_assn
  )\<close>
...
definition propgate_bt_wl_D_int
  ...
  \<open>propgate_bt_wl_D_int = (\<lambda>L L' (M, N, U, D, Q, W, vm, \<phi>, _). do {
      (D'', C) \<leftarrow> list_of_mset2_None (- L) L' D;
      ASSERT(literals_are_in_\<A>\<^sub>i\<^sub>n (mset D''));
...
sepref_thm propgate_bt_wl_D_code
  ...
  :: \<open>[\<lambda>((L, L'), S). get_conflict_wl_int S \<noteq> None \<and> -L \<in># the (get_conflict_wl_int S) \<and>
       ...
       literals_are_in_\<A>\<^sub>i\<^sub>n (the (get_conflict_wl_int S)) \<and>
       distinct_mset (the (get_conflict_wl_int S)) \<and> L \<in># \<L>\<^sub>a\<^sub>l\<^sub>l \<and> L' \<in># \<L>\<^sub>a\<^sub>l\<^sub>l \<and>
...
lemma propgate_bt_wl_D_int_propgate_bt_wl_D:
  \<open>(uncurry2 propgate_bt_wl_D_int, uncurry2 propgate_bt_wl_D) \<in>
    ...
    literals_are_in_\<A>\<^sub>i\<^sub>n (the (get_conflict_wl S))]\<^sub>f
     Id \<times>\<^sub>f Id \<times>\<^sub>f twl_st_ref_no_clvls \<rightarrow> \<langle>twl_st_ref\<rangle>nres_rel\<close>
  apply (intro frefI nres_relI)
  unfolding propgate_bt_wl_D_int_def propgate_bt_wl_D_alt_def twl_st_ref_def list_of_mset2_None_def
    twl_st_ref_no_clvls_def uncurry_def
  apply clarify
  apply refine_vcg
  apply
...
proof -
  have H: \<open>?fun \<in>
     [comp_PRE (nat_lit_lit_rel \<times>\<^sub>f nat_lit_lit_rel \<times>\<^sub>f twl_st_ref_no_clvls)
     (\<lambda>((L, L'), S).
         get_conflict_wl S \<noteq> None \<and>
         - L \<noteq> L' \<and>
         undefined_lit (get_trail_wl S) L \<and>
         literals_are_in_\<A>\<^sub>i\<^sub>n (the (get_conflict_wl S)))
     (\<lambda>_ ((L, L'), S).
     ...
     (\<lambda>_. True)]\<^sub>a hrp_comp
                     ...
                     (nat_lit_lit_rel \<times>\<^sub>f nat_lit_lit_rel \<times>\<^sub>f
                      twl_st_ref_no_clvls) \<rightarrow> hr_comp twl_st_int_assn twl_st_ref
\<close>
    (is \<open>_ \<in> [?pre']\<^sub>a ?im' \<rightarrow> ?f'\<close>)
    ...
    .
  have pre: \<open>?pre' x\<close> if \<open>?pre x\<close> for x
    using that unfolding comp_PRE_def twl_st_ref_def map_fun_rel_def propgate_bt_wl_D_pre_def
...
proof -
  ...
  have
    lits: \<open>literals_are_\<A>\<^sub>i\<^sub>n x\<close> and
    D: \<open>D \<noteq> None\<close> and
    ...
    by auto
  have lits_D': \<open>literals_are_in_\<A>\<^sub>i\<^sub>n D'\<close>
    using literals_are_\<A>\<^sub>i\<^sub>n_conflict_literals_are_in_\<A>\<^sub>i\<^sub>n[OF lits _ struct_invs] D
      literals_are_in_\<A>\<^sub>i\<^sub>n_mono[OF _ D'_D] unfolding xd x by auto
  ...
  show ?thesis
    ...
    subgoal
      using literals_are_\<A>\<^sub>i\<^sub>n_trail_literals_are_in_\<A>\<^sub>i\<^sub>n[OF lits struct_invs] M_nempty
      unfolding x by (cases M) (auto simp: lit_of_hd_trail_st_def literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset)
    subgoal
      using xf lits_D' by (auto dest!: multi_member_split in_diffD simp: literals_are_in_\<A>\<^sub>i\<^sub>n_add_mset)
...
lemma propgate_unit_bt_wl_D_int_propgate_unit_bt_wl_D:
  \<open>(uncurry propgate_unit_bt_wl_D_int, uncurry propgate_unit_bt_wl_D) \<in>
     ...
     Id \<times>\<^sub>f twl_st_ref_no_clvls \<rightarrow> \<langle>twl_st_ref\<rangle>nres_rel\<close>
  by (intro frefI nres_relI)
...
sepref_thm propgate_unit_bt_wl_D_code
  ...
  :: \<open>[\<lambda>(L, S). get_conflict_wl_int S \<noteq> None \<and> size (the (get_conflict_wl_int S)) = 1 \<and>
        undefined_lit (get_trail_wl_int S) L \<and>
         -L \<in># the (get_conflict_wl_int S) \<and> L \<in># \<L>\<^sub>a\<^sub>l\<^sub>l \<and> get_vmtf_int S \<in> vmtf_imp (get_trail_wl_int S)]\<^sub>a
   unat_lit_assn\<^sup>k *\<^sub>a twl_st_int_assn\<^sup>d \<rightarrow> twl_st_int_assn\<close>
  supply [[goals_limit = 1]] rescore_def[simp] image_image[simp] uminus_\<A>\<^sub>i\<^sub>n_iff[simp]
  unfolding propgate_unit_bt_wl_D_int_def cons_trail_Propagated_def[symmetric] twl_st_int_assn_def
  PR_CONST_def
  apply (rewrite at \<open>(_, add_mset _ \<hole>, _)\<close> lms_fold_custom_empty)+
  by sepref
...
proof -
  have H: \<open>?c \<in>
    [comp_PRE (nat_lit_lit_rel \<times>\<^sub>f twl_st_ref_no_clvls)
       (\<lambda>(L, S). get_conflict_wl S \<noteq> None \<and> undefined_lit (get_trail_wl S) L \<and>
       ...
       (\<lambda>_ (L, S). get_conflict_wl_int S \<noteq> None \<and> size (the (get_conflict_wl_int S)) = 1 \<and>
           ...
           L \<in># \<L>\<^sub>a\<^sub>l\<^sub>l \<and> local.get_vmtf_int S \<in> vmtf_imp (get_trail_wl_int S))
       (\<lambda>_. True)]\<^sub>a
   hrp_comp (unat_lit_assn\<^sup>k *\<^sub>a twl_st_int_assn\<^sup>d) (nat_lit_lit_rel \<times>\<^sub>f twl_st_ref_no_clvls) \<rightarrow>
   hr_comp twl_st_int_assn twl_st_ref\<close>
    (is \<open>_ \<in> [?pre']\<^sub>a ?im' \<rightarrow> ?f'\<close>)
  ...
  have pre: \<open>?pre' x\<close> if \<open>?pre x\<close> for x
    using that by (auto simp: comp_PRE_def twl_st_ref_def  twl_st_ref_no_clvls_def
...
proof -
  have H: \<open>L < length xs\<close> (is \<open>?length\<close>) and
    ...
    if L_N: \<open>Pos L \<in># \<L>\<^sub>a\<^sub>l\<^sub>l\<close> and  tr: \<open>((M', xs, lvls, k), M) \<in> trailt_ref\<close>
    for M xs lvls k M' L
  proof -
    have
       \<open>M = M'\<close> and
       \<open>\<forall>L\<in>#\<L>\<^sub>a\<^sub>l\<^sub>l. atm_of L < length xs \<and> xs ! atm_of L = valued_atm_on_trail M (atm_of L)\<close>
...
sepref_thm vmtf_find_next_undef_upd_code
  ...
  :: \<open>trail_assn\<^sup>d *\<^sub>a vmtf_remove_conc\<^sup>d \<rightarrow>\<^sub>a
     (hr_comp trailt_conc trailt_ref *assn vmtf_remove_conc) *assn
        option_assn uint32_nat_assn\<close>
  supply [[goals_limit=1]]
  supply not_is_None_not_None[simp]
...
where
  \<open>find_undefined_atm M _ = SPEC(\<lambda>((M', vm), L). (L \<noteq> None \<longrightarrow> Pos (the L) \<in>snd ` D\<^sub>0 \<and> undefined_atm M (the L)) \<and>
     (L = None \<longrightarrow> (\<forall>K\<in>snd ` D\<^sub>0. defined_lit M K)) \<and> M = M' \<and> vm \<in> vmtf_imp M)\<close>

definition find_unassigned_lit_wl_D_int :: \<open>twl_st_wl_int \<Rightarrow> (twl_st_wl_int \<times> nat literal option) nres\<close>
where
  \<open>find_unassigned_lit_wl_D_int = (\<lambda>(M, N, U, D, WS, Q, vm, \<phi>, clvls). do {
...
lemma find_unassigned_lit_wl_D'_find_unassigned_lit_wl_D:
  \<open>(find_unassigned_lit_wl_D_int, find_unassigned_lit_wl_D) \<in>
     [\<lambda>S. twl_struct_invs (twl_st_of_wl None S) \<and> literals_are_\<A>\<^sub>i\<^sub>n S \<and> get_conflict_wl S = None]\<^sub>f
...
proof -
  ...
  have atms: \<open>atms_of \<L>\<^sub>a\<^sub>l\<^sub>l =
      ...
      if inv: \<open>twl_struct_invs (twl_st_of_wl None (M, N, U, D, NP, UP, WS, Q))\<close> and
        \<A>\<^sub>i\<^sub>n: \<open>literals_are_\<A>\<^sub>i\<^sub>n (M, N, U, D, NP, UP, WS, Q)\<close> and
  ...
  qed
  show ?thesis
   unfolding find_unassigned_lit_wl_D_int_def find_unassigned_lit_wl_D_def find_undefined_atm_def
    ID_R lit_of_found_atm_def
   apply (intro frefI nres_relI)
   apply clarify
   apply refine_vcg
   unfolding RETURN_RES_refine_iff
   by (auto simp: twl_st_ref_def atms in_\<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_in_atms_of_iff Ball_def image_image
...
lemma vmtf_find_next_undef_upd:
  \<open>(uncurry vmtf_find_next_undef_upd, uncurry find_undefined_atm) \<in>
     [\<lambda>(M, vm). vm \<in> vmtf_imp M]\<^sub>f Id \<times>\<^sub>f Id \<rightarrow> \<langle>Id \<times>\<^sub>f Id \<times>\<^sub>f \<langle>nat_rel\<rangle>option_rel\<rangle>nres_rel\<close>
...
sepref_thm find_unassigned_lit_wl_D_code
  ...
  :: \<open>[\<lambda>(M, N, U, D, WS, Q, vm, \<phi>, clvls). vm \<in> vmtf_imp M \<and> phase_saving \<phi>]\<^sub>a
...
definition find_unassigned_lit_wl_D_pre where
  \<open>find_unassigned_lit_wl_D_pre = (\<lambda>S. twl_struct_invs (twl_st_of_wl None S) \<and> literals_are_\<A>\<^sub>i\<^sub>n S \<and>
...
proof -
  ...
  \<in> [comp_PRE twl_st_ref
       ...
       (\<lambda>_ (M, N, U, D, WS, Q, vm, \<phi>, clvls).
           vm \<in> vmtf_imp M \<and> phase_saving \<phi>)
       (\<lambda>_. True)]\<^sub>a hrp_comp (twl_st_int_assn\<^sup>d)
                       twl_st_ref \<rightarrow> hr_comp
         (twl_st_int_assn *assn option_assn unat_lit_assn)
         (twl_st_ref \<times>\<^sub>f \<langle>nat_lit_lit_rel\<rangle>option_rel)\<close>
    (is \<open>_ \<in> [?pre']\<^sub>a ?im' \<rightarrow> ?f'\<close>)
    using hfref_compI_PRE_aux[OF find_unassigned_lit_wl_D_int_hnr[unfolded PR_CONST_def]
...
definition decide_lit_wl_int :: \<open>nat literal \<Rightarrow> twl_st_wl_int \<Rightarrow> twl_st_wl_int\<close> where
  \<open>decide_lit_wl_int = (\<lambda>L' (M, N, U, D, Q, W).
      (Decided L' # M, N, U, D, {#- L'#}, W))\<close>

lemma decide_lit_wl_int_decide_lit_wl:
  \<open>(uncurry (RETURN oo decide_lit_wl_int), uncurry (RETURN oo decide_lit_wl)) \<in>
     [\<lambda>(L, S). undefined_lit (get_trail_wl S) L \<and> get_conflict_wl S = None]\<^sub>f nat_lit_lit_rel \<times>\<^sub>r twl_st_ref \<rightarrow> \<langle>twl_st_ref\<rangle>nres_rel\<close>
...
lemma cons_trail_Decided_tr:
  ...
  [\<lambda>(L, M). undefined_lit M L \<and> L \<in> snd ` D\<^sub>0]\<^sub>f Id \<times>\<^sub>f trailt_ref \<rightarrow> \<langle>trailt_ref\<rangle>nres_rel\<close>
  by (intro frefI nres_relI, rename_tac x y, case_tac \<open>fst x\<close>)
     (auto simp: trailt_ref_def valued_atm_on_trail_def cons_trail_Decided_def uminus_lit_swap
        trailt_ref_def cons_trail_Decided_tr_def Decided_Propagated_in_iff_in_lits_of_l
...
lemma cons_trail_Decided_tr_code_cons_trail_Decided_tr[sepref_fr_rules]:
  \<open>(uncurry cons_trail_Decided_tr_code, uncurry (RETURN oo cons_trail_Decided)) \<in>
    ...
    (is \<open>?c \<in> [?pre]\<^sub>a ?im \<rightarrow> ?f\<close>)
proof -
  have H: \<open>?c \<in> [comp_PRE (nat_lit_lit_rel \<times>\<^sub>f trailt_ref)
     ...
     (\<lambda>_. True)]\<^sub>a hrp_comp
                     (unat_lit_assn\<^sup>k *\<^sub>a trailt_conc\<^sup>d)
                     (nat_lit_lit_rel \<times>\<^sub>f
                      trailt_ref) \<rightarrow> hr_comp trailt_conc
         trailt_ref\<close>
    (is \<open>_ \<in> [?pre']\<^sub>a ?im' \<rightarrow> ?f'\<close>)
...
proof -
  ...
  \<in> [comp_PRE (nat_lit_lit_rel \<times>\<^sub>f twl_st_ref)
    ...
    (is \<open>_ \<in> [?pre']\<^sub>a ?im' \<rightarrow> ?f'\<close>)
    using hfref_compI_PRE_aux[OF decide_lit_wl_int_hnr[unfolded PR_CONST_def]
        decide_lit_wl_int_decide_lit_wl] .
  have pre: \<open>?pre x \<Longrightarrow> ?pre' x\<close> for x
    by (auto simp: comp_PRE_def twl_st_ref_def image_image intro!: ext)
  have im: \<open>?im' = ?im\<close>
    unfolding prod_hrp_comp hrp_comp_dest hrp_comp_keep twl_st_assn_def
    by (auto simp: hrp_comp_def hr_comp_def)
  have f: \<open>?f' = ?f\<close>
