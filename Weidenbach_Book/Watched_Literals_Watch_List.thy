...
(*
  ...
  proof -
    ...
    moreover have [simp]: \<open>NO_MATCH None d \<Longrightarrow> blits_in_\<L>\<^sub>i\<^sub>n(x1b, x1aa, d, x1c, x1d, x2e, g) \<longleftrightarrow>
    ...
    ultimately show ?thesis
      using S_S' SLw w_le j_w n that corr_w n blit_in_lit unfolding set_conflict_wl_def set_conflict_l_def
...
proof -
  ...
  have propagate_unit_bt_wl: \<open>(propagate_unit_bt_wl (lit_of (hd (get_trail_wl S'))) U',
    ...
    \<in> {(T', T). (T', T) \<in> state_wl_l None \<and> correct_watching T' \<and> blits_in_\<L>\<^sub>i\<^sub>n T'} \<close>
  ...
  proof -
    ...
    show ?thesis
      ...
      subgoal using blits by (auto simp: S' blits_in_\<L>\<^sub>i\<^sub>n_def)
      done
  qed
  show ?thesis
