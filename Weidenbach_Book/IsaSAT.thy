...
proof -
  ...
  proof -
    ...
    have conclusive_le: \<open>IsaSAT.conclusive_TWL_run (fst T)
    \<le> \<Down> {(S, T). T = state\<^sub>W_of S}
       (SPEC
	...
	by auto
	 ...
	 by (auto simp: cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_learned_clauses_entailed_by_init_def)
      ...
      have CS_clss: \<open>cdcl\<^sub>W_restart_mset.clauses (state\<^sub>W_of (fst T)) = mset `# mset CS\<close>
        using learned clss oth confl unfolding clauses_def to_init_state0_def init_state0_def cdcl\<^sub>W_restart_mset.clauses_def
	by (cases T) auto
    ...
    if b then do {
        ...
        then RETURN T
          ...
          then do {
             ...
             ASSERT(mset `# ran_mf (get_clauses_wl T) + get_unit_clauses_wl T = mset `# mset CS);
             ASSERT(learned_clss_l (get_clauses_wl T) = {#});
             cdcl_twl_stgy_restart_prog_wl_D T
          } else do {
             T \<leftarrow> rewatch_st (finalise_init T);
             ASSERT (extract_atms_clss CS {} \<noteq> {});
             ASSERT(isasat_input_bounded_nempty (mset_set \<A>\<^sub>i\<^sub>n'));
...
(*TODO kill all_blits_are_in_problem; Move*)
...
proof -
  ...
  have cdcl_twl_stgy_restart_prog_wl_D2: \<open>cdcl_twl_stgy_restart_prog_wl_D U'
    ...
    if 
      \<open>(extract_atms_clss CS {}, \<A>)     \<in> {(x, _). x = extract_atms_clss CS {}}\<close> and
      \<open>(b, b') \<in> bool_rel\<close> and
      \<open>(T, \<A>') \<in> init_dt_wl_rel (to_init_state init_state_wl)\<close> and    \<open>(from_init_state T, T')
       \<in> {(S, S'). S = fst S'} O
	 {(S, S'). remove_watched S = S'} O state_wl_l_init\<close> and
      \<open>\<not> get_conflict_wl (from_init_state T) \<noteq> None\<close> and
      \<open>\<not> get_conflict_l_init T' \<noteq> None\<close> and
      \<open>CS \<noteq> []\<close> and
      \<open>CS \<noteq> []\<close> and
      \<open>(c, c') \<in> bool_rel\<close> and
      \<open>inres (rewatch_st (finalise_init (from_init_state T))) U'\<close> and
      U': \<open>(U', fst T') \<in> {(S, T). (S, T) \<in> state_wl_l None \<and> correct_watching S \<and> blits_in_\<L>\<^sub>i\<^sub>n S}\<close> and
      \<open>extract_atms_clss CS {} \<noteq> {}\<close> and
      \<open>clauses_to_update_l (fst T') = {#}\<close> and
      \<open>mset `# ran_mf (get_clauses_l (fst T')) + get_unit_clauses_l (fst T') =
       mset `# mset CS\<close> and
      \<open>learned_clss_l (get_clauses_l (fst T')) = {#}\<close> and
      \<open>extract_atms_clss CS {} \<noteq> {}\<close> and
      \<open>isasat_input_bounded_nempty (mset_set (extract_atms_clss CS {}))\<close> and
      \<open>mset `# ran_mf (get_clauses_wl U') + get_unit_clauses_wl U' =
       mset `# mset CS\<close> and
  ...
  show ?thesis
    ...
    subgoal using bounded by (auto simp: isasat_input_bounded_nempty_def extract_atms_clss_alt_def
      simp del: isasat_input_bounded_def)
    subgoal for \<A> b ba T \<A>' T' bb bc U
     apply (subst (asm) twl_st_wl(2)[of U _ None])
     apply simp
     apply (subst (asm) twl_st_wl(10)[of U _ None])
     apply simp_all
     done
    subgoal for \<A> b ba T \<A>' T' bb bc U
     apply (subst (asm) (2)twl_st_wl(2)[of U _ None])
     apply simp_all
     done
    subgoal by (rule cdcl_twl_stgy_restart_prog_wl_D2)
    apply (rule rewatch_st_fst; assumption)
    subgoal using bounded by (auto simp: isasat_input_bounded_nempty_def extract_atms_clss_alt_def
      simp del: isasat_input_bounded_def)
    subgoal for \<A> b ba T \<A>' T' bb bc U
  ...
  \<open>IsaSAT_heur opts CS = do{
	...
	T \<leftarrow> rewatch_heur_st T;
        ...
        then RETURN (empty_init_code)
          ...
          if length (get_clauses_wl_heur_init T) \<le> uint64_max
          then do {
           ASSERT(\<A>\<^sub>i\<^sub>n'' \<noteq> {#});
           ASSERT(isasat_input_bounded_nempty \<A>\<^sub>i\<^sub>n'');
           _ \<leftarrow> isasat_information_banner T;
...
lemma
  \<open>IsaSAT_heur b CS \<le> \<Down>{((M, stats), M'). M = M'} (IsaSAT CS)\<close>
proof -
  have init_dt_wl_heur: \<open>init_dt_wl_heur CS S \<le>
       \<Down>(twl_st_heur_parsing_no_WL \<A> O {(S, T). S = remove_watched T \<and>
    ...
    if 
	 ...
	 distinct_mset_set (mset ` set CS)\<close> and
      \<open>((CS, S), CS, T) \<in> \<langle>Id\<rangle>list_rel \<times>\<^sub>f twl_st_heur_parsing_no_WL \<A>\<close>
  for \<A> CS T S
  proof -
    show ?thesis
  ...
  have init_state_wl_heur: \<open>isasat_input_bounded \<A> \<Longrightarrow>
      init_state_wl_heur \<A> \<le> SPEC (\<lambda>c. (c, init_state_wl) \<in>
        {(S, S'). (S, S') \<in> twl_st_heur_parsing_no_WL_wl \<A>})\<close> for \<A>
  ...
  show ?thesis
    unfolding IsaSAT_heur_alt_def IsaSAT_alt_def
    apply (refine_vcg virtual_copy init_state_wl_heur)
    ...
    subgoal
    using rewatch_heur_rewatch
       oops
find_theorems valid_arena fmempty

    thm init_dt_wl_heur_init_dt_wl[THEN fref_to_Down_curry,
      of \<open>mset_set (extract_atms_clss CS {})\<close>]
    thm rewatch_heur_rewatch

    find_theorems init_dt_wl_heur init_dt_wl
    find_theorems  rewatch_heur rewatch
    thm init_dt_wl_heur_init_dt_wl
    thm rewatch_heur_rewatch
    term init_dt_wl'
...
lemma in_class_in_literals_are_in_\<L>\<^sub>i\<^sub>n:
  ...
  shows \<open>literals_are_in_\<L>\<^sub>i\<^sub>n (mset_set (extract_atms_clss CS {})) (mset C)\<close>
  apply (auto simp: literals_are_in_\<L>\<^sub>i\<^sub>n_def extract_atms_clss_alt_def
      \<L>\<^sub>a\<^sub>l\<^sub>l_def all_lits_of_m_def)
...
sepref_definition length_get_clauses_wl_heur_init_code
  is \<open>RETURN o length_get_clauses_wl_heur_init\<close>
  :: \<open>isasat_init_assn\<^sup>k \<rightarrow>\<^sub>a nat_assn\<close>
  unfolding length_get_clauses_wl_heur_init_alt_def isasat_init_assn_def
  by sepref
...
lemma is_\<L>\<^sub>a\<^sub>l\<^sub>l_extract_atms_clss:
    \<open>is_\<L>\<^sub>a\<^sub>l\<^sub>l (mset_set (extract_atms_clss CS' {}))
...
proof -
  ...
  show ?thesis
    by (auto simp: extract_atms_clss_alt_def
          is_\<L>\<^sub>a\<^sub>l\<^sub>l_def \<L>\<^sub>a\<^sub>l\<^sub>l_def all_lits_of_mm_add_mset
...
lemma in_\<L>\<^sub>a\<^sub>l\<^sub>l_extract_atms_clss_in_all_lits_of_mm:
  shows \<open>L \<in># \<L>\<^sub>a\<^sub>l\<^sub>l (mset_set (extract_atms_clss CS' {})) \<longleftrightarrow>
...
proof -
  ...
  also have \<open>\<dots> \<longleftrightarrow> ?B\<close>
    using 1 is_\<L>\<^sub>a\<^sub>l\<^sub>l_extract_atms_clss is_\<L>\<^sub>a\<^sub>l\<^sub>l_def by auto
  finally show ?thesis .
qed

lemma [simp]: \<open>is_\<L>\<^sub>a\<^sub>l\<^sub>l (all_atms N NE) (all_lits N NE)\<close>
proof -
  define NUE where \<open>NUE \<equiv> {#mset (fst C). C \<in># ran_m N#} + NE\<close>
  show ?thesis
  unfolding is_\<L>\<^sub>a\<^sub>l\<^sub>l_def all_atms_def all_lits_def \<L>\<^sub>a\<^sub>l\<^sub>l_def NUE_def[symmetric]
  apply (auto simp: in_all_lits_of_mm_ain_atms_of_iff atm_of_all_lits_of_mm
    image_image[symmetric])
  by (metis (full_types) Neg_atm_of_iff Pos_atm_of_iff imageI)
qed
...
proof -
  ...
  have \<L>\<^sub>a\<^sub>l\<^sub>l:
    \<open>is_\<L>\<^sub>a\<^sub>l\<^sub>l (mset_set (extract_atms_clss CS' {})) (all_lits_of_mm (mset `# mset CS'))\<close>
    for CS'
  ...
  proof
    fix L
    assume L: \<open>L \<in># \<L>\<^sub>a\<^sub>l\<^sub>l (mset_set (extract_atms_clss CS' {}))\<close>
    then obtain C where
  ...
  have CDCL_steps:
    \<open>cdcl_twl_stgy_restart_prog_wl_D (fst T)
      \<le> \<Down> TWL_to_clauses_state_conv
          (SPEC (conclusive_CDCL_run CS (init_state CS)))\<close>
    ...
    clauses: \<open>mset `# ran_mf (get_clauses_wl (fst T)) +
        ...
        (is ?clss) and
    break_CDCL_steps: \<open>cdcl_twl_stgy_restart_prog_wl_D (fst T)
  ...
  proof -
    ...
    have \<L>\<^sub>a\<^sub>l\<^sub>l_NNUE: \<open>set_mset (\<L>\<^sub>a\<^sub>l\<^sub>l (all_atms N NE)) = set_mset (all_lits_of_mm (mset `# mset CS'))\<close>
      unfolding \<L>\<^sub>a\<^sub>l\<^sub>l_def N_NE all_atms_def all_lits_def all_lits_of_mm_def
      by auto
    ...
    next
      ...
      show ?case
        ...
        .
    qed
    have \<L>\<^sub>a\<^sub>l\<^sub>l: \<open>is_\<L>\<^sub>a\<^sub>l\<^sub>l (mset_set (extract_atms_clss CS' {})) (all_lits_of_mm (mset `# mset CS'))\<close>
      by (auto simp add: is_\<L>\<^sub>a\<^sub>l\<^sub>l_extract_atms_clss)
    have [simp]: \<open>isasat_input_bounded (mset_set (extract_atms_clss CS' {}))\<close>
      unfolding isasat_input_bounded_def
    proof
    ...
    have valid_blits: \<open>\<And>L x. x\<in>set (Wa L) \<Longrightarrow>
      ...
      by (auto simp: st N_NE \<L>\<^sub>a\<^sub>l\<^sub>l S\<^sub>0 literals_are_\<L>\<^sub>i\<^sub>n_def
        blits_in_\<L>\<^sub>i\<^sub>n_def correct_watching.simps
    ...
    then have 1: \<open>cdcl_twl_stgy_prog_wl_D_pre (fst T) (fst W)\<close>
      unfolding cdcl_twl_stgy_prog_wl_D_pre_def
      ...
      by (auto simp: st N_NE \<L>\<^sub>a\<^sub>l\<^sub>l S\<^sub>0 literals_are_\<L>\<^sub>i\<^sub>n_def
        ...
        state_wl_l_def correct_watching_init.simps  \<L>\<^sub>a\<^sub>l\<^sub>l_NNUE
        dest!: valid_blits)
    have 2:\<open>cdcl_twl_stgy_restart_prog_wl_D (from_init_state T)
            \<le> \<Down> (state_wl_l None O twl_st_l None)
                 (conclusive_TWL_run (fst W))\<close>
      apply (rule cdcl_twl_stgy_restart_prog_wl_D_spec_final)
  ...
  show ?thesis
    unfolding SAT_wl_def SAT_def from_init_state_def to_init_state_def
      empty_watched_def finalise_init_def id_def
...
lemma twl_st_heur_init_vmtf_next_emptyD:
  \<open>((x1, x1a, x1b, x1c, x1d, ((x1g, x1h, x1i, x1j, x2h), x2i), x1k, x2k), Ta)
       \<in> twl_st_heur_parsing \<A>\<^sub>i\<^sub>n \<Longrightarrow> \<A>\<^sub>i\<^sub>n \<noteq> {#} \<Longrightarrow> x1i \<noteq> None\<close>
  by (auto simp: twl_st_heur_parsing_def vmtf_init_def isa_vmtf_init_def)

lemma twl_st_heur_init_vmtf_fstD:
  \<open>((x1, x1a, x1b, x1c, x1d, ((x1g, x1h, x1i, x1j, x2h), x2i), x1k, x2k), Ta)
       \<in> twl_st_heur_parsing \<A>\<^sub>i\<^sub>n \<Longrightarrow> \<A>\<^sub>i\<^sub>n \<noteq> {#} \<Longrightarrow> x1j \<noteq> None\<close>
  by (auto simp: twl_st_heur_parsing_def vmtf_init_def isa_vmtf_init_def)

(* 
lemma get_conflict_wl_is_None_init_get_conflict_wl_is_None_heur_init[simp]:
  \<open>(Tb, Ta) \<in> twl_st_heur_parsing A \<Longrightarrow>
...
lemma isasat_input_bounded_nempty_cdcl_twl_stgy_restart_prog_wl_D_heur_break_cdcl_twl_stgy_prog_wl_D':
  \<open>isasat_input_bounded_nempty \<A> \<Longrightarrow> (S, S') \<in> twl_st_heur \<Longrightarrow> \<A> = \<A>' \<Longrightarrow>
    cdcl_twl_stgy_restart_prog_wl_heur S
    \<le> \<Down>(twl_st_heur)
         (cdcl_twl_stgy_restart_prog_wl_D S')\<close>
  using cdcl_twl_stgy_restart_prog_wl_heur_cdcl_twl_stgy_restart_prog_wl_D
             [THEN fref_to_Down, unfolded comp_def, of S S']
  by fast
...
proof -
  ...
  have IsaSAT_heur_alt_def: \<open>IsaSAT_heur opts CS = do{
    ...
    then do {
        ...
        S \<leftarrow> init_state_wl_heur;
        S \<leftarrow> f S;
        (T::twl_st_wl_heur_init) \<leftarrow> init_dt_wl_heur_full CS S;
        if \<not>get_conflict_wl_is_None_heur_init T
        then RETURN (empty_init_code)
        else if CS = [] then empty_conflict_code
        else do {
