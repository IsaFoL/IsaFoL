...
proof -
  ...
  have 1: \<open>unit_propagation_inner_loop_body_wl L w S
    ...
    proof -
      let ?K = \<open>N ! (W L ! w) ! the f'\<close>
      ...
      have K_notin_watched[iff]: \<open>?K \<notin> set (watched_l (N ! (W L ! w)))\<close>
        ...
        by (auto simp: S)
      have snd_f'_ge_2: \<open>the f' \<ge> 2\<close>
      proof (rule ccontr)
      ...
      then have [simp]: \<open>L \<notin> set (take 2 (swap (N ! x) i (the f')))\<close> if \<open>W L ! w = x\<close> for x
      ...
      have [simp]: \<open>N ! (W L ! w) ! the f' \<in> set (take 2 (swap (N ! (W L ! w)) i (the f')))\<close>
      ...
      have [simp]: \<open>set (take 2 (swap (N ! (W L ! w)) i (the f'))) = {C''!(1-i), ?K}\<close>
      ...
      then have [simp]: \<open>mset `# mset (tl N[W L ! w - Suc 0 := swap (N ! (W L ! w)) i (the f')]) = mset `# mset (tl N)\<close>
