...
typedef. Moving \<^term>\<open>U\<close> is not an issue, as these are redundant clauses and the basic solver never
...
theorem DPLL_tot_correct_inc:
  ...
  shows \<open>(D1 \<noteq> Some {#} \<and> satisfiable (set (map mset N0) \<union> set (map mset clss)))
    \<or> (D1 = Some {#} \<and> (unsatisfiable (set (map mset N0) \<union> set (map mset clss))))\<close> (is ?A) and
      \<open>satisfiable (set_mset N1) \<longleftrightarrow> satisfiable (set (map mset N0) \<union> set (map mset clss))\<close> (is ?B)
proof -
  ...
  have final: \<open>(D1 \<noteq> Some {#} \<and> satisfiable (set (map mset N')))
                \<or> (D1 = Some {#} \<and> (unsatisfiable (set (map mset N'))))\<close> and
  equisat: \<open>satisfiable (set_mset N1) = satisfiable (set (map mset N'))\<close>
    using DPLL_tot_correct[OF state_equal A3] by auto
  then show ?A
    using sat_preserve unsat_preserve by auto
  then show ?B
    using unsat_preserve H3 equisat final by auto
qed
