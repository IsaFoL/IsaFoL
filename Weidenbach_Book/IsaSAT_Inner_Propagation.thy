...
lemma unit_prop_body_wl_D_invD:
  ...
  assumes \<open>unit_prop_body_wl_inv S j w L\<close>
  shows
...
proof -
  ...
  show \<open>w < length (watched_by S L)\<close> and \<open>j \<le> w\<close>
    using assms unfolding unit_prop_body_wl_inv_def unit_prop_body_wl_inv_def watched_by_app_def
  ...
  have \<open>blits_in_\<L>\<^sub>i\<^sub>n S\<close> and \<open>L \<in># \<L>\<^sub>a\<^sub>l\<^sub>l \<A>\<close>
    using assms unfolding unit_prop_body_wl_inv_def unit_prop_body_wl_inv_def watched_by_app_def
      unit_propagation_inner_loop_body_l_inv_def literals_are_\<L>\<^sub>i\<^sub>n_def \<A>_def
      all_lits_of_all_atms_of
     by fast+
find_theorems "\<L>\<^sub>a\<^sub>l\<^sub>l (all_atms _ _)" all_lits
  ...
  obtain T T' where
    ...
    ui_le: \<open>1 - (if get_clauses_l (remove_one_lit_from_wq (?C) T) \<propto>
          ...
          then 0 else 1)
     < length
        (get_clauses_l (remove_one_lit_from_wq (?C) T) \<propto> (?C))\<close> and
    L_in_watched: \<open>L \<in> set (watched_l
               (get_clauses_l (remove_one_lit_from_wq (?C) T) \<propto> (?C)))\<close> and
    \<open>get_conflict_l (remove_one_lit_from_wq (?C) T) = None\<close>
    using assms C_dom unfolding unit_prop_body_wl_inv_def unit_prop_body_wl_inv_alt_def
...
proof -
  have [refine0]:
    \<open>C \<in># dom_m N \<Longrightarrow> RETURN(arena_length arena C \<le> MAX_LENGTH_SHORT_CLAUSE) \<le>
...
proof -
  ...
  have H: \<open>isa_find_unwatched P M' arena C \<le> \<Down> Id (find_unwatched P' N C')\<close>
    ...
    unfolding isa_find_unwatched_def find_unwatched_def Let_def supply [[goals_limit=1]]
...
sorry
  ...
  subgoal
    ...
    intro!: ASSERT_refine_left valid_arena_swap_lits dest!: multi_member_split[of \<open>arena_lit _ _\<close>])
  subgoal
    by (auto simp: twl_st_heur_def Let_def add_mset_eq_add_mset all_lits_of_all_atms_of
      map_fun_rel_def twl_st_heur'_def update_clause_wl_pre_def arena_lifting arena_lit_pre_def
...
lemma
  ...
  if
    find_unw: \<open>unit_prop_body_wl_find_unwatched_inv None (fst (watched_by_app S L C)) S\<close> and
    inv: \<open>unit_prop_body_wl_inv S j C L\<close> and
  ...
  obtain T T' where
    ...
    \<open>L \<in> set (watched_l
               (get_clauses_l (remove_one_lit_from_wq (?C) T) \<propto>
                (?C)))\<close> and
    \<open>get_conflict_l (remove_one_lit_from_wq (?C) T) = None\<close>
  using inv dom unfolding  unit_prop_body_wl_inv_alt_def
...
lemma propagate_lit_wl_bin_heur_propagate_lit_wl_bin:
  ...
  subgoal by (auto 4 3 simp: twl_st_heur_def twl_st_heur'_def propagate_lit_wl_bin_pre_def swap_lits_pre_def
        arena_lifting phase_saving_def atms_of_def save_phase_def \<L>\<^sub>a\<^sub>l\<^sub>l_all_atms_all_lits
        all_lits_def \<L>\<^sub>a\<^sub>l\<^sub>l_all_atms_all_lits \<L>\<^sub>a\<^sub>l\<^sub>l_atm_of_all_lits_of_mm 
      intro!: ASSERT_refine_left cons_trail_Propagated_tr2 cons_trail_Propagated_tr_pre
      dest: multi_member_split valid_arena_DECISION_REASON)
  subgoal by (auto 4 3 simp: twl_st_heur_def twl_st_heur'_def propagate_lit_wl_bin_pre_def swap_lits_pre_def
...
lemma find_unwatched_le_length:
  ...
  if
    find_unw: \<open>RETURN (Some xj) \<le> find_unwatched_l (get_trail_wl S) (get_clauses_wl S) (fst (watched_by_app S L C))\<close>
  for S L C xj
  using that unfolding find_unwatched_l_def
  by (cases S) auto
...
definition update_blit_wl_heur_pre where
 ...
 lemma update_blit_wl_heur_update_blit_wl:
  ...
  subgoal for aa ab ac ad ae be af ag ah bf aj ak al am an bg ao bh ap aq ar bi at bx "by"
       cf cg ch ci cj ck cl cn co cq y
    apply (subgoal_tac \<open>vdom_m (all_atms cl (cn + co)) (cq(K := (cq K)[ch := (cf, cj, cg)])) cl \<subseteq>
        vdom_m (all_atms cl (cn + co)) cq cl\<close>)
    apply fast
    apply (rule vdom_m_update_subset')
    apply auto
    done
  subgoal for aa ab ac ad ae be af ag ah bf ai aj ak al am an bg ao bh ap aq ar bi at bx
       "by" cf cg ch ci cj ck cl cm cn co cq x
    apply (subgoal_tac \<open>vdom_m (all_atms cl (cn + co)) (cq(K := (cq K)[ch := (cf, cj, cg)]))
         cl \<subseteq> vdom_m (all_atms cl (cn + co)) cq cl\<close>)
    apply fast
    apply (rule vdom_m_update_subset')
    apply auto
    done
  ...
  \<open>unit_propagation_inner_loop_body_wl_D L j w S = do {
      ...
      then RETURN (j+1, w+1, S)
        ...
        if b then do {
             ...
             if val_K = Some False
             then do {
              S \<leftarrow> set_conflict_wl (get_clauses_wl S \<propto> C) S;
              RETURN
                   (j + 1, w + 1, S)}
             else do {
  ...
  proof -
    ...
    show ?thesis
      ...
      apply (simp_all only: twl_st_l \<L>\<^sub>a\<^sub>l\<^sub>l_all_atms_all_lits all_lits_def multiset.map_comp comp_def
        clause_twl_clause_of twl_st_wl in_all_lits_of_mm_uminus_iff)
     done
  qed
(*
...
*)
  ...
  show ?thesis
    ...
    apply (intro frefI nres_relI)
    apply (refine_vcg)
    subgoal by (auto simp: twl_st_heur'_def twl_st_heur_state_simp_watched2 simp flip: \<L>\<^sub>a\<^sub>l\<^sub>l_all_atms_all_lits)
