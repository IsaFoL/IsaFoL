...
definition DPLL_step :: "int dpll\<^sub>W_ann_literals \<times> int literal list list
  \<Rightarrow> int dpll\<^sub>W_ann_literals \<times> int literal list list"  where
...
abbreviation "toS \<equiv> \<lambda>(Ms::(int, unit, unit) ann_literal list)
...
proof -
  ...
  { assume unit: "find_first_unit_clause N Ms = None"
    ...
    have "dpll\<^sub>W (Ms, mset (map mset N))
      ...
      by (auto simp add: Decided_Propagated_in_iff_in_lits_of atms_of_ms_def)
...
function DPLL_ci :: "int dpll\<^sub>W_ann_literals \<Rightarrow> int literal list list
  \<Rightarrow> int dpll\<^sub>W_ann_literals \<times> int literal list list" where
"DPLL_ci Ms N =
...
next
  fix Ms :: "int dpll\<^sub>W_ann_literals" and N x xa y
...
function (domintros) DPLL_part:: "int dpll\<^sub>W_ann_literals \<Rightarrow> int literal list list \<Rightarrow>
  int dpll\<^sub>W_ann_literals \<times> int literal list list" where
"DPLL_part Ms N =
  (let (Ms', N') = DPLL_step (Ms, N) in
...
lemma DPLL_part_dpll\<^sub>W_all_inv_final:
  fixes M Ms':: "(int, unit, unit) ann_literal list" and
...
typedef dpll\<^sub>W_state =
    "{(M::(int, unit, unit) ann_literal list, N::int literal list list).
...
definition Con :: "(int, unit, unit) ann_literal list \<times> int literal list list
...
structure Partial_Annotated_Clausal_Logic : sig
  ...
  val lit_of : ('a, 'b, 'c) ann_literal -> 'a Clausal_Logic.literal
  val lits_of : ('a, 'b, 'c) ann_literal list -> 'a Clausal_Logic.literal Set.set
  val backtrack_split :
    ('a, 'b, 'c) ann_literal list ->
      ('a, 'b, 'c) ann_literal list * ('a, 'b, 'c) ann_literal list
end = struct
...
fun backtrack_split [] = ([], [])
  ...
  | backtrack_split (Decided (la, l) :: mlits) = ([], Decided (la, l) :: mlits);
...
structure DPLL_W_Implementation : sig
  ...
  Con of
    ((Arith.int, unit, unit) Partial_Annotated_Clausal_Logic.ann_literal list *
...
fun equal_dpll_W_state sa s =
  Product_Type.equal_prod
    (List.equal_list
      (Partial_Annotated_Clausal_Logic.equal_ann_literal Arith.equal_int
