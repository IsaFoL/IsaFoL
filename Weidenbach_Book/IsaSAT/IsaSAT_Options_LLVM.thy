...
type_synonym opts_assn = \<open>1 word \<times> 1 word \<times> 1 word \<times> 64 word \<times> 64 word \<times> 64 word \<times> 3 word\<close>
...
definition opts_rel_assn :: \<open>opts_ref \<Rightarrow> _ \<Rightarrow> assn\<close> where
  \<open>opts_rel_assn = bool1_assn \<times>\<^sub>a bool1_assn \<times>\<^sub>a bool1_assn \<times>\<^sub>a word_assn \<times>\<^sub>a word_assn 
   \<times>\<^sub>a snat_assn' TYPE(64) \<times>\<^sub>a word_assn' TYPE(3)\<close>
...
sepref_register opts_restart opts_reduce opts_minimum_between_restart opts_restart_coeff1
  opts_restart_coeff2 opts_target
...
definition default_opts :: opts where
  \<open>default_opts = IsaOptions True True True 50 11 4 1\<close>

definition default_opts2 :: opts_ref where
  \<open>default_opts2 = (True, True, True, 50, 11, 4, 2)\<close>

definition IsaOptions_rel :: \<open>bool \<Rightarrow> bool \<Rightarrow> bool \<Rightarrow> 64 word \<Rightarrow> 64 word \<Rightarrow> nat \<Rightarrow> opts_target \<Rightarrow> opts_ref\<close> where
  \<open>IsaOptions_rel a b c d e f g = (a, b, c, d, e, f, g)\<close>

lemma IsaOptions_rel:
  \<open>(uncurry6 (RETURN ooooooo IsaOptions_rel), uncurry6 (RETURN ooooooo IsaOptions)) \<in>
     bool_rel \<times>\<^sub>f bool_rel \<times>\<^sub>f  bool_rel \<times>\<^sub>f  word_rel \<times>\<^sub>f word_rel \<times>\<^sub>f nat_rel \<times>\<^sub>f word_rel \<rightarrow>
...
sepref_def IsaOptions_rel_impl
  is \<open>uncurry6 (RETURN ooooooo IsaOptions_rel)\<close>
  :: \<open>bool1_assn\<^sup>k *\<^sub>a bool1_assn\<^sup>k *\<^sub>a bool1_assn\<^sup>k *\<^sub>a word_assn\<^sup>k *\<^sub>a word_assn\<^sup>k *\<^sub>a
     (snat_assn' (TYPE(64)))\<^sup>k  *\<^sub>a (word_assn' (TYPE(3)))\<^sup>k \<rightarrow>\<^sub>a
        opts_rel_assn\<close>
