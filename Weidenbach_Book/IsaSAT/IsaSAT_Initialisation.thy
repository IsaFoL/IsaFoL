...
definition inprocessing_interaval_init :: \<open>64 word\<close> where \<open>inprocessing_interaval_init = 10000\<close>
...
definition finalise_init_code :: \<open>opts \<Rightarrow> twl_st_wl_heur_init \<Rightarrow> isasat nres\<close> where
  \<open>finalise_init_code opts =
    (\<lambda>S. case S of Tuple15 M' N' D' Q' W' ((ns, m, fst_As, lst_As, next_search), to_remove) \<phi> clvls cach
     ...
     let heur = Restart_Heuristics ((fema, sema, ccount, 0, (\<phi>, 0, replicate (length \<phi>) False, 0, replicate (length \<phi>) False, 10000, 1000, 1), reluctant_init, False, replicate (length \<phi>) False, (inprocessing_interaval_init, reduce_interval_init)));
