...
private lemma
   ...
   valid: \<open>valid_arena (get_clauses_wl_heur_init T) N (set (get_vdom_heur_init T))\<close> and
    ...
    \<open>get_vmtf_wl_heur_init T \<in> isa_vmtf_init \<A> M\<close> and
...
definition inprocessing_interval_init :: \<open>64 word\<close> where \<open>inprocessing_interval_init = 10000\<close>
...
definition finalise_init_code :: \<open>opts \<Rightarrow> twl_st_wl_heur_init \<Rightarrow> isasat nres\<close> where
  \<open>finalise_init_code opts =
    (\<lambda>S. case S of Tuple15 M' N' D' Q' W' ((ns, m, fst_As, lst_As, next_search), to_remove) \<phi> clvls cach
     ...
     let heur = Restart_Heuristics ((fema, sema, ccount, 0, (\<phi>, 0, replicate (length \<phi>) False, 0, replicate (length \<phi>) False, rephasing_end_of_initial_phase, 0, rephasing_initial_phase), reluctant_init, False, replicate (length \<phi>) False, (inprocessing_interval_init, reduce_interval_init)));
