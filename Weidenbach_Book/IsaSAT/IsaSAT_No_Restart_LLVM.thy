...
lemma restart_info_params[sepref_import_param]:
  \<open>(NORMAL_PHASE,NORMAL_PHASE) \<in> word_rel\<close>
  \<open>(QUIET_PHASE,QUIET_PHASE) \<in> word_rel\<close>
  by auto
...
lemma [llvm_inline]:
  ...
  by (auto simp: NORMAL_PHASE_def DEFAULT_INIT_PHASE_def QUIET_PHASE_def)

experiment
begin

  export_llvm IsaSAT_code_wrapped default_opts_impl IsaSAT_code file \<open>code/isasat_restart.ll\<close>
