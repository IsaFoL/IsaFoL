...
lemma cdcl\<^sub>W_restart_all_struct_inv_add_new_clause_and_update_cdcl\<^sub>W_restart_all_struct_inv:
  assumes
    inv_T: "cdcl\<^sub>W_restart_all_struct_inv T" and
    tr_T_N[simp]: "trail T \<Turnstile>asm N" and
    tr_C[simp]: "trail T \<Turnstile>as CNot C" and
    [simp]: "distinct_mset C"
  shows "cdcl\<^sub>W_restart_all_struct_inv (add_new_clause_and_update C T)" (is "cdcl\<^sub>W_restart_all_struct_inv ?T'")
proof -
  ...
  have [simp]: "no_strange_atm ?T"
    using inv_T unfolding cdcl\<^sub>W_restart_all_struct_inv_def no_strange_atm_def add_new_clause_and_update_def
    cdcl\<^sub>W_restart_M_level_inv_def by (auto 20 1)
  have M_lev: "cdcl\<^sub>W_restart_M_level_inv T"
    using inv_T unfolding cdcl\<^sub>W_restart_all_struct_inv_def by blast
  then have "no_dup (M @ trail (cut_trail_wrt_clause C (trail T) T))"
    unfolding cdcl\<^sub>W_restart_M_level_inv_def unfolding M[symmetric] by auto
  ...
  have "consistent_interp (lits_of_l (M @ trail (cut_trail_wrt_clause C (trail T) T)))"
    using M_lev unfolding cdcl\<^sub>W_restart_M_level_inv_def unfolding M[symmetric] by auto
  then have [simp]: "consistent_interp (lits_of_l (trail (cut_trail_wrt_clause C
    (trail T) T)))"
    unfolding consistent_interp_def by auto

  have [simp]: "cdcl\<^sub>W_restart_M_level_inv ?T"
    using M_lev unfolding cdcl\<^sub>W_restart_M_level_inv_def by (auto dest: H H'
      simp: M_lev cdcl\<^sub>W_restart_M_level_inv_def cut_trail_wrt_clause_backtrack_lvl_length_decided)

  have [simp]: "\<And>s. s \<in># learned_clss T \<Longrightarrow> \<not>tautology s"
    using inv_T unfolding cdcl\<^sub>W_restart_all_struct_inv_def by auto

  have "distinct_cdcl\<^sub>W_restart_state T"
    using inv_T unfolding cdcl\<^sub>W_restart_all_struct_inv_def by auto
  then have [simp]: "distinct_cdcl\<^sub>W_restart_state ?T"
  ...
  then have [simp]: "cdcl\<^sub>W_restart_conflicting ?T"
    unfolding cdcl\<^sub>W_restart_conflicting_def apply simp
    by (metis M \<open>cdcl\<^sub>W_restart_conflicting T\<close> append_assoc cdcl\<^sub>W_restart_conflicting_decomp(2))
  ...
  have [simp]: "cdcl\<^sub>W_restart_learned_clause ?T"
    using inv_T unfolding cdcl\<^sub>W_restart_all_struct_inv_def cdcl\<^sub>W_restart_learned_clause_def
...
proof -
  ...
  then have
    ...
    using cdcl\<^sub>W_restart_M_level_inv_decomp(2) cdcl\<^sub>W_restart_all_struct_inv_def inv
  ...
  then have "trail (add_new_clause_and_update C T) \<Turnstile>as CNot C"
    by (simp add: add_new_clause_and_update_def cut_trail_wrt_clause_CNot_trail
      cdcl\<^sub>W_restart_M_level_inv_def cdcl\<^sub>W_restart_all_struct_inv_def)
  obtain MT where
        ...
        then show False
          ...
          next
            ...
            moreover
              have "lit_of (hd (M' @ Decided K # [])) = -?L"
                ...
                by (cases M', cases "trail (add_init_cls C
                    (cut_trail_wrt_clause C (trail T) T))")
                (auto dest!: arg_cong[of "_ # _" _ hd] simp: hd_append cdcl\<^sub>W_restart_all_struct_inv_def
                  cdcl\<^sub>W_restart_M_level_inv_def)
            ...
            moreover have "no_dup (trail (cut_trail_wrt_clause C (trail T) T))"
              using \<open>cdcl\<^sub>W_restart_all_struct_inv ?T'\<close> unfolding cdcl\<^sub>W_restart_all_struct_inv_def
              cdcl\<^sub>W_restart_M_level_inv_def by (auto simp: add_new_clause_and_update_def)
...
next
  ...
  case 1
  have "cdcl\<^sub>W_restart_all_struct_inv (add_init_cls C S)"
    using inv \<open>distinct_mset C\<close> unfolding cdcl\<^sub>W_restart_all_struct_inv_def no_strange_atm_def
    cdcl\<^sub>W_restart_M_level_inv_def distinct_cdcl\<^sub>W_restart_state_def cdcl\<^sub>W_restart_conflicting_def cdcl\<^sub>W_restart_learned_clause_def
