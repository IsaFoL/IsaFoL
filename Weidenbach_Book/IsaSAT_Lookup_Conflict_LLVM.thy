...
sepref_definition nat_lit_eq_impl
...
sepref_definition minimize_status_rel_eq_impl
...
sepref_definition SEEN_FAILED_impl
...
sepref_definition SEEN_UNKNOWN_impl
...
sepref_definition SEEN_REMOVABLE_impl
  is \<open>uncurry0 (RETURN 2)\<close>
  :: \<open>unit_assn\<^sup>k \<rightarrow>\<^sub>a minimize_status_ref_assn\<close>
  supply [[goals_limit=1]] and [simp] = max_snat_def
  apply (annot_snat_const "TYPE(8)")
  by sepref
...
sepref_definition is_Notin_impl
...
where
 \<open>conflict_option_rel_assn \<equiv> (bool1_assn *a lookup_clause_rel_assn)\<close>
...
abbreviation (in -)ana_refinement_fast_assn where
  \<open>ana_refinement_fast_assn \<equiv> sint64_nat_assn *a uint32_nat_assn *a bool1_assn\<close>
...
sepref_definition lookup_clause_assn_is_None_impl
...
sepref_definition size_lookup_conflict_impl
...
declare size_lookup_conflict_impl.refine[sepref_fr_rules]
...
lemma lookup_clause_assn_is_empty_alt_def:
  ...
  by (auto simp: size_lookup_conflict_def lookup_clause_assn_is_empty_def intro!: ext)

sepref_definition lookup_clause_assn_is_empty_impl
...
declare lookup_clause_assn_is_empty_impl.refine[sepref_fr_rules]
...
sepref_definition the_lookup_conflict_impl
...
declare the_lookup_conflict_impl.refine[sepref_fr_rules]
...
sepref_definition Some_lookup_conflict_impl
...
sepref_definition add_to_lookup_conflict_impl
...
sepref_definition resolve_merge_conflict_fast_code
  ...
  :: \<open>[uncurry6 (\<lambda>M N i (b, xs) clvls lbd outl. length N \<le> sint64_max \<and>
...
sepref_definition lookup_conflict_size_impl
...
sepref_definition set_lookup_empty_conflict_to_none_imple
...
declare set_lookup_empty_conflict_to_none_imple.refine[sepref_fr_rules]
...
sepref_definition isa_get_literal_and_remove_of_analyse_wl_fast_code
  ...
  supply [simp] = uint64_max_def max_unat_def max_snat_def sint64_max_def uint32_max_def
    and [dest] = in_unat_rel_imp_less_max' arena_lit_implI
...
sepref_definition ana_lookup_conv_lookup_fast_code
  ...
  :: \<open>[uncurry ana_lookup_conv_lookup_pre]\<^sub>a arena_fast_assn\<^sup>k *\<^sub>a
    (ana_refinement_fast_assn)\<^sup>k
     \<rightarrow> sint64_nat_assn *a sint64_nat_assn *a sint64_nat_assn *a sint64_nat_assn\<close>
...
lemma isa_lit_redundant_rec_wl_lookup_get_propagation_reason_pol_raw: \<open>do {
    ...
    do {
		 ...
		 if C \<noteq> DECISION_REASON then do {
		     ASSERT(lit_redundant_reason_stack_wl_lookup_pre (-L) NU C);
		     RETURN (cach, analyse @ [lit_redundant_reason_stack_wl_lookup (-L) NU C], False)
		   }
		 else do {
...
sepref_definition lit_redundant_rec_wl_lookup_fast_code
  ...
  :: \<open>[\<lambda>(((((M, NU), D), cach), analysis), lbd). length NU \<le> sint64_max]\<^sub>a
      ...
      cach_refinement_l_assn *a analyse_refinement_fast_assn *a bool1_assn\<close>
  supply [[goals_limit = 1]] neq_Nil_revE[elim] image_image[simp]
  ...
  apply (subst isa_lit_redundant_rec_wl_lookup_get_propagation_reason_pol_raw)
...
apply sepref_dbg_keep
    apply sepref_dbg_trans_keep
    apply sepref_dbg_trans_step_keep
oops term get_propagation_reason_pol
    apply sepref_dbg_side_unfold apply (auto dest!: arena_lit_pre_le)[]

find_theorems "array_assn minimize_status_assn *a al_assn unat_assn"
  apply sepref_dbg_trans
  apply sepref_dbg_opt
  apply sepref_dbg_cons_solve
  apply sepref_dbg_cons_solve
