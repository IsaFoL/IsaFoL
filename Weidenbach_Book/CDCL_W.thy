...
lemma trail_append_trail[simp]:
  "trail (append_trail M S) = rev M @ trail S"
  by (induction M arbitrary: S) auto

lemma learned_clss_append_trail[simp]:
  "learned_clss (append_trail M S) = learned_clss S"
  by (induction M arbitrary: S) auto

lemma init_clss_append_trail[simp]:
  "init_clss (append_trail M S) = init_clss S"
  by (induction M arbitrary: S) auto

lemma conflicting_append_trail[simp]:
  "conflicting (append_trail M S) = conflicting S"
  by (induction M arbitrary: S) auto

lemma backtrack_lvl_append_trail[simp]:
  "backtrack_lvl (append_trail M S) = backtrack_lvl S"
  by (induction M arbitrary: S) auto

lemma clauses_append_trail[simp]:
  "clauses (append_trail M S) = clauses S"
...
decide[intro]: "decide S S' \<Longrightarrow> cdcl_o S S'" |
...
lemma cdcl_all_rules_induct[consumes 1, case_names propagate conflict forget restart decide skip
    resolve backtrack]:
  fixes S  :: "'st"
  ...
  and decide:  "\<And>S T. decide S T \<Longrightarrow> P S T"
  ...
  shows "P S S'"
  using assms(1)
proof (induct S\<equiv>S S' rule: cdcl.induct)
...
lemma cdcl_all_induct[consumes 1, case_names propagate conflict forget restart decide skip
    resolve backtrack]:
  fixes S  :: "'st"
  assumes
    cdcl: "cdcl S S'" and
...
next
  case (decide T)
  thus ?case by (elim decideE) (frule decideH; simp)
next
  case (backtrack S')
  thus ?case by (elim backtrackE) (frule backtrackH; simp del: state_simp add: state_eq_def)
next
  case (forget S')
...
lemma cdcl_o_induct[consumes 1, case_names decide skip resolve backtrack]:
  fixes S  :: "'st"
...
lemma cdcl_o_rule_cases[consumes 1, case_names decide backtrack skip resolve]:
  assumes
    "cdcl_o S T" and
...
lemma cdcl_state_eq_compatible:
  assumes
    ...
    "T \<sim> T'"
  shows "cdcl S' T'"
  using assms by (meson assms backtrack_state_eq_compatible bj cdcl.simps cdcl_bj.simps
...
lemma level_of_marked_ge_1:
  ...
  using assms apply(induct rule: cdcl_all_induct)
...
proof (induct rule: cdcl_o_induct)
  ...
  then show ?case using T unfolding M by (auto dest!: append_cons_eq_upt_length simp del: upt_simps)
qed auto
...
next
  ...
  thus ?case using T by (auto simp add: rev_swap M dest!: append_cons_eq_upt(1) simp del: upt_simps)
qed auto
...
proof (induct rule: cdcl_all_induct)
  ...
  ultimately show ?case using T decomp by simp
...
lemma cdcl_learned_clause_S0_cdcl[simp]:
   "cdcl_learned_clause (init_state N)"
...
lemma cdcl_learned_clss:
  ...
  using assms(1,2)
proof (induct rule: cdcl_all_induct)
  case (backtrack K i M1 M2 L D T) note decomp = this(1) and confl = this(3) and T =this(6) and
    learned = this(7)
  show ?case
    using decomp confl learned T unfolding cdcl_learned_clause_def
...
next
  ...
  then show ?case
    ...
    apply (auto dest!: get_all_marked_decomposition_exists_prepend
      simp: clauses_def state_eq_def cdcl_learned_clause_def
       simp del: state_simp)
    by (metis learned_clss_restart_state set_mset_mono subset_Un_eq true_clss_clss_union_and)+
next
  case propagate
  then show ?case by (auto simp: cdcl_learned_clause_def clauses_def)
next
  case conflict
  then show ?case
    by (auto simp: cdcl_learned_clause_def clauses_def true_clss_clss_in_imp_true_clss_cls)
next
  case forget
  then show ?case by (auto simp: cdcl_learned_clause_def clauses_def split: split_if_asm)
qed  (auto simp: cdcl_learned_clause_def clauses_def)
...
lemma cdcl_no_strange_atm_explicit:
  assumes
    "cdcl S S'" and
    "\<forall>T. conflicting S = C_Clause T \<longrightarrow> atms_of T \<subseteq> atms_of_mu (init_clss S)" and
    ...
    "atm_of ` (lits_of (trail S)) \<subseteq> atms_of_mu (init_clss S)"
  ...
  using assms(1-5)
proof (induct rule: cdcl_all_induct)
  case (propagate C L T) note confl = this(4) and T =this(5)
  ...
  moreover
    have "atms_of (C + {#L#}) \<subseteq> atms_of_mu (init_clss S)"
      by (metis (no_types) atms_of_atms_of_m_mono atms_of_m_union clauses_def mem_set_mset_iff
        propagate.hyps(1) propagate.prems(3) set_mset_union sup.orderE) (* 409 ms *)
    then have "?M (cons_trail (Propagated L (C + {#L#})) S)"
      by (simp add: propagate.prems(2)) (* 76 ms *)
...
next
  ...
  ultimately show ?case using conflict.prems T unfolding atms_of_def atms_of_m_def clauses_def
     by (auto simp add: H )
next
  case (restart T)
  then show ?case
    by (metis (no_types, lifting) atms_of_m_learned_clss_restart_state_in_atms_of_m_learned_clssI
      conflicting_restart_state contra_subsetD empty_iff empty_set image_empty init_clss_restart_state
      lits_of_empty_is_empty state_eq_conflicting state_eq_init_clss state_eq_learned_clss
      state_eq_trail subsetI trail_restart_state)
next
  case (forget C T) note C = this(3) and C_le = this(4) and confl = this(5) and
    T = this(6) and atm_mark = this(8) and atm_le = this(9) and atm_trail = this(10)
  have H: "\<And>L mark. Propagated L mark \<in> set (trail S) \<Longrightarrow> atms_of mark \<subseteq> atms_of_mu (init_clss S)"
    using atm_mark by simp
  show ?case unfolding clauses_def apply standard
    using confl T unfolding clauses_def apply auto[]
    apply standard
     using T atm_trail C apply (auto dest!: H)[]
    apply standard
      using T atm_le C C_le atms_of_m_remove_subset[of "set_mset (learned_clss S)"] apply (auto)[]
    using T atm_trail C apply (auto simp: clauses_def lits_of_def)[]
  done
next
  case (backtrack K i M1 M2 L D T) note decomp = this(1) and confl = this(3) and T =this (6)
  have "?C T"
    using backtrack.prems(3) T by simp
  moreover have "set M1 \<subseteq> set (trail S)"
    using backtrack.hyps(1) by auto
  hence M: "?M T"
    using backtrack.prems(1,2) confl T decomp by (auto simp add: image_subset_iff clauses_def)
  moreover have "?U T"
    using backtrack.prems(1,3) confl T unfolding clauses_def by auto
  moreover have "?V T"
    using M backtrack.prems(4) backtrack.hyps(1) T by fastforce
  ultimately show ?case using T by auto
next
  case (resolve L C M D T) note trail = this(1) and confl = this(2) and T = this(4)
...
lemma distinct_cdcl_state_S0_cdcl[simp]:
  "distinct_mset_mset N \<Longrightarrow>  distinct_cdcl_state (init_state N)"
...
proof (rule ccontr)
  ...
  have [simp]: "L' = L"
    by (metis (mono_tags, lifting) T T' trail_cons_trail list.inject marked_lit.inject(2)
      state_eq_trail)
  have [simp]: "D' = D"
    by (smt T T' add_diff_cancel_left' trail_cons_trail list.inject marked_lit.inject(2)
      state_eq_trail union_commute)
  have [simp]: "i' = i"
    using state_eq_backtrack_lvl[OF T] T' by simp
  have [simp]: "M1' = tl (trail T)"
    using decomp state_eq_trail[OF T'] by auto
  (*     sledgehammer
    by (smt add_diff_cancel_left' backtrackE list.inject marked_lit.inject(2)
      state_eq_trail trail_update_trail union_commute)
  *)
...
next
  ...
  show ?case
    using resolve.prems(1) tr T unfolding all_decomposition_implies_def
...
next
  ...
  show ?case
    using skip.prems(1) tr T unfolding all_decomposition_implies_def
    by (cases "hd (get_all_marked_decomposition M)")
       (auto simp add: M)
next
  case decide note S = this(1) and T = this(4)
  show ?case using decide.prems(1) T unfolding S all_decomposition_implies_def by auto
next
  case (propagate C L) note propa = this(2) and T =this(5) and decomp = this(6) and alien = this(10)
  ...
  hence a_Un_N_M: "(\<lambda>a. {#lit_of a#}) ` set a \<union> set_mset (init_clss S)
  ...
  have "(\<lambda>a. {#lit_of a#}) ` set a \<union> set_mset (init_clss S) \<Turnstile>p {#L#}" (is "?I \<Turnstile>p _")
    proof (rule true_clss_cls_plus_CNot)
      show "?I \<Turnstile>p C + {#L#}"
        ...
        by (metis Un_iff cdcl_learned_clause_def clauses_def mem_set_mset_iff propagate.hyps(1)
  ...
  ultimately show ?case
    using decomp T unfolding ay all_decomposition_implies_def
    using M \<open>(\<lambda>a. {#lit_of a#}) ` set a \<union> set_mset (init_clss S) \<Turnstile>ps (\<lambda>a. {#lit_of a#}) ` set y\<close>
     ay by auto
next
  case (backtrack K i M1 M2 L D T) note decomp = this(1) and lev_L = this(2) and confl = this(3) and
    T = this(6)
  ...
  show ?case unfolding all_decomposition_implies_def
    proof
      ...
      hence x: "x \<in> set (get_all_marked_decomposition (Propagated L ((D + {#L#})) # M1))"
        using T decomp by simp
      ...
      moreover {
        ...
        hence 1: "(\<lambda>a. {#lit_of a#}) ` set M1' \<union> set_mset (init_clss S)
        ...
        moreover
          ...
          hence vars_of_D: "atms_of D \<subseteq> atm_of ` lits_of (trail S)"
            ...
            by (meson image_subsetI mem_set_mset_iff true_annots_CNot_all_atms_defined)
          have "backtrack S T"
            apply (rule backtrack.intros)
            using backtrack.hyps(4) backtrack.hyps(5) confl decomp lev_L T
            by (auto simp del: state_simp simp: state_eq_def)
          hence vars_of_D: "atms_of D \<subseteq> atm_of ` lits_of M1"
            using decomp backtrack_atms_of_D_in_M1[OF _ T] backtrack.prems T by auto
          ...
          hence T': "(\<lambda>a. {#lit_of a#}) ` set M1' \<union> set_mset (init_clss S) \<Turnstile>p D + {#L#}" by auto
          ...
          hence "(\<lambda>a. {#lit_of a#}) ` set M1' \<union> set_mset (init_clss S) \<Turnstile>p {#L#}"
            using true_clss_cls_plus_CNot[OF T' TT] by auto
        ultimately
          have "case x of (Ls, seen) \<Rightarrow> (\<lambda>a. {#lit_of a#}) ` set Ls
            \<union> set_mset (init_clss T)
            \<Turnstile>ps (\<lambda>a. {#lit_of a#}) ` set seen" using T' T unfolding x' by simp
      }
...
lemma cdcl_propagate_is_false:
  ...
  using assms
proof (induct rule: cdcl_all_induct)
  case (propagate C L T) note T =this(5)
  show ?case
    proof (intro allI impI)
      fix L' mark a b
      assume "a @ Propagated L' mark # b = trail T"
      hence "(a=[] \<and> L = L' \<and>  mark = C + {#L#} \<and> b = trail S)
        \<or> tl a @ Propagated L' mark # b = trail S"
        using T by (cases a) fastforce+
      moreover {
        assume "tl a @ Propagated L' mark # b = trail S"
        hence "b \<Turnstile>as CNot ( mark - {#L'#}) \<and> L' \<in>#  mark"
...
next
  ...
  thus ?case using decide.prems(6) T unfolding decide.hyps(1) by fastforce
next
  ...
  show ?case
    proof (intro allI impI)
      fix L' mark a b
      assume "a @ Propagated L' mark # b = trail T"
      hence "a @ Propagated L' mark # b = M" using tr T by simp
...
next
  ...
  show ?case
    proof (intro allI impI)
      ...
      hence "(a = [] \<and> Propagated La mark = Propagated L (D + {#L#}) \<and> b = M1)
        \<or> tl a @ Propagated La mark # b = M1"
        using M T decomp by (cases a) (auto)
      moreover {
        assume A: "a = []" and
        ...
        have "backtrack S  T"
           ...
           by (auto simp del: state_simp simp add: state_eq_def)
        hence vars_of_D: "atms_of D \<subseteq> atm_of ` lits_of M1"
          using backtrack_atms_of_D_in_M1[OF _ T] T backtrack.prems(2-4) decomp by auto
        have "no_dup (trail S)" using backtrack.prems(4) by auto
      ...
      moreover {
        ...
        hence "b \<Turnstile>as CNot ( mark - {#La#}) \<and> La \<in>#  mark"
          using backtrack.prems(6) unfolding backtrack.hyps(1) by blast
      }
...
proof (induct rule: cdcl_all_induct)
  ...
  hence "Propagated L C' # M \<Turnstile>as CNot D" using assms skip by auto
  moreover
    have "L \<notin># D"
      proof (rule ccontr)
        assume "\<not> ?thesis"
      ...
      qed
  ultimately show ?case
    using skip.hyps(1-3) true_annots_CNot_lit_of_notin_skip T unfolding cdcl_M_level_inv_def
     by fastforce
next
...
proof (rule ccontr)
  ...
  hence "total_over_m I (set_mset N)" using tot unfolding total_over_m_def by auto
  moreover have "N \<Turnstile>psm U" using learned_cl state unfolding cdcl_learned_clause_def by auto
  ...
  hence "total_over_m I (set_mset N \<union> (\<lambda>a. {#lit_of a#}) ` (set M))"
...
proof -
  ...
  hence "init_clss S' \<Turnstile>pm {#}" using assms(3) unfolding cdcl_learned_clause_def by auto
  hence "init_clss S \<Turnstile>pm {#}"
...
lemma learned_clss_are_not_tautologies:
  ...
  using assms
proof (induct rule: cdcl_all_induct)
  case (backtrack K i M1 M2 L D) note confl = this(3) and  conflicting = this(8) and
    lev_inv = this(9)
  ...
  thus ?case using backtrack by (auto split: split_if_asm)
next
  case restart
  then show ?case using learned_clss_restart_state state_eq_learned_clss
...
next
  ...
  then show ?case
    using st S by (auto intro: exI[of _ ?S\<^sub>0])
qed
...
proof -
  obtain S where
    ...
    S: "state S = (mapi Marked (length M) M, N, {#}, length M, C_True)"
    using cdcl_can_do_step[OF assms(2-4)] by auto
...
lemma tranclp_conflict_iff[iff]:
  "full1 conflict S S' \<longleftrightarrow> (((\<forall>S''. \<not>conflict S' S'') \<and> conflict S S'))"
proof -
  ...
  thus ?thesis unfolding full1_def by (meson tranclp.r_into_trancl)
qed
...
lemma cdcl_cp_state_eq_compatible:
  assumes
    "cdcl_cp S T" and
    "S \<sim> S'" and
    "T \<sim> T'"
...
lemma tranclp_cdcl_cp_state_eq_compatible:
  assumes
    ...
    "S \<sim> S'" and
    "T \<sim> T'"
  shows "cdcl_cp\<^sup>+\<^sup>+ S' T'"
  using assms
proof induction
  case base
  then show ?case
    using cdcl_cp_state_eq_compatible by blast
next
  case (step U V)
  obtain ss :: 'st where
    "cdcl_cp S ss \<and> cdcl_cp\<^sup>*\<^sup>* ss U"
    by (metis (no_types) step(1) tranclpD)
  then show ?case
    by (meson cdcl_cp_state_eq_compatible rtranclp.rtrancl_into_rtrancl rtranclp_into_tranclp2
      state_eq_ref step(2) step(4) step(5))
qed

lemma conflicting_clause_full_cdcl_cp:
  "conflicting S \<noteq> C_True \<Longrightarrow> full cdcl_cp S S"
unfolding full_def rtranclp_unfold tranclp_unfold by (auto simp add: cdcl_cp.simps)
...
lemma cdcl_cp_no_more_clauses:
  ...
  using assms by (induct rule: cdcl_cp.induct) (auto elim!: conflictE propagateE)
...
proof -
  have "propagate\<^sup>+\<^sup>+ S U \<or> (\<exists>T. propagate\<^sup>*\<^sup>* S T \<and> conflict T U)"
    ...
    (force simp: cdcl_cp.simps tranclp_into_rtranclp dest: no_conflict_after_conflict
...
text \<open>CDCL with the reasonable strategy: we fully propagate the conflict and propagate, then we
  ...
  @{term "full cdcl_cp S' S''"}\<close>
inductive cdcl_s :: "'st \<Rightarrow> 'st \<Rightarrow> bool" where
conflict': "full1 cdcl_cp S S' \<Longrightarrow> cdcl_s S S'" |
other': "cdcl_o S S'  \<Longrightarrow> no_step cdcl_cp S \<Longrightarrow> full cdcl_cp S' S'' \<Longrightarrow> cdcl_s S S''"
...
lemma full1_cdcl_cp_consistent_inv:
  assumes "full1 cdcl_cp S S'"
  and "cdcl_M_level_inv S"
  shows "cdcl_M_level_inv S'"
  using assms unfolding full1_def
proof -
  have "cdcl_cp\<^sup>+\<^sup>+ S S'" and "cdcl_M_level_inv S" using assms unfolding full1_def by auto
...
lemma cdcl_s_consistent_inv:
  ...
  unfolding full_unfold by (blast intro: cdcl_consistent_inv full1_cdcl_cp_consistent_inv cdcl.other)+
...
lemma cdcl_s_no_more_init_clss:
  ...
  unfolding full1_def full_def apply (blast dest: tranclp_cdcl_cp_no_more_init_clss
...
lemma rtranclp_cdcl_s_no_more_init_clss:
  ...
  using cdcl_s_no_more_init_clss by simp
...
lemma cdcl_cp_decreasing_measure:
  assumes cdcl: "cdcl_cp S T" and M_lev: "cdcl_M_level_inv S"
...
proof -
  have "length (trail T) \<le> card (atms_of_mu (init_clss T))"
    apply (rule length_model_le_vars)
      ...
      using cdcl by (auto simp: cdcl_cp.simps)
  with assms
  show ?thesis by induction (auto split: split_if_asm)+
qed

lemma cdcl_cp_wf: "wf {(b,a). (cdcl_M_level_inv a \<and> no_strange_atm a)
  \<and> cdcl_cp a b}"
...
lemma rtranclp_cdcl_all_struct_inv_cdcl_cp_iff_rtranclp_cdcl_cp:
  assumes
    "cdcl_M_level_inv S" and
    "no_strange_atm S"
  shows "(\<lambda>a b. (cdcl_M_level_inv a \<and> no_strange_atm a) \<and> cdcl_cp a b)\<^sup>*\<^sup>* S T
  ...
  thus "?I S T"
    ...
    next
      ...
      hence
        ...
        "no_strange_atm T"
         using \<open>cdcl\<^sup>*\<^sup>* S T\<close> apply (simp add: assms(1) rtranclp_cdcl_consistent_inv)
        using \<open>cdcl\<^sup>*\<^sup>* S T\<close> assms(2) rtranclp_cdcl_no_strange_atm_inv by blast
      hence " (\<lambda>a b. (cdcl_M_level_inv a \<and> no_strange_atm a)
...
proof -
  let ?inv = "\<lambda>a. (cdcl_M_level_inv a \<and> no_strange_atm a)"
  obtain T where T: "full (\<lambda>a b. ?inv a \<and> cdcl_cp a b) S T"
    ...
    hence "cdcl_cp\<^sup>*\<^sup>* S T"
      using rtranclp_cdcl_all_struct_inv_cdcl_cp_iff_rtranclp_cdcl_cp assms unfolding full_def
      by blast
    moreover
...
proof (induct "card (atms_of_mu (init_clss S) - atm_of `lits_of (trail S))" arbitrary: S)
  ...
  ultimately show ?case by (metis cdcl_cp.cases full_def rtranclp.rtrancl_refl)
next
  case (Suc n) note IH = this(1) and card = this(2) and alien = this(3)
  { assume a: "\<exists>S'. conflict S S'"
    then obtain S' where S': "conflict S S'" by metis
    hence "\<forall>S''. \<not>cdcl_cp S' S''" by auto
    hence ?case  unfolding full_def Ex_def using S' cdcl_cp.conflict' by blast
  }
  moreover {
    assume a: "\<exists>S'. propagate S S'"
    ...
    moreover
      ...
      hence "atm_of L \<in> atms_of_mu N" using S' unfolding no_strange_atm_def by auto
      hence "\<And>A. {atm_of L} \<subseteq> atms_of_mu N - A \<or> atm_of L \<in> A" by force
    ...
    ultimately
      ...
      hence "n = card (atms_of_mu (init_clss S') - atm_of ` lits_of (trail S'))"
        using card S S' by simp

    then have a1: "Ex (full cdcl_cp S')" using IH \<open>no_strange_atm S'\<close> by blast
    have ?case
      proof -
        obtain S'' :: "'st" where
        ...
        hence "\<exists>S''. cdcl_cp\<^sup>*\<^sup>* S S'' \<and> (\<forall>S'''. \<not> cdcl_cp S'' S''')"
          using ff1 by blast
        thus ?thesis unfolding full_def
          by meson
      qed

    }
  ultimately show ?case unfolding full_def by (metis cdcl_cp.cases rtranclp.rtrancl_refl)
qed
...
lemma full1_cdcl_cp_not_any_negated_init_clss:
  assumes "full1 cdcl_cp S S'"
  shows "no_clause_is_false S'"
...
lemma cdcl_s_not_non_negated_init_clss:
  ...
  shows "no_clause_is_false S'"
  using assms apply (induct rule: cdcl_s.induct)
  using full1_cdcl_cp_not_any_negated_init_clss full_cdcl_cp_not_any_negated_init_clss by metis+
...
proof (intro allI impI)
  ...
  thus "\<exists>L\<in>#D. get_level L (trail U) = backtrack_lvl U"
    proof cases
      case SD
      hence "S = U"
        by (metis (no_types) assms(1) cdcl_cp_conflicting_not_empty full_def rtranclpD tranclpD)
    ...
    next
      ...
      have "init_clss U = init_clss S" and "learned_clss U = learned_clss S"
        ...
        by (metis (mono_tags, lifting) assms(1) full_def rtranclp_cdcl_cp_learned_clause_inv)
      ...
      moreover
        obtain M where tr_U: "trail U = M @ trail S" and nm: "\<forall>m\<in>set M. \<not>is_marked m"
          by (metis (mono_tags, lifting) assms(1) full_def rtranclp_cdcl_cp_dropWhile_trail)
      ...
      moreover have inv_U: "cdcl_M_level_inv U"
        by (metis cdcl_s.conflict' cdcl_s_consistent_inv full full_unfold lev)
      moreover
        have "backtrack_lvl U = backtrack_lvl S"
          using full unfolding full_def by (auto dest: rtranclp_cdcl_cp_backtrack_lvl)
...
lemma cdcl_then_exists_cdcl_s_step:
  assumes o: "cdcl_o S S'" and alien: "no_strange_atm S"
  shows "\<exists>S'. cdcl_s S S'"
proof -
  obtain S'' where "full cdcl_cp S' S''"
    using always_exists_full1_cdcl_cp_step alien cdcl_no_strange_atm_inv cdcl_o_no_more_init_clss
     o other by blast
  thus ?thesis
    using assms by (metis always_exists_full1_cdcl_cp_step cdcl_s.conflict' full_unfold other')
qed
...
proof -
  ...
  show ?thesis using backtrack_rule[OF S K L L_D] by (meson bj cdcl_bj.simps state_eq_ref)
qed
...
proof -
  ...
  moreover {
    ...
    hence "unsatisfiable (set_mset (init_clss S))"
      ...
      have "atm_of ` (lits_of ?M) = atms_of_mu ?N" (is "?A = ?B")
          ...
          show "?B \<subseteq> ?A"
            proof (rule ccontr)
              ...
              hence "\<exists>S'. cdcl_o S S'"
                using cdcl_o.decide decide.intros \<open>l \<in> ?B\<close> no_strange_atm_def
                by (metis \<open>conflicting S = C_True\<close> literal.sel(1) state_eq_def)
              thus False using termi cdcl_then_exists_cdcl_s_step[OF _ alien] by metis
            qed
          qed
        ...
        hence False
          proof -
            obtain S' where
              f2: "full cdcl_cp S S'"
              by (meson alien always_exists_full1_cdcl_cp_step)
            hence "S' = S"
              using cdcl_s.conflict'[of S] by (metis (no_types) full_unfold termi)
            thus ?thesis
              using f2 \<open>D \<in># init_clss S\<close> \<open>conflicting S = C_True\<close> \<open>trail S \<Turnstile>as CNot D\<close>
              clauses_def full_cdcl_cp_not_any_negated_init_clss by auto
          qed
    }
    hence "?M \<Turnstile>asm ?N" by blast
  }
  moreover {
    ...
    { have M: "?M = hd ?M # tl ?M" using \<open>?M \<noteq> []\<close> list.collapse by fastforce
      ...
      hence False
        ...
        LD  alien termi by (metis cdcl_then_exists_cdcl_s_step)
    }
    moreover {
          ...
          have False
            proof -
              ...
              hence "cdcl_o S (update_conflicting (C_Clause (remdups_mset (D' + C'))) (tl_trail S))"
                using f1 bj[OF cdcl_bj.resolve[OF resolve_rule[of S L' C' "tl ?M" ?N ?U ?k D']]]
                C' D' M by (metis state_eq_def)
              thus ?thesis
                by (meson alien cdcl_then_exists_cdcl_s_step termi)
            qed
        }
        moreover {
          ...
          hence False
            proof -
              ...
              thus ?thesis
                ...
                by (metis (no_types) a1 alien cdcl_then_exists_cdcl_s_step level_inv termi)
            qed
        }
...
proof (induct rule: cdcl_s.induct)
  case conflict'
  thus ?case
   unfolding full1_def by (simp add: tranclp_cdcl_cp_tranclp_cdcl)
next
  case (other' S  S' S'')
...
lemma cdcl_o_conflict_is_false_with_level_inv:
  ...
  using assms
proof (induct rule: cdcl_o_induct)
  case (resolve L C M D T) note tr_S = this(1) and confl = this(2) and T = this(4) and IH = this(5)
    and n_d = this(6) and confl_inv = this(7) and M_lev = this(8)
...
next
  case (skip L C' M D T) note tr_S = this(1) and D = this(2) and T =this(5) and  confl_inv = this(8)
    and lev = this(9)
...
lemma completeness_is_a_full1_propagation:
  ...
  shows "\<exists>S'. propagate\<^sup>*\<^sup>* S S' \<and> full cdcl_cp S S'"
proof -
  obtain S' where full: "full cdcl_cp S S'"
    using always_exists_full1_cdcl_cp_step alien by blast
  then consider (propa) "propagate\<^sup>*\<^sup>* S S'"
    | (confl) "\<exists>X. propagate\<^sup>*\<^sup>* S X \<and> conflict X S'"
    using rtranclp_cdcl_cp_propagate_confl unfolding full_def by blast
  thus ?thesis
    proof cases
    ...
    next
      ...
      have clsX: "init_clss X = init_clss S"
        using X  by (auto dest!: rtranclp_propagate_is_rtranclp_cdcl rtranclp_cdcl_init_clss)
...
text \<open>See also @{thm rtranclp_cdcl_cp_dropWhile_trail}\<close>
lemma rtranclp_propagate_is_trail_append: "propagate\<^sup>*\<^sup>* S T \<Longrightarrow> \<exists>c. trail T = c @ trail S"
  by (induction rule: rtranclp_induct)  auto

lemma rtranclp_propagate_is_update_trail:
  "propagate\<^sup>*\<^sup>* S T \<Longrightarrow> T \<sim> delete_trail_and_rebuild (trail T) S"
...
proof (induction n)
  ...
  moreover have
    ...
    "cdcl_s\<^sup>*\<^sup>* (init_state N) (init_state N)"
    by (auto simp: state_eq_def simp del: state_simp)
...
next
  ...
  { assume no_step: "\<not>no_step propagate S"
    ...
    obtain S' where S': "propagate\<^sup>*\<^sup>* S S'" and full: "full cdcl_cp S S'"
    ...
    moreover
      have full: "full1 cdcl_cp S S'"
        using full no_step no_step_cdcl_cp_no_conflict_no_propagate(2) unfolding full1_def full_def
    ...
    moreover
      have
        ...
        rtranclp_propagate_is_update_trail[of S S'] S unfolding state_eq_def by simp_all
  ...
  moreover {
    ...
    have ?case
      ...
      next
          ...
          proof -
            { fix D
              ...
              moreover have "set M \<Turnstile>s CNot D"
                ...
                by (metis le_iff_sup lits_of_rev true_annots_true_cls true_clss_union_increase)
        ...
        have "no_dup M'" using S M unfolding cdcl_M_level_inv_def by auto
        ...
        ultimately obtain S'' where S'': "propagate\<^sup>*\<^sup>* ?S' S''" and full: "full cdcl_cp ?S' S''"
          using completeness_is_a_full1_propagation[OF assms(1-3), of ?S'] S by auto
        hence "length (trail ?S') \<le> length (trail S'') \<and> lits_of (trail S'') \<subseteq> set M"
        ...
        moreover
          ...
          hence "cdcl_s\<^sup>*\<^sup>* (init_state N) S''"
            using cdcl_s.intros(2)[OF decide[OF dec] _ full] no_step no_confl st
  ...
  show ?case
    proof (intro allI impI)
      ...
      moreover {
        ...
        hence "\<not>M' \<Turnstile>as CNot Da" using IH D T by auto
      }
      moreover {
        assume "Marked K i # M' = Marked L (backtrack_lvl S + 1) # trail S"
        hence "\<not>M' \<Turnstile>as CNot Da" using no_f D confl T by auto
      }
  ...
  show ?case
    proof (intro allI impI)
      ...
      hence "tl M' @ Marked K' ia # M = M1"
        using T decomp by (cases M') auto
        ...
        have "\<not>M \<Turnstile>as CNot Da"
          proof (rule ccontr)
            ...
            moreover
              ...
              hence "no_dup  (Propagated L ( (D + {#L#})) # M1)" using decomp by auto
            ultimately show False by (metis consistent_interp_def distinctconsistent_interp
              insertCI lits_of_cons marked_lit.sel(2))
          qed
      }
      ultimately show "\<not>M \<Turnstile>as CNot Da" using T by (auto split: split_if_asm)
    qed
qed
...
lemma full1_cdcl_cp_no_smaller_confl_inv:
  assumes "full1 cdcl_cp S S'"
  and n_l: "no_smaller_confl S"
  shows "no_smaller_confl S'"
  using assms unfolding full1_def
...
proof (induct rule: cdcl_s.induct)
  case (conflict' S S')
  thus ?case using full1_cdcl_cp_no_smaller_confl_inv[of S S'] by blast
next
  case (other' S S' S'')
  ...
  thus ?case using full_cdcl_cp_no_smaller_confl_inv[of S' S''] other'.hyps by blast
qed
...
lemma cdcl_o_conflict_is_no_clause_is_false:
  ...
  using assms
proof (induct rule: cdcl_o_induct)
  case (decide L T) note S = this(1) and undef = this(2) and T =this(4) and no_f = this(6) and
    lev = this(7)
  show ?case
    proof (rule HOL.disjI2, clarify)
          ...
          have "?M \<Turnstile>as CNot D"
            ...
            proof (intro allI impI)
              ...
              obtain L'' where "L'' \<in># x" and "lits_of (Marked L (?k + 1) # ?M) \<Turnstile>l L''"
                using M_D x T unfolding true_annots_def Ball_def true_annot_def CNot_def true_cls_def
      ...
      thus "\<exists>La. La \<in># D \<and> get_level La ?M'
        = backtrack_lvl T"
        using \<open>-L \<in># D\<close> T by auto
    qed
next
  case resolve
  thus ?case by auto
next
  ...
  thus ?case by auto
next
  case (backtrack K i M1 M2 L D T) note decomp = this(1) and T =this(6) and lev = this(9) and
    no_f = this(8) and no_l = this(10)
  show ?case
    proof (rule HOL.disjI2, clarify)
      fix Da
      ...
      hence "- L \<notin> lits_of M1"
        ...
        proof -
          have "consistent_interp (lits_of (trail S)) \<and> no_dup (trail S)
            ...
            \<and> get_all_levels_of_marked (trail S)
              = rev [1..<1 + length (get_all_levels_of_marked (trail S))]"
            using assms(4) cdcl_M_level_inv_def by blast
      ...
      ultimately have "\<not>M1 \<Turnstile>as CNot Da" using Da T by (auto split: split_if_asm)
      hence "-L \<in># Da"
      ...
      have g_M1: "get_all_levels_of_marked M1 = rev [1..<i+1]"
        using lev' T decomp unfolding cdcl_M_level_inv_def by auto
      have "no_dup (Propagated L ( (D + {#L#})) # M1)" using lev' T decomp by auto
      ...
      thus "\<exists>La. La \<in># Da \<and> get_level La (trail T) = backtrack_lvl T"
        using \<open>-L \<in># Da\<close> T decomp by auto
    qed
qed

lemma full1_cdcl_cp_exists_conflict_decompose:
  assumes confl: "\<exists>D\<in>#clauses S. trail S \<Turnstile>as CNot D"
  and full: "full cdcl_cp S U"
...
proof -
  consider (propa) "propagate\<^sup>*\<^sup>* S U"
   ...
   using full unfolding full_def by (blast dest:rtranclp_cdcl_cp_propa_or_propa_confl)
  thus ?thesis
    proof cases
      case confl
      thus ?thesis by blast
    next
      ...
      moreover
        ...
        obtain M where M: "trail U = M @ trail S"
          using full rtranclp_cdcl_cp_dropWhile_trail unfolding full_def by meson
      ...
      have "\<exists>V. conflict U V"
        ...
        by blast
      hence False using full cdcl_cp.conflict' unfolding full_def by blast
      thus ?thesis by fast
    qed
qed

lemma full1_cdcl_cp_exists_conflict_full1_decompose:
  assumes confl: "\<exists>D\<in>#clauses S. trail S \<Turnstile>as CNot D"
  and full: "full cdcl_cp S U"
  and no_confl: "conflicting S = C_True"
...
proof -
  obtain T where propa: "propagate\<^sup>*\<^sup>* S T" and conf: "conflict T U"
    using full1_cdcl_cp_exists_conflict_decompose[OF assms] by blast
...
proof (induct rule: cdcl_s.induct)
  ...
  show "no_smaller_confl S'"
    using conflict'.hyps conflict'.prems(1) full1_cdcl_cp_no_smaller_confl_inv by blast
next
  case (other' S S' S'')
...
next
  ...
  moreover {
    assume "no_clause_is_false S'"
    {
      ...
      moreover have "full cdcl_cp S' S''"
    ...
    moreover
    {
      ...
      moreover have "cdcl_cp\<^sup>*\<^sup>* S' S''" using other'.hyps(3) unfolding full_def by auto
      hence "S' = S''" using c
  ...
  moreover {
     ...
     moreover {
       ...
       then obtain T D where
         ...
         "conflicting S'' = C_Clause D"
         using full1_cdcl_cp_exists_conflict_full1_decompose[OF _ _  \<open>conflicting S' = C_True\<close>]
       ...
       obtain M where M: "trail S'' = M @ trail S'" and nm: "\<forall>m\<in>set M. \<not>is_marked m"
         using rtranclp_cdcl_cp_dropWhile_trail other'(3) unfolding full_def by meson
       have btS: "backtrack_lvl S'' = backtrack_lvl S'"
         using other'.hyps(3) unfolding full_def by (metis  rtranclp_cdcl_cp_backtrack_lvl)
       have inv: "cdcl_M_level_inv S''"
         by (metis (no_types) cdcl_s.conflict' cdcl_s_consistent_inv full_unfold lev' other'.hyps(3))
...
proof -
  ...
  show ?thesis
    using cdcl_s_normal_forms[OF termi decomp learned level_inv alien no_dup confl confl_k] .
qed


lemma conflict_is_full1_cdcl_cp:
  assumes cp: "conflict S S'"
  shows "full1 cdcl_cp S S'"
proof -
  ...
  ultimately show "full1 cdcl_cp S S'" unfolding full1_def by blast+
qed
...
lemma cdcl_s_fst_empty_conflicting_false:
  ...
  shows False
  using assms apply (induct rule: cdcl_s.induct)
  using tranclpD cdcl_cp_fst_empty_conflicting_false unfolding full1_def apply metis
...
lemma cdcl_s_conflicting_is_false:
  ...
  apply (induction rule: cdcl_s.induct)
    unfolding full1_def apply (metis (no_types) cdcl_cp_conflicting_not_empty tranclpD)
...
lemma full_cdcl_init_clss_with_false_normal_form:
  assumes
...
proof (induction M arbitrary: E D S)
  case Nil
  thus ?case
    using rtranclp_cdcl_s_conflicting_is_false unfolding full_def cdcl_conflicting_def by auto
next
  ...
  thus ?case
    proof cases
      case D
      thus ?thesis
        using full rtranclp_cdcl_s_conflicting_is_false S unfolding full_def E D by auto
    next
      case D'
      hence no_p: "no_step propagate S" and no_c: "no_step conflict S"
        using S E by auto
      hence "no_step cdcl_cp S" by (auto simp: cdcl_cp.simps)
      have res_skip: "\<exists>T. (resolve S T \<and> no_step skip S \<and> full cdcl_cp T T)
        \<or> (skip S T \<and> no_step resolve S \<and> full cdcl_cp T T)"
        proof cases
          ...
          have "full cdcl_cp T T"
            using sk by (auto simp add: conflicting_clause_full_cdcl_cp)
          thus ?thesis
      ...
      hence step_s: "\<exists>T. cdcl_s S T"
        using \<open>no_step cdcl_cp S\<close> other' by (meson bj resolve skip)
      ...
      have full_S_S: "full cdcl_cp S S"
        using S E by (auto simp add: conflicting_clause_full_cdcl_cp)
      hence no_f: "no_step (full1 cdcl_cp) S"
      ...
      have "resolve S T \<or> skip S T"
        using s no_b no_d res_skip full_S_S unfolding cdcl_s.simps cdcl_o.simps full_unfold full1_def
        by (auto dest!: tranclpD simp: cdcl_bj.simps)
...
proof -
  let ?S = "init_state N"
  have "cdcl_s\<^sup>*\<^sup>* ?S S'" and "no_step cdcl_s S'" using full unfolding full_def by auto
  ...
  hence cdcl_s: "\<exists>S'. cdcl_s ?S S'"
    using cdcl_cp.conflict'[of ?S] conflict_is_full1_cdcl_cp cdcl_s.intros(1) by metis
  ...
  have "conflicting St \<noteq> C_True"
    proof (rule ccontr)
      ...
      thus False using fullSt unfolding full1_def by blast
    qed
  ...
  have 2: "full cdcl_s St S'"
    using \<open>cdcl_s\<^sup>*\<^sup>* St S'\<close> \<open>no_step cdcl_s S'\<close> bt unfolding full_def by auto
  have 3: "all_decomposition_implies_m
      (init_clss St)
      (get_all_marked_decomposition
         (trail St))"
   using rtranclp_cdcl_all_inv(1)[OF st] no_d bt by simp
  have 4: "cdcl_learned_clause St"
    using rtranclp_cdcl_all_inv(2)[OF st] no_d bt bt by simp
  have 5: "cdcl_M_level_inv St"
    using rtranclp_cdcl_all_inv(3)[OF st] no_d bt by simp
  have 6: "no_strange_atm St"
    using rtranclp_cdcl_all_inv(4)[OF st] no_d bt by simp
  have 7: "distinct_cdcl_state St"
    using rtranclp_cdcl_all_inv(5)[OF st] no_d bt by simp
  have 8: "cdcl_conflicting St"
    using rtranclp_cdcl_all_inv(6)[OF st] no_d bt by simp
  have "init_clss S' = init_clss St" and "conflicting S' = C_Clause {#}"
     using \<open>conflicting St \<noteq> C_True\<close> full_cdcl_init_clss_with_false_normal_form[OF 1, of _ _ St]
     2 3 4 5 6 7 8 St apply (metis \<open>cdcl_s\<^sup>*\<^sup>* St S'\<close> rtranclp_cdcl_s_no_more_init_clss)
    using \<open>conflicting St \<noteq> C_True\<close> full_cdcl_init_clss_with_false_normal_form[OF 1, of _ _ St _ _
...
lemma full_cdcl_s_normal_forms:
  fixes S' :: 'st
  assumes full: "full cdcl_s (init_state N) S'" and no_d: "distinct_mset_mset N"
  shows "(conflicting S' = C_Clause {#} \<and> unsatisfiable (set_mset (init_clss S')))
    \<or> (conflicting S' = C_True \<and> trail S' \<Turnstile>asm init_clss S')"
...
proof -
  have N: "init_clss S' = N"
    using full unfolding full_def by (auto dest: rtranclp_cdcl_s_no_more_init_clss)
  consider
  ...
  thus ?thesis
    ...
    next
      ...
      hence "cdcl_M_level_inv S'"
        using full rtranclp_cdcl_s_consistent_inv unfolding full_def by blast
