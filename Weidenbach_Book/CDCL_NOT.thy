...
locale dpll_state_ops =
  fixes
    ...
    prepend_trail :: "('v, unit) ann_lit \<Rightarrow> 'st \<Rightarrow> 'st" and
...
locale dpll_state =
  ...
  assumes
    ...
    add_cls\<^sub>N\<^sub>O\<^sub>T:
      "state\<^sub>N\<^sub>O\<^sub>T (add_cls\<^sub>N\<^sub>O\<^sub>T C st) = (trail st, add_mset C (clauses\<^sub>N\<^sub>O\<^sub>T st))" and
...
lemma
  ...
  clauses_add_cls\<^sub>N\<^sub>O\<^sub>T[simp]:
    "clauses\<^sub>N\<^sub>O\<^sub>T (add_cls\<^sub>N\<^sub>O\<^sub>T C st) = add_mset C (clauses\<^sub>N\<^sub>O\<^sub>T st)" and
...
propagate\<^sub>N\<^sub>O\<^sub>T[intro]: "add_mset L C \<in># clauses\<^sub>N\<^sub>O\<^sub>T S \<Longrightarrow> trail S \<Turnstile>as CNot C
...
locale decide_ops =
  ...
  for
    ...
    remove_cls\<^sub>N\<^sub>O\<^sub>T :: "'v clause \<Rightarrow> 'st \<Rightarrow> 'st" +
  fixes
    decide_conds :: "'st \<Rightarrow> 'st \<Rightarrow> bool"
begin
...
text \<open>The condition @{term "atm_of L \<in> atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T S) \<union> atm_of ` (lits_of_l (trail S))"}
  is not implied by the the condition @{term "clauses\<^sub>N\<^sub>O\<^sub>T S \<Turnstile>pm add_mset L C'"} (no negation).\<close>
end
...
locale dpll_with_backjumping_ops =
  ...
  assumes
    ...
    can_propagate_or_decide_or_backjump:
      ...
      inv S \<Longrightarrow>
      no_dup (trail S) \<Longrightarrow>
      \<exists>T. decide\<^sub>N\<^sub>O\<^sub>T S T \<or> propagate\<^sub>N\<^sub>O\<^sub>T S T \<or> backjump S T"
begin
...
lemma dpll_bj_all_induct[consumes 2, case_names decide\<^sub>N\<^sub>O\<^sub>T propagate\<^sub>N\<^sub>O\<^sub>T backjump]:
  ...
  assumes
    ...
    "\<And>C L T. add_mset L C \<in># clauses\<^sub>N\<^sub>O\<^sub>T S \<Longrightarrow> trail S \<Turnstile>as CNot C \<Longrightarrow> undefined_lit (trail S) L
      \<Longrightarrow> T \<sim> prepend_trail (Propagated L ()) S
      \<Longrightarrow> P S T" and
...
next
  ...
  moreover have "unmark_l a \<union> set_mset ?N \<Turnstile>p {#L#}" (is "?I \<Turnstile>p _")
    proof (rule true_clss_cls_plus_CNot)
      show "?I \<Turnstile>p add_mset L C"
...
next
  ...
  have a_N_D_L: "unmark_l a \<union> set_mset (clauses\<^sub>N\<^sub>O\<^sub>T S) \<Turnstile>p add_mset L D"
    by (simp add: N_C)
...
proof -
  ...
  then show ?thesis
    proof cases
      ...
      let ?C = "image_mset lit_of {#L\<in>#mset ?M. is_decided L \<and> L\<noteq>?K#} :: 'v clause"
...
locale learn_ops =
  ...
  for
    ...
    remove_cls\<^sub>N\<^sub>O\<^sub>T :: "'v clause \<Rightarrow> 'st \<Rightarrow> 'st" +
  fixes
    learn_conds :: "'v clause \<Rightarrow> 'st \<Rightarrow> bool"
begin
...
next
  ...
  show ?case
    ...
    proof (intro allI, clarify)
      ...
      moreover
        have a1:"C \<in> set_mset (clauses\<^sub>N\<^sub>O\<^sub>T S)"
          using C by blast
...
next
  ...
  { fix J
    assume
      ...
      "I \<subseteq> J" and
      tot: "total_over_m J (set_mset (add_mset C (clauses\<^sub>N\<^sub>O\<^sub>T S)))" and
      cons: "consistent_interp J"
    then have "J \<Turnstile>sm clauses\<^sub>N\<^sub>O\<^sub>T S" unfolding true_clss_ext_def by auto

    moreover
      with \<open>clauses\<^sub>N\<^sub>O\<^sub>T S\<Turnstile>pm C\<close> have "J \<Turnstile> C"
...
next
  ...
  { fix J
    assume
      "I \<Turnstile>sext set_mset (clauses\<^sub>N\<^sub>O\<^sub>T S) - {C}" and
      "I \<subseteq> J" and
      tot: "total_over_m J (set_mset (clauses\<^sub>N\<^sub>O\<^sub>T S))" and
      cons: "consistent_interp J"
    then have "J \<Turnstile>s set_mset (clauses\<^sub>N\<^sub>O\<^sub>T S) - {C}"
  ...
  then have H: "I \<Turnstile>sext set_mset (clauses\<^sub>N\<^sub>O\<^sub>T S) - {C} \<Longrightarrow> I \<Turnstile>sextm (clauses\<^sub>N\<^sub>O\<^sub>T S)"
...
next
  ...
  have "inv T" using inv st rtranclp_cdcl\<^sub>N\<^sub>O\<^sub>T_inv by blast
   have "atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T U) \<subseteq> A"
...
locale conflict_driven_clause_learning_learning_before_backjump_only_distinct_learnt =
  ...
  "\<lambda>C S. distinct_mset C \<and> \<not>tautology C \<and> learn_restrictions C S \<and>
    (\<exists>F K d F' C' L. trail S = F' @ Decided K # F \<and> C = add_mset L C' \<and> F \<Turnstile>as CNot C'
      \<and> add_mset L C' \<notin># clauses\<^sub>N\<^sub>O\<^sub>T S)"
...
lemma conflicting_bj_clss_remove_cls\<^sub>N\<^sub>O\<^sub>T'[simp]:
  "T \<sim> remove_cls\<^sub>N\<^sub>O\<^sub>T C S \<Longrightarrow> conflicting_bj_clss T = conflicting_bj_clss S - {C}"
  unfolding conflicting_bj_clss_def by fastforce
...
lemma conflicting_bj_clss_add_cls\<^sub>N\<^sub>O\<^sub>T_state_eq:
  ...
  shows "conflicting_bj_clss T
    = conflicting_bj_clss S
     ...
     then {C'} else {})"
proof -
  def P \<equiv> "\<lambda>C L T. distinct_mset (add_mset L C) \<and> \<not> tautology (add_mset L C) \<and>
    (\<exists>F' K F. trail T = F' @ Decided K # F \<and> F \<Turnstile>as CNot C)"
  have conf: "\<And>T. conflicting_bj_clss T = {add_mset L C |C L. add_mset L C \<in># clauses\<^sub>N\<^sub>O\<^sub>T T \<and> P C L T}"
    unfolding conflicting_bj_clss_def P_def by auto
  have P_S_T: "\<And>C L. P C L T = P C L S"
    using T n_d unfolding P_def by auto
  have P: "conflicting_bj_clss T = {add_mset L C |C L. add_mset L C \<in># clauses\<^sub>N\<^sub>O\<^sub>T S \<and> P C L T} \<union>
     {add_mset L C |C L. add_mset L C \<in># {#C'#} \<and> P C L T}"
    using T n_d unfolding conf by auto
  moreover have "{add_mset L C |C L. add_mset L C \<in># clauses\<^sub>N\<^sub>O\<^sub>T S \<and> P C L T} = conflicting_bj_clss S"
    using T n_d unfolding P_def conflicting_bj_clss_def by auto
  moreover have "{add_mset L C |C L. add_mset L C \<in># {#C'#} \<and> P C L T} =
    (if \<exists>C L. C' = add_mset L C \<and> P C L S then {C'} else {})"
  ...
  moreover have "conflicting_bj_clss S \<noteq> conflicting_bj_clss T"
    ...
    proof (elim learn\<^sub>N\<^sub>O\<^sub>TE, goal_cases)
      ...
      then obtain F K F' C' L where
        ...
        C_S:"add_mset L C' \<notin># clauses\<^sub>N\<^sub>O\<^sub>T S"
        by blast
      moreover have "distinct_mset C" "\<not> tautology C" using inv by blast+
      ultimately have "add_mset L C' \<in> conflicting_bj_clss T"
        using T n_d unfolding conflicting_bj_clss_def by fastforce
      moreover have "add_mset L C' \<notin> conflicting_bj_clss S"
...
definition \<mu>\<^sub>C' :: "'v clause set \<Rightarrow> 'st \<Rightarrow> nat" where
"\<mu>\<^sub>C' A T \<equiv> \<mu>\<^sub>C (1+card (atms_of_ms A)) (2+card (atms_of_ms A)) (trail_weight T)"

definition \<mu>\<^sub>C\<^sub>D\<^sub>C\<^sub>L' :: "'v clause set \<Rightarrow> 'st \<Rightarrow> nat" where
...
definition \<mu>\<^sub>C\<^sub>D\<^sub>C\<^sub>L'_bound :: "'v clause set \<Rightarrow> 'st \<Rightarrow> nat" where
...
abbreviation backjump_conds :: "'v clause \<Rightarrow> 'v clause \<Rightarrow> 'v literal \<Rightarrow> 'st \<Rightarrow> 'st \<Rightarrow> bool"
  where
"backjump_conds \<equiv> \<lambda>C C' L' S T. distinct_mset C' \<and> L' \<notin># C' \<and> \<not>tautology (add_mset L' C')"
...
lemma tautology_single_add:
  "tautology (L + {#a#}) \<longleftrightarrow> tautology L \<or> -a \<in># L"
...
lemma backjump_l_implies_exists_backjump:
  ...
  shows "\<exists>U. backjump S U"
proof -
  obtain C F' K F L C' where
    tr: "trail S = F' @ Decided K # F" and
    C: "C \<in># clauses\<^sub>N\<^sub>O\<^sub>T S" and
    T: "T \<sim> prepend_trail (Propagated L ()) (reduce_trail_to\<^sub>N\<^sub>O\<^sub>T F (add_cls\<^sub>N\<^sub>O\<^sub>T (add_mset L C') S))" and
    tr_C: "trail S \<Turnstile>as CNot C" and
    undef: "undefined_lit F L" and
    L: "atm_of L \<in> atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T S) \<union> atm_of ` (lits_of_l (trail S))" and
    S_C_L: "clauses\<^sub>N\<^sub>O\<^sub>T S \<Turnstile>pm add_mset L C'" and
  ...
  { fix S S'
    ...
    then obtain F' K F L C' C D where
      ...
      atm_L:
       ...
       and
      cls_S_C': "clauses\<^sub>N\<^sub>O\<^sub>T S \<Turnstile>pm add_mset L C'" and
      F_C': "F \<Turnstile>as CNot C'" and
      dist: "distinct_mset (add_mset L C')" and
      not_tauto: "\<not> tautology (add_mset L C')" and
...
proof -
   ...
   then have "atms_of (add_mset L C') \<subseteq> atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T S) \<union> atm_of ` (lits_of_l (trail S))"
...
lemma backjump_l_backjump_learn:
  assumes bt: "backjump_l S T" and inv: "inv S"
  shows "\<exists>C' L D S'. backjump S S'
    \<and> learn S' T
    \<and> D = (add_mset L C')
    \<and> T \<sim> add_cls\<^sub>N\<^sub>O\<^sub>T D S'
    \<and> atms_of (add_mset L C') \<subseteq> atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T S) \<union> atm_of ` (lits_of_l (trail S))
    \<and> clauses\<^sub>N\<^sub>O\<^sub>T S \<Turnstile>pm D"
proof -
   obtain C F' K F L l C' D where
     tr_S: "trail S = F' @ Decided K # F" and
     T: "T \<sim> prepend_trail (Propagated L l) (reduce_trail_to\<^sub>N\<^sub>O\<^sub>T F (add_cls\<^sub>N\<^sub>O\<^sub>T D S))" and
     ...
     cond: \<open>backjump_l_cond C C' L S T\<close>
     using bt inv by (elim backjump_lE) simp
   let ?S' = "prepend_trail (Propagated L ()) (reduce_trail_to\<^sub>N\<^sub>O\<^sub>T F S)"
   have atms_C': "atms_of C' \<subseteq> atm_of ` (lits_of_l F)"
     by (metis D(2) atms_of_def image_subsetI true_annots_CNot_all_atms_defined)
   then have "atms_of (add_mset L C') \<subseteq> atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T S) \<union> atm_of ` (lits_of_l (trail S))"
     using atm_L tr_S by auto
   ...
   moreover have bj: "backjump S (prepend_trail (Propagated L ()) (reduce_trail_to\<^sub>N\<^sub>O\<^sub>T F S))"
...
next
   ...
   obtain C' :: "'v clause" and L :: "'v literal" and D :: "'v clause" where
     f3: "learn S (add_cls\<^sub>N\<^sub>O\<^sub>T D S) \<and>
       ...
       atms_of (add_mset L C') \<subseteq> atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T S) \<union> atm_of ` lits_of_l (trail S)" and
     D: "D = add_mset L C'"
...
next
  ...
  moreover have "inv U" using IH cdcl\<^sub>N\<^sub>O\<^sub>T cdcl_merged_inv inv by blast
...
definition \<mu>\<^sub>C\<^sub>D\<^sub>C\<^sub>L'_merged :: "'v clause set \<Rightarrow> 'st \<Rightarrow> nat" where
...
next
  ...
  obtain C' L D S' where
    learn: "learn S' T" and
    bj: "backjump S S'" and
    atms_C: "atms_of (add_mset L C') \<subseteq> atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T S) \<union> atm_of ` (lits_of_l (trail S))" and
...
lemma cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn_atms_of_ms_clauses_decreasing:
  assumes "cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn S T"and "inv S"
  shows "atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T T) \<subseteq> atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T S) \<union> atm_of ` (lits_of_l (trail S))"
  using assms
  apply (induction rule: cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn.induct)
...
lemma cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn_atms_in_trail_in_set:
  assumes
    "cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn S T" and "inv S" and
    "atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T S) \<subseteq> A" and
    "atm_of ` (lits_of_l (trail S)) \<subseteq> A"
  shows "atm_of ` (lits_of_l (trail T)) \<subseteq> A"
  using assms
...
lemma rtranclp_cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn_trail_clauses_bound:
  assumes
    cdcl: "cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn\<^sup>*\<^sup>* S T" and
    inv: "inv S" and
    atms_clauses_S: "atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T S) \<subseteq> A" and
    atms_trail_S: "atm_of `(lits_of_l (trail S)) \<subseteq> A"
  shows "atm_of ` (lits_of_l (trail T)) \<subseteq> A \<and> atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T T) \<subseteq> A"
...
next
  case (step T U) note st = this(1) and cdcl\<^sub>N\<^sub>O\<^sub>T = this(2) and IH = this(3)
  have "inv T" using inv st rtranclp_cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn_is_rtranclp_cdcl\<^sub>N\<^sub>O\<^sub>T_and_inv by blast
...
lemma cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn_trail_clauses_bound:
  assumes
    cdcl: "cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn S T" and
    inv: "inv S" and
    atms_clauses_S: "atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T S) \<subseteq> A" and
    atms_trail_S: "atm_of `(lits_of_l (trail S)) \<subseteq> A"
  shows "atm_of ` (lits_of_l (trail T)) \<subseteq> A \<and> atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T T) \<subseteq> A"
...
lemma cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn_all_decomposition_implies:
  assumes "cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn S T" and inv: "inv S"
    "all_decomposition_implies_m (clauses\<^sub>N\<^sub>O\<^sub>T S) (get_all_ann_decomposition (trail S))"
  shows
    "all_decomposition_implies_m (clauses\<^sub>N\<^sub>O\<^sub>T T) (get_all_ann_decomposition (trail T))"
    using assms
...
lemma rtranclp_cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn_all_decomposition_implies:
  assumes "cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn\<^sup>*\<^sup>* S T" and inv: "inv S"
    "all_decomposition_implies_m (clauses\<^sub>N\<^sub>O\<^sub>T S) (get_all_ann_decomposition (trail S))"
  shows
    "all_decomposition_implies_m (clauses\<^sub>N\<^sub>O\<^sub>T T) (get_all_ann_decomposition (trail T))"
  using assms
...
locale cdcl\<^sub>N\<^sub>O\<^sub>T_merge_bj_learn_with_backtrack_restarts =
  cdcl\<^sub>N\<^sub>O\<^sub>T_merge_bj_learn trail clauses\<^sub>N\<^sub>O\<^sub>T prepend_trail tl_trail add_cls\<^sub>N\<^sub>O\<^sub>T remove_cls\<^sub>N\<^sub>O\<^sub>T
    decide_conds propagate_conds forget_conds
    "\<lambda>C C' L' S T. distinct_mset C' \<and> L' \<notin># C' \<and> backjump_l_cond C C' L' S T" inv
  for
    trail :: "'st \<Rightarrow> ('v, unit) ann_lits" and
    clauses\<^sub>N\<^sub>O\<^sub>T :: "'st \<Rightarrow> 'v clauses" and
    prepend_trail :: "('v, unit) ann_lit \<Rightarrow> 'st \<Rightarrow> 'st" and
...
next
  ...
  have st: "cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn\<^sup>*\<^sup>* S T"
    using bj inv cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn.simps by blast+
  have "atm_of `(lits_of_l (trail T)) \<subseteq> atms_of_ms A" and \<open>atms_of_mm (clauses\<^sub>N\<^sub>O\<^sub>T T) \<subseteq> atms_of_ms A\<close>
  ...
  obtain F' K F L l C' C D where
    ...
    undef: "undefined_lit F L" and
    "clauses\<^sub>N\<^sub>O\<^sub>T S \<Turnstile>pm add_mset L C'" and
    "F \<Turnstile>as CNot C'" and
    D: "D = add_mset L C'" and
    dist: "distinct_mset (add_mset L C')" and
    tauto: "\<not> tautology (add_mset L C')" and
  ...
  then have "simple_clss (atms_of (add_mset L C')) \<subseteq> simple_clss (atms_of_ms A)"
    apply - by (rule simple_clss_mono) (simp_all)
  then have "add_mset L C' \<in> simple_clss (atms_of_ms A)"
...
proof -
  { fix J
    assume
      "I \<Turnstile>sextm M" and
      "I \<subseteq> J" and
      tot: "total_over_m J (set_mset ({#C#} + M))" and
      cons: "consistent_interp J"
    then have "J \<Turnstile>sm M" unfolding true_clss_ext_def by auto

    moreover
      with \<open>M \<Turnstile>pm C\<close> have "J \<Turnstile> C"
        using tot cons unfolding true_clss_cls_def by auto
    ultimately have "J \<Turnstile>sm {#C#} + M" by auto
  }
  then have H: "I \<Turnstile>sextm M \<Longrightarrow> I \<Turnstile>sext insert C (set_mset M)"
    unfolding true_clss_ext_def by auto
  then show ?thesis
...
lemma rtranclp_cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn_bj_sat_ext_iff:
  assumes "cdcl\<^sub>N\<^sub>O\<^sub>T_merged_bj_learn\<^sup>*\<^sup>* S T"and "inv S"
  shows "I\<Turnstile>sextm clauses\<^sub>N\<^sub>O\<^sub>T S \<longleftrightarrow> I\<Turnstile>sextm clauses\<^sub>N\<^sub>O\<^sub>T T"
  using assms apply (induction rule: rtranclp_induct)
    apply simp
