...
fun select_and_remove_from_literals_to_update_wl x =
...
fun valued_impl x =
  (fn ai => fn bi =>
    imp_nfoldli ai (fn sigma => (fn () => (is_none sigma)))
      (fn xa => fn _ =>
        (fn () =>
          (if equal_nat bi (fst xa) then SOME true
            else (if equal_nat
                       ((nat_of_integer(IntInf.~>> (integer_of_nat(bi),
                         Word.fromInt 1))))
                       ((nat_of_integer(IntInf.~>> (integer_of_nat(fst xa),
                         Word.fromInt 1))))
...
fun uminus_lit_imp l = bitXOR_nat l one_nat;
...
fun unit_propagation_inner_loop_body_wl_D_code x =
  (fn ai => fn bia => fn (a1, (a1a, (a1b, (a1c, (a1d, (a1e, (a1f, a2f))))))) =>
   ...
   (plus_nat bia one_nat,
     (op_list_prepend (propagated x_f x_a) a1,
       (a1a, (a1b, (a1c, (a1d, (a1e, (uminus_lit_imp x_f :: a1f, a2f))))))))))
                             else (fn f_ => fn () => f_
...
fun literals_to_update_wll_empty x =
  (fn (_, (_, (_, (_, (_, (_, (q, _))))))) => is_Nil q) x;

fun unit_propagation_outer_loop_wl_D_code x =
  heap_WHILET (fn s => (fn () => (not (literals_to_update_wll_empty s))))
    (fn s => let
               val (a1, a2) = select_and_remove_from_literals_to_update_wl s;
             in
...
fun get_level_wl_code x =
  (fn ai => fn bi => fn () =>
    let
      val xa =
        imp_for zero_nata (op_list_length ai)
          (fn xb => fn (a1, a2) =>
            (if equal_nat
                  ((nat_of_integer(IntInf.~>>
                    (integer_of_nat(fst (op_list_get ai xb)), Word.fromInt 1))))
                  ((nat_of_integer(IntInf.~>> (integer_of_nat(bi),
                    Word.fromInt 1)))) orelse
                  a1
              then (fn f_ => fn () => f_
                     ((is_decided_wl_code (op_list_get ai xb)) ()) ())
                     (fn x_d =>
                       (fn () =>
                         (if x_d then (true, plus_nat a2 one_nat)
                           else (true, a2))))
              else (fn () => (a1, a2))))
          (false, zero_nata) ();
    in
        ...
        imp_for zero_nata xa
          (fn xb => fn (a1, a2) =>
            (fn f_ => fn () => f_ ((arl_get heap_nat bia xb) ()) ())
              (fn xc =>
                (if equal_nat xc bi andalso not a1 then (fn () => (true, a2))
                  else (fn f_ => fn () => f_ ((arl_get heap_nat bia xb) ()) ())
                         (fn xd =>
                           (fn f_ => fn () => f_ ((get_level_wl_code ai xd) ())
                             ())
...
fun count_decided_wl_code x =
  (fn xi =>
    imp_for zero_nata (op_list_length xi)
      (fn xc => fn sigma => fn () =>
        let
          val x_c = is_decided_wl_code (op_list_get xi xc) ();
        in
          (if x_c then plus_nat sigma one_nat else sigma)
        end)
      zero_nata)
    x;
...
fun is_decided_hd_trail_wll_code x =
  (fn (a1, (_, (_, (_, (_, (_, (_, _))))))) =>
    is_decided_wl_code (op_list_hd a1))
    x;
...
fun skip_and_resolve_loop_wl_D_code x =
  (fn xi => fn () =>
      ...
      val a =
        heap_WHILET
          ...
          (fn (_, (a1a, (a1b, (a1c, (a1d, (a1e, (a1f, (a1g, a2g)))))))) =>
            ...
            in
              (fn f_ => fn () => f_ (let
                                       val x_d = op_list_hd a1a;
                                     in
                                       (fn () => (fst x_d, the (snd x_d)))
                                     end
                ()) ())
                (fn (a1h, a2h) =>
                  (fn f_ => fn () => f_
                    ((is_in_arl_code (uminus_lit_imp a1h) x_b) ()) ())
                    (fn xb =>
                      (if not xb
                        then (fn () =>
                               (false,
   ...
   (SOME x_b, (a1e, (a1f, (a1g, a2g)))))))))
                               ...
                               (fn xc =>
                                 (fn f_ => fn () => f_
                                   ((count_decided_wl_code a1a) ()) ())
                                   (fn xaa =>
                                     (if equal_nat xc xaa
                                       then (fn f_ => fn () => f_
      ((remove1_wl_code (uminus_lit_imp a1h) x_b) ()) ())
      (fn x_h =>
        (fn f_ => fn () => f_
          ((if equal_nat a2h zero_nata
             then arl_empty (default_nat, heap_nat) zero_nat
                    ...
                    (fn xd =>
                      (fn f_ => fn () => f_ ((arl_of_array_raa heap_nat xd) ())
                        ())
                        (remove1_wl_code a1h)))
          ()) ())
          (fn x_j =>
    ...
    let
      val x =
        heap_WHILET
          ...
          (fn (_, a2g) =>
            (fn f_ => fn () => f_ ((valued_impl a1 (op_list_hd a2g)) ()) ())
              (fn x =>
                (fn () =>
      ...
      (if not (is_None x)
        then let
             ...
             in
               (fn () =>
                 (false,
                   (op_list_prepend (decided x_c) a1,
                     (a1a, (a1b, (a1c, (a1d,
 (a1e, ([uminus_lit_imp x_c], a2f)))))))))
             end
        else (fn () => (true, xi)))
        ()
    end);
...
fun find_lit_of_max_level_wl_imp_code x =
  (fn ai => fn _ => fn _ => fn bie => fn _ => fn _ => fn _ => fn _ => fn bi =>
    fn () =>
    let
      val xa = maximum_level_remove_code ai bie (uminus_lit_imp bi) ();
      val xb =
        heap_WHILET
          (fn s =>
            (fn f_ => fn () => f_ ((arl_get heap_nat bie s) ()) ())
              (fn xaa =>
                (fn f_ => fn () => f_ ((arl_get heap_nat bie s) ()) ())
                  (fn xb =>
                    (fn f_ => fn () => f_ ((get_level_wl_code ai xb) ()) ())
                      (fn xba =>
                        (fn () =>
                          (if not (equal_nat xaa (uminus_lit_imp bi))
                            then not (equal_nat xba xa) else true))))))
          (fn s => (fn () => (plus_nat s one_nat))) zero_nata ();
    in
...
fun find_decomp_wl_imp_codea x =
  (fn ai => fn bia => fn bi => fn () =>
    let
      val xa = maximum_level_remove_code ai bia (uminus_lit_imp bi) ();
      val x_b = count_decided_wl_code ai ();
      val a =
        heap_WHILET (fn (a1, _) => (fn () => (less_nat xa a1)))
          (fn (a1, a2) =>
            (fn f_ => fn () => f_ ((is_decided_wl_code (op_list_hd a2)) ()) ())
              (fn x_e =>
                (fn () =>
                  (if x_e then (minus_nat a1 one_nat, op_list_tl a2)
                    else (a1, op_list_tl a2)))))
          (x_b, ai) ();
    in
      let
        val (_, aa) = a;
...
fun find_decomp_wl_imp_code x =
  (fn ai => fn _ => fn _ => fn bie => fn _ => fn _ => fn _ => fn _ =>
    find_decomp_wl_imp_codea ai bie)
    x;

fun backtrack_wl_D_code x =
  (fn (a1, (a1a, (a1b, (a1c, (a1d, (a1e, (a1f, a2f))))))) =>
    let
      val x_a = fst (op_list_hd a1);
      val x_c = the a1c;
    in
      (fn () =>
        let
          val x_e =
            find_decomp_wl_imp_code a1 a1a a1b x_c a1d a1e a1f a2f x_a ();
          val xa = arl_length heap_nat x_c ();
        in
          (if less_nat one_nat xa
            then (fn f_ => fn () => f_
                   ((find_lit_of_max_level_wl_imp_code x_e a1a a1b x_c a1d a1e
                      a1f a2f x_a)
                   ()) ())
                   (fn x_g =>
                     (fn f_ => fn () => f_
                       ((remove1_and_add_first_code (uminus_lit_imp x_a) x_g
                          x_c)
                       ()) ())
                       (fn x_j =>
                         (fn f_ => fn () => f_ ((length_ra heap_nat a1a) ()) ())
                           (fn xb =>
                             (fn f_ => fn () => f_
                               ((append_el_aa (default_nat, heap_nat) a2f x_g
                                  xb)
                               ()) ())
                               (fn x_k =>
                                 (fn f_ => fn () => f_ ((length_ra heap_nat a1a)
                                   ...
                                   (fn xc =>
                                     (fn f_ => fn () => f_
                                       ((append_el_aa (default_nat, heap_nat)
  x_k (uminus_lit_imp x_a) xc)
                                       ()) ())
                                       (fn x_m =>
 (fn f_ => fn () => f_ ((length_ra heap_nat a1a) ()) ())
   (fn xd =>
     (fn f_ => fn () => f_ ((array_of_arl_raa heap_nat x_j) ()) ())
       (fn xaa =>
         (fn f_ => fn () => f_
           ((arrayO_raa_append (default_nat, heap_nat) a1a xaa) ()) ())
           (fn xab =>
             (fn () =>
               (op_list_prepend (propagated (uminus_lit_imp x_a) xd) x_e,
                 (xab, (a1b, (NONE, (a1d, (a1e, ([x_a], x_m)))))))))))))))))
            else (fn f_ => fn () => f_ ((single_of_mset_imp_code x_c) ()) ())
                   (fn x_g =>
                     (fn () =>
                       (op_list_prepend
                          (propagated (uminus_lit_imp x_a) zero_nata) x_e,
                         (a1a, (a1b, (NONE,
                                       (a1d,
 ([x_g] :: a1e, ([x_a], a2f))))))))))
            ()
...
fun find_first_eq_map_atm_of_code x =
  (fn ai => fn bi =>
    heap_WHILET
      (fn s =>
        (fn () =>
          (if less_nat s (op_list_length bi)
            then not (equal_nat
                       ((nat_of_integer(IntInf.~>>
                         (integer_of_nat(op_list_get bi s), Word.fromInt 1))))
                       ai)
            else false)))
      (fn s => (fn () => (plus_nat s one_nat))) zero_nata)
    x;

fun extract_lits_cls_code x =
  (fn ai =>
    imp_nfoldli ai (fn _ => (fn () => true))
      (fn xa => fn sigma => fn () =>
        let
          val x_a =
            find_first_eq_map_atm_of_code
              ((nat_of_integer(IntInf.~>> (integer_of_nat(xa),
                Word.fromInt 1))))
              sigma ();
        in
          (if less_nat x_a (op_list_length sigma) then sigma
            else op_list_prepend xa sigma)
        end))
    x;

fun extract_lits_clss_code x =
  (fn ai => imp_nfoldli ai (fn _ => (fn () => true)) extract_lits_cls_code) x;

fun init_state_wl_D n l_Ns =
  let
    val na = suc (suc (fold (max ord_nat) n zero_nata));
  in
    (fn () =>
      let
        val naa = arrayO_raa_empty_sz (default_nat, heap_nat) zero_nat l_Ns ();
        val e = new heap_nat zero_nata zero_nata ();
        val nab = arrayO_raa_append (default_nat, heap_nat) naa e ();
        val ws = arrayO_ara_empty_sz_code (default_nat, heap_nat) na ();
      in
        ([], (nab, (zero_nata, (NONE, ([], ([], ([], ws)))))))
      end)
  end;

fun init_state_wl_D_code n = init_state_wl_D n;
  ...
  (fn _ => fn bia => fn a =>
    (case a
      of (a1, (a1a, (a1b, (NONE, (a1d, (a1e, (a1f, a2f))))))) =>
        (if equal_nat (op_list_length bia) one_nat
          then let
                 val x_b = op_list_hd bia;
               in
                 (fn () =>
                   let
                     val x_d = valued_impl a1 x_b ();
                   in
                     (if is_none x_d
                       then (fn () =>
                              (op_list_prepend (propagated x_b zero_nata) a1,
                                (a1a, (a1b,
(NONE, ([x_b] :: a1d, (a1e, (uminus_lit_imp x_b :: a1f, a2f))))))))
...
fun sAT_wl_code x =
  (fn xi => fn () =>
    let
      val x_b = extract_lits_clss_code xi [] ();
