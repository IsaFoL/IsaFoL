...
fun empty_Q_code x =
  (fn xi =>
    (fn () =>
      let
        val (a1, (a1a, (a1b, (_, (a1d, (a1e,
 (a1f, (a1g, (a1h, (a1i, (a1j, (a1k, (a1l, (a1m, (_, (a1o, a2o))))))))))))))))
          = xi;
      in
        (a1, (a1a, (a1b, ([], (a1d, (a1e, (a1f,
    (a1g, (a1h, (a1i, (a1j, (a1k, (a1l, (a1m,
  ((Word32.fromInt 0), (a1o, a2o))))))))))))))))
      end))
    x;
...
(fn x_k => (fn () => (plus_nat bia one_nat, x_k)))
                                 else (fn f_ => fn () => f_
...
fun get_conflict_count_heur_slow_code x =
  (fn xi =>
    (fn () =>
      let
        val (_, (_, (_, (_, (_, (_, (_, (_,
...
val minimum_number_between_restarts : Word32.word =
  Word32.fromLargeInt (IntInf.toLarge (50 : IntInf.int));
                ...
                (fn f_ => fn () => f_
                  ...
                  (fn x_a =>
                    (fn f_ => fn () => f_
                      ...
                      (fn xa =>
                        (fn f_ => fn () => f_
                          ((nth_u_code (heap_vmtf_node heap_uint32 heap_nat) a1f
                             (atm_of_code xa))
                          ()) ())
                          (fn xb =>
                            let
                              val x_e = less_nat (stamp xb) a1g;
                            in
                              (fn () =>
                                (x_e, (if x_e then a2j
...
fun cdcl_twl_local_restart_wl_D_heur_code x =
  (fn xi => fn () =>
    let
      ...
      val xaa = count_decided_st_code xi ();
    in
      (if ((xa : Word32.word) = xaa) then (fn () => xi)
        else (fn f_ => fn () => f_ ((empty_Q_code xi) ()) ())
               (fn x_b =>
                 (fn f_ => fn () => f_ ((find_decomp_wl_imp_codea xa x_b) ())
                   ())
    ...
    let
      val a =
        heap_WHILET (fn (a1, _) => (fn () => (not a1)))
          (fn (_, (a1a, a2a)) =>
            (fn f_ => fn () => f_ ((unit_propagation_outer_loop_wl_D a1a) ())
              ())
              (fn x_a =>
