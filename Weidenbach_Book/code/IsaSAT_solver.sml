...
fun watched_by_app_heur_fast_code x =
  (fn ai => fn bia => fn bi =>
    ...
    in
      nth_aa_u (heap_prod heap_uint64 heap_uint64) a1d bia bi
    end)
    x;
...
fun update_blit_wl_heur_fast_code x =
  (fn ai => fn bie => fn bid => fn bic => fn bib => fn bia =>
    ...
    let
      val xa =
        update_aa_u (heap_prod heap_uint64 heap_uint64) a1d ai bic
...
fun keep_watch_heur_fast_code x =
  (fn ai => fn bib => fn bia => fn (a1, (a1a, (a1b, (a1c, (a1d, a2d))))) =>
    fn () =>
    let
      val xa = nth_aa_u (heap_prod heap_uint64 heap_uint64) a1d ai bia ();
      val xb = update_aa_u (heap_prod heap_uint64 heap_uint64) a1d ai bib xa ();
    in
...
fun unit_propagation_inner_loop_body_wl_fast_heur_code x =
 ...
 (fn () => (plus_nat bib one_nata, (plus_nat bia one_nata, x_g))))
  ...
  x_b a1 Uint64.zero)
                                       ...
                                       (fn xb =>
 (fn f_ => fn () => f_
   ((propagate_lit_wl_fast_code a1a a1
    ...
    let
      val a =
        heap_WHILET
          (fn (_, (a1f, a2f)) =>
            (fn f_ => fn () => f_
              ((length_aa_u (heap_prod heap_uint64 heap_uint64) a2f bia) ()) ())
              (fn x_a => (fn () => (less_nat a1f x_a))))
          (fn (a1e, (a1f, a2f)) =>
            (fn f_ => fn () => f_
              ((nth_aa_u (heap_prod heap_uint64 heap_uint64) a2f bia a1f) ())
              ())
              (fn xa =>
                (fn f_ => fn () => f_
                  ((update_aa_u (heap_prod heap_uint64 heap_uint64) a2f bia a1e
                     xa)
                  ()) ())
                  (fn xb =>
                    (fn () =>
                      (plus_nat a1e one_nata, (plus_nat a1f one_nata, xb))))))
          (ai, (bib, a1d)) ();
    in
      let
        val (a1e, (_, a2f)) = a;
        ...
        heap_WHILET
          (fn (a1, a2) =>
            (fn f_ => fn () => f_ ((clause_score_extract_code ai a2 a1) ()) ())
              (fn xa =>
                (fn f_ => fn () => f_
                  ((clause_score_extract_code ai a2 (minus_nata a1 one_nata))
                  ()) ())
                  (fn xaa =>
                    (fn () =>
          ...
          (fn (a1, a2) =>
            (fn f_ => fn () => f_
              ((arl_swap heap_nat a2 a1 (minus_nata a1 one_nata)) ()) ())
              (fn x_a => (fn () => (minus_nata a1 one_nata, x_a))))
          (bi, bia) ();
    in
      let
        val (_, aa) = a;
        ...
        heap_WHILET
          ...
          (fn (a1, a2) =>
            (fn f_ => fn () => f_ ((insort_inner_clauses_by_score_code ai a2 a1)
              ()) ())
              (fn x_a => (fn () => (plus_nat a1 one_nata, x_a))))
          (one_nata, bi) ();
    in
      let
        val (_, aa) = a;
...
(a1i, (a1j, (a1k, (a1l, a2l)))))))))))
    ...
    let
      val a =
        resolve_merge_conflict_fast_code a1 a1a ai (a1c, (a1d, a2d)) a1i a1k a1l
          ();
    in
      let
...
fun isa_get_literal_and_remove_of_analyse_wl_fast_code x =
  (fn ai => fn bi => fn () =>
    let
      val a = arl_last (heap_prod heap_uint64 heap_uint64) bi ();
    in
      let
        val (a1, a2) = a;
      in
        (fn f_ => fn () => f_ ((isa_arena_lit_fast_code ai (Uint64.plus a1 a2))
          ()) ())
          (fn x_a =>
            (fn f_ => fn () => f_
                  ...
                  (let
                   ...
                   in
                     (fn f_ => fn () => f_
                       ...
                       (fn xb =>
                         (fn f_ => fn () => f_
                           ((heap_array_set_u heap_minimize_status a1b
    ...
    heap_WHILET
      ...
      (fn (a1, (a1a, _)) => fn () =>
        let
          ...
          val xaa = arl_last (heap_prod heap_uint64 heap_uint64) a1a ();
          val xab = isa_arena_length_fast_code bid (fst xaa) ();
        in
          (if Uint64.less_eq xab (snd xa)
            then (fn f_ => fn () => f_
                   ((arl_last (heap_prod heap_uint64 heap_uint64) a1a) ()) ())
                   (fn xb =>
                     (fn f_ => fn () => f_
                       ((isa_arena_lit_fast_code bid (fst xb)) ()) ())
                       (fn xc =>
                         (fn f_ => fn () => f_
                   ...
                   ((isa_get_literal_and_remove_of_analyse_wl_fast_code bid a1a)
                   ()) ())
                   (fn (a1b, a2b) =>
   ...
   then (fn () => (a1, (a2b, false)))
          ...
          (fn xad =>
            (if not xc orelse equal_minimize_status xad SEEN_FAILED
              then (fn f_ => fn () => f_
                     ((isa_mark_failed_lits_stack_fast_code bid a2b a1) ()) ())
                     (fn x_j =>
                       (fn f_ => fn () => f_
                         ((arl_empty
                            (default_prod default_uint64 default_uint64,
                     ...
                     ((get_propagation_reason_fast_code ai (uminus_code a1b))
                     ()) ())
                     (fn a =>
                       (case a
                         of NONE =>
...
default_uint64,
                                  ...
                                  (fn xc =>
                                    (fn f_ => fn () => f_
                                      ((arl_append
 (default_prod default_uint64 default_uint64, heap_prod heap_uint64 heap_uint64)
 xc (x_c, Uint64.one))
                                      ()) ())
                                      (fn x_d =>
                   ...
                   in
                     (fn f_ => fn () => f_
                       ((isa_arena_lit_code ai
                          (minus_nata (plus_nat a1a a2a) one_nata))
                       ()) ())
                       (fn xb =>
                         (fn f_ => fn () => f_
                           ((heap_array_set_u heap_minimize_status a1b
...
fun isaSAT_code x =
  (fn ai => fn bi => fn () =>
    let
      ...
      val x_e = init_dt_wl_heur_code bi x_d ();
    ...
    in
      (if not xd then empty_init_code
                      ...
                      ((length_get_clauses_wl_heur_init_code x_e) ()) ())
                      (fn xe =>
                        (if less_eq_nat xe
                              (nat_of_integer
                                (18446744073709551615 : IntInf.int))
                          then (fn f_ => fn () => f_
                                 ((isasat_information_banner_code x_e) ()) ())
                                 (fn _ =>
                                   (fn f_ => fn () => f_
                                     ((rewatch_heur_st_fast_code x_e) ()) ())
                                     (fn x_l =>
                                       (fn f_ => fn () => f_
 ((finalise_init_code ai x_l) ()) ())
 (fn x_m =>
   (fn f_ => fn () => f_ ((cdcl_twl_stgy_restart_prog_wl_heur_fast_code x_m) ())
     ())
     (fn x_n =>
       (fn f_ => fn () => f_ ((get_conflict_wl_is_None_code x_n) ()) ())
         (fn x_o =>
           (fn () =>
             (if x_o then get_trail_wl_code x_n else get_stats_code x_n)))))))
                          else (fn f_ => fn () => f_
                                 ((isasat_information_banner_code x_e) ()) ())
                                 (fn _ =>
                                   (fn f_ => fn () => f_
                                     ((isasat_init_fast_slow_code x_e) ()) ())
                                     (fn x_l =>
                                       (fn f_ => fn () => f_
 ((rewatch_heur_st_code x_l) ()) ())
 (fn x_m =>
   (fn f_ => fn () => f_ ((finalise_init_code_unb ai x_m) ()) ())
     (fn x_n =>
