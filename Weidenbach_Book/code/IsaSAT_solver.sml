...
datatype ('a, 'b) ana_ref = Ana_Ref of 'a * 'b * bool;

fun typerep_ana_refa A_ B_ t =
  Typerep
    ("IsaSAT_Lookup_Conflict.ana_ref", [typerep A_ Type, typerep B_ Type]);

fun countable_ana_ref A_ B_ = {} : ('a, 'b) ana_ref countable;

fun typerep_ana_ref A_ B_ = {typerep = typerep_ana_refa A_ B_} :
  ('a, 'b) ana_ref typerep;

fun heap_ana_ref A_ B_ =
  {countable_heap = countable_ana_ref A_ B_,
    typerep_heap = typerep_ana_ref (typerep_heap A_) (typerep_heap B_)}
  : ('a, 'b) ana_ref heap;

val default_bool : bool = false;

fun default_ana_refa A_ B_ = Ana_Ref (default A_, default B_, default_bool);

fun default_ana_ref A_ B_ = {default = default_ana_refa A_ B_} :
  ('a, 'b) ana_ref default;
                  ...
                  (fn xb =>
                    (fn () =>
                      (if not ((xb : Word32.word) = sET_FALSE_code)
                        ...
                        else (NONE, plus_nat a2 one_nata))))))
...
fun isa_arena_length_code x =
  (fn ai => fn bi => fn () =>
    let
      val xa = arl_get heap_uint32 ai (fast_minus_nat bi one_nata) ();
    in
      Uint64.plus (Uint64.fromInt (2 : IntInf.int)) (uint64_of_uint32 xa)
    end)
    x;
...
fun update_blit_wl_heur_code x =
  (fn ai => fn bie => fn bid => fn bic => fn bib => fn bia =>
    ...
    in
      (plus_nat bic one_nata,
        (plus_nat bib one_nata, (a1, (a1a, (a1b, (a1c, (xa, a2d)))))))
    end)
    x;
...
fun update_clause_wl_code x =
  (fn ai => fn bif => fn bie => fn bid => fn bic => fn bib => fn bia =>
    ...
    in
      (bid, (plus_nat bic one_nata, (a1, (x_b, (a1b, (a1c, (x_d, a2d)))))))
    end)
    x;
...
fun unit_propagation_inner_loop_body_wl_heur_code x =
          ...
          (fn x_b =>
            (fn f_ => fn () => f_ ((polarity_st_heur_pol x_b a1a) ()) ())
              (fn x_c =>
                (if ((x_c : Word32.word) = sET_TRUE_code)
                  then (fn () =>
                         (plus_nat bib one_nata, (plus_nat bia one_nata, x_b)))
                  else (if a2a
                         then (if ((x_c : Word32.word) = sET_FALSE_code)
                                then (fn f_ => fn () => f_
                                       ...
                                       ())
                                       (fn x_g =>
 (fn () => (plus_nat bib one_nata, (plus_nat bia one_nata, x_g))))
                                else (fn f_ => fn () => f_
 ...
 (fn f_ => fn () => f_
   ((propagate_lit_wl_bin_code a1a a1
      (if ((xb : Word32.word) = ai) then zero_nata else one_nata) x_b)
   ()) ())
   (fn x_i =>
     (fn () => (plus_nat bib one_nata, (plus_nat bia one_nata, x_i))))))
                         else (fn f_ => fn () => f_
                                ...
                                (fn xb =>
                                  (if not xb
                                    then (fn () =>
   (bib, (plus_nat bia one_nata, x_b)))
                                    else (fn f_ => fn () => f_
   ((access_lit_in_clauses_heur_code x_b a1 zero_nata) ()) ())
   (fn xc =>
     let
       val x_g = (if ((xc : Word32.word) = ai) then zero_nata else one_nata);
     in
       (fn f_ => fn () => f_
         ((access_lit_in_clauses_heur_code x_b a1 (fast_minus_nat one_nata x_g))
         ()) ())
         (fn x_i =>
                            ...
                            of NONE =>
                              (if ((x_k : Word32.word) = sET_FALSE_code)
                                then (fn f_ => fn () => f_
                                       ...
                                       ())
                                       (fn x_p =>
 (fn () => (plus_nat bib one_nata, (plus_nat bia one_nata, x_p))))
                                else (fn f_ => fn () => f_
            ...
            (fn f_ => fn () => f_
              ...
              (fn xb =>
                (fn f_ => fn () => f_
                  ...
                  (fn xc =>
                    (fn () =>
                      (plus_nat a1e one_nata, (plus_nat a1f one_nata, xc))))))
          (ai, (bib, a1d)) ();
    in
      let
        val (a1e, (_, a2f)) = a;
      ...
      in
        less_nat a1u
          (plus_nat (nat_of_integer (3000 : IntInf.int))
            (times_nata (nat_of_integer (500 : IntInf.int))
              (nat_of_uint64 a1o)))
      end))
    x;
...
fun arl_butlast_nonresizing x =
  (fn (xs, a) => (xs, fast_minus minus_nat a one_nata)) x;
          ...
          opts_restart a2u andalso
            less_nat a1u
              (plus_nat (nat_of_integer (2000 : IntInf.int))
                (times_nata (nat_of_integer (300 : IntInf.int))
                  (nat_of_uint64 a1o)))
      end))
    x;
  ...
  (_, (_, (_, ((_, (_, (_, (a1o, _)))), (_, (_, (_, (_, (_, _)))))))))))))))))
      ...
      in
        plus_nat (nat_of_integer (1000 : IntInf.int))
          (times_nata (nat_of_integer (150 : IntInf.int)) (nat_of_uint64 a1o))
      end))
    x;
  ...
  (fn ai => fn bib => fn bia => fn bi => fn () =>
    let
      val xa =
        let
          val xa =
            divide_nat (plus_nat bib bia) (nat_of_integer (2 : IntInf.int));
        in
          (fn f_ => fn () => f_ ((arl_get heap_nat bi bib) ()) ())
            (fn xaa =>
              (fn f_ => fn () => f_ ((clause_score_extract_code ai xaa) ()) ())
                (fn x_b =>
                  (fn f_ => fn () => f_ ((arl_get heap_nat bi xa) ()) ())
                    (fn xab =>
                      (fn f_ => fn () => f_ ((clause_score_extract_code ai xab)
                        ()) ())
                        (fn x_d =>
                          (fn f_ => fn () => f_ ((arl_get heap_nat bi bia) ())
                            ...
                            (fn xac =>
                              (fn f_ => fn () => f_
                                ((clause_score_extract_code ai xac) ()) ())
                                (fn x_f =>
                                  (fn () =>
                                    (if clause_score_ordering
  (equal_uint32, ord_uint32) equal_uint32 x_b x_d andalso
  clause_score_ordering (equal_uint32, ord_uint32) equal_uint32 x_d x_f orelse
  clause_score_ordering (equal_uint32, ord_uint32) equal_uint32 x_f x_d andalso
    clause_score_ordering (equal_uint32, ord_uint32) equal_uint32 x_d x_b
                                      then xa
                                      else (if clause_score_ordering
         (equal_uint32, ord_uint32) equal_uint32 x_b x_f andalso
         clause_score_ordering (equal_uint32, ord_uint32) equal_uint32 x_f
           x_d orelse
         clause_score_ordering (equal_uint32, ord_uint32) equal_uint32 x_d
           x_f andalso
           clause_score_ordering (equal_uint32, ord_uint32) equal_uint32 x_f x_b
     then bia else bib)))))))))
        end
          ();
      ...
      val a =
        imp_for bib (minus_nata bia one_nata)
          (fn xe => fn (a1, a2) =>
                  ...
                  (fn xd =>
                    (if clause_score_ordering (equal_uint32, ord_uint32)
                      ...
                      then (fn f_ => fn () => f_ ((arl_swap heap_nat a2 a1 xe)
                             ()) ())
                             (fn x_h => (fn () => (plus_nat a1 one_nata, x_h)))
                      else (fn () => (a1, a2))))))
          (bib, x_a) ();
    in
      let
...
fun sort_clauses_by_score_code_0 ai x =
  let
    val (a1, (a1a, a2a)) = x;
  in
    (if less_eq_nat a1a (plus_nat a1 one_nata) then (fn () => a2a)
      else (fn () =>
             let
               ...
               in
                 (fn f_ => fn () => f_
                   ((sort_clauses_by_score_code_0 ai
                      (a1, (minus_nata a2b one_nata, a1b)))
                   ()) ())
  ...
  (fn ai => fn bi => fn () =>
    ...
    in
      (if xa then (fn () => bi)
               ...
               (fn xb =>
                 sort_clauses_by_score_code_0 ai
                   (zero_nata, (minus_nata xb one_nata, bi))))
        ()
    end)
    x;
...
fun mark_garbage_heur_code2 x =
  ...
  (a1g, (a1h, (a1i, (a1j, (a1k, (a1l, (a1m,
(a1n, (a1o, (xac, (minus_nata a1q one_nata, a2q))))))))))))))))))
    end)
    x;
...
then (fn f_ => fn () => f_ ((clause_lbd_heur_code2 a2 x_d) ()) ())
 ...
 then (fn f_ => fn () => f_ ((mark_garbage_heur_code2 x_d a1 a2) ()) ())
        (fn x_l => (fn () => (a1, x_l)))
 else (fn () => (plus_nat a1 one_nata, a2))))))))))
          (x_a, xa) ();
    in
      let
        val (_, aa) = a;
    ...
    in
      (if not bi andalso xa
        then (fn f_ => fn () => f_ ((cdcl_twl_restart_wl_heur_fast_code ai) ())
               ())
               (fn x_b => (fn () => (x_b, plus_nat bia one_nata)))
        else (fn () => (ai, bia)))
        ()
    end)
    x;
...
fun to_ana_ref x = (fn a => fn b => Ana_Ref (x, a, b));
...
fun from_ana_ref x = (fn Ana_Ref (a, b, c) => (a, (b, c))) x;

fun isa_get_literal_and_remove_of_analyse_wl_fast_code x =
  (fn ai => fn bi => fn () =>
    let
      val xa = arl_last (heap_ana_ref heap_uint64 heap_uint32) bi ();
    in
      let
      ...
      in
        (fn f_ => fn () => f_
          ...
          (fn x_a =>
            (fn f_ => fn () => f_
              ((arl_length (heap_ana_ref heap_uint64 heap_uint32) bi) ()) ())
              (fn xb =>
                (fn f_ => fn () => f_
                  ((arl_set (heap_ana_ref heap_uint64 heap_uint32) bi
...
fun isa_mark_failed_lits_stack_fast_code x =
  (fn ai => fn bia => fn bi => fn () =>
    let
      val xa = arl_length (heap_ana_ref heap_uint64 heap_uint32) bia ();
      val a =
        heap_WHILET (fn (a1, _) => (fn () => (less_nat a1 xa)))
          (fn (a1, a2) =>
            (fn f_ => fn () => f_
              ((arl_get (heap_ana_ref heap_uint64 heap_uint32) bia a1) ()) ())
              (fn xb =>
                let
...
fun lit_redundant_rec_wl_lookup_fast_code x =
  (fn ai => fn bid => fn bic => fn bib => fn bia => fn bi =>
    heap_WHILET
      (fn (_, (a1a, _)) => fn () =>
        let
          val x_a = arl_is_empty (heap_ana_ref heap_uint64 heap_uint32) a1a ();
        in
          not x_a
        end)
      (fn (a1, (a1a, _)) => fn () =>
        let
          val xa = arl_last (heap_ana_ref heap_uint64 heap_uint32) a1a ();
              ...
              (if not xc orelse equal_minimize_status xab SEEN_FAILED
                then (fn f_ => fn () => f_
                       ...
                       (fn x_j =>
                         (fn f_ => fn () => f_
                           ((arl_empty
                              (default_ana_ref default_uint64 default_uint32,
                                heap_ana_ref heap_uint64 heap_uint32)
                              zero_nat)
                           ()) ())
...
fun arl_swap_u_code A_ xs i j =
  (fn () =>
    let
      val ki = (fn () => Array.sub ((fn (a,b) => a) (xs), Word32.toInt (i))) ();
      val kj = (fn () => Array.sub ((fn (a,b) => a) (xs), Word32.toInt (j))) ();
      val xsa = arl_set_u A_ xs i kj ();
      val x = arl_set_u A_ xsa j ki ();
    in
      x
    end);
  ...
  (fn ai => fn bia => fn bi => fn () =>
    let
      val a =
        heap_WHILET
          (fn (a1, a2) =>
            (if Word32.< ((Word32.fromInt 0), a1)
              then (fn f_ => fn () => f_
                     (((fn () => Array.sub ((fn (a,b) => a) (a2),
                        Word32.toInt (a1))))
                     ()) ())
                     (fn xa =>
                       (fn f_ => fn () => f_
                         ...
                         (fn xb =>
                           (fn f_ => fn () => f_
                             (((fn () => Array.sub ((fn (a,b) => a) (a2),
                                Word32.toInt (fast_minus_uint32 a1
        (Word32.fromInt 1)))))
                             ()) ())
                             (fn xaa =>
                               (fn f_ => fn () => f_
          ...
          (fn (a1, a2) =>
            (fn f_ => fn () => f_
              ((arl_swap_u_code heap_uint32 a2 a1
                 (fast_minus_uint32 a1 (Word32.fromInt 1)))
              ()) ())
              (fn x_a =>
                (fn () => (fast_minus_uint32 a1 (Word32.fromInt 1), x_a))))
          (bi, bia) ();
    in
      let
        val (_, aa) = a;
      ...
      (fn () =>
        let
          val a =
            heap_WHILET
              (fn (a1a, a2a) =>
                (fn f_ => fn () => f_ ((length_arl_u_code heap_uint32 a2a) ())
                  ())
                  (fn x_a => (fn () => (Word32.< (a1a, x_a)))))
              (fn (a1a, a2a) =>
                (fn f_ => fn () => f_ ((insert_sort_inner_nth_code a1 a2a a1a)
                  ()) ())
                  (fn x_a =>
                    (fn () => (Word32.+ (a1a, (Word32.fromInt 1)), x_a))))
              ((Word32.fromInt 1), bi) ();
        in
          let
            val (_, aa) = a;
        ...
        then (valuea, (alpha, (beta, (Uint64.minus wait Uint64.one, period))))
               ...
               val waita =
                 Uint64.plus
                   ...
                   Uint64.one;
               val perioda = waita;
               val betaa = shiftr_uint64 beta one_nata;
               val betab =
            ...
            (fn f_ => fn () => f_ ((nth heap_uint32 ai a1) ()) ())
              (fn xa =>
                (fn f_ => fn () => f_
                  ...
                  (fn x_a => (fn () => (plus_nat a1 one_nata, (x_a, a2a))))))
          (zero_nata, (bia, bi)) ();
    in
      let
        val (_, aa) = a;
...
fun last_trail_code x =
  (fn (a1, (_, (_, (a1c, _)))) => fn () =>
    let
      ...
      val x_a = arl_last heap_uint32 a1 ();
    in
      (x_a, (if equal_nat xb one_nata then NONE else SOME xb))
    end)
    x;
    ...
    in
      (fn () =>
        let
          ...
          val xb =
            (if ((xa : Word32.word) = ai)
              then isa_arena_lit_code bie (plus_nat bid one_nata)
           ...
           ((if not xh then Word32.+ (a1e, (Word32.fromInt 1)) else a1e),
             x_h)))))))
                     ...
                     ai)) andalso
                                  ...
                                  (x_g, (x_a, x_c)))))))))))))))))))
              (plus_nat bid one_nata, (bib, (a2, (bia, bi)))) ();
        in
          let
            val (_, (a1b, (a1c, (a1d, a2d)))) = a;
          in
...
fun tl_trail_tr_code x =
  (fn (a1, (a1a, (a1b, (a1c, (a1d, a2d))))) => fn () =>
    ...
    in
      (arl_butlast_nonresizing a1,
        (xab, (xb, (a1c, ((if equal_nat xc one_nata
                            then fast_minus_uint32 a1d (Word32.fromInt 1)
                            else a1d),
                           (if equal_nat xd one_nata
                             then arl_butlast_nonresizing a2d else a2d))))))
    end)
    x;
  ...
  (fn ai => fn (a1, (a1a, (a1b, (a1c, (a1d, a2d))))) => fn () =>
    let
      ...
      val xb =
        heap_array_set_u heap_uint32 a1b (atm_of_code ai)
          (Word32.+ (a1d, (Word32.fromInt 1))) ();
      val xc = heap_array_set_u heap_nat a1c (atm_of_code ai) one_nata ();
      val xd =
        arl_append (default_uint32, heap_uint32) a2d (uint32_of_nat xa) ();
    in
      (x_a, (xab, (xb, (xc, (Word32.+ (a1d, (Word32.fromInt 1)), xd)))))
...
fun isa_get_literal_and_remove_of_analyse_wl_code x =
  (fn ai => fn bi => fn () =>
    let
      val xa = arl_last (heap_ana_ref heap_nat heap_uint32) bi ();
    in
      let
        val (a1, (a1a, a2a)) = from_ana_ref xa;
      in
        (fn f_ => fn () => f_
          ...
          (fn x_a =>
            (fn f_ => fn () => f_
              ((arl_length (heap_ana_ref heap_nat heap_uint32) bi) ()) ())
              (fn xb =>
                (fn f_ => fn () => f_
                  ((arl_set (heap_ana_ref heap_nat heap_uint32) bi
                     (fast_minus_nat xb one_nata)
                     (to_ana_ref a1 (Word32.+ (a1a, (Word32.fromInt 1))) a2a))
                  ()) ())
                  (fn x_b => (fn () => (x_a, x_b)))))
...
fun isa_mark_failed_lits_stack_code x =
  (fn ai => fn bia => fn bi => fn () =>
    let
      val xa = arl_length (heap_ana_ref heap_nat heap_uint32) bia ();
      val a =
        heap_WHILET (fn (a1, _) => (fn () => (less_nat a1 xa)))
          (fn (a1, a2) =>
            (fn f_ => fn () => f_
              ((arl_get (heap_ana_ref heap_nat heap_uint32) bia a1) ()) ())
              (fn xb =>
                let
                  val (a1a, (a1b, _)) = from_ana_ref xb;
                in
  ...
  (fn ai => fn bi =>
    ...
    in
      (fn () => let
                  val xa = nth_u_code heap_nat a1c (atm_of_code bi) ();
                in
                  (if equal_nat xa one_nata then NONE else SOME xa)
                end)
    end)
    x;
...
fun lit_redundant_rec_wl_lookup_code x =
  (fn ai => fn bid => fn bic => fn bib => fn bia => fn bi =>
    heap_WHILET
      (fn (_, (a1a, _)) => fn () =>
        let
          val x_a = arl_is_empty (heap_ana_ref heap_nat heap_uint32) a1a ();
        in
          not x_a
        end)
      (fn (a1, (a1a, _)) => fn () =>
        let
          val xa = arl_last (heap_ana_ref heap_nat heap_uint32) a1a ();
              ...
              (if not xc orelse equal_minimize_status xab SEEN_FAILED
                then (fn f_ => fn () => f_
                       ...
                       (fn x_j =>
                         (fn f_ => fn () => f_
                           ((arl_empty
                              (default_ana_ref default_nat default_uint32,
                                heap_ana_ref heap_nat heap_uint32)
                              zero_nat)
                           ()) ())
                           (fn xd => (fn () => (x_j, (xd, false)))))
                else (fn f_ => fn () => f_
        ...
        heap_WHILET
          ...
          (fn (a1, a2) =>
            (fn f_ => fn () => f_ ((convert_single_wl_to_nat_code xi a1 a2 a1)
              ()) ())
              (fn x_f => (fn () => (plus_nat a1 one_nata, x_f))))
          (zero_nata, x_b) ();
    in
      let
        val (_, aa) = a;
...
fun init_state_wl_D_code x =
  (fn (a1, a2) =>
    let
      ...
      val x_b = times_nata (nat_of_integer (2 : IntInf.int)) xa;
    in
      (fn () =>
        let
                                 ...
                                 (fn f_ => fn () => f_
                                   ...
                                   (fn xba =>
                                     (fn f_ => fn () => f_
                                       ((arl_get heap_nat ai xaa) ()) ())
                                       (fn xca =>
 (fn f_ => fn () => f_ ((isa_arena_lit_code bia (plus_nat xca one_nata)) ()) ())
   (fn xcb =>
     (fn f_ => fn () => f_ ((arl_get heap_nat ai xaa) ()) ())
       (fn xda =>
       ...
       (2 : IntInf.int)))))
               ...
               (fn x_c =>
                 (fn f_ => fn () => f_ ((arl_get heap_nat ai xaa) ()) ())
                   (fn xf =>
                     (fn f_ => fn () => f_
                       ((isa_arena_lit_code bia (plus_nat xf one_nata)) ()) ())
                       (fn x_d =>
                         (fn f_ => fn () => f_ ((arl_get heap_nat ai xaa) ())
                           ())
                           (fn xg =>
