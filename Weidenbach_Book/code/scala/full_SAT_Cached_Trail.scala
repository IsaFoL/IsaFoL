...
def extract_lits_cls_imp:
  ...
  ((ai: List[nat]) =>
    ((a: (hashtable[nat, Unit], List[nat])) =>
      imp_nfoldli[nat, (hashtable[nat, Unit],
                         List[nat])](ai, ((_: (hashtable[nat, Unit], List[nat]))
    =>
   (_: Unit) => true),
                                      ((x: nat) =>
(sigma: (hashtable[nat, Unit], List[nat])) => (b: Unit) =>
{
  val (): Unit = b
  val x_a: Boolean =
    nat_lit_lits_init_assn_assn_in.apply(shiftr1(x)).apply(sigma).apply(());
  (if (x_a) (_: Unit) => sigma
    else nat_lit_lits_init_assn_assn_prepend.apply(x).apply(sigma))(())
}),
                                      a)))
