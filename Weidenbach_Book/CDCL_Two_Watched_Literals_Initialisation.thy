...
next
  ...
  have 
    alien: \<open>cdcl\<^sub>W_restart_mset.no_strange_atm (convert_to_state (convert_lits M, twl_clause_of `# N,
        twl_clause_of `# U, map_option mset D, NP, UP, 
        {#(watched b ! a, twl_clause_of b). (a, b) \<in># {#}#}, Q))\<close> and
    \<open>cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_M_level_inv (convert_to_state (convert_lits M, twl_clause_of `# N,
        twl_clause_of `# U, map_option mset D, NP, UP,
        {#(watched b ! a, twl_clause_of b). (a, b) \<in># {#}#}, Q))\<close> and
    \<open>(\<forall>s\<in>#learned_clss (convert_to_state (convert_lits M, twl_clause_of `# N, twl_clause_of `# U, 
    ...
    \<open>cdcl\<^sub>W_restart_mset.distinct_cdcl\<^sub>W_state (convert_to_state (convert_lits M, twl_clause_of `# N,
        twl_clause_of `# U, map_option mset D, NP, UP, 
        {#(watched b ! a, twl_clause_of b). (a, b) \<in># {#}#}, Q))\<close> and
    \<open>cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_conflicting (convert_to_state (convert_lits M, twl_clause_of `# N,
        twl_clause_of `# U, map_option mset D, NP, UP,
        {#(watched b ! a, twl_clause_of b). (a, b) \<in># {#}#}, Q))\<close> and
    \<open>all_decomposition_implies_m (cdcl\<^sub>W_restart_mset.clauses (convert_to_state
     ...
     (get_all_ann_decomposition (trail (convert_to_state (convert_lits M, twl_clause_of `# N,
            twl_clause_of `# U, map_option mset D, NP, UP,
            {#(watched b ! a, twl_clause_of b). (a, b) \<in># {#}#}, Q))))\<close> and
    \<open>cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_learned_clause (convert_to_state (convert_lits M, twl_clause_of `# N,
        twl_clause_of `# U, map_option mset D, NP, UP,
  ...
  proof (cases \<open>length a = 1\<close>)
    ...
    proof (cases D)
      ...
      have all_inv': 
        ...
        subgoal 
          using all_struct by (auto simp: cdcl\<^sub>W_restart_mset_state 
              ...
              split: if_splits)
        subgoal 
          using all_struct by (auto simp: a cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_conflicting_def cdcl\<^sub>W_restart_mset_state
              cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_all_struct_inv_def S Let_def split: if_splits)
        subgoal 
          using all_struct by (auto simp: a cdcl\<^sub>W_restart_mset.no_strange_atm_def cdcl\<^sub>W_restart_mset_state
              cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_all_struct_inv_def S Let_def split: if_splits)
        subgoal 
          using all_struct by (auto simp: a cdcl\<^sub>W_restart_mset.no_strange_atm_def cdcl\<^sub>W_restart_mset_state
              cdcl\<^sub>W_restart_mset.cdcl\<^sub>W_all_struct_inv_def S Let_def split: if_splits)
      ...
      show ?thesis
        ...
        subgoal
          using H S' valid by (auto simp: twl_struct_invs_def a S twl_st_inv.simps
            twl_clause_of_def get_level_cons_if
              split: if_splits)
        subgoal
          using all_inv' S' apply simp
          using S' all_struct (* apply (auto simp: twl_struct_invs_def a S twl_st_inv.simps
            twl_clause_of_def get_level_cons_if
              valid_annotation.simps
        ...
        subgoal
          using S' excep (* apply (auto simp: a S cdcl\<^sub>W_restart_mset_state twl_clause_of_def
        ...
        subgoal
          using S' no_dup by (auto simp: a S cdcl\<^sub>W_restart_mset_state twl_clause_of_def
            twl_exception_inv.simps uminus_lit_swap
              cdcl\<^sub>W_restart_mset.no_smaller_propa_def clauses_def split: if_splits)
        subgoal
          using S' dist_q in_pending apply (auto simp add: a S cdcl\<^sub>W_restart_mset_state twl_clause_of_def
            twl_exception_inv.simps Decided_Propagated_in_iff_in_lits_of_l
              cdcl\<^sub>W_restart_mset.no_smaller_propa_def clauses_def split: if_splits) 
        sorry
          using S' apply (auto simp: a S cdcl\<^sub>W_restart_mset_state twl_clause_of_def
            twl_exception_inv.simps uminus_lit_swap
              cdcl\<^sub>W_restart_mset.no_smaller_propa_def clauses_def split: if_splits)
        subgoal
