...
theory WoLLIC_FOL imports Main begin
...
In applications @{text \<open>Fun a ts\<close>} and predicates
@{text \<open>Pre a ts\<close>}, the length of @{text \<open>ts\<close>} is considered
to be a part of the function or predicate name, so @{text \<open>Fun a [t]\<close>}
and @{text \<open>Fun a [t,u]\<close>} refer to different functions.
...
a problem with the treatment of formulae of the form @{text \<open>Exi P\<close>} and
@{text \<open>Neg (Uni P)\<close>}. Fitting therefore suggests to define an {\em alternative
...
text \<open>
Note that in the clauses for @{text \<open>Exi P\<close>} and @{text \<open>Neg (Uni P)\<close>},
