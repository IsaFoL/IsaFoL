    ...
    proof -
      ...
      proof
       ...
       have \<open>{to_V C |C. C \<in> X \<and> is_Pos C} \<union> {to_V C |C. C \<in> Y \<and> \<not> is_Pos C} = M'\<close>
        unfolding X_def Y_def using rm_all_ext[of M] rm_all_ext_neg[of N] rm_all_ext_comp[of N]
          rm_all_ext_comp_neg[of M]
...
sorry
  ...
  qed
    ...
    then obtain X Y where \<open>M' = (to_V ` {C \<in> X. is_Pos C}) \<union> (to_V ` {C \<in> Y. \<not> is_Pos C})\<close> and
      \<open>N' =  (to_V ` {C \<in> Y. is_Pos C}) \<union> (to_V ` {C \<in> X. \<not> is_Pos C})\<close> and
      \<open>M \<subseteq> X\<close> and \<open>N \<subseteq> Y\<close>
sorry
    ...
    define X where "X = {C. to_V C \<in> M' \<and> is_Pos C} \<union> {C. to_V C \<in> N' \<and> \<not> is_Pos C}"
    define Y where "Y = {C. to_V C \<in> M' \<and> \<not> is_Pos C} \<union> {C. to_V C \<in> N' \<and> is_Pos C}"
    have \<open>X \<union> Y = UNIV\<close>
      unfolding X_def Y_def using union_univ by blast
    moreover have \<open>M \<subseteq> X\<close>
      unfolding X_def using m_pos_subs m_neg_subs by blast
    moreover have \<open>N \<subseteq> Y\<close>
      unfolding Y_def using n_pos_subs n_neg_subs by blast
    ultimately have \<open>X \<Turnstile>\<^sub>\<sim> Y\<close> using all_supsets_entails by auto 
    then show \<open>M' \<Turnstile> N'\<close>
      unfolding X_def Y_def entails_neg_def 
      sorry
  qed
