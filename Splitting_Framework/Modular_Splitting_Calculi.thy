...
interpretation AF_cons_rel: consequence_relation "to_AF bot"  \<open>(\<Turnstile>\<^sub>A\<^sub>F)\<close>
  using core.AF_cons_rel.consequence_relation_axioms by auto
...
proof -
  ...
  proof (intro ballI)
    ...
    next
      case other
      then show ?thesis
        using base_calculus.sound \<C>_is_consq_of_\<iota> by auto
    qed
  qed
qed
...
proof clarsimp
  ...
  then have \<open>\<exists> \<C>' \<in> \<M> \<union> {AF.Pair (F_of \<C>) B}. (F_of \<C>' = F_of \<C> \<and> A_of \<C>' |\<subset>| A_of \<C>)\<close>
    by auto
  then show \<open>\<C> \<in> core.SRed\<^sub>F (AF.Pair (F_of \<C>) B \<triangleright> \<M>)\<close>
