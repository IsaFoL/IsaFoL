...
definition line_assm :: "valuation \<Rightarrow> nat set \<Rightarrow> form set" where
"line_assm \<A> \<equiv> op ` (\<lambda>k. turn_true \<A> (Atom k))"
definition line_suitable :: "nat set \<Rightarrow> form \<Rightarrow> bool" where
...
lemma line_assm_Cons[simp]: "line_assm \<A> (k\<cdot>ks) = (if \<A> k then Atom k else Not (Atom k)) \<cdot> line_assm \<A> ks"
unfolding line_assm_def by simp

lemma NotD: "\<Gamma> \<turnstile> \<^bold>\<not> F \<Longrightarrow> F\<cdot>\<Gamma> \<turnstile> \<bottom>" by (meson Not2I NotE Weaken subset_insertI)
...
using assms proof(induction F)
  case (Atom k) thus ?case using Ax
    by (simp add: line_suitable_def line_assm_def)
next
  case Bot
...
next
  have [simp]: "\<Gamma> \<turnstile> \<^bold>\<not> (\<^bold>\<not> F) \<longleftrightarrow> \<Gamma> \<turnstile> F" for F \<Gamma> using NotI NotE by (meson NDtrans Not2E Not2I)
  case (Not F)
  hence "line_assm \<A> Z \<turnstile> turn_true \<A> F" by simp
  thus ?case by(cases "\<A> \<Turnstile> F"; simp)
next
  have [simp]: "\<lbrakk>line_assm \<A> Z \<turnstile> \<^bold>\<not> F; \<not> \<A> \<Turnstile> F\<rbrakk> \<Longrightarrow> F \<^bold>\<and> G\<cdot> line_assm \<A> Z \<turnstile> \<bottom>" for F G by(blast intro!: NotE[where F=F] intro: AndE1[OF Ax] Weaken[OF _ subset_insertI])
  have [simp]: "\<lbrakk>line_assm \<A> Z \<turnstile> \<^bold>\<not> G; \<not> \<A> \<Turnstile> G\<rbrakk> \<Longrightarrow> F \<^bold>\<and> G\<cdot> line_assm \<A> Z \<turnstile> \<bottom>" for F G by(blast intro!: NotE[where F=G] intro: AndE2[OF Ax] Weaken[OF _ subset_insertI]) 
...
lemma deconstruct_assm_set:
  assumes IH: "\<And>\<A>. line_assm \<A> (k\<cdot>Z) \<turnstile> F"
...
next
  ...
  from IH have av: "line_assm (\<A>(k := v)) (k\<cdot>Z) \<turnstile> F" for v by blast
  text\<open>However, that modification is only relevant for @{term "k\<cdot>Z"}, nothing from @{term Z} gets touched.\<close>
  ...
  hence "line_assm (\<A>(k := v)) (k\<cdot>Z) = 
      (if v then Atom k else Not (Atom k)) \<cdot> line_assm \<A> Z" for v by simp
  text\<open>Inserting @{const True} and @{const False} for \<open>v\<close> yields the two alternatives.\<close>
  with av have "Atom k \<cdot> line_assm \<A> Z \<turnstile> F" "Not (Atom k) \<cdot> line_assm \<A> Z \<turnstile> F"
    by(metis (full_types))+
...
proof -
  ...
  next
    case (3 k Z)
    from \<open>k \<in> Z\<close> have *: \<open>k \<cdot> Z - {k} = Z\<close> by blast
  ...
  have "line_assm \<A> {} \<turnstile> F" for \<A>
    using deconstruct_assm_set[OF spec[where P="\<lambda>\<A>. line_assm \<A> (_ \<cdot> _) \<turnstile> F"]]
