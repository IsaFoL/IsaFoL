...
lemma extended_AxE[intro!]: "A, \<Gamma> \<Rightarrow> A, \<Delta>" by (intro extended_Ax) (simp add: add.commute inter_add_right2)
...
proof(induction rule: HC.induct)
  case (Ax F) thus ?case proof
    ...
    have G: "\<Gamma> \<Rightarrow> {#(F \<^bold>\<rightarrow> \<bottom>) \<^bold>\<rightarrow> \<^bold>\<not> F#}" for F using SCp.BotL by blast
    have H: "\<Gamma> \<Rightarrow> {#\<^bold>\<not> F \<^bold>\<rightarrow> (F \<^bold>\<rightarrow> \<bottom>)#}" for F  by blast
    have I: "\<Gamma> \<Rightarrow> {#(\<^bold>\<not> F \<^bold>\<rightarrow> \<bottom>) \<^bold>\<rightarrow> F#}" for F using BotL by blast
    have K: "\<Gamma> \<Rightarrow> {#F \<^bold>\<rightarrow> (G \<^bold>\<rightarrow> F)#}" for F G by blast
    have L: "\<Gamma> \<Rightarrow> {#(F \<^bold>\<rightarrow> (G \<^bold>\<rightarrow> H)) \<^bold>\<rightarrow> ((F \<^bold>\<rightarrow> G) \<^bold>\<rightarrow> (F \<^bold>\<rightarrow> H))#}" for F G H by auto
...
next
  case (MP F G) 
  from MP have "\<Gamma> \<Rightarrow> {#F#}" "\<Gamma> \<Rightarrow> {#F \<^bold>\<rightarrow> G#}" by blast+
  with ImpR_inv'[where \<Delta>=Mempty, simplified] have "F,\<Gamma> \<Rightarrow> {#G#}" by simp
  moreover from MP.IH(1) weakenR' have "\<Gamma> \<Rightarrow> F, {#G#}" by blast
  ultimately show "\<Gamma> \<Rightarrow> {#G#}" using cut'[where A=F] by simp
qed
