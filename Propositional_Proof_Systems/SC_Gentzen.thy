...
inductive SCg :: "form list \<Rightarrow> form list \<Rightarrow> bool" (infix "\<Rightarrow>" 30) where
...
proof -
  ...
  show ?thesis unfolding \<Gamma> using AnfangTauschA * by simp
qed
...
theorem "mset \<Gamma> \<Rightarrow> mset \<Delta> \<longleftrightarrow> \<Gamma> \<Rightarrow> \<Delta>" proof
  ...
  proof(induction n arbitrary: \<Gamma> \<Delta> rule: nat.induct)
    ...
    next
      ...
      from Suc.IH[OF this] show ?thesis unfolding \<Gamma> using AnfangTauschA NEA by simp
    next
      case (NotR F \<Delta>')
      ...
      from Suc.IH[OF this] show ?thesis using \<Delta> using AnfangTauschS NES by simp
    next
      case (AndR F \<Delta>' G)
      ...
      from this[THEN Suc.IH] show ?thesis using \<Delta> using AnfangTauschS UES by simp
    next
      case (OrR F G \<Delta>')
      ...
      thus ?thesis unfolding \<Delta>[THEN conjunct1] using AnfangTauschS by simp
    next
      case (ImpR F G \<Delta>')
      ...
      from this[THEN Suc.IH] show ?thesis using \<Delta> using AnfangTauschS FES by simp
    next
      case (AndL F G \<Gamma>')
      ...
      thus ?thesis unfolding \<Gamma>[THEN conjunct1] using AnfangTauschA by simp
    next
      case (OrL F \<Delta>' G)
      from sr[OF this(1)] guess \<Gamma>1 .. then guess \<Gamma>2 .. note \<Gamma> = this
      ...
      from this[THEN Suc.IH] show ?thesis using \<Gamma> using AnfangTauschA OEA by simp
    next
      case (ImpL \<Gamma>' F G)
      from sr[OF this(1)] guess \<Gamma>1 .. then guess \<Gamma>2 .. note \<Gamma> = this
  ...
  qed blast
next
  have mset_Cons[simp]: "mset (A # S) = A, mset S" for A::form and S by (simp add: add.commute)
  ...
  show "\<Gamma> \<Rightarrow> \<Delta> \<Longrightarrow> mset \<Gamma> \<Rightarrow> mset \<Delta>" proof(induction rule: SCg.induct)
    ...
    case (Anfang \<DD>) thus ?case using extended_Ax SC_SCp_eq by simp
  next
    case (FalschA) thus ?case using SCp.BotL by simp
  next
    case (VerduennungA \<Gamma> \<Theta> \<DD>) thus ?case using weakenL' by force
  next
    case (VerduennungS \<Gamma> \<Theta> \<DD>) thus ?case using weakenR' by force
  next
    case (ZusammenziehungA \<DD> \<Gamma> \<Theta>) thus ?case using contractL' by force
  next
    case (ZusammenziehungS \<Gamma> \<DD> \<Theta>) thus ?case using contractR' by force
  next
  ...
  next
    ...
    from \<open>mset (\<AA> # \<Gamma>) \<Rightarrow> mset \<Theta>\<close> have "\<AA>,\<BB>,mset \<Gamma> \<Rightarrow> mset \<Theta>" using weakenL' by auto
    thus ?case using SCp.AndL by force
  next
    case (UEA2 \<BB> \<Gamma> \<Theta> \<AA>)
    from \<open>mset (\<BB> # \<Gamma>) \<Rightarrow> mset \<Theta>\<close> have "\<AA>,\<BB>,mset \<Gamma> \<Rightarrow> mset \<Theta>" using weakenL' by force
    thus ?case using SCp.AndL by force
  next
    case (OEA \<AA> \<Gamma> \<Theta> \<BB>) thus ?case unfolding SC_SCp_eq by (simp add: SCp.OrL)
  next
    case (OES1 \<Gamma> \<AA> \<Theta> \<BB>) thus ?case using weakenR' unfolding SC_SCp_eq by (simp add: SCp.OrR inR1')
  next
    case (OES2 \<Gamma> \<BB> \<Theta> \<AA>) thus ?case using weakenR' unfolding SC_SCp_eq by (simp add: SCp.OrR)
  next
  ...
  next
    ...
    show ?case using S A SC_SCp_eq SCp.ImpL unfolding mset_Cons by meson
  next
