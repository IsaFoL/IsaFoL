...
next
  ...
  next
    case True
    then obtain \<Gamma>'' where "\<Gamma>' = \<^bold>\<not> F\<cdot>\<Gamma>''" "\<^bold>\<not> F \<notin> \<Gamma>''" by (meson Set.set_insert)
    hence "\<Gamma>'' \<subseteq> \<Gamma>" "finite \<Gamma>''" "\<^bold>\<not> F\<cdot>\<Gamma>'' \<turnstile> \<bottom>" using \<Gamma>' by auto
    thus ?thesis using ND.CC by meson
  qed
next
...
next
  ...
  from IH have "?w \<turnstile> F \<^bold>\<or> G" using Weaken[OF _ sup_ge1] by presburger moreover
  from IH have "F\<cdot>?w \<turnstile> H" "G\<cdot>?w \<turnstile> H" using Weaken by (metis Un_commute Un_insert_right Un_upper1 Weaken insert_Diff_single)+ ultimately
