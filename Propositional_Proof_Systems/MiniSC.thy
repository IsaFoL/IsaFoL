...
proof -
  ...
  next
    ...
    thus ?case using SCp.Ax by blast
  next
...
proof -
  ...
  next
    ...
    next
      ...
      from ImpL(2)[OF this(1,2)] ImpL(4)[OF this(3-)]
      show ?thesis using Cut_Bot_pre' by(force intro!: SCp.OrL dest!: ImpR_inv' simp: *)
    next
      fix S2 H J
      ...
      show ?thesis using Cut_Bot_pre' using SCp.AndL ImpR_inv' * by (metis add_mset_remove_trivial inL1')
    next
      fix S2 H
      ...
      from ImpL(2)[OF this(1,2)]
      show ?thesis using Cut_Bot_pre by(force intro!: SCp.NotL dest!: ImpR_inv simp: *)
    qed
  next
    case (ImpR F G \<Delta>')
    note whai[OF ImpR(3)]
    thus ?case proof(elim disjE  exE conjE)
      ...
      thus ?thesis using ImpR.hyps(2,3) SCp.ImpR by auto
    next
      fix S2 H J
      ...
      thus ?thesis using SCp.OrR * ImpR.hyps(2) by (simp add: NotL_inv')
    next
      have botoff: "\<Gamma> \<Rightarrow> H, \<bottom>, S2 \<Longrightarrow> \<Gamma> \<Rightarrow> H, S2" for \<Gamma> H S2 using Cut_Bot_pre' by fastforce
      fix S2 H J
    ...
    next  
      ...
      hence "\<Gamma> \<Rightarrow> \<^bold>\<not> H, S2" using SCp.NotR Cut_Bot_pre' by fastforce
