...
proof -
  ...
  hence "\<^bold>\<not>F, S' \<Rightarrow> {#}" using NotR_inv' by simp
  ...
  from Resolution_weaken[OF this, of "?mun S"] show ?thesis using su by (metis Sup_union_distrib Un_insert_right image_Un insert_is_Un sup.orderE)
qed
