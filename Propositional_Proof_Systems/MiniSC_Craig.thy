...
(\<forall>\<Gamma> \<Delta> \<Gamma>' \<Delta>'. \<Gamma> + \<Gamma>' \<Rightarrow> \<Delta> + \<Delta>' \<longrightarrow> (\<exists>F. \<Gamma> \<Rightarrow> F,\<Delta> \<and> F,\<Gamma>' \<Rightarrow> \<Delta>' \<and> atoms F \<subseteq> atoms_mset (\<Gamma> + \<Delta>) \<and> atoms F \<subseteq> atoms_mset (\<Gamma>' + \<Delta>')))
  \<longleftrightarrow>
(\<forall>\<Gamma> \<Delta> \<Gamma>' \<Delta>'. \<Gamma> \<Rightarrow> \<Delta> \<longrightarrow> (\<exists>F. \<Gamma> \<Rightarrow> {#F#} \<and> {#F#} \<Rightarrow> \<Delta> \<and> atoms F \<subseteq> atoms_mset \<Gamma> \<and> atoms F \<subseteq> atoms_mset \<Delta>))
"
...
next
  ...
  from 2(1)[THEN spec, THEN spec, THEN spec, THEN spec, THEN mp, OF this]
...
proof -
  ...
  have mini: "\<exists> F. \<Gamma> \<Rightarrow> F, \<Delta> \<and> F, \<Gamma>' \<Rightarrow> \<Delta>' \<and> 
    ...
    if "\<Gamma> + \<Gamma>' \<Rightarrow> \<Delta> + \<Delta>'" "is_mini_mset (\<Gamma>+\<Gamma>'+\<Delta>+\<Delta>')" for \<Gamma> \<Gamma>' \<Delta> \<Delta>' n
  ...
  next
    ...
    have ff: "?ss \<bottom>" if "Atom k \<in># \<Gamma>" "Atom k \<in># \<Delta>" (* troelstra uses \<top> in this case. That can't be right. (p. 117, case Ax, right bottom split) *)
      using SCp.BotL SCp.Ax that by simp_all
    ...
    have ss: "?ss \<top>" if "Atom k \<in># \<Gamma>'" "Atom k \<in># \<Delta>'"
      unfolding Top_def using that SCp.ImpR SCp.Ax SCp.BotL by simp
  ...
  next
    ...
    thus ?case proof(elim disjE exE)
      ...
      thus ?thesis unfolding \<Delta> proof(intro exI[where x=Fa] conjI \<open>is_mini_formula Fa\<close>)
        ...
        show "Fa, \<Gamma>' \<Rightarrow> \<Delta>'" using Fam deeper_suc by blast
    ...
    next
      ...
      show ?thesis unfolding \<Delta>' proof(intro exI[where x=Fa] conjI \<open>is_mini_formula Fa\<close>)
         show "\<Gamma> \<Rightarrow> Fa, \<Delta>" using Fam deeper_suc by fast
    ...
    thus ?case proof(elim disjE exE)
      ...
      show ?thesis proof(intro exI[where x="?w"] conjI)  
        ...
        from IH1(2) IH2(1) show "\<Gamma> \<Rightarrow> ?w, \<Delta>" unfolding \<Gamma>
          using deeper_suc by(intro SCp.ImpL inR1'[OF SCp.ImpR] SCp.ImpR) (simp_all add: weakenR' weakenL')
    ...
    next
      ...
      show ?thesis proof(intro exI[of _ ?w] conjI)
        from IH1(1) IH2(1) show "\<Gamma> \<Rightarrow> ?w, \<Delta>"
          by(intro SCp.ImpR SCp.ImpL) (simp_all add: inR1' weakenR' PlainBotL')
  ...
  def tms \<equiv> "image_mset to_mini_formula"
  have [simp]: "tms (A + B) = tms A + tms B" "tms {#F#} = {#to_mini_formula F#}" for A B F unfolding tms_def by simp_all
  have [simp]: "atoms_mset (tms \<Gamma>) = atoms_mset \<Gamma>" for \<Gamma> unfolding tms_def using mini_formula_atoms by simp
  ...
  from hp mf have "tms \<Gamma> \<Rightarrow> tms (F, \<Delta>)" "tms (F, \<Gamma>') \<Rightarrow> tms \<Delta>'" using mini_to_mini unfolding tms_def by simp_all
...
We won't consider that, and it's unclear whether the interpolants we use fulfil that property.\<close>
(*
  Gallier:
   - wouldn't hold without bottom. Try interpolating A \<rightarrow> A and B \<rightarrow> B 
   - is an application of cut elimination? oh well, if you'd have a cut rule here, you'd be in trouble, ok.
*)
