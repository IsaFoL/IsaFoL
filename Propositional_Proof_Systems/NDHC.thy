...
theorem NDHC: "B \<turnstile> F \<Longrightarrow> AX10 \<union> B \<turnstile>\<^sub>H F"
...
next
  ...
  from HC_intros(11) have HC_NotI: "AX10 \<union> \<Gamma> \<turnstile>\<^sub>H A \<^bold>\<rightarrow> \<bottom> \<Longrightarrow> AX10 \<union> \<Gamma> \<turnstile>\<^sub>H \<^bold>\<not> A" for A 
    using MP HC_mono by (meson sup_ge1)
...
next
  ...
  thus "AX10 \<union> \<Gamma> \<turnstile>\<^sub>H F" using AX10.intros(10) by (meson HC.simps UnCI)
next
...
next
  ...
  from \<open>AX10 \<union> (F\<cdot>\<Gamma>) \<turnstile>\<^sub>H H\<close> \<open>AX10 \<union> (G\<cdot>\<Gamma>) \<turnstile>\<^sub>H H\<close> have
  ...
  have "AX10 \<union> \<Gamma> \<turnstile>\<^sub>H  (F \<^bold>\<or> G) \<^bold>\<rightarrow> H" by meson
