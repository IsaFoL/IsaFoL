...
imports LSC_Resolution Resolution SC_Sema CNF_Sema
begin
...
proof -
  ...
  hence "S \<TTurnstile> \<^bold>\<not>(nnf (\<^bold>\<not>F))" using nnf_semantics by simp
  hence "S \<TTurnstile> \<^bold>\<not>(cnf_form_of (nnf (\<^bold>\<not>F)))" using cnf_form_semantics[OF is_nnf_nnf] by simp
  hence "set_mset (image_mset nnf S') \<TTurnstile> \<^bold>\<not>(cnf_form_of (nnf (\<^bold>\<not>F)))" using nnf_semantics S by simp
  hence "set_mset (image_mset (cnf_form_of \<circ> nnf) S') \<TTurnstile> \<^bold>\<not>(cnf_form_of (nnf (\<^bold>\<not>F)))" using cnf_form_semantics[OF is_nnf_nnf] by simp
  ...
  thus ?thesis using cnf_cnf[OF is_nnf_nnf] unfolding set_image_mset image_comp comp_def S by simp
qed
...
thm_deps Resolution_complete_single (* okay, this is so much not the thing I want that it looks just plain wrong. :( *)
