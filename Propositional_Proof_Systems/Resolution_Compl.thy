...
text\<open>The next lemma describes what we have (or can) to do to a CNF after it has been mangled by @{const restrict_cnf_atom} to get back the original CNF.
The idea behind this will be clearer upon usage.\<close>
lemma unrestrict_effects:
  "(\<lambda>c. if make_lit (\<not>v) a \<cdot> c \<in> S then make_lit (\<not>v) a \<cdot> c else c) ` restrict_cnf_atom a v S \<subseteq> S"
proof -
  have "\<lbrakk>xa \<in> restrict_cnf_atom a v S; make_lit (\<not> v) a \<cdot> xa \<notin> S; x = xa\<rbrakk> \<Longrightarrow> xa \<in> S" for x xa
    unfolding restrict_cnf_atom_def using insert_Diff by fastforce
  hence "x \<in> (\<lambda>c. if make_lit (\<not> v) a \<cdot> c \<in> S then make_lit (\<not> v) a \<cdot> c else c) ` restrict_cnf_atom a v S \<Longrightarrow> x \<in> S" for x
...
lemma can_cope_with_unrestrict_effects:
  ...
  shows "\<exists>R \<subseteq> {make_lit v a}. (\<lambda>c. if c \<in> n then make_lit v a \<cdot> c else c) ` T \<turnstile> R"
proof -
  ...
  hence SI: "(\<lambda>c. if c \<in> n then make_lit v a \<cdot> c else c) ` S =
    (insert (make_lit v a) ` {c \<in> S. c \<in> n}) \<union> {c \<in> S. c \<notin> n}
  " by auto
  from pr have "\<exists>R \<subseteq> {make_lit v a}.
  (\<lambda>c. if c \<in> n then make_lit v a \<cdot> c else c) ` S \<turnstile> R" 
    apply(subst SI)
    apply(subst(asm) S)
    apply(elim taint)
  done
...
proof -
  ...
  text\<open>The idea is that the restricted set lost some clauses, and that some others were crippled.
   ...
   @{term C} and @{term "k\<inverse> \<cdot> C"} might be in there and get reduced to one @{term C}.
    You then heal that @{term C} to @{term "k\<inverse> \<cdot> (C::clause)"} and insert the shadowed @{term C}@{text "\<dots>"} Details.)
 \<close>
  obtain n where S:
    "(\<lambda>c. if c \<in> n then make_lit (\<not> v) a \<cdot> c else c) ` restrict_cnf_atom a v S \<subseteq> S"
    using exI[where x="{c. make_lit (\<not>v) a \<cdot> c \<in> S}"] by force
...
proof(induction "atoms_of_cnf S" arbitrary: S rule: finite_psubset_induct)
  ...
  next
    ...
    hence "atoms_of_cnf (restrict_cnf_atom a v S) \<subset> atoms_of_cnf S" for v
      using restrict_cnf_remove by blast
    from psubset(2)[OF this unsat_restrict]
    have IH: "restrict_cnf_atom a v S \<turnstile> \<box>" for v .
    from unrestrict'[OF IH, of "\<not> _"] have unr_IH: "\<exists>R\<subseteq>make_lit v a \<cdot> \<box>. S \<turnstile> R" for v by simp (* unbelievable, how much worry finding that "\<not> _" cost\<dots> otherwise, you get weird sledgehammer results. *)
...
text\<open>What you might actually want is  @{term "\<forall>\<A>. \<not>cnf_semantics \<A> S \<Longrightarrow> S \<turnstile> \<box>"}.
  ...
  Since the notion of a formula with an infinite number of atoms is not exactly standard, it is probably better to just skip this and instead do:
 \<close>

theorem Resolution_complete_standalone_single:
  assumes vld: "\<Turnstile> F"
  shows "cnf (nnf (\<^bold>\<not>F)) \<turnstile> \<box>"
proof -
  have fin: "finite (atoms_of_cnf (cnf (nnf (\<^bold>\<not>F))))"
    using atoms_of_cnf_def cnf_fin[OF is_nnf_nnf] by simp 
  from vld
  have "\<Turnstile> \<^bold>\<not>(\<^bold>\<not>F)" by simp
  hence "\<Turnstile> \<^bold>\<not>(nnf (\<^bold>\<not>F))" using nnf_semantics by simp
  hence "\<forall>\<A>. \<not> \<A> \<Turnstile> nnf (\<^bold>\<not>F)" by simp
  hence "\<forall>\<A>. \<not> cnf_semantics \<A> (cnf (nnf (\<^bold>\<not>F)))"
    using cnf_semantics[OF is_nnf_nnf] by blast
  with Resolution_complete_standalone_finite
