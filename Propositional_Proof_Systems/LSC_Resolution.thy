...
lemma literal_subset_sandwich:
  assumes "is_lit_plus F" "cnf F = {S}" "R \<subseteq> S"
  shows "R = \<box> \<or> R = S"
...
proof(induction rule: LSC.induct)
  ...
  have "?s \<turnstile> {k\<^sup>+}" "?s \<turnstile> {k\<inverse>}" using Resolution.Ass by simp_all
...
next
  (* The idea for the whole trickery with F being a literal (is_disj has to only allows right deep formulas)
    and the sandwitching came to me while looking at Gallier. They do it, too, but in one little sentence. *)
  case (OrL F \<Gamma> G)
  ...
  show ?case proof(cases "trivial_disj (F \<^bold>\<or> G)")
    case True
    hence "(\<Union>(cnf ` set_mset (F \<^bold>\<or> G, \<Gamma>))) = (\<Union>(cnf ` set_mset \<Gamma>))" (* slight redundancy with the next have\<dots> :/ *)
      using trivial_disj by fastforce
    moreover have "(\<Union>(cnf ` set_mset \<Gamma>)) \<turnstile> \<box>" if "(\<Union>(cnf ` set_mset (F, \<Gamma>))) \<turnstile> \<box>" "trivial_disj F" for F
      using trivial_disj that by simp
    ultimately show ?thesis using True IH unfolding trivial_disj.simps by presburger
  next
    case False
    hence nt: "\<not> trivial_disj F" "\<not> trivial_disj G" by simp_all
    with db  nontrivial_disj obtain S T where ST: "cnf F = {S}" "cnf G = {T}" by presburger (* hint: card S \<le> 1 *)
    hence R: "cnf (F \<^bold>\<or> G) = { S \<union> T }" by simp
    let ?\<Gamma> = "(\<Union>(cnf ` set_mset \<Gamma>))"
    have "\<lbrakk>S\<cdot>?\<Gamma> \<turnstile> \<box>; T\<cdot>?\<Gamma> \<turnstile> \<box>\<rbrakk> \<Longrightarrow> S \<union> T\<cdot> ?\<Gamma> \<turnstile> \<box>" proof -
      assume s: "S\<cdot>?\<Gamma> \<turnstile> \<box>" and t: "T\<cdot>?\<Gamma> \<turnstile> \<box>"
      hence s_w: "S \<cdot> S \<union> T \<cdot> \<Union>(cnf ` set_mset \<Gamma>) \<turnstile> \<box>" using Resolution_weaken by (metis insert_commute insert_is_Un)
      note Resolution_taint_assumptions[of "{T}" ?\<Gamma> "\<box>" S] t
      then obtain R where R: "S \<union> T \<cdot> \<Union>(cnf ` set_mset \<Gamma>) \<turnstile> R" "R\<subseteq>S" by (auto simp: Un_commute)
      ...
      next
        ...
        have "(R \<cdot> S \<union> T \<cdot> \<Union>(cnf ` set_mset \<Gamma>) \<turnstile> \<box>) = (S \<union> T \<cdot> \<Union>(cnf ` set_mset \<Gamma>) \<turnstile> \<box>)"  .
        moreover assume "R = S" 
...
proof -
  ...
  moreover have "\<forall>\<gamma> \<in> set_mset (image_mset (cnf_form_of \<circ> nnf) \<Gamma>). is_cnf \<gamma>" 
    using cnf_form_of_is is_nnf_nnf by simp
...
proof -
  ...
  thus ?thesis using is_nnf_nnf_id is_nnf_nnf by auto
qed
...
lemma Resolution_LSC_pre:
  ...
  shows "\<exists>S' R'. \<forall>\<Gamma>. is_nnf_mset \<Gamma> \<longrightarrow> set R' = R \<and> set (map set S') = S \<and> 
...
using assms proof(induction S R rule: Resolution.induct)
  ...
  def Sm \<equiv> "S - {F}"
  hence Sm: "S = F\<cdot>Sm" "F \<notin> Sm" using Ass by fast+ (* try doing this with obtain\<dots> *)
...
next
  have pnf: "is_nnf_mset (image_mset disj_of_clause (mset S))" for S by simp
  case (R S F G k)
  ...
  from R.IH(1)[OF fin(1) R.prems(2,3)] obtain FR FS where IHF:
    ...
    "\<And>\<Gamma> GS. is_nnf_mset \<Gamma> \<Longrightarrow> (disj_of_clause FR, image_mset disj_of_clause (mset (FS@GS)) + \<Gamma> \<Rightarrow>\<^sub>n 
      \<Longrightarrow> image_mset disj_of_clause (mset (FS@GS)) + \<Gamma> \<Rightarrow>\<^sub>n)"
      by simp (metis (mono_tags, lifting) UnE add.assoc pnf set_mset_union)
  from R.IH(2)[OF fin(2) R.prems(2,3)] obtain GR GS where IHG:
    "set GR = G" "set (map set GS) = S"
    "\<And>\<Gamma> HS. is_nnf_mset \<Gamma> \<Longrightarrow> (disj_of_clause GR, image_mset disj_of_clause (mset (GS@HS)) + \<Gamma> \<Rightarrow>\<^sub>n 
  ...
  have IH: "image_mset disj_of_clause (mset (FS @ GS)) + \<Gamma> \<Rightarrow>\<^sub>n"
    ...
    and "is_nnf_mset \<Gamma>"
    for \<Gamma> using IHF(3)[of \<Gamma> GS] IHG(3)[of "disj_of_clause FR, \<Gamma>" FS] that 
...
lemma Resolution_LSC_pre_nodisj: (* TODO: we should probably show this directly with Resolution_LSC_pre. But for now, this is fine. *)
  assumes "S \<turnstile> R"
  assumes "finite R"
  assumes "finite S" "Ball S finite"
...
proof -
  ...
  from Resolution_LSC_pre[OF assms]
