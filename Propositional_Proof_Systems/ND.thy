...
inductive ND :: "form set \<Rightarrow> form \<Rightarrow> bool" (infix "\<turnstile>" 30) where
...
NotI: "F\<cdot>\<Gamma> \<turnstile> \<bottom> \<Longrightarrow> \<Gamma> \<turnstile> Not F" |
CC: "Not F\<cdot> \<Gamma> \<turnstile> \<bottom> \<Longrightarrow> \<Gamma> \<turnstile> F" |
...
OrE: "\<lbrakk> \<Gamma> \<turnstile> Or F G; F\<cdot>\<Gamma> \<turnstile> H; G\<cdot>\<Gamma> \<turnstile> H \<rbrakk> \<Longrightarrow> \<Gamma> \<turnstile> H" |
ImpI: "F\<cdot>\<Gamma> \<turnstile> G \<Longrightarrow> \<Gamma> \<turnstile> Imp F G" |
...
next
  case AndE1 thus ?case using ND.AndE1 by meson
next
  case AndE2 thus ?case using ND.AndE2 by meson
next
  case AndI thus ?case using ND.AndI by simp
next
...
next
  case ImpE thus ?case using ND.ImpE by meson
qed

lemma BotE: "\<Gamma> \<turnstile> \<bottom> \<Longrightarrow> \<Gamma> \<turnstile> F"
  by (meson CC subset_insertI Weaken)

lemma Not2E: "Not(Not F)\<cdot> \<Gamma> \<turnstile> F"
  by (metis CC ND.Ax NotE insertI1 insert_commute)

lemma Not2I: "F\<cdot>\<Gamma> \<turnstile> Not(Not F)"
  by (metis CC ND.Ax NotE insertI1 insert_commute)

lemma Not2IE: "F\<cdot>\<Gamma> \<turnstile> G \<Longrightarrow> Not (Not F)\<cdot> \<Gamma> \<turnstile> G"
  by (metis (full_types) CC Not2I NotE NotI insert_commute subset_insertI Weaken)

lemma NDtrans: "\<Gamma> \<turnstile> F \<Longrightarrow> F\<cdot> \<Gamma> \<turnstile> G \<Longrightarrow> \<Gamma> \<turnstile> G"
  using ImpE ImpI by blast

lemma AndL_sim: "F\<cdot> G\<cdot> \<Gamma> \<turnstile> H \<Longrightarrow> And F G\<cdot> \<Gamma> \<turnstile> H"
  apply(drule Weaken[where \<Gamma>' = "And F G\<cdot> F\<cdot> G\<cdot> \<Gamma>"])
   apply blast
by (metis AndE1 AndE2 ND.Ax NDtrans insertI1 insert_commute)

lemma NotSwap: "Not F\<cdot> \<Gamma> \<turnstile> G \<Longrightarrow> Not G\<cdot> \<Gamma> \<turnstile> F"
  using CC Not2I NotE insert_commute subset_insertI Weaken by (metis (full_types))

lemma AndR_sim: "\<lbrakk> Not F\<cdot> \<Gamma> \<turnstile> H; Not G\<cdot> \<Gamma> \<turnstile> H \<rbrakk> \<Longrightarrow> Not(And F G)\<cdot> \<Gamma> \<turnstile> H"
  using AndI NotSwap by blast

lemma OrL_sim: "\<lbrakk> F\<cdot> \<Gamma> \<turnstile> H; G\<cdot>\<Gamma> \<turnstile> H \<rbrakk> \<Longrightarrow> F \<^bold>\<or> G\<cdot> \<Gamma> \<turnstile> H"
  using Weaken[where \<Gamma>' = "F\<cdot> Or F G\<cdot> \<Gamma>"] Weaken[where \<Gamma>' = "G\<cdot> Or F G\<cdot> \<Gamma>"]
  by (meson ND.Ax OrE insertI1 insert_mono subset_insertI)

lemma OrR_sim: "\<lbrakk> \<^bold>\<not> F\<cdot> \<^bold>\<not> G\<cdot> \<Gamma> \<turnstile> \<bottom>\<rbrakk> \<Longrightarrow> \<^bold>\<not> (G \<^bold>\<or> F)\<cdot> \<Gamma> \<turnstile> \<bottom>"
proof -
  assume "\<^bold>\<not> F \<cdot> \<^bold>\<not> G \<cdot> \<Gamma> \<turnstile> \<bottom>"
  then have "\<And>f. f \<cdot> \<^bold>\<not> F \<cdot> \<^bold>\<not> G \<cdot> \<Gamma> \<turnstile> \<bottom>" by (meson Weaken subset_insertI)
  then have "\<And>f. \<^bold>\<not> G \<cdot> \<^bold>\<not> (f \<^bold>\<or> F) \<cdot> \<Gamma> \<turnstile> \<bottom>" by (metis NDtrans Not2E NotSwap OrI2 insert_commute)
  then show ?thesis by (meson NDtrans Not2I NotSwap OrI1)
qed

lemma ImpL_sim: "\<lbrakk> \<^bold>\<not> F\<cdot> \<Gamma> \<turnstile> \<bottom>; G\<cdot> \<Gamma> \<turnstile> \<bottom>\<rbrakk> \<Longrightarrow> F \<^bold>\<rightarrow> G\<cdot> \<Gamma> \<turnstile> \<bottom>"
  by (meson CC ImpE ImpI ND.Ax Weaken insertI1 subset_insertI)

lemma ImpR_sim: "\<lbrakk> \<^bold>\<not> G\<cdot> F\<cdot> \<Gamma> \<turnstile> \<bottom>\<rbrakk> \<Longrightarrow> \<^bold>\<not> (F \<^bold>\<rightarrow> G)\<cdot> \<Gamma> \<turnstile> \<bottom>"
  by (metis ImpI NotSwap insert_commute)
...
lemma ND_caseDistinction: "\<lbrakk> F\<cdot>\<Gamma> \<turnstile> H; Not F\<cdot>\<Gamma> \<turnstile> H \<rbrakk> \<Longrightarrow> \<Gamma> \<turnstile> H"
  by (meson ND_lem OrE empty_subsetI Weaken)

lemma "\<lbrakk>\<^bold>\<not> F\<cdot> \<Gamma> \<turnstile> H; G\<cdot> \<Gamma> \<turnstile> H\<rbrakk> \<Longrightarrow> F \<^bold>\<rightarrow> G\<cdot> \<Gamma> \<turnstile> H"
...
lemma sim_sim: "F\<cdot> \<Gamma> \<turnstile> H \<Longrightarrow> G\<cdot>\<Gamma> \<turnstile> F \<Longrightarrow> G\<cdot> \<Gamma> \<turnstile> H"
...
lemma NotBot_provable[simp,intro!]: "\<Gamma> \<turnstile> \<^bold>\<not> \<bottom>" using NotI BotE Ax by simp
 ...
 proof(induction G arbitrary: F)
  ...
  next
    case (Cons G GS) show ?case proof
      ...
      hence "{G,\<^bold>\<And>GS} \<turnstile> F" proof -
        have *: "{\<^bold>\<And>GS} \<turnstile> G \<^bold>\<rightarrow> F" 
          using Weaken[OF * empty_subsetI] ImpE[where \<Gamma>="{\<^bold>\<And> GS}" and F="\<^bold>\<And> GS"] ND.Ax by simp
        show "{G,\<^bold>\<And>GS} \<turnstile> F" using Weaken[OF *] ImpE[where \<Gamma>="{G,\<^bold>\<And> GS}" and F="G"] ND.Ax by simp
      qed
    ...
    next
      ...
      hence "{G \<^bold>\<and> \<^bold>\<And>GS} \<turnstile> F" using ImpE[OF _ Ax[OF singletonI]] Weaken  by simp      
      ...
      thus "set (G # GS) \<turnstile> F" using ImpE Weaken by (meson Ax list.set_intros(1) set_subset_Cons)
  qed
qed
