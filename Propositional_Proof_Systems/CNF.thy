...
theory CNF
imports Formulas "~~/src/HOL/Library/Multiset"  "~~/src/HOL/Library/Simps_Case_Conv"
begin

context begin

(* attempts to eliminate \<bottom> from formulas while converting to nnf have been made.
But that just gives you additional cases *) 
...
lemma "nnf ((Atom k) \<^bold>\<rightarrow> (Not ((Atom l) \<^bold>\<or> (Not (Atom m))))) = \<^bold>\<not> (Atom k) \<^bold>\<or> (\<^bold>\<not> (Atom l) \<^bold>\<and> Atom m)" by code_simp
...
lemma nnf_no_imp: "A \<^bold>\<rightarrow> B \<notin> subformulae (nnf F)"
  by(induction F rule: nnf.induct; simp)
lemma subformulae_nnf: "is_nnf F \<Longrightarrow> G \<in> subformulae F \<Longrightarrow> is_nnf G" (* fun fact *)
apply(induction F)
  subgoal by simp
  subgoal by simp
  subgoal Not for F by(cases F) auto
by(clarsimp; elim disjE conjE; simp; fail)+

datatype literal = Pos nat ("(_\<^sup>+)" [1000] 999) | Neg nat ("(_\<inverse>)" [1000] 999)

type_synonym clause = "literal set"
abbreviation empty_clause ("\<box>") where "\<box> \<equiv> {} :: clause"
...
"cnf (Or F G) = {f \<union> g| f g. f \<in> (cnf F) \<and> g \<in> (cnf G)}"
...
fun trivial_disj where
"trivial_disj (Not \<bottom>) = True" |
"trivial_disj (Or F G) = (trivial_disj F \<or> trivial_disj G)" |
"trivial_disj _ = False"

lemma mset_empty_Uns: "\<Union>#{#{#}. s \<in># S#} = {#}"
  by (induction S; simp)

lemma trivial_disj: "trivial_disj F \<Longrightarrow> cnf F = {}"
  by(induction rule: trivial_disj.induct; (simp;fail)?) (auto simp add: mset_empty_Uns)

lemma nontrivial_disj: "\<not>trivial_disj F \<Longrightarrow> is_disj F \<Longrightarrow> \<exists>S. cnf F = {S}"
proof(induction F)
  case Bot thus ?case by auto
next
  case And hence False by simp thus ?case ..
next
  case (Or F1 F2) 
  hence d: "is_disj F1 \<and> is_disj F2" by (cases F1; simp) 
  show ?case proof(cases "trivial_disj F1"; cases "trivial_disj F2")
    assume "\<not> trivial_disj F1" "\<not> trivial_disj F2"
    from Or.IH(1)[OF this(1)] Or.IH(2)[OF this(2)] d
    show ?case by clarsimp
  qed (insert \<open>\<not> trivial_disj (F1 \<^bold>\<or> F2)\<close>, simp_all)
next
  case Imp hence False by simp thus ?case ..
next
  case Atom thus ?case by auto
next
  case (Not F) thus ?case by(cases F; simp) 
qed
  

fun cnf_lists :: "form \<Rightarrow> literal list list" where
...
private lemma BigOr_is_not_cnf': "(\<forall>l \<in> set ls. is_lit_plus l) \<Longrightarrow> is_cnf (\<^bold>\<Or>ls)" 
  by(induction ls; simp) (metis BigOr.simps(1, 2) form.distinct(25) is_cnf.elims(2) is_cnf.simps(3) list.exhaust)
  (* ugh *)
...
lemma disj_is_nnf: "is_disj F \<Longrightarrow> is_nnf F"
  by(induction F rule: is_disj.induct; simp add: is_lit_plus_cases split: form.splits)
