...
lemma Deduction1: "F \<cdot> (AX1 \<union> \<Gamma> ) \<turnstile>\<^sub>H \<bottom> \<longleftrightarrow> (AX1 \<union> \<Gamma>) \<turnstile>\<^sub>H (F \<^bold>\<rightarrow> \<bottom>)" unfolding AX1_away by (metis Deduction_theorem HC.simps HC_mono Un_commute Un_insert_left insertI1 subset_insertI)
lemma Deduction2: "(F \<^bold>\<rightarrow> \<bottom>) \<cdot> (AX1 \<union> \<Gamma>) \<turnstile>\<^sub>H \<bottom> \<longleftrightarrow> (AX1 \<union> \<Gamma>) \<turnstile>\<^sub>H F" (is "?l \<longleftrightarrow> ?r")
proof
  assume l: ?l
  ...
  moreover have "AX1 \<union> \<Gamma> \<turnstile>\<^sub>H ((F \<^bold>\<rightarrow> \<bottom>) \<^bold>\<rightarrow> \<bottom>) \<^bold>\<rightarrow> F" using AX1.intros(2) HC.Ax by simp
...
proof(induction "\<Gamma>" "\<Delta>" rule: SCp.induct)
  ...
  hence IH1: "F \<^bold>\<rightarrow> \<bottom> \<cdot> set_mset \<Gamma> \<union> (\<lambda>F. F \<^bold>\<rightarrow> \<bottom>) ` set_mset \<Delta> \<union> AX1 \<turnstile>\<^sub>H \<bottom>" by simp
  ...
  hence IH2: "G \<cdot> (set_mset \<Gamma> \<union> (\<lambda>F. F \<^bold>\<rightarrow> \<bottom>) ` set_mset \<Delta> \<union> AX1) \<turnstile>\<^sub>H \<bottom>" by simp
  have R: "F \<^bold>\<rightarrow> G \<cdot> AX1 \<union> \<Gamma> \<turnstile>\<^sub>H \<bottom>" if "G \<cdot> AX1 \<union> \<Gamma> \<turnstile>\<^sub>H \<bottom>" "F \<^bold>\<rightarrow> \<bottom> \<cdot> AX1 \<union> \<Gamma> \<turnstile>\<^sub>H \<bottom>" for \<Gamma> 
  ...
  have "F \<^bold>\<rightarrow> G \<cdot> (set_mset \<Gamma> \<union> (\<lambda>F. F \<^bold>\<rightarrow> \<bottom>) ` set_mset \<Delta> \<union> AX1) \<turnstile>\<^sub>H \<bottom>" using IH2 IH1 by(simp add: Un_commute)
  thus ?case by simp
next
  case (ImpR F \<Gamma> G \<Delta>)
  hence "is_mini_mset (F, \<Gamma>)" "is_mini_mset (G, \<Delta>)" by simp_all
  with ImpR.IH have IH: "F \<cdot> G \<^bold>\<rightarrow> \<bottom> \<cdot> set_mset \<Gamma> \<union> (\<lambda>F. F \<^bold>\<rightarrow> \<bottom>) ` set_mset \<Delta> \<union> AX1 \<turnstile>\<^sub>H \<bottom>" by(simp add: insert_commute)
  have R: "(F \<^bold>\<rightarrow> G) \<^bold>\<rightarrow> \<bottom> \<cdot> \<Gamma> \<union> AX1 \<turnstile>\<^sub>H \<bottom>" if "F \<cdot> G \<^bold>\<rightarrow> \<bottom> \<cdot> \<Gamma> \<union> AX1 \<turnstile>\<^sub>H \<bottom>" for \<Gamma> using that
    by (metis AX1_away Deduction2 Deduction_theorem Un_commute Un_insert_right)
  thus ?case using IH by simp
next
  case (Ax k \<Gamma> \<Delta>)
  have R: "F \<cdot> F \<^bold>\<rightarrow> \<bottom> \<cdot> \<Gamma> \<union> AX1 \<turnstile>\<^sub>H \<bottom>" for F \<Gamma> by (meson HC.simps insert_iff)
  from R[where F="Atom k" and \<Gamma>="set_mset \<Gamma> \<union> (\<lambda>F. F \<^bold>\<rightarrow> \<bottom>) ` set_mset \<Delta>"] show ?case using Ax.hyps
    by (simp add: insert_absorb singletonI)
next
  case BotL thus ?case using HC.Ax by simp
qed simp_all
