...
primrec extender :: "nat \<Rightarrow> form set \<Rightarrow> form set" where
"extender 0 S = S" |
"extender (Suc n) S = (
  let r = extender n S; 
...
lemma not_fin_sat_extended_UNIV: assumes "\<not>fin_sat S" shows "extended S = UNIV" 
text\<open>Note that this crucially depends on the fact that we check \emph{first} whether adding @{term "\<^bold>\<not>F"} makes the set not satisfiable, 
  and add @{term F} otherwise \emph{without any further checks}.\<close>
proof -
  from assms[unfolded fin_sat_def, simplified] obtain s where "finite s" "\<not> sat s" by clarify
...
lemma extended_tran: assumes su: "S \<subseteq> T" shows "extended S \<subseteq> extended T"
oops
...
non-tautological formula in the extension S. Showing that this is possible is not worth the effort, since we can't influence the ordering of formulae.\<close>
...
proof(goal_cases)
  ...
  thus False using 1 extended_fin_sat fin_sat_def by(auto intro: 1(2))
qed
...
proof -
  ...
  next
    ...
    have False if "\<bottom> \<in> extended S" proof -
      ...
      ultimately have "\<exists>A. A \<Turnstile> \<bottom>" using fs unfolding fin_sat_def sat_def by blast
      thus False by simp
    qed
    thus ?case by auto
  next
...
proof goal_cases
  ...
  ultimately show ?case by(intro 1)  (blast, simp add: entailment_def)
qed
