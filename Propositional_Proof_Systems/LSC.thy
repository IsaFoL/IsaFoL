...
text\<open>We can transform a proof of a sequent into a proof of the same sequent in NNF.
However, doing so does not help much, because we then need to then transform that proof of a sequent in NNF to a proof in left-handed SC, and that's more tricky than doing it the following way:\<close>
...
text\<open>One downside of the whole approach is that we nearly always need everything to be in NNF. To shorten:\<close>
...
proof(induction \<Gamma> "{#}::form multiset " rule: SCp.induct)
  case (BotL \<Gamma>)
...
proof(induction rule: LSC.induct)
  show "\<^bold>\<not> (Atom k), Atom k, \<Gamma> \<Rightarrow> {#}" for k \<Gamma> by(auto intro!: SCp.NotL intro: extended_Ax)
  show "\<bottom>, \<Gamma> \<Rightarrow> {#}" for \<Gamma> by auto
next
  case (OrL F \<Gamma> G)
  hence IH: "F, \<Gamma> \<Rightarrow> {#}" "G, \<Gamma> \<Rightarrow> {#}" by -
  thus ?case using SCp.OrL by simp
next
  case (AndL F G \<Gamma>)
  hence IH: "F, G, \<Gamma> \<Rightarrow> {#}" by -
  thus ?case using SCp.AndL by simp
next
  case (NotImpNNF F G \<Gamma>) thus ?case by(auto intro: NotL_inv' intro!: SCp.intros(3-))
qed (auto dest!: NotL_inv' intro: SCp.intros(3-))
...
next
  ...
  with LSC.AndL have "nnf F \<^bold>\<and> nnf G, image_mset nnf \<Gamma> \<Rightarrow>\<^sub>n" by simp
  thus ?case by simp
next  
...
proof -
  have GO(* get out *)[simp]:   
    ...
    for F G S by (simp add: add_mset_commute)
  have ?r1 if ?l1 proof -
    ...
    show ?r1 using f gh LSC_weaken add_mset_commute by(intro LSC.intros; simp)+
  qed
  moreover have ?r2 if ?l2 proof -
    ...
    thus ?r2 using LSC_weaken add_mset_commute by(intro LSC.intros; simp)+
  qed
...
proof -
  ...
  show ?thesis proof(rule LHCut[where F="Atom k"])
    have 2: "T, Atom k, Atom k \<^bold>\<or> S, \<Gamma> \<Rightarrow>\<^sub>n" using r using LSC_weaken r(1) by (metis add_mset_commute)
  ...
  next text\<open>analogously\<close>(*<*)
    have 2: "S, \<^bold>\<not>(Atom k), \<^bold>\<not>(Atom k) \<^bold>\<or> T, \<Gamma> \<Rightarrow>\<^sub>n" using LSC_weaken r(2) by (metis add_mset_commute)
...
lemma distrib1: "\<lbrakk>disj_of_clause a, \<Gamma> \<Rightarrow>\<^sub>n; image_mset disj_of_clause (mset G) + \<Gamma> \<Rightarrow>\<^sub>n\<rbrakk>
    \<Longrightarrow> mset (map (\<lambda>d. disj_of_clause a \<^bold>\<or> disj_of_clause d) G) + \<Gamma> \<Rightarrow>\<^sub>n"
...
proof(induction G arbitrary: \<Gamma>)
  ...
  case (Cons g G)
  from \<open>disj_of_clause a, \<Gamma> \<Rightarrow>\<^sub>n\<close> have 1: "disj_of_clause a, disj_of_clause g, \<Gamma> \<Rightarrow>\<^sub>n" by (metis LSC_weaken add_mset_commute)
  ...
  from Cons.IH[OF 1 2] have IH: "disj_of_clause g, mset (map (\<lambda>d. disj_of_clause a \<^bold>\<or> disj_of_clause d) G) + \<Gamma> \<Rightarrow>\<^sub>n" 
    by(simp add: add_mset_commute)
  from \<open>disj_of_clause a, \<Gamma> \<Rightarrow>\<^sub>n\<close> have 3: "disj_of_clause a, mset (map (\<lambda>d. disj_of_clause a \<^bold>\<or> disj_of_clause d) G) + \<Gamma> \<Rightarrow>\<^sub>n"
...
proof(induction rule: LSC.induct)
  case Ax thus ?case by (metis LSC.Ax add.commute diff_union_swap form.distinct(1) form.distinct(3) form.inject(2))
next
 case BotL thus ?case by (metis LSC.BotL add.commute diff_union_swap form.distinct(11))
next
  case (AndL F G \<Gamma>)
  ...
  thus ?case by (metis add.commute diff_union_swap form.distinct(19))
next
  case (OrL F \<Gamma> G)
  ...
  thus ?case by (metis diff_union_swap form.distinct(21))
qed auto
...
(* you know, I just had a funky idea: we could do the same thing with dnf and right handed SC. 
completeness should be imminent\<dots>? but\<dots> NOPE! Hm. Why shouldn't that work with CNFs and LH-LSC? *)
lemma LSC_cnf: "\<Gamma> \<Rightarrow>\<^sub>n \<Longrightarrow> is_nnf_mset \<Gamma> \<Longrightarrow> image_mset cnf_form_of \<Gamma> \<Rightarrow>\<^sub>n" (* marvel at the fact that I do not require \<Gamma> to be in nnf *)
...
next
  ...
  have 1: "\<lbrakk>\<^bold>\<And> (map disj_of_clause (cnf_lists F)), \<Gamma> \<Rightarrow>\<^sub>n; \<^bold>\<And> (map disj_of_clause (cnf_lists G)), \<Gamma> \<Rightarrow>\<^sub>n\<rbrakk>
    ...
    (* the actual disjunction is happening here *)
      for \<Gamma> using distrib 2 by(auto dest!: LSC_BigAndL_inv intro!: LSC_BigAndL simp: mset_map)
  from OrL show ?case
