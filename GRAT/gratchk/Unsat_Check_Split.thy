...
(*
  Test for split mode:
    ...
    Separate between proof iterator and lemma iterator.
    Lemma iterator formally refined to array.
    Proof iterator left abstract through whole refinement process.
      Equipped with fuel to ensure termination.

      * Do we need the item structure?
  ...
  (candidates,cand,prf) \<leftarrow> EWHILET
    ...
    (\<lambda>(candidates,cand,prf). doE {
      if cand \<in> candidates then doE {
        ...
        prf \<leftarrow> parse_skip_listZ prf; (* Skip over unit propagation *)
        (_,prf) \<leftarrow> parse_prf prf; (* Skip over conflict clause *)
        (cand,prf) \<leftarrow> parse_idZ prf;
        ERETURN (candidates,cand,prf)
      }
    }) (candidates,cand,prf);
...
(* id id* 0 id, advance iterator by one clause *)
definition check_rup_proof :: "state \<Rightarrow> 'it \<Rightarrow> (nat\<times>'prf) \<Rightarrow> (_, state \<times> 'it \<times> (nat\<times>'prf)) enres" where
  "check_rup_proof \<equiv> \<lambda>(last_id,CM,A\<^sub>0) it prf. doE {
    (i,prf) \<leftarrow> parse_id prf;
    CHECK (i>last_id) (mkp_errNprf STR ''Ids must be strictly increasing'' i prf);
...
lemma check_rup_proof_correct[THEN ESPEC_trans, refine_vcg]: 
  ...
  assumes "it_invar it"
  shows
  "check_rup_proof s it prf \<le> ESPEC (\<lambda>_. True) (\<lambda>((last_id',CM',A'),it',prf'). 
      cm_invar CM'
...
definition check_rat_proof :: "state \<Rightarrow> 'it \<Rightarrow> (nat \<times> 'prf) \<Rightarrow> (_, state \<times> 'it \<times> (nat \<times> 'prf)) enres" where
  "check_rat_proof \<equiv> \<lambda>(last_id,CM,A\<^sub>0) it prf. doE {
...
lemma check_rat_proof_correct[THEN ESPEC_trans, refine_vcg]: 
  ...
  assumes "it_invar it"
  shows
  "check_rat_proof s it prf \<le> ESPEC (\<lambda>_. True) (\<lambda>((last_id',CM',A'),it',prf'). 
      cm_invar CM'
    \<and> (\<forall>i\<in>cm_ids CM'. i\<le>last_id')
    \<and> (sat' (cm_F CM) A \<longrightarrow> sat' (cm_F CM') A')
    \<and> it_invar it' \<and> (prf',prf)\<in>prfWF\<^sup>+
...
definition check_item :: "state \<Rightarrow> 'it \<Rightarrow> (nat \<times> 'prf) \<Rightarrow> (_, (state \<times> 'it \<times> (nat \<times> 'prf)) option) enres"
  where "check_item \<equiv> \<lambda>(last_id,CM,A) it prf. doE {
    ...
    | RUP_LEMMA \<Rightarrow> doE {
        s \<leftarrow> check_rup_proof (last_id,CM,A) it prf;
        ERETURN (Some s)
      }
    | RAT_LEMMA \<Rightarrow> doE {
        s \<leftarrow> check_rat_proof (last_id,CM,A) it prf;
        ERETURN (Some s)
      }
...
lemma check_item_correct_pre: 
  ...
  shows "check_item s it prf \<le> ESPEC (\<lambda>_. True) (\<lambda>
      Some ((last_id',CM',A'),it',prf') \<Rightarrow> 
          cm_invar CM' 
        \<and> (\<forall>i\<in>cm_ids CM'. i\<le>last_id') 
        \<and> (sat' (cm_F CM) A \<longrightarrow> sat' (cm_F CM') A')
        \<and> it_invar it' \<and> (prf',prf)\<in>prfWF\<^sup>+
...
lemma check_item_correct[THEN ESPEC_trans, refine_vcg]: 
  ...
  assumes "it_invar it"  
  shows "check_item s it prf \<le> ESPEC (\<lambda>_. True) (case s of (last_id,CM,A) \<Rightarrow> (\<lambda>
      Some ((last_id',CM',A'),it',prf') \<Rightarrow> 
          cm_invar CM' 
        \<and> (\<forall>i\<in>cm_ids CM'. i\<le>last_id') 
        \<and> (sat' (cm_F CM) A \<longrightarrow> sat' (cm_F CM') A')
  ...
  using check_item_correct_pre[of s _ _ _ it "prf"] assms
...
definition read_cnf_new 
  :: "'it \<Rightarrow> 'it \<Rightarrow> clausemap \<Rightarrow> (_, clausemap \<times> nat) enres"    
...
definition "init_rat_counts prf \<equiv> doE {
  ...
  (CM,_,prf) \<leftarrow> EWHILET (\<lambda>(CM,l,prf). l\<noteq>None) (\<lambda>(CM,l,prf). doE {
    ...
    let l = the l;
    (_,prf) \<leftarrow> parse_prf prf;  (* Just ignoring count, silently assuming it to be >0. TODO: Add count-down and stop optimization? *)
...
definition "verify_unsat F_begin F_end it prf \<equiv> doE {
  ...
  CHECK (is_None s) (mkp_err STR ''Proof did not contain conflict declaration'')
}"
...
lemma verify_unsat_correct: 
  ...
  apply (refine_vcg 
    EWHILEIT_expinv_rule[where 
          ...
          | (Some ((last_id,CM,A), it', prf')) \<Rightarrow> it_invar it'
                \<and> cm_invar CM 
                \<and> (\<forall>i\<in>cm_ids CM. i\<le>last_id) 
...
definition check_rup_proof_bt :: "state \<Rightarrow> 'it \<Rightarrow> (nat\<times>'prf) \<Rightarrow> (_, state \<times> 'it \<times> (nat\<times>'prf)) enres" where
  "check_rup_proof_bt \<equiv> \<lambda>(last_id,CM,A) it prf. doE {
    (i,prf) \<leftarrow> parse_id prf;
    CHECK (i>last_id) (mkp_errNprf STR ''Ids must be strictly increasing'' i prf);
...
definition check_rat_proof_bt :: "state \<Rightarrow> 'it \<Rightarrow> (nat \<times> 'prf) \<Rightarrow> (_,state \<times> 'it \<times> (nat \<times> 'prf)) enres" where
  "check_rat_proof_bt \<equiv> \<lambda>(last_id,CM,A) it prf. doE {
    (reslit,prf) \<leftarrow> parse_prf_literal prf;
...
lemma check_rat_proof_bt_refine[refine]:
  ...
  apply refine_rcg
  apply refine_dref_type
  apply (auto simp: bt_in_bta_rel dest!: bt_rel_simps) (* Takes long *)
  done
...
definition check_item_bt :: "state \<Rightarrow> 'it \<Rightarrow> (nat \<times> 'prf) \<Rightarrow> (_, (state \<times> 'it \<times> (nat \<times> 'prf)) option) enres"
  where "check_item_bt \<equiv> \<lambda>(last_id,CM,A) it prf. doE {
    (ty,prf) \<leftarrow> parse_type prf;
    case ty of
      INVALID \<Rightarrow> THROW (mkp_err STR ''Invalid item'')
    | UNIT_PROP \<Rightarrow> doE {
    ...
    | RUP_LEMMA \<Rightarrow> doE {
        s \<leftarrow> check_rup_proof_bt (last_id,CM,A) it prf;
        ERETURN (Some s)
      }
    | RAT_LEMMA \<Rightarrow> doE {
        s \<leftarrow> check_rat_proof_bt (last_id,CM,A) it prf;
        ERETURN (Some s)
      }
    | CONFLICT \<Rightarrow> doE {
        (i,prf) \<leftarrow> parse_id prf;
...
context unsat_input begin
  ...
  definition check_rup_proof1 
    ...
    where
    "check_rup_proof1 \<equiv> \<lambda>(last_id,CM,A) it prf. doE {
      (i,prf) \<leftarrow> parse_id prf;
      CHECK (i>last_id) (mkp_errNprf STR ''Ids must be strictly increasing'' i prf);
  ...
  proof -
    ...
    show ?thesis
      ...
      apply refine_dref_type
      apply vc_solve
      subgoal by (intro refine_dref_RELATES) (* TODO: refine_dref_type should always try to discharge RELATES-goals, 
                                                even if they do not contain schematic vars *)
  ...
  definition check_rat_proof1 
    :: "('it) state1 \<Rightarrow> 'it \<Rightarrow> (nat \<times> 'prf) \<Rightarrow> (_,('it) state1 \<times> 'it \<times> (nat \<times> 'prf)) enres" 
    where
    "check_rat_proof1 \<equiv> \<lambda>(last_id,CM,A) it prf. doE {
      (reslit,prf) \<leftarrow> parse_prf_literal prf;
      CHECK (sem_lit' reslit A \<noteq> Some False) 
            (mkp_errprf STR ''Resolution literal is false'' prf);
      (i,prf) \<leftarrow> parse_id prf;
  ...
  proof -
    ...
    show ?thesis
      ...
      apply (vc_solve solve: asm_rl RELATESI) (* Takes its time ... *)
      done
  qed
  ...
  lemma remove_ids1_refine[refine]: 
    ...
    unfolding remove_ids1_def remove_ids_def EWHILET_def (* TODO: Register EWHILET \<le> EWHILEIT refine rule *)
  ...
  definition check_item1 
    ...
    where "check_item1 \<equiv> \<lambda>(last_id,CM,A) it prf. doE {
      (ty,prf) \<leftarrow> parse_type prf;
      case ty of
        INVALID \<Rightarrow> THROW (mkp_err STR ''Invalid item'')
      | UNIT_PROP \<Rightarrow> doE {
      ...
      | RUP_LEMMA \<Rightarrow> doE {
          s \<leftarrow> check_rup_proof1 (last_id,CM,A) it prf;
          ERETURN (Some s)
        }
      | RAT_LEMMA \<Rightarrow> doE {
          s \<leftarrow> check_rat_proof1 (last_id,CM,A) it prf;
          ERETURN (Some s)
        }
      | CONFLICT \<Rightarrow> doE {
          (i,prf) \<leftarrow> parse_id prf;
  ...
  lemma check_item1_deforest: "check_item1 = (\<lambda>(last_id,CM,A) it prf. doE {
  ...
  definition read_cnf_new1 
    :: "'it \<Rightarrow> 'it \<Rightarrow> 'it clausemap1 \<Rightarrow> (_, 'it clausemap1 \<times> nat) enres"    
...
begin
  ...
  (*synth_definition parse_type_bd is [enres_unfolds]: "parse_type prf = \<hole>"  
    apply (rule CNV_eqD)
    unfolding parse_type_def
    apply opt_enres_unfold
    apply (rule CNV_I)
...
(*concrete_definition (in GRAT_loc) goto_next_item2_loc 
  ...
  *)
  ...
  sepref_definition read_cnf_new3 is "uncurry3 read_cnf_new2" 
    :: "liti.a_assn\<^sup>k *\<^sub>a liti.it_assn\<^sup>k *\<^sub>a liti.it_assn\<^sup>k *\<^sub>a cm_assn\<^sup>d 
      \<rightarrow>\<^sub>a errorp_assn +\<^sub>a cm_assn \<times>\<^sub>a nat_assn"   
  ...
  sepref_register read_cnf_new2 
    :: "int list \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> i_cm \<Rightarrow> ('prf i_prfi error + i_cm \<times> nat) nres"      
