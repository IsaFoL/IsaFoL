...
\documentclass{llncs}     % onecolumn (ditto)
...
\institute{UTwente, \email{p.lammich@utwente.nl}}
...
These specifications state that the bound \isai{F_end} is in range, and that \isai{DB[1..<F_end]} (in Isabelle: \isai{tl (take F_end DB)}) is a valid (\isai{F_invar}) representation of a satisfiable or unsatisfiable, respectively, formula.

To increase the trust in these specifications, we have proved an equivalent form that only relies on basic list operations:
First, we use \isai{tokenize :: int list => int list list}, which splits a list into its zero-terminated components.
