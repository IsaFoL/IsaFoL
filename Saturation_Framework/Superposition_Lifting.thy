  ...
  next
    ...
    next
      ...
      then have \<open>ground_term t'\<close>
        using ordered_trs_preserve_groundness ordered_canonical_trs by presburger
      then have \<open>t' \<prec> u'\<close> using trans ordered_rewriting ordered_canonical_trs by metis
