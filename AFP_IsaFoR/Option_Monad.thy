...
lemma bind_Some_ex_conv [simp]:
  "(m \<bind> f) = Some y \<longleftrightarrow> (\<exists>x. m = Some x \<and> f x = Some y)"
  by (cases m) simp_all

lemma bind_cong [fundef_cong]:
  assumes "m1 = m2"
    and "\<And>x. m2 = Some x \<Longrightarrow> f1 x = f2 x"
...
(* leads to nontermination in Semantic_Labeling_Carrier
  "zip_option xs ys = Some zs \<Longrightarrow> length ys = length xs"
  "zip_option xs ys = Some zs \<Longrightarrow> length zs = length xs"*)
  by (simp_all add: zip_option_None zip_option_zip_conv)


subsection \<open>map for option monad\<close>
