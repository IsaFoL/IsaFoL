...
imports Inference_System Ground_Resolution_Model Clauses
begin
...
definition str_maximal_in :: "'a \<Rightarrow> 'a literal multiset \<Rightarrow> bool" where
  "str_maximal_in A CA \<equiv> (\<forall>B \<in> atms_of CA. B < A)"

inductive eligible :: "'a list \<Rightarrow> 'a clause \<Rightarrow> bool" where
  eligible:
    "S DA = negs (mset Ai) \<or> S DA = {#} \<and> length Ai = 1 \<and> maximal_in (Ai ! 0) DA \<Longrightarrow> eligible Ai DA"
...
proof -
  ...
  define C_of where "C_of \<equiv> \<lambda>A. {#L \<in># CA_of A. L \<noteq> Pos A#}"
  ...
  hence a_ni_c': "\<And>A. A \<in> set Ai \<Longrightarrow> A \<notin> atms_of (C_of A)"
  ...
  hence "\<And>A. A \<in> set Ai \<Longrightarrow> C_of A \<noteq> {#} \<Longrightarrow> Max (atms_of (C_of A)) \<le> A"
  ...
  moreover have "eligible Ai DA"
    using s_d by auto
  hence "eligible Ai (D + negs (mset Ai))"
  ...
  hence "\<And>A. A \<in> set Ai \<Longrightarrow> \<not> Interp N (CA_of A) \<Turnstile> C_of A"
    unfolding prod_c0 C_of_def Interp_def true_cls_def
    using true_lit_def not_gr_zero
    using prod_c0 by auto
  hence c'_at_n: "\<And>A. A \<in> set Ai \<Longrightarrow> \<not> INTERP N \<Turnstile> C_of A"
...
using res_e proof (cases rule: ord_resolve.cases)
  ...
  hence "\<forall>C \<in> set Ci. C \<subseteq># \<Union>#(mset CAi)"
