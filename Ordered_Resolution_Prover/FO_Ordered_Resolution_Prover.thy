...
lemma ord_resolve_unique:
  assumes
    "ord_resolve S CAs DA AAs As \<sigma> E" and
...
proof (induction St)
  ...
  then show ?case
    ...
    by (metis (no_types) src.Rf_mono sup_ge1 SUP_absorb contra_subsetD)
qed
...
next
  case (inference_computation N Q C P)
  {
    ...
    then obtain \<gamma> where
      \<gamma>_p: "\<gamma> \<in> ord_FO_\<Gamma> \<and> infer_from (Q \<union> {C}) \<gamma> \<and> C \<in># prems_of \<gamma> \<and> concl_of \<gamma> = E"
...
proof -
  ...
  from empty_in have in_Liminf_not_Rf: "{#} \<in> Liminf_llist Ns - src.Rf (Liminf_llist Ns)"
