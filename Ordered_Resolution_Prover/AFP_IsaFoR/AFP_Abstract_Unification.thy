theory AFP_Abstract_Unification
  imports
    AFP_Unifiers
    AFP_Term_Pair_Multiset
...
text \<open>
  The reflexive and transitive closure of \<open>UNIF1\<close> collecting substitutions
  produced by intermediate steps.
\<close>
inductive
...
definition compose :: "('f, 'v) subst list \<Rightarrow> ('f, 'v) subst"
  where
    "compose ss = List.foldr (op \<circ>\<^sub>s) ss Var"
...
lemma compose_append [simp]:
  "compose (ss @ ts) = compose ss \<circ>\<^sub>s compose ts"
    using foldr_assoc [of "op \<circ>\<^sub>s" ss Var "foldr op \<circ>\<^sub>s ts Var"]
...
text \<open>
  The inference rules of unification are sound in the sense that
...
text \<open>
  Every unifiable set of equations can be reduced to the empty
...
text \<open>
  The inference rules for unification are complete in the
  sense that whenever it is not possible to reduce a set of equations
  \<open>E\<close> to the empty set, then \<open>E\<close> is not unifiable.
\<close>
...
text \<open>
  It is enough to reach an irreducible set of equations
