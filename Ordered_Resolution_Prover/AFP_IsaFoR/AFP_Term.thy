...
text \<open>
  A substitution is a mapping \<open>\<sigma>\<close> from variables to terms. We call a substitution that
  alters the type of variables a general substitution, since it does not have all properties
...
subsection \<open>Singleton substitutions\<close>
...
text \<open>
  A substitution is idempotent iff the variables in its range are disjoint from its domain.
  (See also "Term Rewriting and All That" Lemma 4.5.7.)
\<close>
